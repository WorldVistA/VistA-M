LRARC1 ;DALISC/CKA - CLONED WKLD REP GENERATOR-MAIN FOR ARCHIVING;5/8/95
 ;;5.2;LAB SERVICE;**59**;August 31, 1995
 ;same as LRCAP1 except for archived wkld file reference
GO ;
 G TRIAL
EN ;
...
TRIAL ; entry point for work load lookup
 ...
 D ACCN^LRARC1A G:Y<0 EXIT
 D DATE^LRARC1A G:Y<0 EXIT S %=2
...
B D SPEC^LRARC1A G:$D(DUOUT)!($D(DTOUT)) EXIT
C D COLL^LRARC1A G:$D(DUOUT)!($D(DTOUT)) EXIT G A
S D SPEC^LRARC1A G:$D(DUOUT)!($D(DTOUT)) EXIT
A W !,"Do you want to select by TESTS or WKLD CODES (YES or NO )"
 ...
 I Y="A" G L
 I Y="W" D CAP^LRARC1A G:$D(DUOUT)!($D(DTOUT)) EXIT G L
 D TEST^LRARC1A G:$D(DUOUT)!($D(DTOUT)) EXIT
I D INSTR^LRARC1A G:$D(DUOUT)!($D(DTOUT)) EXIT
L ;
 D STAT^LRARC1A G:$D(DUOUT)!($D(DTOUT))!(%<0) EXIT
 D LOC^LRARC1A G:$D(DUOUT)!($D(DTOUT)) EXIT
 D IOPAT^LRARC1A G:$D(DUOUT)!($D(DTOUT)) EXIT
 D CONTROL^LRARC1A G:LREND EXIT
 D REPTYP^LRARC1A G:LREND EXIT
...
ASK ;
 ...
 Q
QUE ;
 S ZTSAVE("LR*")="",ZTRTN="LRARCR2",ZTDESC="WORKLOAD REPORT",ZTIO=ION
