SROSNR1 ;B'HAM ISC/MAM - SCRUB NURSE REPORT (ALL) ; [ 07/27/98   2:33 PM ]
...
SET ; set variables
 ...
 K SCRUB S SCRUB(1)="",(SCRUB,CNT)=0 F  S SCRUB=$O(^SRF(SRTN,23,SCRUB)) Q:'SCRUB  S CNT=CNT+1,X=$P(^SRF(SRTN,23,SCRUB,0),"^"),X=$P(^VA(200,X,0),"^") S:$L(X)>17 X=$P(X,",")_", "_$E($P(X,",",2)) S SCRUB(CNT)=X
 K CIRC S CIRC(1)="",(CIRC,CNT)=0 F  S CIRC=$O(^SRF(SRTN,19,CIRC)) Q:'CIRC  S CNT=CNT+1,X=$P(^SRF(SRTN,19,CIRC,0),"^"),X=$P(^VA(200,X,0),"^") S:$L(X)>17 SX=$P(X,",")_", "_$E($P(X,",",2)) S CIRC(CNT)=X
OPS S SROPER=$P(^SRF(SRTN,"OP"),"^"),OP=0 F  S OP=$O(^SRF(SRTN,13,OP)) Q:'OP  D OTHER
...
HDR ; print heading
 ...
 S PAGE=PAGE+1 I $D(NURSE) W !!,?(132-$L("** "_NURSE_" **")\2),"** "_NURSE_" **"
 Q
BEG ; entry when queued
 ...
 .S NURSE=0 F  S NURSE=$O(^SRF(SRTN,23,NURSE)) Q:'NURSE  D UTL
 K NURSE D HDR S NURSE=0 F  S NURSE=$O(^TMP("SR",$J,NURSE)) Q:NURSE=""!SRSOUT  W !!,?(132-$L("** "_NURSE_" **")\2),"** "_NURSE_" **" S SRSDT=0 F  S SRSDT=$O(^TMP("SR",$J,NURSE,SRSDT)) Q:'SRSDT!(SRSOUT)  D SRTN
...
OTHER ; other operations
 ...
 Q
UTL ; set ^TMP(
 S SRNUR=$P(^SRF(SRTN,23,NURSE,0),"^"),SRNUR=$P(^VA(200,SRNUR,0),"^")
