...
NUM S X="" F JJ=1:1:$L(V) S:$A(V,JJ)>32 X=X_$E(V,JJ)
 S V=X Q
IN S:'$D(^LA(TSK,"C",0)) ^LA(TSK,"C")=0,^("C",0)=0 I ^LA(TSK,"C")=^LA(TSK,"C",0) D ^LAKERM3 Q:TOUT
...
OUT S CNT=^LA(TSK,"O")+1,^("O")=CNT,^("O",CNT)=OUT
 ...
 Q
QUIT G:^LA(TSK,"I")>^LA(TSK,"I",0) LA2 I $D(^LA(TSK,"P")),^("P")'["QUIT" H 60 G QUIT
 I $D(^LA(TSK,"O",0)),^LA(TSK,"O")'=^LA(TSK,"O",0) H 60 G QUIT
 K ^LA("LOCK",TSK) LOCK ^LA(TSK) H 1 K ^LA(TSK),^TMP("LA",$J)
 Q
