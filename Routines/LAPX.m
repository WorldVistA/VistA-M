...
LA1 Q  S:$D(ZTQUEUED) ZTREQ="@" S LANM=$T(+0),TSK=$O(^LAB(62.4,"C",LANM,0)) Q:TSK<1
 Q:'$D(^LA(TSK,"I",0))
 K LATOP D ^LASET Q:'TSK  S X="TRAP^"_LANM,@^%ZOSF("TRAP")
LA2 ;K TV,Y S TOUT=0,A=1 D IN G QUIT:TOUT,LA2:IN'[ D QC
 S TOUT=0,BAD=0 F A=2:1 D IN,QC G QUIT:TOUT
 ...
 ;S V=$E(Y(),,) D NUM S ID=+V,V=$E(Y(),,) D NUM S IDE=+V,TRAY=$E(Y(),,),CUP=$E(Y(),,)
