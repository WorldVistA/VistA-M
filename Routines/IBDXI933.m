...
DE S DIE="^IBE(357.93,D0,1,",DIC=DIE,DP=357.931,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^IBE(357.93,D0,1,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,6) S:%]"" DE(1)=% S %=$P(%Z,U,7) S:%]"" DE(3)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET N DIR S DIR(0)="SV"_$E("o",$D(DB(DQ)))_U_DU,DIR("V")=1
 ...
 Q
BEGIN S DNM="IBDXI933",DQ=1
1 S DW="0;6",DV="RNJ3,0XO",DU="",DLB="BUBBLE COLUMN",DIFLD=.06
...
X2 S IBX=X+5 I (X<$$MINX^IBDFU1B)!((X+2)>$$MAXX^IBDFU1B) D WARNING^IBDFU1B("bubble")
 Q
3 S DW="0;7",DV="RNJ3,0XO",DU="",DLB="BUBBLE ROW",DIFLD=.07
 S DQ(3,2)="S Y(0)=Y S Y=+Y+1"
 S X=IBY
 S Y=X
 G Y
...
X4 S IBY=X I (X<$$MINY^IBDFU1B)!(X>$$MAXY^IBDFU1B) D WARNING^IBDFU1B("bubble")
 Q
