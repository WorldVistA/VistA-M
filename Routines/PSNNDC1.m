PSNNDC1 ;BIR/WRT-auto set of translation file (for new drugs added since V1.0) if ndc in national file ; 10/20/98 12:49
...
SECOND I $P(^PS(59.7,1,10),"^",3)=1 F PSNB=0:0 S PSNB=$O(^PSDRUG(PSNB)) Q:'PSNB  U IO W:'(PSNB#100) "." D STRT,SYNO
 Q
STRT Q:'$D(^PSDRUG(PSNB,0))
 I $D(^PSDRUG(PSNB,"ND")),$P(^PSDRUG(PSNB,"ND"),"^",2)]"" Q
 S PSNP=$G(^PSDRUG(PSNB,"I")) I PSNP,PSNP<DT Q
 I $D(^PSNTRAN(PSNB,0)),$P(^PSNTRAN(PSNB,0),"^",2)]"" Q
...
NDC F VV=1:1:3 S VV1=$S(VV=1:6,VV=2:4,VV=3:2) D NDCSET
 ...
 I '$D(^PSNDF(50.67,"NDC",ANS)) K ANS Q
 ...
 S PSNNDF=$P(^PSNDF(50.68,PSNFNM,0),"^",2),PSNFORM=$P(^PSNDF(50.68,PSNFNM,0),"^"),PSNCLASS=$P(^PSNDF(50.68,PSNFNM,3),"^") D SET
 Q
...
SET S:'$D(^PSNTRAN(PSNB,0)) $P(^PSNTRAN(0),"^",4)=($P(^PSNTRAN(0),"^",4))+1,$P(^PSNTRAN(0),"^",3)=PSNB
 ...
 Q
SYNO I $D(^PSDRUG(PSNB,"ND")) Q:$P(^PSDRUG(PSNB,"ND"),"^",2)]""
 I $D(^PSNTRAN(PSNB,0)) Q:$P(^PSNTRAN(PSNB,0),"^",2)]""
 I $O(^PSDRUG(PSNB,1,0)) F XXX=0:0 S XXX=$O(^PSDRUG(PSNB,1,XXX)) Q:'XXX  S ANS=$P(^PSDRUG(PSNB,1,XXX,0),"^",2) I ANS?1.6N1"-"1.4N1"-"1.2N Q:$P($G(^PSNTRAN(PSNB,0)),"^",2)]""  D NDC
 Q
