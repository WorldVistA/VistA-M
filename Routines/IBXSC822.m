...
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGCR(399,DA,""))=""
 I $D(^("U2")) S %Z=^("U2") S %=$P(%Z,U,10) S:%]"" DE(1)=% S %=$P(%Z,U,11) S:%]"" DE(8)=%
 I $D(^("U3")) S %Z=^("U3") S %=$P(%Z,U,3) S:%]"" DE(6)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
1 S DW="U2;10",DV="*P355.93X",DU="",DLB="NON-VA FACILITY",DIFLD=232
 S DE(DW)="C1^IBXSC822"
 S DU="IBA(355.93,"
 G RE
...
C1F1 Q
X1 S DIC("S")="I $P(^(0),U,2)=1,$P(^(0),U)'["",""" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=2 D X2 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X2 S DIPA("NVA_FC")=X S:DIPA("NVA_FC")="" Y="@852"
 Q
3 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=3 D X3 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X3 S DIPA("NVA_FC-0")=$G(^IBA(355.93,+DIPA("NVA_FC"),0)) S:$P(DIPA("NVA_FC-0"),U,5)'=""&($P(DIPA("NVA_FC-0"),U,6)'="")&($P(DIPA("NVA_FC-0"),U,7)'="") Y="@851"
 Q
4 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=4 S I(0,0)=D0 S Y(1)=$S($D(^DGCR(399,D0,"U2")):^("U2"),1:"") S X=$P(Y(1),U,10),X=X S D(0)=+X S X=$S(D(0)>0:D(0),1:"")
 S DGO="^IBXSC823",DC="^355.93^IBA(355.93," G DIEZ^DIE0
R4 D DE G A
 ;
5 S DQ=6 ;@851
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW="U3;3",DV="*P8932.1'",DU="",DLB="NON-VA FACILITY TAXONOMY",DIFLD=244
 S DU="USC(8932.1,"
 G RE
X6 S DIC("S")="I $P($G(^(90002)),U,2)=""N""" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
7 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=7 D X7 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X7 D DISPTAX^IBCEP81($P($G(^DGCR(399,DA,"U3")),U,3),"Non-VA Facility")
 Q
8 S DW="U2;11",DV="S",DU="",DLB="NON-VA CARE TYPE",DIFLD=233
