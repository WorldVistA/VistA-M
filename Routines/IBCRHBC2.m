IBCRHBC2 ;ALB/ARH - RATES: UPLOAD HOST FILES (CMAC 2000+) ; 14-FEB-2000
 ...
 ;
 ; ROUTINE SPECIFIC FOR FORMAT OF YEAR 2000+ CMAC FILES
 ;
...
ENDF() N IBX S IBX=1 I $T,IBFLINE'="" S IBX=0
 ...
 . U IO(0)
 . W !!,"**** Error while reading file: line not expected format (91 numeric characters):"
...
LNFORM(LINE) ; check an individual line of the file for proper format
 N IBX S IBX=0,LINE=$G(LINE) I (LINE?91N)!(LINE?3N1A87N) S IBX=1
 Q IBX
 ;
PARSE ; process a single lin from a CMAC file: parse out into individual fields and store the line in XTMP
 ...
 S IBEFDT=$E(IBFLINE,36,43) ; effective date
 S IBTRDT=$E(IBFLINE,52,59) ; termination date
 S IBCL1P=$E(IBFLINE,60,67) ; class 1 professional component
 S IBCL1T=$E(IBFLINE,68,75) ; class 1 technical component
 S IBCL4P=$E(IBFLINE,76,83) ; class 4 professional component
 S IBCL4T=$E(IBFLINE,84,91) ; class 4 technical component
 Q
 ;
STORE ;
...
LNDT(LINE) ; return the date of an individual line, in FM format
 N IBX S IBX=$E($G(LINE),36,43) S IBX=$$DATE(IBX)
 Q IBX
