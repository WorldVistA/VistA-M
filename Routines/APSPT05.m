APSPT05 ; GENERATED FROM 'PSO INTERVENTION NEW' INPUT TEMPLATE(#870), FILE 9009032.4;04/16/99
 D DE G BEGIN
DE S DIE="^APSPQA(32.4,",DIC=DIE,DP=9009032.4,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^APSPQA(32.4,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,3) S:%]"" DE(1)=% S %=$P(%Z,U,4) S:%]"" DE(2)=% S %=$P(%Z,U,6) S:%]"" DE(3)=% S %=$P(%Z,U,7) S:%]"" DE(5)=% S %=$P(%Z,U,8) S:%]"" DE(9)=% S %=$P(%Z,U,9) S:%]"" DE(13)=% S %=$P(%Z,U,11) S:%]"" DE(15)=%
 I  S %=$P(%Z,U,12) S:%]"" DE(17)=% S %=$P(%Z,U,13) S:%]"" DE(19)=% S %=$P(%Z,U,16) S:%]"" DE(4)=% S %=$P(%Z,U,17) S:%]"" DE(21)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET N DIR S DIR(0)="SV"_$E("o",$D(DB(DQ)))_U_DU,DIR("V")=1
 ...
 Q
BEGIN S DNM="APSPT05",DQ=1
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIEZ=870,U="^"
1 S DW="0;3",DV="*P200'",DU="",DLB="PROVIDER",DIFLD=.03
 S DE(DW)="C1^APSPT05"
 S DU="VA(200,"
 G RE
...
X1 S DIC("S")="S X(1)=$G(^(""PS"")) I +X(1),$S('$P(X(1),""^"",4):1,1:$P(X(1),""^"",4)'<DT)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="0;4",DV="*P200'",DU="",DLB="PHARMACIST",DIFLD=.04
 S DE(DW)="C2^APSPT05"
 S DU="VA(200,"
 S X=DUZ
 S Y=X
...
C2S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X2 Q
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW="0;6",DV="RS",DU="",DLB="INSTITUTED BY",DIFLD=.06
 S DU="1:PHARMACY;2:PROVIDER;3:NURSING;4:PATIENT OR FAMILY;5:OTHER;"
 S Y="PHARMACY"
 G Y
X3 Q
4 S DW="0;16",DV="P59'",DU="",DLB="DIVISION",DIFLD=.16
 S DU="PS(59,"
 S X=$G(PSOSITE)
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD:X="@",Z
X4 Q
5 S DW="0;7",DV="RP9009032.3'",DU="",DLB="INTERVENTION",DIFLD=.07
 S DE(DW)="C5^APSPT05"
 S DU="APSPQA(32.3,"
 G RE
...
X6 I X'=17 S Y="@1"
 Q
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,D=0 K DE(1) ;1100
...
X14 I X=1 S Y="@3"
 Q
15 S DW="0;11",DV="*P200'",DU="",DLB="PROVIDER CONTACTED",DIFLD=.11
 S DU="VA(200,"
 G RE
X15 S DIC("S")="S X(1)=$G(^(""PS"")) I +X(1),$S('$P(X(1),""^"",4):1,1:$P(X(1),""^"",4)'<DT)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
16 S DQ=17 ;@3
17 S DW="0;12",DV="S",DU="",DLB="RECOMMENDATION ACCEPTED",DIFLD=.12
 S DU="0:NO;1:YES;"
 G RE
X17 Q
...
X18 I X=1 S Y="@4"
 Q
19 S DW="0;13",DV="S",DU="",DLB="AGREE WITH PROVIDER",DIFLD=.13
 S DU="0:YES;1:NO;"
 G RE
X19 Q
20 S DQ=21 ;@4
21 S DW="0;17",DV="NJ12,2",DU="",DLB="FINANCIAL COST",DIFLD=.17
 G RE
X21 S:X["$" X=$P(X,"$",2) K:X'?.N.1".".2N!(X>999999999)!(X<0) X
 Q
 ;
22 S D=0 K DE(1) ;1300
 S Y="REASON FOR INTERVENTION^WL^^0;1^Q",DG="13",DC="^9009032.413" D DIEN^DIWE K DE(1) G A
 ;
23 S D=0 K DE(1) ;1400
 S Y="ACTION TAKEN^WL^^0;1^Q",DG="14",DC="^9009032.414" D DIEN^DIWE K DE(1) G A
 ;
24 S D=0 K DE(1) ;1500
 S Y="CLINICAL IMPACT^WL^^0;1^Q",DG="15",DC="^9009032.415" D DIEN^DIWE K DE(1) G A
 ;
25 S D=0 K DE(1) ;1600
