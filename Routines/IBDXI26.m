...
DE S DIE="^IBE(357.2,D0,2,",DIC=DIE,DP=357.22,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^IBE(357.2,D0,2,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,2) S:%]"" DE(3)=%,DE(6)=%,DE(8)=% S %=$P(%Z,U,3) S:%]"" DE(1)=% S %=$P(%Z,U,7) S:%]"" DE(13)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET N DIR S DIR(0)="SV"_$E("o",$D(DB(DQ)))_U_DU,DIR("V")=1
 ...
 Q
BEGIN S DNM="IBDXI26",DQ=1
1 S DW="0;3",DV="NJ3,0",DU="",DLB="SUBCOLUMN WIDTH",DIFLD=.03
 S X=+$G(IBLEN(IBD))
 S Y=X
 G Y
X1 K:+X'=X!(X>150)!(X<1)!(X?.E1"."1N.N) X
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=2 D X2 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X2 I $P(^IBE(357.2,D0,2,D1,0),U,2)'="" S Y="@11"
 Q
3 S DW="0;2",DV="F",DU="",DLB="SUBCOLUMN HEADER",DIFLD=.02
 S X=$P($G(IBLEN(IBD)),U,2)
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X3 K:$L(X)>150!($L(X)<1) X
 I $D(X),X'?.ANP K X
 Q
 ;
4 S DQ=5 ;@11
...
X5 S X=X
 Q
6 S DW="0;2",DV="F",DU="",DLB="WHAT TEXT SHOULD APPEAR AT THE TOP OF THE SUBCOLUMN?",DIFLD=.02
 G RE
X6 K:$L(X)>150!($L(X)<1) X
 I $D(X),X'?.ANP K X
 Q
 ;
...
X7 I X'="" S Y="@9"
 Q
8 S DW="0;2",DV="F",DU="",DLB="SUBCOLUMN HEADER",DIFLD=.02
 S X=" "
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD:X="@",Z
...
X12 S:IBDYN Y="@99"
 Q
13 S DW="0;7",DV="SR",DU="",DLB="ALLOW EDITING OF THE TEXT APPEARING IN THIS SUBCOLUMN? (YES/NO)",DIFLD=.07
 S DU="0:NO;1:YES;"
 G RE
X13 Q
14 S DQ=15 ;@99
