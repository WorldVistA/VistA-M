...
SET N DIR S DIR(0)="SV"_$E("o",$D(DB(DQ)))_U_DU,DIR("V")=1
 ...
 Q
BEGIN S DNM="PRCATE4",DQ=1+D G B
...
X1 K:X'?2N X I $D(X) S DINUM=$S(+X<70:"3"_X,1:"2"_X)
 ...
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="0;8",DV="RNJ9,2",DU="",DLB="FY ORIGINAL AMOUNT",DIFLD=7
 S DE(DW)="C2^PRCATE4"
 G RE
...
C2S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X2 S:X["$" X=$P(X,"$",2) K:X'?.N.1".".2N!(X>999999.99)!(X<0) X
 Q
 ;
3 G 1^DIE17
