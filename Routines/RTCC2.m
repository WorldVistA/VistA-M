...
DE S DIE="^RT(",DIC=DIE,DP=190,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^RT(DA,""))=""
 I $D(^("CL")) S %Z=^("CL") S %=$P(%Z,U,1) S:%]"" DE(6)=% S %=$P(%Z,U,5) S:%]"" DE(15)=% S %=$P(%Z,U,14) S:%]"" DE(1)=% S %=$P(%Z,U,15) S:%]"" DE(7)=%
 I $D(^("COMMENT")) S %Z=^("COMMENT") S %=$P(%Z,U,1) S:%]"" DE(8)=%,DE(11)=%
 I $D(^("I")) S %Z=^("I") S %=$P(%Z,U,1) S:%]"" DE(4)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET I X'?.ANP S DDER=1 Q 
 ...
 Q
BEGIN S DNM="RTCC2",DQ=1
1 S DW="CL;14",DV="*P195.9X",DU="",DLB="ASSOCIATED BORROWER",DIFLD=114
 S DU="RTV(195.9,"
 S Y="@"
...
X3 S:$S('$D(^RT(DA,"I")):1,'^("I"):1,1:0) Y="@18"
 Q
4 S DW="I;1",DV="D",DU="",DLB="INACTIVATION DATE",DIFLD=50
 S Y="@"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X4 S %DT="X" D ^%DT S X=Y K:Y<1 X
 ...
 ;
5 S DQ=6 ;@18
6 S DW="CL;1",DV="*P190.1'",DU="",DLB="ASSOCIATED REQUEST",DIFLD=101
 S DU="RTV(190.1,"
 S X=$S($D(RTQ):RTQ,1:"")
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z
X6 Q
7 S DW="CL;15",DV="S",DU="",DLB="LAST TIME SELECTED BY BARCODE?",DIFLD=115
 S DU="y:YES;n:NO;"
 ...
 G Z
X7 Q
8 S DW="COMMENT;1",DV="F",DU="",DLB="TEMPORARY COMMENT",DIFLD=75
...
X14 S:'$D(RTB) Y="@20"
 Q
15 S DW="CL;5",DV="R*P195.9X",DU="",DLB="CURRENT BORROWER/FILE ROOM",DIFLD=105
 S DE(DW)="C15^RTCC2"
 S DU="RTV(195.9,"
 S X=RTB
 S Y=X
