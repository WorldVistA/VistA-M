IB20P441 ;ALB/TDM - POST-INIT FOR IB*2.0*441; 11/25/2008 ; 12/9/10 3:29pm
 ;;2.0;INTEGRATED BILLING;**441**;21-MAR-94;Build 6
 ;
PRE ; set up check points for pre/post-init
 N %
 S %=$$NEWCP^XPDUTL("THRESH","THRESH^IB20P441")
 S %=$$NEWCP^XPDUTL("MCRDED","MCRDED^IB20P441")
 S %=$$NEWCP^XPDUTL("PRIOR","PRIOR^IB20P441")
 Q
 ;
THRESH ; Pension Threshold
 N IBA,IBERRM,IBRN,IBTYPE,IBX,DA,DIK
 S IBTYPE="Pension Threshold"
 D BMES^XPDUTL("Filing CY 2011 Pension Threshold rates.")
 S IBX=3091201
 F  S IBX=$O(^IBE(354.3,"B",IBX)) Q:'IBX  D  ; remove any records since 12/01/2009
 . S IBRN=0
 . F  S IBRN=$O(^IBE(354.3,"B",IBX,IBRN)) Q:'IBRN  D
 .. S DIK="^IBE(354.3,",DA=IBRN D ^DIK
 S IBA(354.3,"+1,",.01)=3101201 ; effective date for CY 2011 values
 S IBA(354.3,"+1,",.02)=1 ;     internal value 1 = BASIC PENSION
 S IBA(354.3,"+1,",.03)=11830 ;  base rate for veteran
 S IBA(354.3,"+1,",.04)=15493 ; 1 dependent
 S IBA(354.3,"+1,",.05)=17513 ; 2 dependents
 S IBA(354.3,"+1,",.06)=19533 ; 3 dependents
 S IBA(354.3,"+1,",.07)=21553 ; 4 dependents
 S IBA(354.3,"+1,",.08)=23573 ; 5 dependents
 S IBA(354.3,"+1,",.09)=25593 ; 6 dependents
 S IBA(354.3,"+1,",.10)=27613 ; 7 dependents
 S IBA(354.3,"+1,",.11)=29633 ; 8 dependents
 S IBA(354.3,"+1,",.12)=2020 ;  additional dependent amount
 D UPDATE^DIE("","IBA","","IBERRM") ; file the new record for CY 2011
 I $D(IBERRM) D
...
PRIOR ;This code sets up the variables and calls the routine to print or print-and-update the
 ...
 ;
 Q:'$D(^IBA(354.1,"APRIOR",3091201))  ; quit if the "APRIOR" x-ref is not set for 12/1/09.
 ...
 S IBBMES=$S($D(ZTSK):"This job has been queued for NOW, as task number "_ZTSK_".",1:"This job could not be queued. Please edit the 12/1/10 threshold through the 'Add Income Thresholds' option, which allows you to queue this job.")
...
MMSG ; MailMan message to report update problem to billing groups, patch installer and patch developer
 ...
 S IBPARAM("FROM")="PATCH IB*2.0*441 CY 2011 RATE UPDATE"
 F IBGROUP="IB EDI SUPERVISOR","IB ERROR","MCCR" D
 . I $D(^XMB(3.8,"B",IBGROUP)) S IBGROUP="G."_IBGROUP,XMY(IBGROUP)=""
 S XMY(DUZ)=""
 ;
 S IBC=0
 S IBC=IBC+1,IBTXT(IBC)="This message has been sent by patch IB*2.0*441. If you have received this"
 S IBC=IBC+1,IBTXT(IBC)="message, it indicates that the patch encountered some difficulty in filing"
 S IBC=IBC+1,IBTXT(IBC)="the CY 2011 "_IBTYPE_" rates as outlined in the patch description."
 ...
 S IBC=IBC+1,IBTXT(IBC)="You can consult the IB*2.0*441 patch description for additional information."
