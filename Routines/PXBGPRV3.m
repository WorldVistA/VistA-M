PXBGPRV3 ;ISL/JVS - DOUBLE ?? GATHERING OF CLINIC PROVIDER ;11/5/96  14:23
...
NEW ;
 ...
 N TOTAL,PRV,CNT,PXBPMT,SUB2
 ;---SETUP VARIABLES
 S BACK="",INDEX=""
 S START=DATA,SUB=0,SUB2=0
 ;
START1 ;--RECYCLE POINT
 S TITLE="- - C L I N I C    P R O V I D E R S - -"
 ;
 D PRV^PXBUTL2(CLINIC)
 ;
 I '$D(PXBPMT) S TOTAL=0
 I $D(PXBPMT) D
 .S (PRV,CNT)="" F  S PRV=$O(PXBPMT("PRV",PRV)) Q:PRV=""  S CNT=CNT+1 D
 ..S ^TMP("PXBTOTAL",$J,"DILIST","ID",CNT,.01)=PRV
 ..S ^TMP("PXBTOTAL",$J,"DILIST",2,CNT)=$O(PXBPMT("PRV",PRV,0))
 I $D(CNT) S TOTAL=CNT
 ;
 ;--DISPLAY IF NO MATCH FOUND
 I TOTAL=0 W IOCUU,IOCUU,!,IOELEOL D
 .D LOC W !
 .S RESULTS="NO PROVIDERS ARE SET UP FOR THIS CLINIC" W !!!,?(IOM-$L(RESULTS))\2,RESULTS D HELP1^PXBUTL1("CON") R OK:DTIME
 I TOTAL=0 Q TOTAL
 ;
 ;
 ;----DISPLAY LIST TO THE SCREEN
 S HEADING="W !,""ITEM"",?6,""NAME   "",IOINHI,TOTAL,"" MATCHES"",IOINLOW,?30,""       PERSON CLASS IN NEW PERSON FILE"""
