IBXEXI ; GENERATED FROM 'IB INACTIVATE EXEMPTION' INPUT TEMPLATE(#518), FILE 354.1;06/27/96
 D DE G BEGIN
DE S DIE="^IBA(354.1,",DIC=DIE,DP=354.1,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^IBA(354.1,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,10) S:%]"" DE(2)=% S %=$P(%Z,U,15) S:%]"" DE(3)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET I X'?.ANP S DDER=1 Q 
 ...
 Q
BEGIN S DNM="IBXEXI",DQ=1
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIEZ=518,U="^"
1 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=1 D X1 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X1 I '$D(IBX) S Y="@99"
 Q
2 S DW="0;10",DV="R*S",DU="",DLB="ACTIVE",DIFLD=.1
 S DE(DW)="C2^IBXEXI"
...
C2S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X2 Q
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW="0;15",DV="D",DU="",DLB="PRIOR YEAR THRESHOLDS",DIFLD=.15
 S DE(DW)="C3^IBXEXI"
 S Y="@"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
