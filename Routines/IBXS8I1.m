...
BEGIN S DNM="IBXS8I1",DQ=1+D G B
...
X4 K DLAYGO S DIPA("PRF")=X S:DIPA("PRF")="" Y="@8399"
 Q
...
7 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=7 N DIERR X DR(99,1,9.2) K DIC S DIC="^IBA(355.93,",DIC(0)="MF" D ^DIC S (D,D0,D(0))=+Y S Y(101)=$S($D(^IBA(355.93,D0,0)):^(0),1:"") S X=$P(Y(101),U,1) S D0=I(0,0) S D1=I(1,0) S X=$S(D(0)>0:D(0),1:"")
 S DGO="^IBXS8I3",DC="^355.93^IBA(355.93," G DIEZ^DIE0
R7 D DE G A
 ;
8 S DQ=9 ;@838
...
X15 S DIPA("CRD")=$$CRED^IBCEU($P($G(^DGCR(399,DA(1),"PRV",DA,0)),U,2))
 Q
...
X17 K DIPA("W1") S:$G(DIPA("CRD"))'=$P($G(^DGCR(399,DA(1),"PRV",DA,0)),U,3) DIPA("W1")=1
 Q
