LEX2039 ; ISL/KER - Environment Check/Pre/Post Install ; 12/06/2005
 ...
 ;                            
ENV ; LEX*2.0*39 Environment Check
 ;
 ;   General
 ;
 N LEXBUILD,LEXIGHF,LEXREQP,LEXLREV,LEXG,LEXE
 W !," Code Set Update for FY06 2nd Quarter - includes CPT/HCPCS Annual Release",!
 D IMP S U="^"
 ;     No user
...
ABRT ;     Abort  Failed Environment Check, KILL the distribution
 D:$D(LEXE) ED S XPDABORT=1,XPDQUIT=1,XPDQUIT("LEX*2.0*39")=1
...
CPD(X) ;  Check Current Patched Data is installed
 ...
 S:'$D(^LEX(757.3,"AS","I $L($$ICDONE^LEXU(+Y,+($G(LEXVDT))))!($L($$CPTONE^LEXU(+Y,+($G",2)) INS=0
 S:$D(^LEX(757.3,"AS","I $L($$ICDONE^LEXU(+Y))!($L($$CPTONE^LEXU(+Y)))!($L($$CPCONE^LE",2)) INS=0
 ;    Check Last ICD Diagnosis Set
 S:'$D(^ICD9(13131,0)) INS=0
 ;    Check Last CPT/HCPCS Procedures Set/Kill
 S:'$D(^ICPT("F","V2788",108367)) INS=0
 S:$D(^ICPT("D",78,75956)) INS=0
 ;    Check Last CPT Modifiers Set/Kill
 S:'$D(^DIC(81.3,"M",8400001,8409999,250)) INS=0
 S:$D(^DIC(81.3,"C","HOSP-BASED DIALYSIS TO RESIDEN",250)) INS=0
 ;    Check v2
 S:$G(^LEX(757.3,1,1))'["LEXVDT"!($G(^LEXT(757.2,4,6))'["LEXVDT") INS=0
 ;    Check v3
 S:$G(^ICPT(108371,0))'["0144T" INS=0
 S X=+($G(INS))
 Q X
 ;
 ; Error messages
...
ED ;   Error Display
 ...
 ;
IMP ;   Import names
 S LEXLREV=39,LEXREQP="LEX*2.0*37",LEXBUILD="LEX*2.0*39"
 S LEXIGHF="LEX_2_39.GBL"
 Q
