...
BEGIN S DNM="IBXSC712",DQ=1
1 S DW="0;15",DV="FXO",DU="",DLB="RX PROCEDURE",DIFLD=.15
 S DQ(1,2)="S Y(0)=Y S Y=Y_"" - ""_$P($$PRCNM^IBCSCH1($P($G(^DGCR(399,D0,""CP"",+Y,0)),U)),U)"
 S DE(DW)="C1^IBXSC712",DE(DW,"INDEX")=1
 G RE
...
C1X1(DION) K X
 ...
 Q
C1F2 Q
X1 S X=$$RXPRLOOK^IBCEU4(X) K:'X X
 I $D(X),X'?.ANP K X
 Q
 ;
