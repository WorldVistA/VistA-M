...
DE S DIE="^DVB(396,",DIC=DIE,DP=396,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DVB(396,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,17) S:%]"" DE(9)=%
 I $D(^(1)) S %Z=^(1) S %=$P(%Z,U,2) S:%]"" DE(1)=%,DE(5)=% S %=$P(%Z,U,3) S:%]"" DE(12)=%,DE(16)=% S %=$P(%Z,U,16) S:%]"" DE(4)=% S %=$P(%Z,U,17) S:%]"" DE(15)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
1 S DW="1;2",DV="D",DU="",DLB="OTHER/EXAM COMPLETION DATE",DIFLD=8.5
 S Y="@"
...
X2 S Y="@8"
 Q
3 S DQ=4 ;@7
4 S DW="1;16",DV="F",DU="",DLB="EDIT8",DIFLD=8.7
 S X=OPER
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z
X4 Q
5 S DW="1;2",DV="D",DU="",DLB="OTHER/EXAM COMPLETION DATE",DIFLD=8.5
 G RE
X5 S %DT="EX" D ^%DT S X=Y K:Y<1 X
 Q
 ;
6 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=6 D X6 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X6 I X=""&(DVBASTAT="C") W *7,!!,"Completed status must have date.",!! S Y="@7"
 Q
7 S DQ=8 ;@8
8 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=8 D X8 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X8 S CODE=$P(^DVB(396,D0,0),U,17) S:CODE="" Y="@10" S:DVBACORR="N"&(CODE'="P") Y="@10"
 Q
9 S DW="0;17",DV="S",DU="",DLB="STATUS OF SPECIAL REPORT",DIFLD=10
 S DU="P:PENDING;C:COMPLETED;"
 G RE
X9 Q
10 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=10 D X10 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X10 S DVBASTAT=X S:X="C" Y="@9"
 Q
11 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=11 D X11 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X11 S DVBADT=$S($D(^DVB(396,D0,1)):$P(^(1),U,3),1:"") S:DVBADT=""&(X="P") Y="@10"
 Q
12 S DW="1;3",DV="D",DU="",DLB="SPECIAL REPORT COMPLETION DATE",DIFLD=10.5
 S Y="@"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
...
X13 S Y="@10"
 Q
14 S DQ=15 ;@9
15 S DW="1;17",DV="F",DU="",DLB="EDIT10",DIFLD=10.7
 S X=OPER
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z
X15 Q
16 S DW="1;3",DV="D",DU="",DLB="SPECIAL REPORT COMPLETION DATE",DIFLD=10.5
 G RE
X16 S %DT="EX" D ^%DT S X=Y K:Y<1 X
 Q
 ;
17 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=17 D X17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X17 I X=""&(DVBASTAT="C") W *7,!!,"Completed status must have date.",!! S Y="@9"
 Q
18 S DQ=19 ;@10
19 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=19 D X19 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X19 S CODE=$P(^DVB(396,D0,0),U,19) S:CODE="" Y="@12" S:DVBACORR="N"&(CODE'="P") Y="@12"
 Q
20 D:$D(DG)>9 F^DIE17 G ^DVBAXS3
