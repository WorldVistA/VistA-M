...
LA1 S:$D(ZTQUEUED) ZTREQ="@" S LANM=$T(+0),TSK=$O(^LAB(62.4,"C",LANM,0)) Q:TSK<1
 Q:'$D(^LA(TSK,"I",0))
 K LRTOP D ^LASET Q:'TSK  S X="TRAP^"_LANM,@^%ZOSF("TRAP")
LA2 S TOUT=0,A=1 D IN G QUIT:TOUT,LA2:IN'?3N1"  "2N1"/"2N.E D QC
 S TOUT=0,BAD=0 F A=2:1:17 D IN D QC G QUIT:TOUT
 F I=0:0 S I=$O(TC(I)) Q:I<1  X TC(I,2) S @TC(I,1)=V
 S ID=+$E(Y(1),1,3),TRAY=1,CUP=ID,V=$E(Y(2),1,7) D NUM S IDE=+V
LA3 X LAGEN ;can be changed by the cross-link code
