PRCATP1 ; GENERATED FROM 'PRCA PROFILE' PRINT TEMPLATE (#384) ; 06/27/96 ; (FILE 430, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(384,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
 ...
 W ?24 D EN3^PRCADR K DIP K:DN Y
