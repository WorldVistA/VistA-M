PSXDQUE ;BIR/WPB-Terminate Auto Processing ;[ 04/08/97   2:06 PM ]
 ;;2.0;CMOP;;11 Apr 97
DQ ;
 G:'$D(^PSX(550,"AT")) DQ1
 S REC=$O(^PSX(550,"AT",""))
 S TASK=$P($G(^PSX(550,+PSXSYS,"T",REC,0)),U,7)
DQ1 I '$D(TASK) W !,"There isn't an automatic job queued to run under this option.",! G EXIT
 S TIME=$$FMTE^XLFDT($P(^PSX(550,+PSXSYS,"T",REC,0),U,2),"1P"),FREQ=$P(^(0),U,3),THRU=$P(^(0),U,8)
 ...
 S DIR("A",5)="Next transmission scheduled for:  "_$$FMTE^XLFDT($P(^PSX(550,+PSXSYS,"T",REC,0),"^",4),1),DIR("A",6)="Rescheduling Frequency         :  "_FREQ_$S(FREQ=1:" hour",FREQ>1:" hours",1:"")
 S DIR("A",1)="",DIR("A",7)=""
 ...
 .S DA=REC,DA(1)=+PSXSYS,DIE="^PSX(550,"_+PSXSYS_",""T"",",DR=".01////2;6////@" D ^DIE L -^PSX(550,+PSXSYS,0) K DIE,DA,DR,PSXZTSK,REC,TREC,TASK W !,"Job Unscheduled.",!
...
MSG D NOW^%DTC S (PSXDATE,DTE)=%,SITE=$P($G(PSXSYS),U,3),NAME=$P($G(^VA(200,DUZ,0)),U,1),TIME=$$FMTE^XLFDT(DTE,"1")
 S XMDUZ=.5,XMSUB=("CMOP Auto-Transmission Terminated"),LCNT=5
 D XMZ^XMA2 G:XMZ<1 MSG
 S ^XMB(3.9,XMZ,2,1,0)="Cancellation of Auto-Transmission Schedule."
 S ^XMB(3.9,XMZ,2,2,0)=""
