MCOBPV ; GENERATED FROM 'MCARPACVLEADBRPR' PRINT TEMPLATE (#1033) ; 10/04/96 ; (FILE 698.1, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(1033,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
 ...
 G A1R
A1 ;
 S X=$G(^MCAR(698.1,D0,10,D1,0)) S DIWL=1,DIWR=55 D ^DIWP
 Q
A1R ;
 D A^DIWW
 D T Q:'DN  D N D N:$X>4 Q:'DN  W ?4 W "PROCEDURE SUMMARY: "
 S X=$G(^MCAR(698.1,D0,.2)) S DIWL=1,DIWR=55 S Y=$P(X,U,2) S X=Y D ^DIWP
 D A^DIWW
