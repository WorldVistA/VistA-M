IBXA2 ; COMPILED XREF FOR FILE #350 ; 10/29/09
 ; 
 S DIKZK=1
 S DIKZ(0)=$G(^IB(DA,0))
 S X=$P(DIKZ(0),U,1)
 ...
 I X'="" D
 .N DIK,DIV,DIU,DIN
 .K DIV S DIV=X,D0=DA,DIV(0)=D0 S Y(1)=$S($D(^IB(D0,1)):^(1),1:"") S X=$P(Y(1),U,4),X=X S DIU=X K Y S X=DIV S %=$P($H,",",2),X=DT_(%\60#60/100+(%\3600)+(%#60/10000)/100) X ^DD(350,.05,1,3,1.4)
 S X=$P(DIKZ(0),U,5)
