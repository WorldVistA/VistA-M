...
DE S DIE="^IBE(357.5,D0,2,",DIC=DIE,DP=357.52,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^IBE(357.5,D0,2,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,6) S:%]"" DE(3)=% S %=$P(%Z,U,7) S:%]"" DE(5)=% S %=$P(%Z,U,8) S:%]"" DE(1)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET N DIR S DIR(0)="SV"_$E("o",$D(DB(DQ)))_U_DU,DIR("V")=1
 ...
 Q
BEGIN S DNM="IBDXI53",DQ=1
1 S DW="0;8",DV="NJ3,0",DU="",DLB="LENGTH OF DATA",DIFLD=.08
 S X=$G(IBLEN(IBP))
 S Y=X
 G Y
...
X2 S IBW=+X
 Q
3 S DW="0;6",DV="NJ3,0XO",DU="",DLB="STARTING ROW FOR DATA",DIFLD=.06
...
X4 S IBY=X+1
 Q
5 S DW="0;7",DV="NJ3,0XO",DU="",DLB="STARTING COLUMN FOR DATA",DIFLD=.07
 S DQ(5,2)="S Y(0)=Y S:Y=+Y Y=Y+1"
 S X=+IBX
 S Y=X
 G Y
X5 S:+X=X X=X-1 K:+X'=X!(X>200)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
