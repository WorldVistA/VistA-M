RMPREO23 ;HINES/HNC - suspense processing display protocols ; Jan 11, 2024@11:43:17
 ;;3.0;PROSTHETICS;**45,55,62,77,80,215**;Feb 09, 1996;Build 3
 ...
 ;
 ; RMPR*3.0*215 - Prevent actions for cancelled requests.
 Q
...
CANCEL ;cancel suspense
 ...
 D NUM^RMPREOU
 S RMPREOY=Y
 F RN=1:1 S XDA=$P(RMPREOY,",",RN) Q:XDA=""  Q:$G(DUOUT)=1  I $D(^TMP($J,"RMPREOEE",XDA,0)) S DA=^TMP($J,"RMPREOEE",XDA,0) D  ;
 . ;
 . ;  If request is cancelled, do not allow action.  p215 wtc 12/20/23
 . ;
 . I $$GET1^DIQ(668,DA,14)="CANCELLED" R !,"ACTION NOT AVAILABLE FOR CANCELLED REQUEST. PRESS [RETURN] TO CONTINUE",RMPRCNTU:$S($G(DTIME):DTIME,1:60) Q  ;
 . D CANCEL^RMPREOS
 K RMPREOY,XDA,DA,DUOUT,RMPRCNTU ; P215
 D INIT^RMPREO
 S VALMBCK="R"
 Q
...
EMAN ;edit manual suspense
 ...
 . ;  If request is cancelled, do not allow action.  p215 wtc 12/20/23
 . ;
 . I $$GET1^DIQ(668,DA,14)="CANCELLED" R !,"ACTION NOT AVAILABLE FOR CANCELLED REQUEST. PRESS [RETURN] TO CONTINUE",RMPRCNTU:$S($G(DTIME):DTIME,1:60) D INIT^RMPREO S VALMBCK="R" Q  ;
 . D EMANP
 K RMPREOY,XDA,DA,DUOUT,RN,RMPRCNTU ; P215
 Q
