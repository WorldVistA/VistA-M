RMPRESI ;PHX/JLT-ENTER/EDIT SITE PARAMETERS ROUTINE; Nov 12, 2024@12:15
 ;;3.0;PROSTHETICS;**51,55,62,90,125,218**;Feb 09, 1996;Build 12
 ;
 ...
 ;                        DVN IP ADDRESS
 ;                        DVN FILE NAME
 ; HDSO patch 218 - deprecated fields 19, 20, & 21
 ;
...
START S RMPRA=^DIC(4,SITE,0),RMPRB=$S($D(^DIC(4,SITE,1)):^(1),1:" "),SIG="",ST=$P(^DIC(4,SITE,0),U,2)
 ...
 S STRT=$P(RMPRB,U,1)
 ;RMPR*3.0*218: removed PG, COP, & CDP variables from line below
 ;              which correspond to deprecated fields 19, 20, & 21
 S CITY=$P(RMPRB,U,3),RIP=$P(RMPRB,U,4),ART=125,WCHR=100,BRSS=75,BAS=75
CON S RMPRSITE=$O(^RMPR(669.9,0)),AIN=$P(^RMPR(669.9,RMPRSITE,0),U,3) I $D(^(2)) S SIG=$P(^(2),U,6),SBT=$P(^(2),U,1)
 S DA=RMPRIEN,DIE="^RMPR(669.9,"
 ;RMPR*3.0*218: removed fields 19, 20, & 21 from DR string
 S DR="2////^S X=AIN;4////^S X=STRT;5////^S X=CITY;6////^S X=ST;7////^S X=RIP;8////^S X=SIG;8.5///^S X=SBT;11////^S X=99999999;15////^S X=ART;16////^S X=WCHR;17////^S X=BRSS;18////^S X=BAS" D ^DIE
EDIT ;EDIT SITE PARAMENTERS
 ; Added fields 8.1 and 10 in patch 90
 ; Added fields (subfile 8.1 field 3,4) 12,13,14.
 ;RMPR*3.0*218: Removed fields 19, 20, & 21 from DR string
 S DIE="^RMPR(669.9,",DA=RMPRIEN,DR=".01;40;3;4;5;6;7;7.1;12;13;14;8;8.1;10;8.5;2;31;32;33;34;9;27;28;29;30;15;16;17;18;22;52" D ^DIE G DIR
