BPSNCPD6 ;ALB/SS - Pharmacy API part 6 ;10-JAN-08
 ;;1.0;E CLAIMS MGMT ENGINE;**7,8,10,11,20,40,41**;JUN 2004;Build 11
 ...
 ;== New Claim 
NEWCLAIM ;
 S BPRETV=$$NEWCLM^BPSNCPD5(.BP77NEW,BRXIEN,BFILL,DOS,BWHERE,BILLNDC,REVREAS,DURREC,BPOVRIEN,BPSCLARF,BPSAUTH,BPSDELAY,BPCOBIND,BPJOBFLG,IEN59,BPACTTYP,DFN,.BPSTART,$G(BPREQIEN),.BPSELIG,$G(BPSRTYPE),$G(BPSPLAN),.BPSPRDAT,BPSDX,BPSPREG)
 S RESPONSE=+BPRETV
...
RVPAID ;
 S BPRETV=$$REVERSAL^BPSNCPD5(.BP77NEW,BRXIEN,BFILL,OLDRESP,DOS,BWHERE,$G(BILLNDC),REVREAS,DURREC,BPOVRIEN,BPSCLARF,BPSAUTH,BPSDELAY,IEN59,BPCOBIND,BPJOBFLG,BPACTTYP,.BPSTART,$G(BPREQIEN),.BPSCLOSE,$G(BPSRTYPE),.BPSPRDAT,BPSDX,BPSPREG)
 S RESPONSE=+BPRETV
 ;to make LOG backward compatible
...
RVRSPAID ;
 S BPRETV=$$REVRESUB^BPSNCPD4(.BP77NEW,BRXIEN,BFILL,DOS,BWHERE,BILLNDC,REVREAS,DURREC,BPOVRIEN,BPSCLARF,BPSAUTH,BPSDELAY,BPCOBIND,BPJOBFLG,IEN59,DFN,.BPSTART,$G(BPREQIEN),OLDRESP,.BPSELIG,$G(BPSRTYPE),$G(BPSPLAN),.BPSPRDAT,BPSDX,BPSPREG)
 S RESPONSE=+BPRETV
...
RVNPAID ;
 ...
 ;== Also used for Resubmits w/out Reversals no matter the claim status
RVRSNPD ;
 ;
 S BPREQIEN=$G(BPREQIEN)
 S BPSRTYPE=$G(BPSRTYPE)
 S BPSPLAN=$G(BPSPLAN)
 ; resubmit a claim
 S BPRETV=$$REVRESNP^BPSNCPD5(.BP77NEW,BRXIEN,BFILL,DOS,BWHERE,BILLNDC,REVREAS,DURREC,BPOVRIEN,BPSCLARF,BPSAUTH,BPSDELAY,BPCOBIND,BPJOBFLG,IEN59,BPACTTYP,DFN,.BPSTART,BPREQIEN,OLDRESP,.BPSELIG,BPSRTYPE,BPSPLAN,.BPSPRDAT,BPSDX,BPSPREG)
 S RESPONSE=+BPRETV
 ;to make LOG backward compatible
...
RVNEW ;
 ...
 Q
 ;
 ; This was meant to be called by BPSSCR04 to collect for the User
 ; Screen requests that don't have BPS TRANSACTION records.  However,
 ; it is currently not called.  This is still here in case it is needed
 ; in the future.
