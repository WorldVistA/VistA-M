...
BEGIN ;
 ...
 D N:$X>2 Q:'DN  W ?2 W "Remote By"
 W ?13 X DXS(5,9.2) S X1=DIP(2) S %=$X,%1=$S(X>%:X-%,1:0),%="",$P(%,".",%1)="." W %,X1 K %,%1 S (X,X1)="" K DIP K:DN Y W X
 D N:$X>42 Q:'DN  W ?42 S X="Date entered" K DIP K:DN Y W X
 W ?53 X DXS(6,9.2) S X1=DIP(3) S %=$X,%1=$S(X>%:X-%,1:0),%="",$P(%,".",%1)="." W %,X1 K %,%1 S (X,X1)="" K DIP K:DN Y W X
 D N:$X>2 Q:'DN  W ?2 W "Mult Sign-on"
 W ?16 X DXS(7,9.2) S X1=DIP(3) S %=$X,%1=$S(X>%:X-%,1:0),%="",$P(%,".",%1)="." W %,X1 K %,%1 S (X,X1)="" K DIP K:DN Y W X
 D N:$X>42 Q:'DN  W ?42 W "Fileman codes"
 W ?57 X DXS(8,9.2) S X1=DIP(2) S %=$X,%1=$S(X>%:X-%,1:0),%="",$P(%,".",%1)="." W %,X1 K %,%1 S (X,X1)="" K DIP K:DN Y W X
 D N:$X>2 Q:'DN  W ?2 W "Time-out"
 W ?12 X DXS(9,9.2) S X1=DIP(2) S %=$X,%1=$S(X>%:X-%,1:0),%="",$P(%,".",%1)="." W %,X1 K %,%1 S (X,X1)="" K DIP K:DN Y W X
 D N:$X>42 Q:'DN  W ?42 W "Type-ahead"
 W ?54 X DXS(10,9.2) S X1=DIP(3) S %=$X,%1=$S(X>%:X-%,1:0),%="",$P(%,".",%1)="." W %,X1 K %,%1 S (X,X1)="" K DIP K:DN Y W X
 D N:$X>2 Q:'DN  W ?2 W "Title"
 W ?9 X DXS(11,9.2) S X1=DIP(2) S %=$X,%1=$S(X>%:X-%,1:0),%="",$P(%,".",%1)="." W %,X1 K %,%1 S (X,X1)="" K DIP K:DN Y W X
 D N:$X>42 Q:'DN  W ?42 W "Office Phone"
 W ?56 X DXS(12,9.2) S X1=DIP(2) S %=$X,%1=$S(X>%:X-%,1:0),%="",$P(%,".",%1)="." W %,X1 K %,%1 S (X,X1)="" K DIP K:DN Y W X
 D N:$X>2 Q:'DN  W ?2 W "Auto-Menu"
 W ?13 X DXS(13,9.2) S X1=DIP(3) S %=$X,%1=$S(X>%:X-%,1:0),%="",$P(%,".",%1)="." W %,X1 K %,%1 S (X,X1)="" K DIP K:DN Y W X
 D N:$X>42 Q:'DN  W ?42 W "Voice Pager"
 W ?55 X DXS(14,9.2) S X1=DIP(2) S %=$X,%1=$S(X>%:X-%,1:0),%="",$P(%,".",%1)="." W %,X1 K %,%1 S (X,X1)="" K DIP K:DN Y W X
 D N:$X>2 Q:'DN  W ?2 W "Last Sign-on"
 W ?16 X DXS(15,9.2) S X1=DIP(3) S %=$X,%1=$S(X>%:X-%,1:0),%="",$P(%,".",%1)="." W %,X1 K %,%1 S (X,X1)="" K DIP K:DN Y W X
 D N:$X>42 Q:'DN  W ?42 W "Digital Pager"
 W ?57 X DXS(16,9.2) S X1=DIP(2) S %=$X,%1=$S(X>%:X-%,1:0),%="",$P(%,".",%1)="." W %,X1 K %,%1 S (X,X1)="" K DIP K:DN Y W X
 D N:$X>2 Q:'DN  W ?2 W "Has a E-SIG"
 S DIPA("ESIG")=$S($L($P($G(^VA(200,D0,20)),U,4)):"Yes",1:"No") K DIP K:DN Y
 W ?15 X DXS(17,9.2) S X1=DIP(1) S %=$X,%1=$S(X>%:X-%,1:0),%="",$P(%,".",%1)="." W %,X1 K %,%1 S (X,X1)="" K DIP K:DN Y W X
 D N:$X>42 Q:'DN  W ?42 W "Write Med's"
 W ?55 X DXS(18,9.2) S X1=DIP(3) S %=$X,%1=$S(X>%:X-%,1:0),%="",$P(%,".",%1)="." W %,X1 K %,%1 S (X,X1)="" K DIP K:DN Y W X
 D N:$X>0 Q:'DN  W ?0 W " "
 D N:$X>2 Q:'DN  W ?2 W "NPI"
 W ?7 X DXS(19,9.2) S X1=DIP(2) S %=$X,%1=$S(X>%:X-%,1:0),%="",$P(%,".",%1)="." W %,X1 K %,%1 S (X,X1)="" K DIP K:DN Y W X
 D N:$X>42 Q:'DN  W ?42 W "Taxonomy"
 S DIPA("TAX")=$P($$TAXINQ^XUSTAX(D0),"^") K DIP K:DN Y
 W ?52 X DXS(20,9.2) S X1=DIP(1) S %=$X,%1=$S(X>%:X-%,1:0),%="",$P(%,".",%1)="." W %,X1 K %,%1 S (X,X1)="" K DIP K:DN Y W X
 ...
 X DXS(22,9.2) S X1=DIP(2) S:X]""&(X?.ANP)&(X1'[U)&(X1'["$C(94)") DIPA($E(X,1,30))=X1 S X="" K DIP K:DN Y W X
 W ?25 W:$G(DIPA("POS"))'="" !,?2,"Current Degree Level: ",DIPA("F12.1") K DIP K:DN Y
 W ?36 X DXS(23,9.2) S X1=DIP(2) S:X]""&(X?.ANP)&(X1'[U)&(X1'["$C(94)") DIPA($E(X,1,30))=X1 S X="" K DIP K:DN Y W X
 W ?47 W:$G(DIPA("POS"))'="" !,?2,"Program of Study: ",DIPA("F12.2") K DIP K:DN Y
 W ?58 N DIERR X DXS(24,9.2) S X1=DIP(2) S:X]""&(X?.ANP)&(X1'[U)&(X1'["$C(94)") DIPA($E(X,1,30))=X1 S X="" K DIP K:DN Y W X
 W ?69 W:$G(DIPA("POS"))'="" !,?2,"Last Training Year: ",DIPA("F12.3") K DIP K:DN Y
 D T Q:'DN  W ?2 X DXS(25,9.2) S X1=DIP(2) S:X]""&(X?.ANP)&(X1'[U)&(X1'["$C(94)") DIPA($E(X,1,30))=X1 S X="" K DIP K:DN Y W X
 ...
 D N:$X>39 Q:'DN  W ?39 X DXS(26,9.2) S X=$P(DIP(101),U,2) S D0=I(0,0) K DIP K:DN Y W X
 D N:$X>0 Q:'DN  W ?0 S X="Secondary Menu(s)",%=$L(X),%1=$X,$P(%2,"-",%)="-" W X,!,?%1,%2 K %1,%2 S X="" K DIP K:DN Y W X
 S I(1)=203,J(1)=200.03 F D1=0:0 Q:$O(^VA(200,D0,203,D1))'>0  X $G(DSC(200.03))  S D1=$O(^(D1)) Q:D1'>0   D:$X>11 T Q:'DN  D A1
 G A1R
A1 ;
 ...
 D N:$X>41 Q:'DN  W ?41 X DXS(27,9.2) S X=$P(DIP(101),U,2) S D0=I(0,0) S D1=I(1,0) K DIP K:DN Y W X
 Q
A1R ;
 ...
 S X=$G(^VA(200,D0,101)) D N:$X>13 Q:'DN  W ?13 S Y=$P(X,U,1) W:Y]"" $E($$SET^DIQ(200,101.01,Y),1,3)
 ...
 W ?11 X DXS(28,9) K DIP K:DN Y
 S I(1)="""ORD""",J(1)=200.010113 F D1=0:0 Q:$O(^VA(200,D0,"ORD",D1))'>0  X $G(DSC(200.010113))  S D1=$O(^(D1)) Q:D1'>0   D:$X>22 T Q:'DN  D B1
 G B1R
B1 ;
 S X=$G(^VA(200,D0,"ORD",D1,0)) D N:$X>2 Q:'DN  W ?2 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^ORD(101.13,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,3)
 D N:$X>8 Q:'DN  W ?8 X DXS(29,9.2) S DIP(101)=$S($D(^ORD(101.13,D0,0)):^(0),1:"") S X=$P(DIP(101),U,2) S D0=I(0,0) S D1=I(1,0) K DIP K:DN Y W X
