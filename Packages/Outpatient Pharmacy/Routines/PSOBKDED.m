PSOBKDED ;BIR/SAB - Edit backdoor Rx Order entry ;Aug 11, 2020@11:13:08
 ;;7.0;OUTPATIENT PHARMACY;**11,46,91,78,99,117,133,143,268,378,416,282,450,402,518,525,538,457,557,574,598,441,770,824**;DEC 1997;Build 1
...
ASK1 S STRE=$P($G(DOSE("DD",PSODRUG("IEN"))),"^",5),UNITN=$P($G(DOSE("DD",PSODRUG("IEN"))),"^",6),DOSE("LD")=$P($G(DOSE("DD",PSODRUG("IEN"))),"^",11)
 W ! S DIR(0)="F^1:60"
 I '$G(PSODOSCT) D
 .S PSODOSCT=0 F I=0:0 S I=$O(DOSE(I)) Q:'I!('$D(DOSE(I)))  S PSODOSCT=I
 ...
 D ^DIR S X=$$REMAMP(X),Y=X G:'$L(X) ASK ;p824
...
REMAMP(DOSEIN) ;p824
 ...
 Q:'$L(DOSEIN) DOSEIN
 N DOSEOUT,I,DOSESTR
 S DOSEOUT="",DOSEIN=$TR(DOSEIN,"&","")
 F I=1:1:$L(DOSEIN) D
 .S DOSESTR=$P(DOSEIN," ",I) Q:DOSESTR=""  S DOSEOUT=DOSEOUT_$S(DOSEOUT="":"",1:" ")_DOSESTR
 K DOSEIN
 Q DOSEOUT
KX K X,Y
