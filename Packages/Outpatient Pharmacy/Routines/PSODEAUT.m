PSODEAUT ;ALB/BI - DEA MANUAL ENTRY ;10/30/23  13:47
 ;;7.0;OUTPATIENT PHARMACY;**529,684,731,743,802**;DEC 1997;Build 2
...
FILEFM(RET,DATA,NPIEN)  ; -- File DEA Information in the DEA NUMBERS FILE #8991.9
 ...
 I $G(NPIEN),$$DEANUM($P(DATA,U,14)) D         ; ONLY CLEAR AND SET IF VALIDATED  ;p802 Adding $G check
 ...
 I $G(NPIEN),$G(RET),$P(DATA,U,21)="YES" S FDA(200,NPIEN_",",53.2)=$P(DATA,U,11) D UPDATE^DIE(,"FDA")     ;p802 Adding $G checks
