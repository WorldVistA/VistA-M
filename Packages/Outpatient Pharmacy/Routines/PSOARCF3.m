PSOARCF3 ;BHAM ISC/LGH,LC - RX RETRIEVE ; 07/07/92
 ...
 U PSOAT D PSOAT
 S PSOAPF=0
R R X:DTIME G END:X="" G:X'="!" R
PAR R X:DTIME G:'$T END G:$P(X,"^")=NM&($G(SS)=$P(X,"^",2)) PR G PAR
...
Q ;I $D(PSOAP) U IO(0) S IOP=PSOAP D ^%ZIS D ^%ZISC K IOP
 ...
 Q
PR ;PATIENT READ
 S T(1)=X R X:DTIME G:'$T END S T(2)=X,D=$P(T(2),"^",2),A=$P(T(2),"^",3),DG=$P(T(2),"^",4),GD=$P(T(2),"^",5)
 I D>0 F I=1:1:D R X:DTIME G:'$T END S T(2,I)=X
 I A>0 F I=1:1:A R X:DTIME G:'$T END S T(3,I)=X
 I DG>0 F I=1:1:DG R X:DTIME G:'$T END S T(4,I)=X
 I GD>0 F I=1:1:GD R X:DTIME G:'$T END S T(5,I)=X
 D:'PSOAPF DPR,HD1^PSOARCF4 S PSOAPF=1 ;DISPLAY DEMO INFO
RXR R X:DTIME G:'$T END G END:X="" I X="!" R X:DTIME G:'$T END G:$P(X,"^")'=NM END G PR
 S T(6)=X R X:DTIME G:'$T END S T(10)=X R X:DTIME G:'$T END S T(7)=X S RR=$P(T(7),"^",14),RA=$P(T(7),"^",15)
 I RR>0 F I=1:1:RR R X:DTIME G:'$T END S T(8,I)=X
 I RA>0 F I=1:1:RA R X:DTIME G:'$T END S T(9,I)=X
...
REM W ! I $P(T(9,I),"^",5)]"" W ?5,"COMMENT: ",$P(T(9,I),"^",5)
 ...
 Q
PSOAT ;CHECK FOR EOF
 U PSOAT S:$$STATUS^%ZISH PSORWND=$$REWIND^%ZIS(PSOAIO,PSOAIOT,PSOAPAR) I $G(PSORWND) U IO(0) W !!,"HFS file failed to rewind" G END
 U PSOAT
 Q
HEAD D:$Y>(PSOACPL-20) HD1^PSOARCF4
 W !,"#",?3,"LOG DATE",?14,"REFILL DATE",?27,"QTY",?32,"ROUTING",?40,"LOT #",?52,"PHARMACIST",?70,"DIVISION",! F I=1:1:79 W "="
 S FFX=0 Q
H1 D:$Y>(PSOACPL-20) HD1^PSOARCF4
