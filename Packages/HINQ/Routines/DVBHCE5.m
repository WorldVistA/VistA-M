...
X6 S %DT="E",%DT(0)=-DT D ^%DT K %DT S X=Y K:Y<1 X I $D(X) D EK^DGLOCK
 ...
 ;
7 S D=0 K DE(1) ;361
 S DIFLD=361,DGO="^DVBHCE6",DC="3^2.0361IP^E^",DV="2.0361M*P8'Xa",DW="0;1",DOW=$$LABEL^DIALOGZ(DP,DIFLD),DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="DIC(8,"
...
C24S S X="" G:DG(DQ)=X C24F1 K DB
 ...
 D AUTOUPD^DGENA2(DA)
C24F1 Q
X24 D DOL^DGLOCK2 K:+X'=X&(X'?.N1"."2N)!(X>999999.99)!(X<0) X I $D(X) S DFN=DA D MV^DGLOCK I $D(X),('$$TOTCHK^DGLOCK2(DFN)) D TOTCKMSG^DGLOCK2 K X
 Q
 ;
25 S DQ=26 ;@2062
