...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 ...
 I $D(^(.52)) S %Z=^(.52) S %=$P(%Z,U,5) S:%]"" DE(55)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW=".11;7",DV="NJ3,0XOa",DU="",DIFLD=.117,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X17 I $L(DVBCN)=9,(DVBCN?9N),(DVBCN'=$P(^DPT(D0,0),U,9)) W !!,*7,"HINQ claim # is a SSN, does not match patient file SSN  NO UPDATING claim #" R !,?25,"<RET to continue>",DVBQ:DTIME K DVBQ S Y="@22",JP=JP+1
 Q
18 D:$D(DG)>9 F^DIE17,DE S DQ=18,DW=".31;3",DV="FXOa",DU="",DIFLD=.313,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DQ(18,2)="S Y(0)=Y S Y=$E(Y,1,10)"
 S DE(DW)="C18^DVBHCE18"
 S X=DVBCN
 S Y=X
...
X36 I $D(^DPT(D0,.105)) W !!,"Patient is currently in-house." W !,"Discharge patient with a discharge type of DEATH." R !,?25,"<RET to continue>",DVBQ:DTIME K DVBQ S Y="@24",JP=JP+1
 Q
37 D:$D(DG)>9 F^DIE17,DE S DQ=37,DW=".35;1",DV="DXa",DU="",DIFLD=.351,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C37^DVBHCE18",DE(DW,"INDEX")=1
 S X=Z1
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
...
C37X2(DION) K X
 ...
 S X=$G(X(1))
 Q
C37F3 S DIXR=1351 D C37X3(U) K X2 M X2=X D C37X3("O") K X1 M X1=X
 K X M X=X2 D
...
X43 I '$D(DVBSICK) S Y="@25",JP=JP+1
 Q
44 D:$D(DG)>9 F^DIE17,DE S DQ=44,DW=".29;12",DV="S",DU="",DIFLD=.293,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C44^DVBHCE18"
 S DU="0:NO;1:YES;"
...
X54 D POW^DVBHUTIL
 Q
55 D:$D(DG)>9 F^DIE17,DE S DQ=55,DW=".52;5",DV="RSX",DU="",DIFLD=.525,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C55^DVBHCE18",DE(DW,"INDEX")=1
...
C55S S X="" G:DG(DQ)=X C55F1 K DB
 ...
 S X=DG(DQ),DIC=DIE
 D EVENT^IVMPLOG(DA)
C55F1 N X,X1,X2 S DIXR=627 D C55X1(U) K X2 M X2=X D C55X1("O") K X1 M X1=X
 D
