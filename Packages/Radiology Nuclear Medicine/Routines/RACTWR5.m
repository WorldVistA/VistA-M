...
DE S DIE="^RARPT(",DIC=DIE,DP=74,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^RARPT(DA,""))=""
 ...
 I $D(^("P")) S %Z=^("P") S %=$E(%Z,1,240) S:%'?." " DE(4)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET I X'?.ANP S DDER=1 Q 
 ...
 Q
BEGIN S DNM="RACTWR5",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="0;7",DV="DI",DU="",DLB="VERIFIED DATE",DIFLD=7
 S DE(DW)="C1^RACTWR5"
 S X="NOW"
 S Y=X
...
X2 S Y="@15"
 Q
3 S DQ=4 ;@50
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW="P;E1,240",DV="F",DU="",DLB="PROBLEM STATEMENT",DIFLD=25
 G RE
X4 K:$L(X)>240!($L(X)<2) X
 I $D(X),X'?.ANP K X
 Q
 ;
5 S DQ=6 ;@99
6 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=6 D X6 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X6 K RASTATX,RATRSC,RANODE
 Q
7 G 0^DIE17
