...
MRPF2 ;SECOND ENTRY POINT
 ...
 ;I '$D(^RAMRPF(71.99,"C",RACPT)) D MSG G MRPFL
 ;F I=1:1 S RAPROIEN=$O(^RAMRPF(71.99,"C",RACPT,RAPROIEN)) Q:RAPROIEN=""  D ACTIVE^RANPROU2 D:$G(RA99)'="INACTIVE"
 ;. ;check if procedure has a MRPF already
 ;. ;I $P($G(^RAMRPF(71.11,DA,"NTRT")),"^",1)'="" Q
 ;. ; check if the MRPF is already in use
 ;. I $D(^RAMIS(71,"MRPF",RAPROIEN)) D  Q
 ;. . S II=0 F  S II=$O(^RAMIS(71,"MRPF",RAPROIEN,II)) Q:'II  S CNT1=CNT1+1,RAMERR(CNT1)=$E($P($G(^RAMRPF(71.99,RAPROIEN,0)),"^",1),1,30)_"^"_$E($P($G(^RAMIS(71,II,0)),"^",1),1,30)
 ;. ; set up selected items array
 ;. S:$G(RACODE)="" RACODE="" S CNT=CNT+1,RACINF=$$CPT^ICPTCOD(RACPT),RACODE=RACODE_CNT_":"_$P(^RAMRPF(71.99,RAPROIEN,0),U,1)_"/"_$P(RACINF,"^",3)_";",^XTMP("RAMAIN4",$J,I)=RAPROIEN
 ;; List out found associations
 ;I CNT1>0 D
 ;. S II=0 W !,"Procedure To MRPF associations For CPT: "_RACPT,!
 ;. F  S II=$O(RAMERR(II)) Q:'II  S A=RAMERR(II) W $P(A,"^",2),?40,$P(A,"^",1),!
