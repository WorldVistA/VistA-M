RACTOE ; GENERATED FROM 'RA ORDER EXAM' INPUT TEMPLATE(#1087), FILE 75.1;06/23/17
 D DE G BEGIN
...
C72F1 S DIEZRXR(75.1,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=1370 S DIEZRXR(75.1,DIXR)=""
 Q
...
C75S S X="" G:DG(DQ)=X C75F1 K DB
 S X=DG(DQ),DIC=DIE
 S ^RAO(75.1,"AP",+$P(^RAO(75.1,DA,0),U),+$P(^(0),U,2),9999999.9999-$E(X,1,30),DA)=""
C75F1 S DIEZRXR(75.1,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=1370 S DIEZRXR(75.1,DIXR)=""
 Q
...
C79S S X="" G:DG(DQ)=X C79F1 K DB
 S X=DG(DQ),DIC=DIE
 S ^RAO(75.1,"AP",+$P(^RAO(75.1,DA,0),U),+$P(^(0),U,2),9999999.9999-$E(X,1,30),DA)=""
C79F1 S DIEZRXR(75.1,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=1370 S DIEZRXR(75.1,DIXR)=""
 Q
...
C82F1 S DIEZRXR(75.1,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=1371 S DIEZRXR(75.1,DIXR)=""
 Q
X82 Q
...
86 D:$D(DG)>9 F^DIE17,DE S DQ=86,D=0 K DE(1) ;75
 S DIFLD=75,DGO="^RACTOE4",DC="4^75.12DA^T^",DV="75.12D",DW="0;1",DOW=$$LABEL^DIALOGZ(DP,DIFLD),DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
