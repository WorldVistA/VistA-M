ONCORIST ;HINES OIFO/RTK,AJB - OncoTrax Data to ORIS ;04/16/26
 ;;2.2;ONCOLOGY;**23**;Jul 31, 2013;Build 6
 ; SAC EXEMPTION ID 202602221239-01 : For use of $ZF function
 ;
EN ;this code (copy of ONCORIS) will be run if the option is QUEUED
 ;
 ;W @IOF
 ;W !!!!,"========================================"
 ;W "======================================",!!
...
JSON ;
 ...
 ;W !,"Tarball created: ",!
 S X=$ZF(-100,"/SHELL","ls","-lh","/tmp/onc2oris.scheduler.tar")
 ;W !!,"Contents of onc2oris.scheduler.tar: ",!
 S X=$ZF(-100,"/SHELL","tar","-tvf","/tmp/onc2oris.scheduler.tar")
 ;N ONCT,ONCTZONE,ONCTHR,ONCTMN,ONCTSN D TIMENOW W !!,"Timestamp after Tarball created: ",ONCDTNW
 ;W !,"Sending tar file to ORIS...",!
 S X=$ZF(-100,"/SHELL","curl","-X","POST",ONCSVPTH,"-H",ONCACKEY,"-F","data=@/tmp/onc2oris.scheduler.tar")
 ;W !,"Done.",!
 ;
 ;Cleanup after ourselves
 S X=$ZF(-100,"/SHELL","rm","-f","/tmp/onc2oris.scheduler.tar", "/tmp/onc2oris/metadata.scheduler.txt")
 S X=$ZF(-100,"/SHELL","rm","-rf","/tmp/onc2oris.scheduler")
 ;
...
GETFILES ;get all the files/data in json format
 ...
 S X=$ZF(-100,"/SHELL","rm","-rf","/tmp/onc2oris.scheduler")
 S X=$ZF(-100,"/SHELL","rm","-f","/tmp/onc2oris.scheduler.txt", "/tmp/onc2oris.scheduler.tar")
 S X=$ZF(-100,"/SHELL","mkdir", "/tmp/onc2oris.scheduler")
 ;First build header
 S IOP="ORIS-HFS-SCHEDULED",%ZIS="Q" D ^%ZIS
 U IO
 W !,"{"
 W !,"  ","""_MetaData"""_" :"
 W !,"  {"
 ...
 W !,"}"
 D ^%ZISC
 S X=$ZF(-100,"/SHELL", "mv", "/tmp/onc2oris.scheduler.txt", "/tmp/onc2oris.scheduler/metadata.txt")
 ;
 ;Create a new tarball and load in the metadata file.
 S X=$ZF(-100,"/SHELL","tar","-cf","/tmp/onc2oris.scheduler.tar", "-C", "/tmp/onc2oris.scheduler/", "metadata.txt")
 ;
 ...
 .S IOP="ORIS-HFS-SCHEDULED",%ZIS="QA" D ^%ZIS
 .U IO
 ...
 .D ^%ZISC
 .S X=$ZF(-100,"/SHELL", "mv", "/tmp/onc2oris.scheduler.txt", "/tmp/onc2oris.scheduler/"_FILENUM_".txt")
 .S X=$ZF(-100,"/SHELL","wc","-c","/tmp/onc2oris.scheduler/"_FILENUM_".txt")
 .;Compress the current file into a .gz file and then delete the original txt file
 .S X=$ZF(-100,"/SHELL","gzip","-f","/tmp/onc2oris.scheduler/"_FILENUM_".txt")
 .S X=$ZF(-100,"/SHELL","rm","-f","/tmp/onc2oris.scheduler/"_FILENUM_".txt")
 .;Load the compressed file into the tarball and then delete it
 .S X=$ZF(-100,"/SHELL","tar","-rf","/tmp/onc2oris.scheduler.tar", "-C", "/tmp/onc2oris.scheduler/",FILENUM_".txt.gz")
 .S X=$ZF(-100,"/SHELL","rm","-f","/tmp/onc2oris.scheduler/"_FILENUM_".txt.gz")
 .Q
 ;
 ;
