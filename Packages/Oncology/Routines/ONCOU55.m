ONCOU55 ;HINES OIFO/GWB - Utility routine # 1 ;06/23/10
 ;;2.2;ONCOLOGY;**1,10,12,13,15,17,20,21,22**;Jul 31, 2013;Build 6
 ;
...
TNMED(IEN) ;AJCC Cancer Staging Manual Edition
 N CSG,PSG,TNMED,TNMMO,TOP,YR,TNMAJCC,ONC5000
 S YR=$E($P($G(^ONCO(165.5,IEN,0)),U,16),1,3)
 S ONC5000=$P($G(^ONCO(165.5,IEN,"AJCC8")),U,1)  ;the AJCC ID
 ...
 I $$LYMPHOMA^ONCFUNC(IEN),ONC5000'?1"9"3N G TNMEX
 ...
 .I YR>323,$P($G(^ONCO(165.5,IEN,"SSD1")),"^",1)'="","09290^09301^09302^09310^09320^09330^09340^09500"[$P($G(^ONCO(165.5,IEN,"SSD1")),"^",1) S TNMED=9 Q
 .I YR>324,$P($G(^ONCO(165.5,IEN,"SSD1")),"^",1)'="","09090^09350^09360^09370"[$P($G(^ONCO(165.5,IEN,"SSD1")),"^",1) S TNMED=9 Q
TNMEX ;
 Q TNMED
 ;
