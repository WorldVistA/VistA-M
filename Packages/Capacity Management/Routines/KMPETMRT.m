KMPETMRT ;SP/JML - Collect Cache Metrics for the Real-Time Error Trap Monitor ;7/1/2025
 ...
 ; Reference to ERROR LOG file (^%ZTER(1)) in ICR #7138
 ; Reference to ERROR TRAP SUMMARY file (^%ZTER(3.077)) in ICR $7552
 ;
RUN ;
...
COLLECT(KMPVNODE) ;
 ...
 N KMPTS,KMPUNAM,KMPUTCE,KMPUTCO,KMPZA,KMPZB,KMPZIO,KMPOPT,KMPVARI
 ;
 ...
 .S KMPVARI=0,KMPOPT=""
 .F  S KMPVARI=$O(^%ZTER(1,KMPDAT,1,KMPENUM,"ZV",KMPVARI)) Q:KMPVARI=""  D
 ..I $G(^%ZTER(1,KMPDAT,1,KMPENUM,"ZV",KMPVARI,0))="XQYO" D
 ...S KMPOPT=$G(^%ZTER(1,KMPDAT,1,KMPENUM,"ZV",KMPVARI,"D"))
 .S KMPEDATA=KMPERRN_B_KMPTS_B_KMPTS_B_KMPDB_B_KMPINST_B_KMPNT_B_KMPERR_B_KMPLGR_B_KMPCIO_B
 .S KMPEDATA=KMPEDATA_KMPZA_B_KMPZB_B_KMPZIO_B_KMPJNUM_B_KMPPNAM_B_KMPUNAM_B_KMPAJN_B_KMPELINE_B_KMPOPT
...
RETRY ;  retry failed POSTS
 ...
 ;
ERRSUM() ;
 N KMPI,KMPDAT,KMPERR,KMPEDETAIL,KMPDISTRO,KMPESUM,KMPEFRST,KMPECURR,KMPVAR,KMPX
 S KMPESUM=##class(%DynamicObject).%New()
 S KMPI=0
 F  S KMPI=$O(^%ZTER(3.077,KMPI)) Q:+KMPI=0  D
 .S KMPDAT=0
 .F  S KMPDAT=$O(^%ZTER(3.077,KMPI,4,KMPDAT)) Q:KMPDAT=""  D
 ..I KMPDAT=+$H D
 ...S KMPERR=$G(^%ZTER(3.077,KMPI,0))
 ...S KMPDISTRO=$G(^%ZTER(3.077,KMPI,4,KMPDAT,0))
 ...S KMPEDETAIL=##class(%DynamicObject).%New()
 ...S KMPEFRST=$$TSTAMP^KMPUTLW($P(KMPERR,"^",2),"FILEMAN")
 ...S KMPECURR=$$TSTAMP^KMPUTLW($P(KMPERR,"^",3),"FILEMAN")
 ...D KMPEDETAIL.%Set("Location",$P($P(KMPERR,"^",1),",",1))
 ...D KMPEDETAIL.%Set("Error",$P($P(KMPERR,"^",1),",",2))
 ...D KMPEDETAIL.%Set("FirstTimeStamp",$P(KMPEFRST,"^",1))
 ...D KMPEDETAIL.%Set("LastTimeStamp",$P(KMPECURR,"^",1))
 ...D KMPEDETAIL.%Set("Routine",$P(KMPERR,"^",4))
 ...F KMPX=1:1:24 S KMPVAR="Hour"_(KMPX-1) D KMPEDETAIL.%Set(KMPVAR,+$P(KMPDISTRO,"~",KMPX))
 ...D KMPESUM.%Set(KMPI,KMPEDETAIL)
 Q KMPESUM
