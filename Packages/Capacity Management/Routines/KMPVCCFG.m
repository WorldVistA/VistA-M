KMPVCCFG ;SP/JML - VSM configuration functions -- APIs for data access ;7/1/2025
 ;;4.0;CAPACITY MANAGEMENT;**1,2,4,5**;3/1/2018;Build 9
 ;
...
CFGARR(KMPVMKEY,KMPVCFG,KMPVFLAG) ; Return configuration by monitor in array
 ...
 S KMPVCFG("COLLECTION INTERVAL")=$P(KMPV1,U,2),KMPVCFG("HTTP REQUEST MAX LENGTH")=$P(KMPV1,U,5)
 S KMPVCFG("MONITOR START DELAY")=$P(KMPV1,U,6),KMPVCFG("ALLOW TEST SYSTEM")=$$GETVAL(KMPVMKEY,"ALLOW TEST SYSTEM",8969,$G(KMPVFLAG))
 S KMPVCFG("CACHE DAILY TASK")=$P(KMPV1,U,3),KMPVCFG("COVER SHEET EXPIRATION")=$P(KMPV1,U,7)
 S KMPVCFG("ENCRYPT")=$P(KMPV1,U,9),KMPVCFG("FUTURE TASK CHECK")=$$GETVAL(KMPVMKEY,"FUTURE TASK CHECK",8969)
 ;
 S KMPVCFG("NATIONAL DATA EMAIL ADDRESS")=$P(KMPV3,U),KMPVCFG("NATIONAL SUPPORT EMAIL ADDRESS")=$P(KMPV3,U,2)
 S KMPVCFG("LOCAL SUPPORT EMAIL ADDRESS")=$P(KMPV3,U,4)
...
CFGSTR(KMPVMKEY,KMPVFLAG) ; Return configuration in "^" delimited string
 ...
 ; ^CACHE DAILY TASK^COVER SHEET EXPIRATION^*REALTIME^*START PERFMON^ENCRYPT^FUTURE TASK CHECK
 ...
 S KMPVCFG=KMPVCFG_$P(KMPV1,U,3)_U_$P(KMPV1,U,10)_U_U_U_$P(KMPV1,U,9)_U_$$GETVAL(KMPVMKEY,"FUTURE TASK CHECK",8969)
 Q KMPVCFG
 ;
