...
RUN ; Collect metrics per configured interval and store in ^KMPTMP("KMPV","VMCM","DLY" -- CALLED VIA CACHE TASK MANAGER
 ...
 ; ^KMPTMP("KMPV","VMCM","PREVIOUS"
 ;   Data in "TRANSMIT" node is deleted upon transmission
 ;   Data in "PREVIOUS" node stores data for prior collection interval
 ;    
 ;-----------------------------------------------------------------------
 ;
 N $ES,$ETRAP S $ETRAP="D ERR^ZU Q"
 I $$GETVAL^KMPVCCFG("VMCM","ONOFF",8969,"I") D RU^%ZOSVKR("KMP VMCM DRIVER")
 N KMPD,KMPPARMS,KMPFM,KMPH,KMPNDTYP,KMPSC,KMPVCHKH,KMPVH,KMPVHANG,KMPVNODE,KMPVSINF,KMPVSINT,KMPVSTOP,KMPVTEST
 ...
 S KMPH=$H,KMPD=$P(KMPH,","),KMPFM=+$$HTFM^XLFDT(KMPH,1)
 ...
 S KMPPARMS("KMPD")=KMPD,KMPPARMS("KMPFM")=KMPFM
...
COLLECT(KMPVNODE) ;
 ...
 S KMPD=KMPPARMS("KMPD"),KMPFM=KMPPARMS("KMPFM")
 ...
 .I KMPSTAT'=200 S ^KMPTMP("KMPV","VMCM","RETRY",KMPVNODE,+$H,$H)=KMPJSON.%ToJSON()
