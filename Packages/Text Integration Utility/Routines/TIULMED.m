TIULMED ; SLC/JM,JH,AJB - ACTIVE/RECENT MED OBJECTS ;Aug 29, 2025@06:55:28
 ;;1.0;TEXT INTEGRATION UTILITIES;**38,73,92,94,183,193,197,198,202,213,238,290,289,318,365,372**;Jun 20, 1997;Build 5
 ;
 ...
 ;
 Q
LIST(DFN,TARGET,ACTVONLY,DETAILED,ALLMEDS,ONELIST,CLASSORT,SUPPLIES,MEDREC) ;
 ...
 ;  MEDREC    0 - Exclude TIUDATE for PSOORRL call (supported in ^TIUMOBJ only)
 ;            1 - Include TIUDATE (Default) (supported in ^TIUMOBJ only)
 N XPARSYS I $$GET^XPAR("SYS","TIUMOBJ STATUS") D  Q TARGET ; ajb *365 redirect on param def.
 . S TARGET=$$LIST^TIUMOBJ(DFN,.TARGET,$G(ACTVONLY),$G(DETAILED),$G(ALLMEDS),$G(ONELIST),$G(CLASSORT),$G(SUPPLIES),$G(MEDREC))
