TIUEPN ; GENERATED FROM 'TIU ENTER/EDIT PROGRESS NOTE' INPUT TEMPLATE(#1493), FILE 8925;05/26/16
 D DE G BEGIN
...
1 S DW="0;2",DV="P9000001'",DU="",DIFLD=.02,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
C1F1 S DIEZRXR(8925,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 ...
 Q
X1 Q
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="0;3",DV="P9000010'",DU="",DIFLD=.03,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C2^TIUEPN"
 S DU="AUPNVSIT("
 S X=$P($G(TIU("VISIT")),U)
 S Y=X
...
C2F1 Q
X2 Q
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW="0;7",DV="D",DU="",DIFLD=.07,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
C3F1 S DIEZRXR(8925,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 ...
 Q
X3 Q
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW="0;8",DV="DX",DU="",DIFLD=.08,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
C4F1 S DIEZRXR(8925,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 ...
 Q
X4 Q
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW="14;1",DV="P405'",DU="",DIFLD=1401,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="DGPM("
 S X=$P($G(TIU("AD#")),U)
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X7 I +$P($G(^TIU(8925,+DA,12)),U,5) S Y="@13"
 Q
8 S DW="12;5",DV="P44'",DU="",DIFLD=1205,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X10 I +$G(^TIU(8925,+DA,13)) S Y="@14"
 Q
11 D:$D(DG)>9 F^DIE17,DE S DQ=11,DW="13;1",DV="D",DU="",DIFLD=1301,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
C11F1 S DIEZRXR(8925,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=247,323 S DIEZRXR(8925,DIXR)=""
 Q
X11 Q
12 S DQ=13 ;@14
...
X13 I +$P($G(^TIU(8925,+DA,12)),U,2) S Y="@1"
 Q
14 D:$D(DG)>9 F^DIE17,DE S DQ=14,DW="12;2",DV="*P200'",DU="",DIFLD=1202,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X17 I $P($G(^SC(+$G(TIU("LOC")),0)),U,3)="W" S Y="@3"
 Q
18 D:$D(DG)>9 F^DIE17,DE S DQ=18,DW="12;5",DV="P44'",DU="",DIFLD=1205,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X21 Q
22 S DQ=23 ;@4
23 D:$D(DG)>9 F^DIE17,DE S DQ=23,DW="13;1",DV="D",DU="",DIFLD=1301,DLB="DATE/TIME OF NOTE"
