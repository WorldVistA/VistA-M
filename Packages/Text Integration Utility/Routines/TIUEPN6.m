...
DE S DIE="^TIU(8925,",DIC=DIE,DP=8925,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^TIU(8925,DA,""))=""
 I $D(^(12)) S %Z=^(12) S %=$P(%Z,U,1) S:%]"" DE(1)=% S %=$P(%Z,U,8) S:%]"" DE(7)=%,DE(11)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET N DIR S DIR(0)="SV"_$E("o",$D(DB(DQ)))_U_DU,DIR("V")=1
 ...
 Q
BEGIN S DNM="TIUEPN6",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="12;1",DV="D",DU="",DLB="ENTRY DATE/TIME",DIFLD=1201
 S DE(DW)="C1^TIUEPN6"
 S X=$$NOW^TIULC
 S Y=X
...
C7S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X7 S DIC("S")="I +$$SCRCSNR^TIULA3(DA,+Y)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
X9 S Y="@9"
 Q
10 S DQ=11 ;@11
11 D:$D(DG)>9 F^DIE17,DE S DQ=11,DW="12;8",DV="*P200'R",DU="",DLB="EXPECTED COSIGNER",DIFLD=1208
 S DE(DW)="C11^TIUEPN6"
 S DU="VA(200,"
 G RE
...
C11S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 D SACLEC^TIUDD0(1208,X)
 Q
X11 S DIC("S")="I +$$SCRCSNR^TIULA3(DA,+Y)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
