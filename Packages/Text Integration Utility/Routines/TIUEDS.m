TIUEDS ; GENERATED FROM 'TIU ENTER/EDIT DS' INPUT TEMPLATE(#1491), FILE 8925;05/26/16
 D DE G BEGIN
...
X3 I $P(^TIU(8925,+DA,0),U,7)]"" S Y="@1"
 Q
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW="0;7",DV="D",DU="",DIFLD=.07,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X6 I $P(^TIU(8925,+DA,0),U,8)]"" S Y="@2"
 Q
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW="0;8",DV="DX",DU="",DIFLD=.08,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X7 Q
8 S DQ=9 ;@2
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW="0;9",DV="*S",DU="",DIFLD=.09,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X11 I +$$PROVIDER^TIUPXAP1(DUZ,DT)'>0 S Y="@8"
 Q
12 S DW="12;2",DV="*P200'R",DU="",DIFLD=1202,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X13 S Y="@3"
 Q
14 S DQ=15 ;@8
15 D:$D(DG)>9 F^DIE17,DE S DQ=15,DW="12;2",DV="*P200'R",DU="",DIFLD=1202,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X15 S DIC("S")="I '+$$ISTERM^USRLM(+Y)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 ...
 ;
16 S DQ=17 ;@3
17 D:$D(DG)>9 F^DIE17,DE S DQ=17,DW="13;7",DV="DR",DU="",DIFLD=1307,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
...
X20 I +$P(TIUREFDT,U,2)'>0 S Y="@4"
 Q
21 S DW="0;12",DV="S",DU="",DIFLD=.12,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C21^TIUEDS"
 S DU="1:YES;"
 S X=1
 S Y=X
...
X21 Q
22 S DQ=23 ;@4
23 D:$D(DG)>9 F^DIE17,DE S DQ=23,DW="13;1",DV="D",DU="",DIFLD=1301,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
