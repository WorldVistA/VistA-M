...
DE S DIE="^TIU(8925,",DIC=DIE,DP=8925,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^TIU(8925,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,3) S:%]"" DE(1)=% S %=$P(%Z,U,7) S:%]"" DE(2)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="0;3",DV="P9000010'",DU="",DLB="VISIT",DIFLD=.03
 S DE(DW)="C1^TIUEPN1"
 S DU="AUPNVSIT("
 S X=$P($G(TIU("VISIT")),U)
 S Y=X
...
C1S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X1 Q
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="0;7",DV="D",DU="",DLB="EPISODE BEGIN DATE/TIME",DIFLD=.07
 S DE(DW)="C2^TIUEPN1"
 S X=$P($G(TIU("EDT")),U)
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
