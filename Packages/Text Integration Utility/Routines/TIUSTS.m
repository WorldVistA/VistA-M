TIUSTS ; GENERATED FROM 'TIU PRINT SERVICE STATS' PRINT TEMPLATE (#1118) ; 03/03/04 ; (FILE 8925, MARGIN=132)
 G BEGIN
CP G CP^DIO2
C S DQ(C)=Y
...
BEGIN ;
 ...
 I $D(DXS)<9 M DXS=^DIPT(1118,"DXS")
 S I(0)="^TIU(8925,",J(0)=8925
 S X=$G(^TIU(8925,D0,14)) W ?0 S Y=$P(X,U,4),C=1 D D S Y=$S(Y="":Y,$D(^DIC(49,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,15)
...
HEAD ;
 W !,?20,"Line"
 W !,?0,"Service",?19,"Count",?27,"Ref Date",?41,"Patient",?58,"Disch-Dict",?74,"Dict-Transcr",?89,"Transcr-Sign",?104,"Sign-Cosign"
 W !,"------------------------------------------------------------------------------------------------------------------------------------",!!
