PSSDSAPI ;BIR/RTR - Dose Check APIs routine ; Jun 09, 2008@16:00
 ;;1.0;PHARMACY DATA MANAGEMENT;**117,160,173,178,206,224,254**;9/30/97;Build 109
 ;
...
MRT(PSSRS1) ;Return Standard Medication Route and First DataBank Route
 ...
 F  S PSSRTIEN=$O(^PS(51.2,"FDBMRT",PSSRTNAM,PSSRTIEN)) Q:PSSRTIEN=""  D  ;check for duplicate route names; if mapped, defined and has a valid effective date set array
...
UNITP ;
 K PSSVUTZ S PSSVUTX=$O(^PS(51.24,"FDBNAME",PSSVUTAA,0)) I PSSVUTX,'$$SCREEN^XTID(51.24,.01,PSSVUTX_",") S PSSVUTZ=$P($G(^PS(51.24,PSSVUTX,0)),"^",2) I PSSVUTZ'="" G UNITX
 K PSSVUTZ S PSSVUTX=$O(^PS(51.24,"FDBUNIT",PSSVUTAA,0)) I PSSVUTX,'$$SCREEN^XTID(51.24,.01,PSSVUTX_",") S PSSVUTZ=$P($G(^PS(51.24,PSSVUTX,0)),"^",2) I PSSVUTZ'="" G UNITX
 K PSSVUTZ S PSSVUTX=$O(^PS(51.24,"FDBSYN",PSSVUTAA,0)) I PSSVUTX,'$$SCREEN^XTID(51.24,.01,PSSVUTX_",") S PSSVUTZ=$P($G(^PS(51.24,PSSVUTX,0)),"^",2) I PSSVUTZ'="" G UNITX
...
NUMB ;Frequency passed in as a number
 ...
 I PSSFWFR'>10080 S PSSFWXWK=10080/PSSFWFR I PSSFWXWK?.N S PSSFWFLG=1,PSSFWRST=PSSFWXWK_"XW",PSSFWPR1=1 Q
 S PSSFWXMN=43200/PSSFWFR I PSSFWXMN?.N S PSSFWFLG=1,PSSFWRST=PSSFWXMN_"XL",PSSFWPR1=1 Q
 Q
 ;
STN ;Standard Logic
 ...
 I $G(PSSOSN)]"" S PSSFWLP2=PSSOSN D STN1^PSSDSAPB  ;254
 ;PSS*1*224
 I $G(PSSOSN)="" S PSSFWSC=$$PRNSCHD^PSSDSUTL(PSSFWSC) F PSSFWLP2=0:0 S PSSFWLP2=$O(^PS(51.1,"APPSJ",PSSFWSC,PSSFWLP2)) Q:'PSSFWLP2!(PSSFWFLG)!$G(PSSDCFLG)!$G(PSSDIVFG)  D STN1^PSSDSAPB  ;254
 K PSSDIVFG
 Q:$G(PSSDCFLG)
...
STNO ;Standard Logic part 2, using File 51, For Outpatient Orders only
 ...
 I $G(PSSOMEDN)]"" S PSSFWLP3=PSSOMEDN D STNO1^PSSDSAPB  ;254
 ;PSS*1*224
 I $G(PSSOMEDN)="" S PSSFWSC=$$PRNMI^PSSDSUTL(PSSFWSC) F PSSFWLP3=0:0 S PSSFWLP3=$O(^PS(51,"B",PSSFWSC,PSSFWLP3)) Q:'PSSFWLP3!(PSSFWFLG)!$G(PSSDIVFG)  D STNO1^PSSDSAPB  ;254
 K PSSDIVFG
 Q:$G(PSSDCFLG)
 Q:$G(PSSDECNO)
 I PSSFWFLG D DURLS I PSSFWFLG S PSSFWPR1=1 Q
...
UNITD(PSSVUTUN) ;Find First DataBank Unit, can't do DIC Lookup because of exact match check
 ...
 S PSSVUTAA=$$UP^XLFSTR(PSSVUTUN)
UNITDP ;
 K PSSVUTZ S PSSVUTX=$O(^PS(51.24,"FDBNAME",PSSVUTAA,0)) I PSSVUTX,'$$SCREEN^XTID(51.24,.01,PSSVUTX_",") S PSSVUTZ=$P($G(^PS(51.24,PSSVUTX,0)),"^",2) I PSSVUTZ'="",$P($G(^PS(51.24,PSSVUTX,0)),"^",3)=0 G UNITDX
 K PSSVUTZ S PSSVUTX=$O(^PS(51.24,"FDBUNIT",PSSVUTAA,0)) I PSSVUTX,'$$SCREEN^XTID(51.24,.01,PSSVUTX_",") S PSSVUTZ=$P($G(^PS(51.24,PSSVUTX,0)),"^",2) I PSSVUTZ'="",$P($G(^PS(51.24,PSSVUTX,0)),"^",3)=0 G UNITDX
 K PSSVUTZ S PSSVUTX=$O(^PS(51.24,"FDBSYN",PSSVUTAA,0)) I PSSVUTX,'$$SCREEN^XTID(51.24,.01,PSSVUTX_",") S PSSVUTZ=$P($G(^PS(51.24,PSSVUTX,0)),"^",2) I PSSVUTZ'="",$P($G(^PS(51.24,PSSVUTX,0)),"^",3)=0 G UNITDX
