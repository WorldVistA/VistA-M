PSSDSAPA ;BIR/RTR,TMK - Dose Check APIs routine (continued) ; Oct 27, 2009@12:22
 ;;1.0;PHARMACY DATA MANAGEMENT;**151,160,173,178,254**;9/30/97;Build 109
 ;
...
SGENA ;
 ...
 ;
 ;
REM ;Remove certain output globals to comply with 2.1 requirements
 ...
 ....S PSSPERR=$P(PSSDBCAR(PSSRMV2),"^",9) I PSSPERR="OTIC (EAR)"!(PSSPERR="OPHTHALMIC (EYE)")!(PSSPERR="INTRANASAL") D
 .....S ^TMP($J,PSSDBASG,"OUT",PSSRMV1,PSSRMV2,"MESSAGE",".1_INTRO")="Dosing Information provided is PER "_$S(PSSPERR="OTIC (EAR)":"EAR:",PSSPERR="OPHTHALMIC (EYE)":"EYE:",1:"NOSTRIL:")
 ;
 ...
 . D FLTR,TWEAK27B^PSSDSEXF
 ;
 I PSSDBASA D
 . D FLTR1,TWEAK27A^PSSDSEXF
 . D UPCPRS^PSSDSUTL
 ;
...
DUNIT() ;Find unit
 ...
 ;
 ;
FLTR ; backdoor - filtering out top-level messages - *254 FDB 4.5
 N PSSPAR1,PSSPAR2,PSSPAR3,PSSPARX,PSSARR,TEXT,TEXT1,TEXT2,FSTPOS,TEXT2L
 S PSSPAR1="" F  S PSSPAR1=$O(^TMP($J,PSSDBASG,"OUT",PSSPAR1)) Q:PSSPAR1=""  D
 .S PSSPAR2="" F  S PSSPAR2=$O(^TMP($J,PSSDBASG,"OUT",PSSPAR1,PSSPAR2)) Q:PSSPAR2=""  D
 ..S PSSPAR3="" F  S PSSPAR3=$O(^TMP($J,PSSDBASG,"OUT",PSSPAR1,PSSPAR2,"ERROR",PSSPAR3)) Q:PSSPAR3=""  D
 ...I '$D(PSSARR(PSSPAR1,$G(^TMP($J,PSSDBASG,"OUT",PSSPAR1,PSSPAR2,"ERROR",PSSPAR3,"MSG")))) D
 ....S PSSARR(PSSPAR1,$G(^TMP($J,PSSDBASG,"OUT",PSSPAR1,PSSPAR2,"ERROR",PSSPAR3,"MSG")))=PSSPAR3_U_$G(^TMP($J,PSSDBASG,"OUT",PSSPAR1,PSSPAR2,"ERROR",PSSPAR3,"TEXT"))
 ...E  I $G(PSSARR(PSSPAR1,$G(^TMP($J,PSSDBASG,"OUT",PSSPAR1,PSSPAR2,"ERROR",PSSPAR3,"MSG"))))[$P($G(^TMP($J,PSSDBASG,"OUT",PSSPAR1,PSSPAR2,"ERROR",PSSPAR3,"TEXT")),":",2) D
 ....S PSSPARX=$P($G(PSSARR(PSSPAR1,$G(^TMP($J,PSSDBASG,"OUT",PSSPAR1,PSSPAR2,"ERROR",PSSPAR3,"MSG")))),U)
 ....S TEXT2=$P($G(^TMP($J,PSSDBASG,"OUT",PSSPAR1,PSSPAR2,"ERROR",PSSPAR3,"TEXT")),":",2),TEXT2L=$L(TEXT2)
 ....S TEXT1=$G(^TMP($J,PSSDBASG,"OUT",PSSPAR1,PSSPAR2,"ERROR",PSSPARX,"TEXT"))
 ....S FSTPOS=$F(TEXT1,TEXT2)
 ....S TEXT=$E(TEXT1,1,FSTPOS-(TEXT2L))_$E(TEXT1,FSTPOS,999)
 ....I $G(^TMP($J,PSSDBASE,"OUT","DOSE","ERROR",PSSPAR2,PSSPAR1,"TYPE"))'["NoDosageforProfileandOrder" D
 .....S TEXT=$P(TEXT,"Reason(s)",1)_"Reason(s):"_$P(TEXT,":",2)
 ....S ^TMP($J,PSSDBASG,"OUT",PSSPAR1,PSSPAR2,"ERROR",PSSPARX,"TEXT")=TEXT
 ....K ^TMP($J,PSSDBASG,"OUT",PSSPAR1,PSSPAR2,"ERROR",PSSPAR3)
 Q
 ;
 ;
FLTR1 ; CPRS - filtering out top-level messages - *254 FDB 4.5
 N PSSPAR1,PSSPAR2,PSSPARX,PSSARR,TEXT,TEXT1,TEXT2,FSTPOS,TEXT2L
 S PSSPAR1="" F  S PSSPAR1=$O(^TMP($J,PSSDBASF,"OUT","DOSE","ERROR",PSSPAR1)) Q:PSSPAR1=""  D
 .S PSSPAR2="" F  S PSSPAR2=$O(^TMP($J,PSSDBASF,"OUT","DOSE","ERROR",PSSPAR1,PSSPAR2)) Q:PSSPAR2=""  D
 ..I '$D(PSSARR(PSSPAR1,$G(^TMP($J,PSSDBASF,"OUT","DOSE","ERROR",PSSPAR1,PSSPAR2,"MSG")))) D
 ...S PSSARR(PSSPAR1,$G(^TMP($J,PSSDBASF,"OUT","DOSE","ERROR",PSSPAR1,PSSPAR2,"MSG")))=PSSPAR2_U_$G(^TMP($J,PSSDBASF,"OUT","DOSE","ERROR",PSSPAR1,PSSPAR2,"TEXT"))
 ..E  I $G(PSSARR(PSSPAR1,$G(^TMP($J,PSSDBASF,"OUT","DOSE","ERROR",PSSPAR1,PSSPAR2,"MSG"))))[$P($G(^TMP($J,PSSDBASF,"OUT","DOSE","ERROR",PSSPAR1,PSSPAR2,"TEXT")),":",2) D
 ...S PSSPARX=$P($G(PSSARR(PSSPAR1,$G(^TMP($J,PSSDBASF,"OUT","DOSE","ERROR",PSSPAR1,PSSPAR2,"MSG")))),U,1)
 ...S TEXT2=$p($G(^TMP($J,PSSDBASF,"OUT","DOSE","ERROR",PSSPAR1,PSSPAR2,"TEXT")),":",2),TEXT2L=$L(TEXT2)
 ...S TEXT1=$G(^TMP($J,PSSDBASF,"OUT","DOSE","ERROR",PSSPAR1,PSSPARX,"TEXT"))
 ...S FSTPOS=$F(TEXT1,TEXT2)
 ...S TEXT=$E(TEXT1,1,FSTPOS-(TEXT2L))_$E(TEXT1,FSTPOS,999)
 ...I $G(^TMP($J,PSSDBASE,"OUT","DOSE","ERROR",PSSPAR2,PSSPAR1,"TYPE"))'["NoDosageforProfileandOrder" D
 ....S TEXT=$P(TEXT,"Reason(s)",1)_"Reason(s):"_$P(TEXT,":",2)
 ...S ^TMP($J,PSSDBASF,"OUT","DOSE","ERROR",PSSPAR1,PSSPARX,"TEXT")=TEXT
 ...K ^TMP($J,PSSDBASF,"OUT","DOSE","ERROR",PSSPAR1,PSSPAR2)
 Q
