PSJO ;BIR/CML3,PR - GET AND PRINT INPATIENT ORDERS ;Apr 30, 2018@10:05
 ;;5.0;INPATIENT MEDICATIONS;**31,58,110,181,267,275,317,336**;16 DEC 97;Build 6
 ;
...
PUD ; print unit dose
 ...
 ;S PSJRNDT=$$LASTREN^PSJLMPRI(DFN,PSJO) S:PSJRNDT PSJRNDT=$E($$ENDTC^PSGMI(+PSJRNDT),1,5) ;#336
 S PSJRNDT=$$LASTREN^PSJLMPRI(DFN,PSJO) S:PSJRNDT PSJRNDT=$E($$ENDTC2^PSGMI(+PSJRNDT),1,10) ;#336
 D DRGDISP^PSJLMUT1(PSGP,+PSJO_PSJORFLG,40,54,.MARX,0)
 F X=0:0 S X=$O(MARX(X)) Q:'X  W @($S(X=1:"?9",1:"!?11")),$S($E(PSJS)="*":$P(PSJS,"^"),1:MARX(X)) D:X=1
 . ;W ?50,$S(PSJC["NZ":"?",PSJSCHT'="z":PSJSCHT,1:"?") ;#336
 . W ?48,$S(PSJC["NZ":"?",PSJSCHT'="z":PSJSCHT,1:"?") ;#336
 . ;W:'$D(PSJEXTP) ?53,$S(PSJC["NZ":"*****",1:$E($$ENDTC^PSGMI(PSGID),1,5)),?60,$S(PSJC["NZ":"*****",1:$E($$ENDTC^PSGMI(SD),1,5)),?67,STAT ;#336
 . W:'$D(PSJEXTP) ?51,$S(PSJC["NZ":"*****",1:$E($$ENDTC2^PSGMI(PSGID),1,10)),?62,$S(PSJC["NZ":"*****",1:$E($$ENDTC2^PSGMI(SD),1,10)),?73,STAT ;#336
 . ;W:$D(PSJEXTP) ?53,$S(PSJC["NZ":"*****",1:$E($$ENDTC^PSGMI(PSGID),1,8)),?63,$S(PSJC["NZ":"*****",1:$E($$ENDTC^PSGMI(SD),1,8)),?73,STAT ;#336
 . W:$D(PSJEXTP) ?51,$S(PSJC["NZ":"*****",1:$E($$ENDTC2^PSGMI(PSGID),1,10)),?62,$S(PSJC["NZ":"*****",1:$E($$ENDTC2^PSGMI(SD),1,10)),?73,STAT ;#336
 . ;I NF!WS!SM!PF!$G(PADE)!(PSJRNDT]"") W ?71 W:NF "NF " W:(WS&'PADE) "WS " W:(WS&PADE) "WP " W:(PADE&'WS) "PD " W:SM $E("HSM",SM,3) W:$G(PSJRNDT) PSJRNDT W:PF ?79,"*" ;#336
 . I NF!WS!SM!PF!$G(PADE) W ?76 W:NF "NF" W:(WS&'PADE) "WS" W:(WS&PADE) "WP" W:(PADE&'WS) "PD" W:SM $E("HSM",SM,3) W:PF ?77,"*" ;#336
 . I (PSJRNDT]"") W !,?51,"Renewed: ",PSJRNDT ;#336
