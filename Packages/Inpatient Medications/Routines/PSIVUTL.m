PSIVUTL ;BIR/MLM - IV UTILITIES ;Apr 30, 2018@08:47
 ;;5.0;INPATIENT MEDICATIONS ;**69,58,81,85,110,133,181,263,275,279,336**;16 DEC 97;Build 6
 ;
...
PIV(ON) ; Display IV orders.
 ...
 .S ND14=$G(^PS(55,DFN,"IV",+ON,14,0)),ND14=$P(ND14,U,3) S:ND14 ND14=+$G(^(ND14,0))  ;#336 - Retrieve Renewal Dt, if any
 ...
 NEW RNDTPRT S RNDTPRT=0   ;#336 - Keep track if Renewal Date printed
...
SOL ;
 ...
 . ;F X=0:0 S X=$O(NAME(X)) Q:'X  W ?12 W NAME(X) I X=1,DRGX=1,'$D(DRG("AD",1)) D PIV1 ;#336
 . F X=0:0 S X=$O(NAME(X)) Q:'X  W ?12 W NAME(X) D:(DRGX=2!(X=2))&('$D(DRG("AD",1))) RENEWDT I X=1,DRGX=1,'$D(DRG("AD",1)) D PIV2  ;#336
 I 'RNDTPRT,$G(ND14)]"" W ! D RENEWDT
 Q
PIVAD ; Print IV Additives.
 NEW NAME,PSGX
 D NAME(DRG("AD",DRGX),39,.NAME,1)
 ;F PSGX=0:0 S PSGX=$O(NAME(PSGX)) Q:'PSGX  W:(DRGX'=1!(PSGX'=1)) ! W ?9,NAME(PSGX) I PSGX=1,DRGX=1 D PIV1 ;#336
 F PSGX=0:0 S PSGX=$O(NAME(PSGX)) Q:'PSGX  W:(DRGX'=1!(PSGX'=1)) ! W ?9,NAME(PSGX) D:(DRGX=2!(PSGX=2)) RENEWDT I PSGX=1,DRGX=1 D PIV2  ;#336
 Q
 ;
PIV1 ; Print Sched type, start/stop dates, and status.
 ...
 Q
PIV2 ; Print Sched type, start/stop dates with four digit year, and status. ;#336
 F X=2,3 S P(X)=$E($$ENDTC2^PSGMI(P(X)),1,$S($D(PSJEXTP):10,1:10))
 I '$D(PSJEXTP) W ?48,TYP,?51,P(2),?62,P(3),?73,$S($G(P(25))]"":P(25),1:P(17)) Q
 W ?48,TYP,?51,P(2),?62,P(3),?73,$S($G(P(25))]"":P(25),1:P(17))
 Q
RENEWDT ; 336 - Put renewal date on 2nd line instead of 1st.
 Q:$G(ND14)'[""  Q:RNDTPRT  S ND14=$$ENDTC2^PSGMI(ND14)
 W ?51,"Renewed: ",$P(ND14," ") S RNDTPRT=1
 Q
