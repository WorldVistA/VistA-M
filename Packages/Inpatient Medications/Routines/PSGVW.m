PSGVW ;BIR/CML3 - EXPANDED VIEW OF AN ORDER ;Feb 16, 2018@08:59
 ;;5.0;INPATIENT MEDICATIONS;**50,58,85,80,104,110,267,315,336**;16 DEC 97;Build 6
...
L S SCH=$P(AT,"^"),STD=$S(STAT'["P":$P(AT,"^",2),1:""),FD=$S(STAT'["P":$P(AT,"^",4),1:""),FQC=$P(AT,"^",6),AT=$P(AT,"^",5),RT=$P(RT,U,2) I FQC="D",AT="" S AT=$E($P(STD,".",2)_"0000",1,4) ;*315
 ...
 ;F Q="FD","LID","OD","STD" S @Q=$$ENDTC^PSGMI(@Q) ;#336
 F Q="LID","OD" S @Q=$$ENDTC^PSGMI(@Q) ;#336
 F Q="FD","STD" S @Q=$$ENDTC2^PSGMI(@Q) ;#336
 Q:$D(PSJLM)
 ;
WRT ;
 Q:($G(@(F_"0)"))="")
 ...
 ;N RNDT,PSGLRN S PSGLRN=$$LASTREN^PSJLMPRI(PSGP,PSGORD) I PSGLRN S RNDT=$$ENDTC^PSGMI(+PSGLRN) W ?46,"Renewed: ",RNDT ;#336
 N RNDT,PSGLRN S PSGLRN=$$LASTREN^PSJLMPRI(PSGP,PSGORD) I PSGLRN S RNDT=$$ENDTC2^PSGMI(+PSGLRN) W ?46,"Renewed: ",RNDT ;#336
