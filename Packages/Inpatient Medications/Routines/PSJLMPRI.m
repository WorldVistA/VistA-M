PSJLMPRI ;BIR/MLM - INPATIENT LISTMAN IV PROFILE UTILITIES ;Apr 24, 2018@12:13
 ;;5.0;INPATIENT MEDICATIONS;**58,85,118,110,133,154,181,275,336**;16 DEC 97;Build 6
 ;
...
SOL ;
 ...
 NEW DRGX,NAME
 ; #336 - Added dot notation (prompted by new call to RENEWDT API) beginning on next line to avoid ln longer than 245 bytes issue.
 S DRG=0 F  S DRG=+$O(DRG("SOL",DRG)) Q:'DRG  D
 . D NAME^PSIVUTL(DRG("SOL",DRG),37,.NAME,0)
 . S DRGX=0 F  S DRGX=$O(NAME(DRGX)) Q:'DRGX  D
 .. S PSJL=$$SETSTR^VALM1(NAME(DRGX),PSJL,12,60)
 ...
 D RENEWDT I PSJL["Renewed" D SETTMP,SETSTAT   ; #336 - in case only one output line, get RENEW info, if any.
 Q
PIVAD ; Print IV Additives.
 NEW NAME
 D NAME^PSIVUTL(DRG("AD",DRG),39,.NAME,1)
 ; #336 - Added calls to RENEWDT API in two lines immediately below.
 I $D(NAME(2)) S PSJL=$$SETSTR^VALM1(NAME(1),PSJL,9,60) D:'$G(PSJIVFLG) RENEWDT D:$G(PSJIVFLG) PIV1 D SETTMP,SETSTAT S PSJL="",PSJL=$$SETSTR^VALM1(NAME(2),PSJL,9,60) D RENEWDT,SETTMP,SETSTAT
 I '$D(NAME(2)) S PSJL=$$SETSTR^VALM1(NAME(1),PSJL,9,60) D:'$G(PSJIVFLG) RENEWDT D:$G(PSJIVFLG) PIV1 D SETTMP,SETSTAT
 Q
 ;
PIV1 ; Print Sched type, start/stop dates, and status.
 ;K PSJIVFLG,PSJCLP I $G(PSJCLORD),$G(ON)["P" N PSJCLP S PSJCLP(2)=$P(^PS(53.1,+ON,2),"^",2),PSJCLP(3)=$P(^PS(53.1,+ON,2),"^",4) F X=2,3 S PSJCLP(X)=$E($$ENDTC^PSGMI(PSJCLP(X)),1,$S($D(PSJEXTP):8,1:5)) ;#336
 K PSJIVFLG,PSJCLP I $G(PSJCLORD),$G(ON)["P" N PSJCLP S PSJCLP(2)=$P(^PS(53.1,+ON,2),"^",2),PSJCLP(3)=$P(^PS(53.1,+ON,2),"^",4) F X=2,3 S PSJCLP(X)=$E($$ENDTC2^PSGMI(PSJCLP(X)),1,$S($D(PSJEXTP):8,1:10)) ;#336
 ;F X=2,3 S P(X)=$E($$ENDTC^PSGMI(P(X)),1,$S($D(PSJEXTP):8,1:5)) ;#336
 F X=2,3 S P(X)=$E($$ENDTC2^PSGMI(P(X)),1,$S($D(PSJEXTP):8,1:10)) ;#336
 ;I '$D(PSJEXTP) S PSJL=$$SETSTR^VALM1(TYP,PSJL,50,1),PSJL=$$SETSTR^VALM1($S($G(PSJCLP(2)):PSJCLP(2),1:P(2)),PSJL,53,7) D   ;#336
 ;.S PSJL=$$SETSTR^VALM1($S($G(PSJCLP(3)):PSJCLP(3),1:P(3)),PSJL,60,7),PSJL=$$SETSTR^VALM1($S($G(P(25))]"":P(25),1:P(17)),PSJL,66,2)  ;#336
 ...
 ;    ,PSJL=$$SETSTR^VALM1($S(PSJOL'="L"&($G(P(25))]""):P(25),1:P(17)),PSJL,73,2) ;#336 (line too long so breaking into dot structure necessary)
 I '$D(PSJEXTP) S PSJL=$$SETSTR^VALM1(TYP,PSJL,50,1),PSJL=$$SETSTR^VALM1($S($G(PSJCLP(2)):PSJCLP(2),1:P(2)),PSJL,53,10) D  ;#336
 . S PSJL=$$SETSTR^VALM1($S($G(PSJCLP(3)):PSJCLP(3),1:P(3)),PSJL,64,10),PSJL=$$SETSTR^VALM1($S($G(P(25))]"":P(25),1:P(17)),PSJL,75,2) ;#336
 E  D   ;#336
 . S PSJL=$$SETSTR^VALM1(TYP,PSJL,50,1)  ;#336
 . S PSJL=$$SETSTR^VALM1($S($G(PSJCLP(2)):PSJCLP(2),1:P(2)),53,10)  ;#336
 . S PSJL=$$SETSTR^VALM1($S($G(PSJCLP(3)):PSJCLP(3),1:P(3)),PSJL,64,10)  ;#336
 . S PSJL=$$SETSTR^VALM1($S(PSJOL'="L"&($G(P(25))]""):P(25),1:P(17)),PSJL,75,2)  ;#336
 ;I $G(ND14) S ND14=$$ENDTC^PSGMI((ND14)) S PSJL=$$SETSTR^VALM1(ND14,PSJL,$S($D(PSJEXTP):75,1:72),5) K ND14  ;#336
 ;* D SETTMP
 Q
RENEWDT ; 336 - Put renewal date on 2nd line instead of 1st.
 Q:$G(ND14)']""  S PSJL=$$SETSTR^VALM1("Renewed:",PSJL,53,8)
 S ND14=$$ENDTC2^PSGMI((ND14)) S PSJL=$$SETSTR^VALM1(ND14,PSJL,62,10) K ND14
 Q
 ;
SETTMP ;
