EHMAPPTZ ;ALB/WTC - EHRM APPOINTMENT MAINTENANCE; Jun 05, 2025@14:52:28
 ...
 ;
 ;  Clone of EHMAPPT but reports show Encounter IEN.
 ;
 Q  ;
 ;
...
CNVTDAPT(RPTYPE,CONVDATE,SORTORDR,CLINFLTR,CLINICS,FILTER,NONCOUNT,QUEUED,INCLCANC,ACTREQ) ;
 ...
 . ;
 . S APPTDTTM=CONVDATE-.000001 F  S APPTDTTM=$O(^SC(IEN,"S",APPTDTTM)) Q:'APPTDTTM  S IEN2=0 F  S IEN2=$O(^SC(IEN,"S",APPTDTTM,1,IEN2)) Q:'IEN2  S X=^(IEN2,0) D  ;
 .. ;
...
CNVTDLST ;
 ...
 ;
 N RPTYPE,CONVDATE,SORTORDR,DFN,APPTDTTM,CLINIC,X1,X2,X3,LASTFI,SORT1,SORT2,SORT3,OUTPTFMT,Y,POP,%ZIS,DIRUT,QUEUED,OUTPTFMT,TITLE ;
 ;
 ...
 . S ZTRTN="CNVTDLS1^EHMAPPTZ",ZTDESC="Converted Appointment List" ;
 . S ZTSAVE("*")="" ;
...
HEADER(TITLE,CONVDATE,SORTORDR,LISTFMT,PAGE) ;
 ...
 . W "Date/Time",?16,"Patient",?48,"Clinic",?80,"Status",?88,"Status",?111,"Unique ID/IEN" W:$G(LISTFMT)=2 ?125,"Made/Made By" W ! ;
 ...
 . W "Clinic",?32,"Date/Time",?48,"Patient",?80,"Status",?88,"Status",?111,"Unique ID/IEN" W:$G(LISTFMT)=2 ?125,"Made/Made By" W ! ;
 ...
 I SORTORDR=3 D  ;
 . W ?32,"Appointment",?80,"Appt",?88,"[-------------Encounter------------]" W:$G(LISTFMT)=2 ?125,"Appointment" W ! ;
 . W "Patient",?32,"Date/Time",?48,"Clinic",?80,"Status",?88,"Status",?111,"Unique ID/IEN"  W:$G(LISTFMT)=2 ?125,"Made/Made By" W ! ;
 . W $$DASHES^EHM13UTIL(30),?32,$$DASHES^EHM13UTIL(14),?48,$$DASHES^EHM13UTIL(30),?80,$$DASHES^EHM13UTIL(6),?88,$$DASHES^EHM13UTIL(21),?111,$$DASHES^EHM13UTIL(13) ;
 . W:$G(LISTFMT)=2 ?125,$$DASHES^EHM13UTIL(20) W ! ;
 ;
 Q  ;
...
APPTLSTF(TITLE,CONVDATE,SORTORDR,LISTFMT,QUEUED) ;  Formatted Report
 ...
 .... I $G(LISTFMT)=2 W ?125,$$FMTE^XLFDT($P($P(PTAPPT,U,19),".",1),2) ;
 .... W !?111,ENCNTR ;
 .... I $G(LISTFMT)=2 W ?125,$$LASTFI^EHM13UTIL(,$$GET1^DIQ(200,$P(PTAPPT,U,18),.01)) ;
 .... W ! ;
 ... I SORTORDR=2 D  ;
 ...
 .... I $G(LISTFMT)=2 W ?125,$$LASTFI^EHM13UTIL(,$$GET1^DIQ(200,$P(PTAPPT,U,18),.01)) ;
 .... W ! ;
 ... S LINES=LINES+2,RECRDCT=RECRDCT+1 ;
...
APPTLSTC(TITLE,SORTORDR,LISTFMT) ;  Comma-delimited file
 ...
 ; 
 I SORTORDR=1 D  ;
 . I $G(LISTFMT)'=2 W $$COMMAOUT^EHM13UTIL(8,"Appt Date/Time","Patient","SSN","Clinic","Appt Status","Encounter Status","Encounter ID","Encounter IEN"),! ;
 . E  W $$COMMAOUT^EHM13UTIL(10,"Appt Date/Time","Patient","SSN","Clinic","Status","Encounter Status","Encounter ID","Encounter IEN","Appt Made","Made By"),! ;
 I SORTORDR=2 D  ;
 . I $G(LISTFMT)'=2 W $$COMMAOUT^EHM13UTIL(8,"Clinic","Appt Date/Time","Patient","SSN","Appt Status","Encounter Status","Encounter ID","Encounter IEN",),! ;
 . E  W $$COMMAOUT^EHM13UTIL(10,"Clinic","Appt Date/Time","Patient","SSN","Appt Status","Encounter Status","Encounter ID","Encounter IEN","Appt Made","Made By"),! ;
 I SORTORDR=3 D  ;
 . I $G(LISTFMT)'=2 W $$COMMAOUT^EHM13UTIL(8,"Patient","SSN","Appt Date/Time","Clinic","Appt Status","Encounter Status","Encounter ID","Encounter IEN"),! ;
 . E  W $$COMMAOUT^EHM13UTIL(10,"Patient","SSN","Appt Date/Time","Clinic","Appt Status","Encounter Status","Encounter ID","Encounter IEN","Appt Made","Made By"),! ;
 ;
 ;  Scan sorted data in ^TMP($J)
 ;
 ...
 ... ;
 ... I SORTORDR=1 D  ;
 .... I $G(LISTFMT)'=2 W $$COMMAOUT^EHM13UTIL(8,FMTDTTM,VADM(1),$P($P(VADM(2),U,2),"-",3),$$GET1^DIQ(44,CLINIC,.01),APPTSTS,ENCTRSTS,UNIQUEID,ENCNTR),! ;
 .... E  W $$COMMAOUT^EHM13UTIL(10,FMTDTTM,VADM(1),$P($P(VADM(2),U,2),"-",3),$$GET1^DIQ(44,CLINIC,.01),APPTSTS,ENCTRSTS,UNIQUEID,ENCNTR,$$FMTE^XLFDT($P($P(PTAPPT,U,19),".",1),2),$$GET1^DIQ(200,$P(PTAPPT,U,18),.01)),! ;
 ... I SORTORDR=2 D  ;
 .... I $G(LISTFMT)'=2 W $$COMMAOUT^EHM13UTIL(8,$$GET1^DIQ(44,CLINIC,.01),FMTDTTM,VADM(1),$P($P(VADM(2),U,2),"-",3),APPTSTS,ENCTRSTS,UNIQUEID,ENCNTR),! ;
 .... E  W $$COMMAOUT^EHM13UTIL(10,$$GET1^DIQ(44,CLINIC,.01),FMTDTTM,VADM(1),$P($P(VADM(2),U,2),"-",3),APPTSTS,ENCTRSTS,UNIQUEID,ENCNTR,$$FMTE^XLFDT($P($P(PTAPPT,U,19),".",1),2),$$GET1^DIQ(200,$P(PTAPPT,U,18),.01)),! ;
 ... I SORTORDR=3 D  ;
 .... I $G(LISTFMT)'=2 W $$COMMAOUT^EHM13UTIL(8,VADM(1),$P($P(VADM(2),U,2),"-",3),FMTDTTM,$$GET1^DIQ(44,CLINIC,.01),APPTSTS,ENCTRSTS,UNIQUEID,ENCNTR),! ;
 .... E  W $$COMMAOUT^EHM13UTIL(10,VADM(1),$P($P(VADM(2),U,2),"-",3),FMTDTTM,$$GET1^DIQ(44,CLINIC,.01),APPTSTS,ENCTRSTS,UNIQUEID,ENCNTR,$$FMTE^XLFDT($P($P(PTAPPT,U,19),".",1),2),$$GET1^DIQ(200,$P(PTAPPT,U,18),.01)),! ;
 ;
 Q  ;
 ;
