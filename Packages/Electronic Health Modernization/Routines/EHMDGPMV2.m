EHMDGPMV2 ;ALB/WTC - EHRM - AUTOMATED PATIENT DISCHARGE TOOL; Mar 18, 2025@13:02:27
...
SEL2 ; DISABLED EHM*1*15 ;S DGPMN=0 W !! W:'DGPM1X "Select " W DGPMUC," DATE:  ",DEF W $S(DEF]"":"// ",1:"") R X:DTIME G Q:'$T!(X["^") I X["?" D SHOW G SEL2
 S DGPMN=0 W !! W DGPMUC," DATE:  ",DEF W $S(DEF]"":"// ",1:"") S X="" ;R X:DTIME G Q:'$T!(X["^") I X["?" D SHOW G SEL2 - WTC 7/15/2024
...
CONT S DGPMY=+Y,DGPMDA=$S($D(^UTILITY("DGPMVD",$J,+Y)):+^(Y),1:"") I DGPMT=1!(DGPMT=4) S DGPMCA=+DGPMDA,DGPMAN=$S($D(^DGPM(DGPMCA,0)):^(0),1:DGPMY)
 ; DISABLED EHM*1*15 ;K %DT D ^DGPMV21,SCHDADM^DGPMV22:DGPMT=1&DGPMN,^DGPMV3:DGPMY I $D(DGPME) W:DGPME'="***" !,DGPME G SEL
 K %DT D ^DGPMV21,SCHDADM^DGPMV22:DGPMT=1&DGPMN,DGPMV3^EHMDGPMV3:DGPMY I $D(DGPME) W:DGPME'="***" !,DGPME G SEL ; WTC 7/16/24
...
OLD ;for previous entries (discharges and check-outs) skip select
 ...
 ; DISABLED EHM*1*15 ;I DGPMY D ^DGPMV3 I $D(DGPME) W !,DGPME G OLD
 I DGPMY D DGPMV3^EHMDGPMV3 I $D(DGPME) W !,DGPME G OLD ; WTC 7/16/24
 D Q Q
