...
DE S DIE="^VAS(393,",DIC=DIE,DP=393,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^VAS(393,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,11) S:%]"" DE(9)=%
 I $D(^("DT")) S %Z=^("DT") S %=$P(%Z,U,7) S:%]"" DE(1)=% S %=$P(%Z,U,8) S:%]"" DE(4)=%
 I $D(^("MSG")) S %Z=^("MSG") S %=$P(%Z,U,1) S:%]"" DE(10)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X2 I X]"",X<$P(^VAS(393,D0,"DT"),"^",1) W !!?5,"Date must be after the DATE DICTATED" S $P(^VAS(393,D0,"DT"),"^",7)="",Y="@7"
 Q
3 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=3 D X3 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X3 I X="" S DGJTST="SIGNED",Y="@2"
 Q
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW="DT;8",DV="P200'",DU="",DLB="REVIEWED BY",DIFLD=10.08
 S DE(DW)="C4^DGJXA3"
 S DU="VA(200,"
 G RE
...
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW="0;11",DV="R*P393.2'",DU="",DLB="STATUS",DIFLD=.11
 ...
 G Z
X9 Q
10 S DW="MSG;1",DV="F",DU="",DLB="COMMENTS",DIFLD=20.01
 G RE
X10 K:$L(X)>200!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
 ;
11 G 0^DIE17
