SD53MY13 ;ALB/DEP- Stop Code/DSS Identifier Update  ; 11/14/12 1:26pm
 ;;5.3;Scheduling;**601**;AUG 13, 1993;Build 11
 ;
 ;**  This patch is used as a Post-Init in a KIDS build to modify the
 ;**  the CLINIC STOP file [^DIC(40.7,] for FY2013 mid year updates.
 ;
 Q
 ;
...
ADD ;** Add DSS IDs
 ...
 D BMES^XPDUTL(" [NOTE: These Stop Codes CANNOT be used UNTIL 4/1/2013]")
...
REACT ;** Reactivate DSS IDs
 ...
 ;Inactivation date is an uneditable field, cannot use DIE to delete  ; so must manually set piece to null if stop code being reactivated.
...
NEW ;DSS IDs to add- ex ;;STOP CODE NAME^NUMBER^RESTRICTION TYPE^RESTRICTION DATE^CDR
 ;;STORE & FORWARD HOME PROV SITE^189^S^4/1/2013
 ;;OBSTETRICS^339^E
 ;;TELE-ICU PATIENT SITE^901^P^4/1/2013
 ;;QUIT
 ;
...
CHNG ;DSS ID name changes- example ;;STOP CODE NAME^NUMBER^^NEW NAME
 ;;PHYSICIAN RESIDENT 2ND ONLY^188^^FELLOW/RESIDENT
 ;;SPEECH PATHOLOGY^204^^SPEECH-LANGUAGE PATHOLOGY
 ;;OB/GYN^404^^GYNECOLOGY
 ;;PRIMARY CARE/SURGERY^427^^ANES SPECIAL PROCS IN OR SUITE
 ;;DES EXAM^448^^INTGRTED DIS EVAL (IDES) EXAM
 ;;WOMENS GEN SPEC CA CARE^704^^WMS SPECIFIC PREVENTIVE CARE
 ;;QUIT
 ;
