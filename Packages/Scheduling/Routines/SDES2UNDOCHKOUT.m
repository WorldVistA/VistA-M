SDES2UNDOCHKOUT ;ALB/ANU/TJB,MCB,MCB,AGW,JDJ - VISTA SCHEDULING RPCS - SDES2 UNDO CHECKOUT ;MAY 22,2026
 ;;5.3;Scheduling;**877,919,909,942,945**;Aug 13, 1993;Build 2
...
UNDOCHECKOUT(JSONRETURN,SDCONTEXT,SDINPUT) ;
 N ERRORS,SDRETURN,PARAMETERS,Y
 ; Newed Y due to it leaking
 D VALCONTEXT^SDES2VALCONTEXT(.ERRORS,.SDCONTEXT)
 I $D(ERRORS) S ERRORS("UndoCheckOut",1)="",ERRORS("Appointment",1)="" D BUILDJSON^SDES2JSON(.JSONRETURN,.ERRORS) Q
 I $G(SDCONTEXT("USER DUZ"))'="" N DUZ D DUZ^XUP(SDCONTEXT("USER DUZ"))
 ;
 S PARAMETERS=$$VALPARAMS(.SDINPUT,.ERRORS)
 I $D(ERRORS) S ERRORS("UndoCheckOut",1)="",ERRORS("Appointment",1)="" D BUILDJSON^SDES2JSON(.JSONRETURN,.ERRORS) Q
 ;
 D UNDOCKOUT(.SDRETURN,PARAMETERS)
 I $D(SDRETURN) D
 . S PARAMETERS("APPOINTMENT IEN")=PARAMETERS
 . D BYAPPTIEN^SDES2GETAPPTRPCS(.SDRETURN,.SDCONTEXT,.PARAMETERS)
 I '$D(SDRETURN) S SDRETURN("UndoCheckOut",1)="",SDRETURN("Appointment",1)=""
...
UNDO(APPTIEN,CLINICIEN,APPTSTARTTIME,DFN) ;
 N FDA84,ERR84,FDA44,ERR44,IENS44,ENCOUNTERIEN,VISITFILEIEN,FDA9000010,ERR9000010,FDA40968,ERR40968,SUBIEN
 ;Newed SUBIEN due to it leaking from SDESCANCELAPPTS
 S FDA84(409.84,APPTIEN_",",.14)="@"
 S FDA84(409.84,APPTIEN_",",100)=$G(SDCONTEXT("ACHERON AUDIT ID"))
...
AFTEREVENT(APPTSTARTTIME,DFN,CLINICIEN,APPTSUBIEN44,SDCIHDL,SDATA) ;
 N %,%DT,%H,%I,ANS,C,DA,DGMT,DGNOCOPF,DGWRT,DSIHCL,DSIHFS,DSPV150,EVN,FS,HL16,HL7STRG,HLA,HLECH,HLFS,HLHDR
 N HLN,HLPARAM,HLQ,HLSAN,HLTYPE,HLX,IEN,IVMZ10F,J,PIVOT,R,REF,S,SDLOC,VA,VACNTRY,VAERR,XQXFLG,XX,Z0,ZMID,ZTSK
 ; Newed all the leaking variables from EVT^SDAMEVT
