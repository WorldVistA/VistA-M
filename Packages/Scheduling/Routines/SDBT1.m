...
BEGIN S DNM="SDBT1",DQ=1+D G B
1 S DW="0;1",DV="M*P200'",DU="",DIFLD=.01,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C1^SDBT1"
 S DU="VA(200,"
 G RE:'D S DQ=2 G 2
...
X1 S DIC("S")="I $$SCREEN^SDUTL2(Y,DT)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="0;2",DV="SX",DU="",DIFLD=.02,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C2^SDBT1"
 S DU="0:NO;1:YES;"
 G RE
