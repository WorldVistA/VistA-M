SDESVVS ;ALB/RJT,TAW,LAB,DJS,ANU - SDEC AND VIDEO VISIT SERVICE (VVS) INTEGRATION ;JAN 06, 2023@10:40
 ;;5.3;Scheduling;**781,784,785,788,790,792,800,801,804,805,818,823,828,833**;;Build 9
...
GETVVSINFO(RETURN,VVSPATIENT,PROVIDERINFO,VVSSYSTEMINFO) ;GET RPC: "SDEC GETVVSBASE" RETURN
 ...
 Q
SAVEVVSID(JSONRETURN,DFN,CLINIC,APPTDATETIME,VVSID) ;Save VVS ID in the SDEC APPOINTMENT file
 ;INPUT:
 ;DFN        - Required Patient ID Pointer to the PATIENT file 2
 ;CLINIC     - Required
 ;APPT       - Required Appointment Date of search in ISO8601 format
 ;VVSID      - Required
 ;RETURN:
 ...
 S ISCLNVALID=$$VALIDATECLN(.ERRORS,$G(CLINIC))
 ...
 ;
 I '$D(ERRORS) S HASFIELDS=$$SVVSID(.ELGFIELDSARRAY,DFN,CLINIC,APPTDATETIME,VVSID)
 I HASFIELDS M RETURN=ELGFIELDSARRAY
 ;
 D BUILDJSON^SDESBUILDJSON(.JSONRETURN,.RETURN)
 D CLEANUP
 Q
 ;
SVVSID(ELGARRAY,DFN,CLINIC,APPT,VVSID) ;Save VVS ID in the SDEC APPOINTMENT file
 ;
 N IEN40984,REC40984,IEN44,RESOURCEIEN,RESOURCECLN
 ;
 ...
 ..S RESOURCECLN=$$GET1^DIQ(409.831,RESOURCEIEN,.04,"I") ; Clinic
 ..Q:CLINIC'=$G(RESOURCECLN)
