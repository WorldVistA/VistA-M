SDES2CANCELAPPT2 ;ALB/MCB,AGW - SDES2 CANCEL APPOINTMENT 2 ; MAY 6, 2026
 ;;5.3;Scheduling;**943,944**;Aug 13, 1993;Build 2
...
CANCELAPPT(JSONRETURN,SDCONTEXT,PARAMS) ;
 N APPTIEN,APPTIENS,GOODPARAMS,SDERRORS,SDRETURN,APPT
 ;
 D VALCONTEXT^SDES2VALCONTEXT(.SDERRORS,.SDCONTEXT)
 I $D(SDERRORS) M SDRETURN=SDERRORS S SDRETURN("Appointment",1)="" S SDRETURN("CompensationPensionCancel",1)="" D BUILDJSON^SDES2JSON(.JSONRETURN,.SDRETURN) Q
 I $G(SDCONTEXT("USER DUZ"))'="" N DUZ D DUZ^XUP(SDCONTEXT("USER DUZ"))
 ;
 S GOODPARAMS=$$VALPARAMS(.PARAMS,.SDERRORS)
 I 'GOODPARAMS,$D(SDERRORS) M SDRETURN=SDERRORS S SDRETURN("Appointment",1)="" S SDRETURN("CompensationPensionCancel",1)="" D BUILDJSON^SDES2JSON(.JSONRETURN,.SDRETURN) Q
 ;
 ...
 I $D(SDERRORS) M SDRETURN=SDERRORS S SDRETURN("Appointment",1)="" S SDRETURN("CompensationPensionCancel",1)="" D BUILDJSON^SDES2JSON(.JSONRETURN,.SDRETURN) Q
 K APPT
 D BUILDAPPTOBJ^SDES2BLDAPPTOBJ(.APPT,PARAMS("APPT IEN"),1,.SDERRORS)
 I $D(SDERRORS) D  Q
 . D ERRLOG^SDESJSON(.SDERRORS,52,"Appointment successfully cancelled; however, return object could not be returned")
 . M SDRETURN=SDERRORS S SDRETURN("Appointment",1)="" S SDRETURN("CompensationPensionCancel",1)="" D BUILDJSON^SDES2JSON(.JSONRETURN,.SDRETURN)
 M APPT("CompensationPensionCancel")=SDRETURN("CompensationPensionCancel")
 D BUILDJSON^SDES2JSON(.JSONRETURN,.APPT)
 Q
 ;
...
DUPPIDCHK(REQUESTIEN,NEWPID) ;
 ...
 Q DUPPID
 ;Cancel Patient's Appt
TRY2CANCEL(SDRETURN,SDCONTEXT,PARAMS,SDERRORS) ;
 ; New line are from leaking variables
 N %DT,C,DSIHCL,DSIHFS,DSPV150,EVN,HL16,HL7STRG,HLA,HLECH,HLFS,HLQ,IEN,J,PIVOT,R,REF,RTN,S,SDCODES,SEQ,SLOTSTATUSSTRING,VA,VACNTRY,VAERR,XQXFLG,XS,XX,Y,Z0
 ...
 ; cancel appointments
 D CANCEL40984^SDES2CANCELAPPT1(.SDERRORS,APPTIEN,CANCELREASONIEN,CANBYCLINORPAT,WALKIN,$G(EAS),.ROLLBACK)
 Q:$D(SDERRORS)
...
UPDATEREQUEST(REQUESTIEN,APPTIEN,CLINICIEN,DFN,CANBYCLINORPAT,NOTE,APPTSTARTTIME,RESOURCE,MRTC,PARENTREQUEST,PARENTSTATUS,NEWPID,SDERROR,REQUESTTYPE) ;
 ...
 .D REOPEN^SDESRECALLREQ(.RECALLRET,APPTIEN,,NEWPID,CANBYCLINORPAT,,$G(SDCONTEXT("ACHERON AUDIT ID")))
 .I '$D(RECALLRET) Q
