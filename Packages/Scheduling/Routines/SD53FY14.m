SD53FY14 ;ALB/DEP- Stop Code/DSS Identifier Update 6/18/07 ; 9/25/12 2:21pm
 ;;5.3;Scheduling;**608**;AUG 13, 1993;Build 13
 ;
 ;**  This patch is used as a Post-Init in a KIDS build to modify the
 ;**  the CLINIC STOP file [^DIC(40.7,] for FY2014 updates.
 ;
 Q
 ;
POST ;** Add/inactivate/change/reactivate DSS IDs (stop codes)
 ;**  The following code executes if file modifications exist
 ;
 N SDI,SDTAG,SDVAR,SDAUMF,TYPE
 S SDAUMF=1
...
ADD ;** Add DSS IDs
 ...
 D BMES^XPDUTL(" [NOTE: These Stop Codes CANNOT be used UNTIL 10/1/2013]")
...
NEW ;DSS IDs to add- ex ;;STOP CODE NAME^NUMBER^RESTRICTION TYPE^RESTRICTION DATE^CDR
 ;;ALS CENTER^346^E
 ;;ALS HOME CARE PROGRAM^347^E
 ;;HOSPITAL AT HOME^354^E
 ;;TELETRANSPLANT PT SITE^490^S^10/1/2013
 ;;TELETRANSPLANT PROV SITE^491^S^10/1/2013
 ;;RRTP INDIVIDUAL^586^E
 ;;RRTP GROUP^587^E
 ;;QUIT
 ;
...
CHNG ;DSS ID name changes- example ;;STOP CODE NAME^NUMBER^^NEW NAME
 ;;COMPLEMENTARY & ALTERNATIVE RX^159^^COMP AND ALTERNATIVE MED
 ;;QUIT
 ;
...
REST ;Change restriction - ;;STOP CODE NAME^NUMBER^REST TYPE^RES DATE^OLD
 ;;COMP AND ALTERNATIVE MED^159^S^10/1/2013^E
 ;;QUIT
