SDCCRSEN ;CCRA/LB,PB - Appointment retrieval API;APR 4, 2019
 ;;5.3;Scheduling;**707,730,735,764,768,741,795,808,822,841,865,912,936**;APR 4, 2019;Build 65
 Q
 ...
 ; Reference to $$HLPHONE^HLFNC in ICR #10106
...
PROCMSG(MSG1) ; Process message
 ...
 N GMRCDFN,GMRCTIU,GMRCTIUS,CID,ADDTXT,GMRCATIU,STID,PROVIDERADDRESS,PROVIDERPHONE,RAWSEG,APTTM,DFN,CONID,CONTITLE,PROVIDER,SRVNAME1,SRVNAMEX,LOC,PROV,SDECRESA,DIVID,NEWAPPT
 K SDECSTART,SDECEND,SDDFN,PROVADD,SITECODE,SDECRES,SDECLEN,SDECNOTE,SDECATID,SDECCR,SDMRTC,SDDDT,SDREQBY,SDLAB,PROVIEN,SDID,SDAPTYP,SDSVCP,SDSVCPR,SDCL,SDEKG,SDXRAY,APPTYPE,EESTAT,OVB,SDPARENT,SDEL
 ...
 I NEWAPPT=1&(MSGARY("EVENT")="SCHEDULE")&(+$G(QUIT)'=1)  D ADDCOMMENT^SDCCRSEN2(SDECSTART,PROV,PROV1,PROVIDERADDRESS,PROVIDERPHONE)
 I NEWAPPT=1&(MSGARY("EVENT")="CANCEL")&(+$G(QUIT)'=1)  D ADDCANCOMMENT^SDCCRSEN2(SDECSTART,PROV,PROV1,PROVIDERADDRESS,PROVIDERPHONE)
 I NEWAPPT=1&(MSGARY("EVENT")="NOSHOW")&(+$G(QUIT)'=1)  D NOSHOWCOMMENT^SDCCRSEN2(SDECSTART,PROV,PROV1,PROVIDERADDRESS,PROVIDERPHONE)
 I $G(NEWAPPT)=1 Q 0
 I +$G(ABORT)=2 D APPMSG^SDCCRCOR(MID,.ABORT) Q 1
 I +$G(QUIT)=1 Q 1
 S QUIT=0
 S NEWAPPT=0 S NEWAPPT=$$CHKAPPT^SDCCRSEN2(CONID,MSGARY("EVENT"))
 I MSGARY("EVENT")="SCHEDULE" D MAKE^SDCCRSEN1 ;D:NEWAPPT=1 ADDCOMMENT^SDCCRSEN2(SDECSTART,PROV,PROV1,PROVIDERADDRESS,PROVIDERPHONE)
 I MSGARY("EVENT")="CANCEL" D CANCEL^SDCCRSEN1 ;D:NEWAPPT=1 ADDCANCOMMENT^SDCCRSEN2(SDECSTART,PROV,PROV1,PROVIDERADDRESS,PROVIDERPHONE)
 I MSGARY("EVENT")="NOSHOW" D NOSHOW^SDCCRSEN1 ;D:NEWAPPT=1 NOSHOWCOMMENT^SDCCRSEN2(SDECSTART,PROV,PROV1,PROVIDERADDRESS,PROVIDERPHONE)
 D DONEINC^SDCCRCOR
...
AIL(AIL,MSGARY,PROVDTL,BASEDTE) ;AIL segment processing.
 ...
 S PROVADD=$G(AIL(3,1,1))
 ;N CH1 
 ;S CH1("\\T\\")="&" S PROVADD=$$REPLACE^XLFSTR(PROVADD,.CH1)
 N CH1  ;,PROVIDERADDRESS,SITE,PROVIDERPHONE
 S CH1("&&")="AND"
 ...
 S SITE=$P(AIL(3,1,1),"-",1),PROVIDERADDRESS=$P(AIL(3,1,1),"-",2)
 S PROVIDERADDRESS=$$REPLACE^XLFSTR(PROVIDERADDRESS,.CH1)
 K CH1 S CH1("\T\")="&",PROVIDERADDRESS=$$REPLACE^XLFSTR(PROVIDERADDRESS,.CH1)
 S PROVIDERPHONE=$P(AIL(3,1,1),"-",3),PROVIDERPHONE=$P(PROVIDERPHONE,";",2),PROVIDERPHONE=$$HLPHONE^HLFNC(PROVIDERPHONE)
...
AIP(AIP,MSGARY,PROVDTL,BASEDTE) ;AIP segment processing.
 ...
 S PROV=$G(AIP(3,1,2)),PROV1=$G(AIP(3,1,3)),PROV=PROV_" "_PROV1
 N CHG1 S CHG1("\\T\\")="&",PROV=$$REPLACE^XLFSTR(PROV,.CHG1),PROV1=$$REPLACE^XLFSTR(PROV1,.CHG1)
 N CHG1
 S CHG1("&&")="AND"
 S PROV=$$REPLACE^XLFSTR(PROV,.CHG1),NPI=AIP(3,1,1)
 Q
 ;
