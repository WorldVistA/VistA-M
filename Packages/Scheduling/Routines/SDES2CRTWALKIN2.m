SDES2CRTWALKIN2  ;ALB/AGW - VISTA SCHEDULING WALK-IN RPC ; APR 03 2026
 ;;5.3;Scheduling;**941**;Aug 13, 1993;Build 3
...
CREATEWALKIN(JSONRETURN,SDCONTEXT,PARAMS) ;
 ;
 N X,Y,REQRESULT,REQRESULT1,REQARRAY,APPTARRAY,APPTRESULT,%,APPOINTMENT,DISPRESULT,IEN,APPTIEN,DSIHFS
 N APPTRETURN,REQRETURN,CHECKIN,DA,DIK,SDERRORS,SDPARAMS,VALRETURN,SDCLINSTOP,RLIST,NOD
 N APPTRESULTS,C,DGMT,DGNOCOPF,DSIHCL,DSPV150,EVN,HL16,HL7STRG,HLA,HLECH,HLFS,HLQ,SDMODE
 N IVMZ10F,J,VA,VAERR,VEFS12,VEFS9,VEFSFAC,VESPID,VEFSQ,XX,Z0,ZTSK,REF,R,PIVOT,S,VACNTRY,VEFSPID
 ;
 ...
 . M REQRESULT1("Error")=REQRESULT("Error")
 . D BUILDERROR(.REQRESULT1,.JSONRETURN)
 ;
 D BLDAPPTARRAY(.APPTARRAY,.PARAMS)
 K TMPJSONRETURN
 D CREATE^SDES2CREATEAPPT2(.APPTRETURN,.SDCONTEXT,.APPTARRAY)
 ...
 ;
 S SDPARAMS("APPOINTMENT IEN")=APPTRESULT("Appointment",1,"AppointmentIEN")
 S SDPARAMS("CHECKIN DATE TIME")=$$FMTISO^SDAMUTDT($$NOW^XLFDT)
 D CHECKIN^SDES2CHECKIN(.CHECKIN,.SDCONTEXT,.SDPARAMS)
 I CHECKIN(1)="{""Checkin"":""Checked in""}" D BUILDAPPTOBJ^SDES2BLDAPPTOBJ(.APPTRESULTS,APPTRESULT("Appointment",1,"AppointmentIEN"),1,.SDERRORS)
