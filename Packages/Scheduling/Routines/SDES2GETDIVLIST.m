SDES2GETDIVLIST ;ALB/BWF,AGW - SDES2 GET DIVISION LIST ;Aug 14,2025
 ;;5.3;Scheduling;**899,909**;Aug 13, 1993;Build 12
 ;;Per VHA Directive 6402, this routine should not be modified
 ; Reference to DUZ^XUP is supported by IA #7487
 ;
 Q
 ; SDCONTEXT               - standard SDCONTEXT array
 ; SDINPUT("SEARCH TERM")  - search term for division lookup
 ;
GETDIVISIONLIST(RESULTS,SDCONTEXT,SDINPUT) ;
 N DATA,FACILITYNUM,FACARRY,ERRORS,ERR,SEARCHTRM
 ;
 D VALCONTEXT^SDES2VALCONTEXT(.ERRORS,.SDCONTEXT)
 I $D(ERRORS) S ERRORS("Division",1)="" D BUILDJSON^SDESBUILDJSON(.RESULTS,.ERRORS) Q
 I $G(SDCONTEXT("USER DUZ"))'="" N DUZ D DUZ^XUP(SDCONTEXT("USER DUZ"))
 ;
 S SEARCHTRM=$G(SDINPUT("SEARCH TERM"))
