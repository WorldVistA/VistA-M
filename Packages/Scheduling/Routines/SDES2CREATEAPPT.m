SDES2CREATEAPPT  ;ALB/BLB,LAB/BLB,ANU,TJB,LAB,TJB,ANU,BLB,TJB,BLB,JAS,MCB,JDJ,JAS,JAS,LAB,TJB,JHC,TJB,AGW - SDES2 CREATE APPOINTMENT ; MAY 08, 2026
 ;;5.3;Scheduling;**866,869,873,875,877,880,881,886,893,895,898,901,905,907,925,919,920,909,930,944**;Aug 13, 1993;Build 2
...
CREATE(JSON,SDCONTEXT,APPOINTMENT) ;
 ...
 N IEN,IVMZ10F,J,PIVOT,R,REF,RTN,S,SCHEMAIL,SEQ,VA,VACNTRY,VAERR,XQXFLG,XX,Y,Z0
 ;
 D VALCONTEXT^SDES2VALCONTEXT(.ERRORS,.SDCONTEXT)
 I $D(ERRORS) S ERRORS("Appointment","IEN")="",ERRORS("Appointment","Message")="" D BUILDJSON^SDES2JSON(.JSON,.ERRORS) Q
 I $G(SDCONTEXT("USER DUZ"))'="" N DUZ D DUZ^XUP(SDCONTEXT("USER DUZ"))
 ;
 D VALIDATE(.ERRORS,.SDCONTEXT,.APPOINTMENT,.VAL)
 I $D(ERRORS) S ERRORS("Appointment","IEN")="",ERRORS("Appointment","Message")="" D BUILDJSON^SDES2JSON(.JSON,.ERRORS) Q
 ;
 D LOCKREQ^SDES2APPTUTIL(.ERRORS,.APPOINTMENT,$P($G(APPOINTMENT("REQUEST TYPE")),"|"),$P($G(APPOINTMENT("REQUEST TYPE")),"|",2),$G(APPOINTMENT("DFN")),"L")
 I $D(ERRORS) S ERRORS("Appointment","IEN")="",ERRORS("Appointment","Message")="" D BUILDJSON^SDES2JSON(.JSON,.ERRORS) Q
 ...
 I $D(ERRORS) D  Q
 .D LOCKREQ^SDES2APPTUTIL(.ERRORS,.APPOINTMENT,$P($G(APPOINTMENT("REQUEST TYPE")),"|"),$P($G(APPOINTMENT("REQUEST TYPE")),"|",2),$G(APPOINTMENT("DFN")),"U")
 .S ERRORS("Appointment","IEN")="",ERRORS("Appointment","Message")="" D BUILDJSON^SDES2JSON(.JSON,.ERRORS)
 .Q
 ...
 ;
 ;
 I $D(ERRORS) S ERRORS("Appointment","IEN")=APPTIEN,ERRORS("Appointment","Message")=$G(APPTMSG) D BUILDJSON^SDES2JSON(.JSON,.ERRORS) Q
 ;
