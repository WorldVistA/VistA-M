...
R1 D DE
 ...
 ;
2 S DQ=3 ;@30
3 S DW="0;11",DV="F",DU="",DLB="PHYSICAL LOCATION",DIFLD=10
 G RE
X3 K:$L(X)>25!($L(X)<1) X
 I $D(X),X'?.ANP K X
 Q
 ;
4 S DW="SL;5",DV="*P44'",DU="",DLB="PRINCIPAL CLINIC",DIFLD=1916
 S DU="SC("
 G RE
X4 S DIC("S")="I $P(^(0),""^"",3)=""C"",'$G(^(""OOS""))" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
5 S DW="SL;7",DV="RNJ4,0",DU="",DLB="OVERBOOKS/DAY MAXIMUM",DIFLD=1918
 G RE
...
R6 D DE
 G A
 ;
7 S DW="SL;1",DV="RNJ2,0X",DU="",DLB="LENGTH OF APP'T",DIFLD=1912
 G RE
...
X8 I '$D(SDLA) S SDLA=X
 Q
9 S DW="SL;2",DV="S",DU="",DLB="VARIABLE APP'NTMENT LENGTH",DIFLD=1913
...
X10 S:+$O(^SC(DA,"ST",0))>0 Y="@99"
 Q
11 S DW="SL;6",DV="RSX",DU="",DLB="DISPLAY INCREMENTS PER HOUR",DIFLD=1917
