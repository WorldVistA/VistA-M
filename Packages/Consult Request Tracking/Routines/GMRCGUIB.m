...
REASON(GMRCFN,GMRCRQ,GMRCDT) ;Load the reason for the request into ^GMR(123,GMRCO,20
 ...
 S L=0,LN=1 F  S L=$O(GMRCRQ(L)) Q:L=""  S GMRCRQ(L)=$$CHKTXT^GMRCHL7U(GMRCRQ(L)),^GMR(123,GMRCFN,20,LN,0)=GMRCRQ(L),LN=LN+1
 S LN=LN-1,$P(^GMR(123,GMRCFN,20,0),"^",3)=LN
 K LN,L
 Q
SETCOM(COMMENT,WHO) ;Set comment array into tracking actions
 N GMRCNOW,DR,DIE,L
 S L=0 F  S L=$O(COMMENT(L)) Q:L=""  S COMMENT(L)=$$CHKTXT^GMRCHL7U(COMMENT(L))
 S GMRCNOW=$$NOW^XLFDT
