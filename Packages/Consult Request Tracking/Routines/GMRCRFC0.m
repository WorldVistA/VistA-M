GMRCRFC0 ;EHRM/JCH - Process IFC HL7 Messages; May 23, 2025@11:20:28
 ;;3.0;CONSULT/REQUEST TRACKING;**154,184,207**;DEC 27, 1997;Build 10
 ;
 Q
...
TST ;
 ...
 .D EXIT
 ;
 ;  File comment in ACTION DATE multiple.  p207 wtc 1/13/25
 ;
 I RMPRST="IP" D  Q
 . N RMPRORC2,RMPRORC3,RMPROBR4,RMPRORC5 ;
 . S RMPRORC5=$P(^TMP("RMPRIF",$J,"ORC"),"|",5) Q:RMPRORC5'="IP"  ;
 . S RMPRORC2=$P($G(^TMP("RMPRIF",$J,"ORC")),"|",2)
 . S RMPRORC3=$P($G(^TMP("RMPRIF",$J,"ORC")),"|",3)
 . S RMPROBR4=$P($G(^TMP("RMPRIF",$J,"OBR")),"|",4)
 . I ($P(RMPRORC2,U,2)=$P(RMPRORC3,U,2)),((RMPROBR4["PROSTHETICS IFC")!(RMPROBR4["PSAS")),$$IEN^XUAF4($P(RMPRORC2,U,2))=$$KSP^XUPARAM("INST"),$D(^GMR(123,+RMPRORC3,0)) D
 .. N RMPRIEN ;
 .. S RMPRIEN="" F  S RMPRIEN=$O(^RMPR(668,"D",RMPR123A,RMPRIEN)) Q:RMPRIEN=""  D  ; ICR #7131
 ... N ACTIEN,FDA,I,N,ERR,X ;
 ... ;
 ... S FDA(1,668.011,"+1,"_RMPRIEN_",",.01)=$$NOW^XLFDT ;
 ... D UPDATE^DIE("","FDA(1)","ACTIEN","ERR") S ACTIEN=ACTIEN(1) ;
 ... ;
 ... D TRIMWP^GMRCIUTL($NA(^TMP("RMPRIF",$J,"OBX",3)),5) ;
 ... D WP^DIE(668.011,ACTIEN_","_RMPRIEN_",",1,"K",$NA(^TMP("RMPRIF",$J,"OBX",3))) ;
 . D EXIT
 ;
