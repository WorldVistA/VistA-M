...
X2 S X=$$UP^XLFSTR(X) K:$L(X)>30!($L(X)<1)!$$BADADD^IBCEP8B(X) X I $D(X),$P($G(^IBA(355.93,DA,0)),U,2)'=1 K X
 ...
 Q
 ;
3 S DW="0;10",DV="FX",DU="",DIFLD=.1,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
X3 K:$L(X)>30!($L(X)<1) X I $D(X),$P($G(^IBA(355.93,DA,0)),U,2)'=1 K X
 I $D(X),X'?.ANP K X
 Q
 ;
4 S DW="0;6",DV="FX",DU="",DIFLD=.06,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
X4 K:$L(X)>20!($L(X)<1) X I $D(X),$P($G(^IBA(355.93,DA,0)),U,2)'=1 K X
 I $D(X),X'?.ANP K X
 Q
 ;
5 S DW="0;7",DV="P5'X",DU="",DIFLD=.07,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="DIC(5,"
 G RE
X5 I $D(X),$P($G(^IBA(355.93,DA,0)),U,2)'=1 K X
 Q
 ;
6 S DW="0;8",DV="RFX",DU="",DIFLD=.08,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
X6 K:$L(X)>10!($L(X)<9)!'((X?9N)!(X?5N1"-"4N))!($E(X,$L(X)-3,$L(X))="0000") X I $D(X),$P($G(^IBA(355.93,DA,0)),U,2)'=1 K X
 I $D(X),X'?.ANP K X
 Q
 ;
7 S DQ=8 ;@1041
8 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=8 D X8 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X8 S:$P(DIPA("NVA_FC-0"),U,9)'="" Y="@1042"
 Q
9 S DW="0;9",DV="FX",DU="",DIFLD=.09,DLB="LAB OR FACILITY PRIMARY ID"
 S DE(DW)="C9^IBXSAH2"
 G RE
...
X11 S:$P(DIPA("NVA_FC-0"),U,11)'="" Y="@1043"
 Q
12 D:$D(DG)>9 F^DIE17,DE S DQ=12,DW="0;11",DV="S",DU="",DIFLD=.11,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X14 S:$P(DIPA("NVA_FC-0"),U,14)'="" Y="@1044"
 Q
...
X17 S:$D(^IBA(355.93,+DIPA("NVA_FC"),"TAXONOMY"))>0 Y="@1045"
 Q
18 S D=0 K DE(1) ;42
 S DIFLD=42,DGO="^IBXSAH12",DC="3^355.9342IP^TAXONOMY^",DV="355.9342M*P8932.1'",DW="0;1",DOW=$$LABEL^DIALOGZ(DP,DIFLD),DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="USC(8932.1,"
