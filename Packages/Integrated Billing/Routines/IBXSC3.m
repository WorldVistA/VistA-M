IBXSC3 ; GENERATED FROM 'IB SCREEN3' INPUT TEMPLATE(#574), FILE 399;08/13/09
 D DE G BEGIN
...
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW="0;19",DV="R*P353'",DU="",DLB="FORM TYPE",DIFLD=.19
 S DE(DW)="C7^IBXSC3",DE(DW,"INDEX")=1
 S DU="IBE(353,"
 G RE
...
C7S S X="" G:DG(DQ)=X C7F1 K DB
 ...
 S X=DG(DQ),DIC=DIE
 D ATTREND^IBCU1(DA,"","")
C7F1 S DIEZRXR(399,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=820 S DIEZRXR(399,DIXR)=""
 Q
X7 S DIC("S")="N Z S Z=$G(^IBE(353,Y,2)) I $P(Z,U,2)=""P"",$P(Z,U,4)" D ^DIC K DIC S DIC=$G(DIE),X=+Y K:Y<0 X
 Q
 ;
...
11 D:$D(DG)>9 F^DIE17,DE S DQ=11,DW="0;19",DV="R*P353'",DU="",DLB="FORM TYPE",DIFLD=.19
 S DE(DW)="C11^IBXSC3",DE(DW,"INDEX")=1
 S DU="IBE(353,"
 S X=$G(DIPA("FT1"))
 S Y=X
...
C11S S X="" G:DG(DQ)=X C11F1 K DB
 ...
 S X=DG(DQ),DIC=DIE
 D ATTREND^IBCU1(DA,"","")
C11F1 S DIEZRXR(399,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=820 S DIEZRXR(399,DIXR)=""
 Q
X11 S DIC("S")="N Z S Z=$G(^IBE(353,Y,2)) I $P(Z,U,2)=""P"",$P(Z,U,4)" D ^DIC K DIC S DIC=$G(DIE),X=+Y K:Y<0 X
 Q
 ;
