IB20P842 ;MNTVBB/FSB - POST-INIT FOR IB*2.0*842 ; DEC 1, 2025@13:00
 ;;2.0;INTEGRATED BILLING;**842**;21-MAR-94;Build 1
 ;;Per VA Directive 6402, this routine should not be modified.
 ;
 Q
PRE ; set up check points for pre/post-init
 N %
 S %=$$NEWCP^XPDUTL("THRESH","THRESH^IB20P842")
 S %=$$NEWCP^XPDUTL("MCRDED","MCRDED^IB20P842")
 S %=$$NEWCP^XPDUTL("PRIOR","PRIOR^IB20P842")
 Q
 ;
THRESH ; Pension Threshold
 N IBA,IBERRM,IBRN,IBTYPE,IBX,DA,DIK,IBADLDEP
 S IBTYPE="Pension Threshold"
 D BMES^XPDUTL("Filing CY 2026 Pension Threshold rates.")
 S IBX=3251200 ;set IBX so that it will pick up all record on or after the new effective date
 F  S IBX=$O(^IBE(354.3,"B",IBX)) Q:'IBX  D  ; remove any records since 12/01/2023
 . S IBRN=0
 . F  S IBRN=$O(^IBE(354.3,"B",IBX,IBRN)) Q:'IBRN  D
 .. S DIK="^IBE(354.3,",DA=IBRN D ^DIK
 S IBA(354.3,"+1,",.01)=3251201 ; effective date for CY 2026 values
 S IBA(354.3,"+1,",.02)=1 ;     internal value 1 = BASIC PENSION
 S IBA(354.3,"+1,",.03)="17441" ;  0 dependents - base rate for veteran
 S IBA(354.3,"+1,",.04)="22839" ; 1 dependent
 S IBADLDEP="2984" ;  additional dependent amount
 ...
 D UPDATE^DIE("","IBA","","IBERRM") ; file the new record for CY 2026
 I $D(IBERRM) D
...
PRIOR ;This code sets up the variables and calls the routine to print or print-and-update the
 ...
 ;
 Q:'$D(^IBA(354.1,"APRIOR",3241201))  ; quit if the "APRIOR" x-ref is not set for 12/1/24.
 ...
 S IBBMES=$S($D(ZTSK):"This job has been queued for NOW, as task number "_ZTSK_".",1:"This job could not be queued. Please edit the 12/1/24 threshold through the 'Add Income Thresholds' option, which allows you to queue this job.")
...
MMSG ; MailMan message to report update problem to billing groups, patch installer and patch developer
 ...
 S IBPARAM("FROM")="PATCH IB*2.0*842 CY 2026 RATE UPDATE"
 F IBGROUP="IB EDI SUPERVISOR","IB ERROR","MCCR" D
 . I $D(^XMB(3.8,"B",IBGROUP)) S IBGROUP="G."_IBGROUP,XMY(IBGROUP)=""
 S XMY(DUZ)=""
 ;
 S IBC=0
 S IBC=IBC+1,IBTXT(IBC)="This message has been sent by patch IB*2.0*842. If you have received this"
 S IBC=IBC+1,IBTXT(IBC)="message, it indicates that the patch encountered some difficulty in filing"
 S IBC=IBC+1,IBTXT(IBC)="the CY 2026 "_IBTYPE_" rates as outlined in the patch description."
 ...
 S IBC=IBC+1,IBTXT(IBC)="You can consult the IB*2.0*842 patch description for additional information."
