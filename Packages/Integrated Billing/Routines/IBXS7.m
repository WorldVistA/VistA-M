IBXS7 ; GENERATED FROM 'IB SCREEN7' INPUT TEMPLATE(#513), FILE 399;10/26/11
 D DE G BEGIN
...
X2 S:$D(IBOUT) Y="@999"
 Q
3 S D=0 K DE(1) ;43
 S DIFLD=43,DGO="^IBXS71",DC="1^399.043DA^OP^",DV="399.043MDX",DW="0;1",DOW="OP VISITS DATE(S)",DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
...
C17F1 Q
X17 Q
18 D:$D(DG)>9 F^DIE17,DE S DQ=18,DW="U;12",DV="*P399.1'",DU="",DLB="DISCHARGE STATUS",DIFLD=162
 S DU="DGCR(399.1,"
 G RE
X18 S DIC("S")="I $P(^DGCR(399.1,+Y,0),""^"",6)=1" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
19 S DW="0;22",DV="P40.8'",DU="",DLB="DEFAULT DIVISION",DIFLD=.22
...
C19F1 S DIEZRXR(399,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 ...
 Q
X19 Q
20 D:$D(DG)>9 F^DIE17,DE S DQ=20,DW="U3;2",DV="*P8932.1'",DU="",DLB="SERVICE FACILITY TAXONOMY",DIFLD=243
 S DU="USC(8932.1,"
 G RE
...
X21 D DISPTAX^IBCEP81($P($G(^DGCR(399,DA,"U3")),U,2),"Service Facility")
 Q
22 S DW="0;27",DV="S",DU="",DLB="BILL CHARGE TYPE",DIFLD=.27
 S DE(DW)="C22^IBXS7"
...
X25 D CKFT^IBCIUT1(IBIFN)
 Q
26 S DQ=27 ;@714
27 D:$D(DG)>9 F^DIE17,DE S DQ=27,DW="0;19",DV="R*P353'",DU="",DLB="FORM TYPE",DIFLD=.19
...
X30 W !,*7,"Must be a printable national form type"
 Q
31 D:$D(DG)>9 F^DIE17,DE S DQ=31,DW="0;19",DV="R*P353'",DU="",DLB="FORM TYPE",DIFLD=.19
...
X36 S:IBDR20'["72" Y="@73"
 Q
37 D:$D(DG)>9 F^DIE17,DE S DQ=37,DW="U;5",DV="RFOX",DU="",DLB="IS THIS A SENSITIVE RECORD?",DIFLD=155
...
X38 S:X=0 Y=156
 Q
39 S DW="U;7",DV="FOX",DU="",DLB="R.O.I. FORM(S) COMPLETED?",DIFLD=157
...
X39 I $D(X) D YN^IBCU S:$G(X)=1 X=$$ROI399^IBNCPDR4($G(DA))
 ...
 Q
 ;
40 S DW="U;6",DV="RFOX",DU="",DLB="ASSIGNMENT OF BENEFITS",DIFLD=156
...
X41 S:'$D(IBOX) Y="@73"
 Q
42 S DW="U;3",DV="RFOX",DU="",DLB="POWER OF ATTORNEY COMPLETED?",DIFLD=153
...
X44 S:IBDR20'["73" Y="@75"
 Q
45 S DW="U;1",DV="RDX",DU="",DLB="STATEMENT COVERS FROM",DIFLD=151
 S DE(DW)="C45^IBXS7"
 G RE
...
X45 S %DT="ETP" D ^%DT S X=Y K:Y<1 X I $D(X) D DDAT^IBCU4 K IB00
 Q
 ;
46 D:$D(DG)>9 F^DIE17,DE S DQ=46,DW="U;2",DV="RDX",DU="",DLB="STATEMENT COVERS TO",DIFLD=152
 S DE(DW)="C46^IBXS7"
 G RE
