IBXS7 ; GENERATED FROM 'IB SCREEN7' INPUT TEMPLATE(#513), FILE 399;01/29/18
 D DE G BEGIN
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGCR(399,DA,""))=""
 ...
 I $D(^("U")) S %Z=^("U") S %=$P(%Z,U,1) S:%]"" DE(45)=% S %=$P(%Z,U,2) S:%]"" DE(46)=% S %=$P(%Z,U,3) S:%]"" DE(42)=% S %=$P(%Z,U,5) S:%]"" DE(37)=% S %=$P(%Z,U,6) S:%]"" DE(40)=% S %=$P(%Z,U,7) S:%]"" DE(39)=% S %=$P(%Z,U,12) S:%]"" DE(18)=%
...
C27F3 S DIEZRXR(399,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=820 S DIEZRXR(399,DIXR)=""
 Q
X27 S DIC("S")="N Z S Z=$G(^IBE(353,Y,2)) I $P(Z,U,2)=""P"",$P(Z,U,4)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
C31F3 S DIEZRXR(399,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=820 S DIEZRXR(399,DIXR)=""
 Q
X31 S DIC("S")="N Z S Z=$G(^IBE(353,Y,2)) I $P(Z,U,2)=""P"",$P(Z,U,4)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
X38 S:X=0 Y=156
 Q
39 S DW="U;7",DV="FOX",DU="",DIFLD=157,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DQ(39,2)="S Y(0)=Y S Y=$S(Y:""YES"",Y=0:""NO"",1:"""")"
 G RE
X39 I $D(X) D YN^IBCU S:$G(X)=1 X=$$ROI399^IBNCPDR4($G(DA))
 I $D(X),X'?.ANP K X
 Q
 ;
40 S DW="U;6",DV="RFOX",DU="",DIFLD=156,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DQ(40,2)="S Y(0)=Y S Y=$S(Y="""":"""",""Yy1""[Y:""YES"",""Nn0""[Y:""NO"",1:"""")"
 G RE
X40 I $D(X) D YN^IBCU I $D(X) X:X=0 ^DD(399,156,9.3) K IBRATY
 I $D(X),X'?.ANP K X
 Q
 ;
...
X41 S:'$D(IBOX) Y="@73"
 Q
42 S DW="U;3",DV="RFOX",DU="",DIFLD=153,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DQ(42,2)="S Y(0)=Y S Y=$S(Y:""YES"",Y=0:""NO"",1:"""")"
 G RE
X42 I $D(X) D YN^IBCU
 I $D(X),X'?.ANP K X
 Q
...
X44 S:IBDR20'["73" Y="@75"
 Q
45 S DW="U;1",DV="RDX",DU="",DIFLD=151,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C45^IBXS7"
 G RE
...
C45S S X="" G:DG(DQ)=X C45F1 K DB
 D ^IBXS72
C45F1 Q
X45 S %DT="ETP" D ^%DT S X=Y K:Y<1 X I $D(X) D DDAT^IBCU4 K IB00
 Q
 ;
46 D:$D(DG)>9 F^DIE17,DE S DQ=46,DW="U;2",DV="RDX",DU="",DIFLD=152,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C46^IBXS7"
 G RE
...
C46S S X="" G:DG(DQ)=X C46F1 K DB
 ...
 S DGRVRCAL=1
C46F1 Q
X46 S %DT="ETP" D ^%DT S X=Y K:Y<1 X I $D(X) D DDAT1^IBCU4 K IB00
 Q
 ;
47 S DQ=48 ;@75
48 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=48 D X48 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X48 S:IBDR20'["75" Y="@76"
 Q
49 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=49 D X49 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X49 D RCD^IBCU1
 Q
50 D:$D(DG)>9 F^DIE17,DE S DQ=50,D=0 K DE(1) ;42
 S DIFLD=42,DGO="^IBXS73",DC="16^399.042IPA^RC^",DV="399.042MR*P399.2'",DW="0;1",DOW=$$LABEL^DIALOGZ(DP,DIFLD),DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="DGCR(399.2,"
 G RE:D I $D(DSC(399.042))#2,$P(DSC(399.042),"I $D(^UTILITY(",1)="" X DSC(399.042) S D=$O(^(0)) S:D="" D=-1 G M50
 S D=$S($D(^DGCR(399,DA,"RC",0)):$P(^(0),U,3,4),$O(^(0))'="":$O(^(0)),1:-1)
M50 I D>0 S DC=DC_D I $D(^DGCR(399,DA,"RC",+D,0)) S DE(50)=$P(^(0),U,1)
 G RE
R50 D DE
 S D=$S($D(^DGCR(399,DA,"RC",0)):$P(^(0),U,3,4),1:1) G 50+1
 ;
51 D:$D(DG)>9 F^DIE17 G ^IBXS74
