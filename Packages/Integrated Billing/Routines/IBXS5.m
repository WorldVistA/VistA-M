IBXS5 ; GENERATED FROM 'IB SCREEN5' INPUT TEMPLATE(#511), FILE 399;11/02/18
 D DE G BEGIN
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGCR(399,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,3) S:%]"" DE(9)=% S %=$P(%Z,U,9) S:%]"" DE(24)=%,DE(32)=%
...
X2 S:$D(IBOUT) Y="@999"
 Q
3 S D=0 K DE(1) ;43
 S DIFLD=43,DGO="^IBXS51",DC="1^399.043DA^OP^",DV="399.043MDX",DW="0;1",DOW=$$LABEL^DIALOGZ(DP,DIFLD),DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
...
R3 D DE
 ...
 ;
4 S DW="U;8",DV="S",DU="",DIFLD=158,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X8 S:IBDR20'["51" Y="@52"
 Q
9 S DW="0;3",DV="RDX",DU="",DIFLD=.03,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C9^IBXS5"
 G RE
...
15 D:$D(DG)>9 F^DIE17,DE S DQ=15,D=0 K DE(1) ;41
 S DIFLD=41,DGO="^IBXS52",DC="4^399.041IPA^OC^",DV="399.041M*P399.1'X",DW="0;1",DOW=$$LABEL^DIALOGZ(DP,DIFLD),DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="DGCR(399.1,"
...
X18 S:IBDR20'["58" Y="@54"
 Q
19 S D=0 K DE(1) ;40
 S DIFLD=40,DGO="^IBXS53",DC="1^399.04PA^CC^",DV="399.04M*P399.1'X",DW="0;1",DOW=$$LABEL^DIALOGZ(DP,DIFLD),DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="DGCR(399.1,"
...
X23 I IBZ20="",$$FT^IBCEF(DA)=7 S Y="@80"
 Q
24 S DW="0;9",DV="SX",DU="",DIFLD=.09,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X27 S:IBDR20'["59" Y="@999"
 Q
28 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=28 D X28 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X28 I $P(^DGCR(399,DA,0),U,19)=2 S Y="@999"
 Q
29 S D=0 K DE(1) ;47
 S DIFLD=47,DGO="^IBXS54",DC="2^399.047PA^CV^",DV="399.047M*P399.1'X",DW="0;1",DOW=$$LABEL^DIALOGZ(DP,DIFLD),DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="DGCR(399.1,"
 G RE:D I $D(DSC(399.047))#2,$P(DSC(399.047),"I $D(^UTILITY(",1)="" X DSC(399.047) S D=$O(^(0)) S:D="" D=-1 G M29
 S D=$S($D(^DGCR(399,DA,"CV",0)):$P(^(0),U,3,4),$O(^(0))'="":$O(^(0)),1:-1)
M29 I D>0 S DC=DC_D I $D(^DGCR(399,DA,"CV",+D,0)) S DE(29)=$P(^(0),U,1)
 G RE
R29 D DE
 S D=$S($D(^DGCR(399,DA,"CV",0)):$P(^(0),U,3,4),1:1) G 29+1
 ;
30 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=30 D X30 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X30 S Y="@999"
 Q
31 S DQ=32 ;@80
32 S DW="0;9",DV="SX",DU="",DIFLD=.09,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="4:CPT-4;5:HCPCS (HCFA COMMON PROCEDURE CODING SYSTEM);9:ICD;"
 S Y="HCPCS"
 G Y
X32 S:X=4 X=5
