...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=2,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 ...
 I $D(^(.25)) S %Z=^(.25) S %=$P(%Z,U,1) S:%]"" DE(18)=% S %=$P(%Z,U,2) S:%]"" DE(20)=%
...
X2 K:$L(X)>30!($L(X)<1) X
 ...
 Q
 ;
3 S DW=".311;15",DV="RSa",DU="",DLB="EMPLOYMENT STATUS",DIFLD=.31115
 S DE(DW)="C3^IBXSC21"
...
18 S DW=".25;1",DV="FX",DU="",DLB="SPOUSE'S EMPLOYER NAME",DIFLD=.251
 S DE(DW)="C18^IBXSC21"
 G RE
C18 G C18S:$D(DE(18))[0 K DB S X=DE(18),DIC=DIE
 X "S DGXRF=.251 D ^DGDDC Q"
C18S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ;
 Q
...
20 D:$D(DG)>9 F^DIE17,DE S DQ=20,DW=".25;2",DV="FX",DU="",DLB="SPOUSE'S EMP STREET [LINE 1]",DIFLD=.252
 S DE(DW)="C20^IBXSC21"
 G RE
C20 G C20S:$D(DE(20))[0 K DB S X=DE(20),DIC=DIE
 X "S DGXRF=.252 D ^DGDDC Q"
C20S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ;
 Q
