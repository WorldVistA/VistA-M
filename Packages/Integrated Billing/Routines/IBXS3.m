IBXS3 ; GENERATED FROM 'IB SCREEN3' INPUT TEMPLATE(#574), FILE 399;05/12/15
 D DE G BEGIN
...
X26 S IBHPID=$$HPD^IBCNHUT1(IBINS)
 Q
27 D:$D(DG)>9 F^DIE17,DE S DQ=27,DW="M1;13",DV="NJ10,0X",DU="",DLB="PRIMARY INSURANCE HPID",DIFLD=471
 S X=IBHPID
 S Y=X
 G Y
X27 K:+X'=X!(X>9999999999)!(X<1000000000)!(X?.E1"."1N.N)!$$HOD^IBCNHUT1(X) X
 Q
 ;
28 S DW="M;13",DV="FOX",DU="",DLB="SECONDARY INSURANCE POLICY",DIFLD=113
...
X31 S IBHPID=$$HPD^IBCNHUT1(IBINS)
 Q
32 D:$D(DG)>9 F^DIE17,DE S DQ=32,DW="M1;14",DV="NJ10,0X",DU="",DLB="SECONDARY INSURANCE HPID",DIFLD=472
 S X=IBHPID
 S Y=X
 G Y
X32 K:+X'=X!(X>9999999999)!(X<1000000000)!(X?.E1"."1N.N)!$$HOD^IBCNHUT1(X) X
 Q
 ;
33 S DW="M;14",DV="FOX",DU="",DLB="TERTIARY INSURANCE POLICY",DIFLD=114
...
X36 S IBHPID=$$HPD^IBCNHUT1(IBINS)
 Q
37 D:$D(DG)>9 F^DIE17,DE S DQ=37,DW="M1;15",DV="NJ10,0X",DU="",DLB="TERTIARY INSURANCE HPID",DIFLD=473
 S X=IBHPID
 S Y=X
 G Y
