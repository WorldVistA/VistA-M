IBTRKRBA ;WAS/RFJ - claims tracking - random selection bulletin ; 1 Mar 96
 ;;Version 2.0 ; INTEGRATED BILLING ;**56**; 21-MAR-94
 Q
 ;
 ;
ADMTBULL(DFN,IBTRN,DGPMA,IBSPEC) ; -- send admission bulletin for rs
 ...
 S IBT(2)=$$DAT2^IBOUTL($$NOW^XLFDT())_"."
 S IBT(3)=""
 ;
 D PID^VADPT
 ...
 D SEND("UR Random Sample Patient Selection")
 Q
 ;
 ;
SEND(XMSUB)        ;  send bulletin with subject XMSUB
 N %X,XCNP,XMDUZ,XMTEXT,XMY,XMZ
 S XMDUZ="INTEGRATED BILLING PACKAGE",XMTEXT="IBT("
 ;  if no members in mail group, do not send the message
 I '$O(^XMB(3.8,+$O(^XMB(3.8,"B","DGPM UR ADMISSION",0)),1,0)) Q
