IB20P425 ;ALB/LBD - POST-INIT FOR IB*2.0*425; 11/25/2008 ; 11/10/09 2:36pm
 ;;2.0;INTEGRATED BILLING;**425**;21-MAR-94;Build 3
 ;
PRE ; set up check points for pre/post-init
 N %
 S %=$$NEWCP^XPDUTL("THRESH","THRESH^IB20P425")
 S %=$$NEWCP^XPDUTL("MCRDED","MCRDED^IB20P425")
 S %=$$NEWCP^XPDUTL("PRIOR","PRIOR^IB20P425")
 Q
 ;
THRESH ; Pension Threshold
 N IBA,IBERRM,IBRN,IBTYPE,IBX,DA,DIK
 S IBTYPE="Pension Threshold"
 D BMES^XPDUTL("Filing CY 2010 Pension Threshold rates.")
 S IBX=3081201
 F  S IBX=$O(^IBE(354.3,"B",IBX)) Q:'IBX  D  ; remove any records since 12/01/2008
 . S IBRN=0
 . F  S IBRN=$O(^IBE(354.3,"B",IBX,IBRN)) Q:'IBRN  D
 .. S DIK="^IBE(354.3,",DA=IBRN D ^DIK
 S IBA(354.3,"+1,",.01)=3091201 ; effective date for CY 2010 values
 ...
 D UPDATE^DIE("","IBA","","IBERRM") ; file the new record for CY 2010
 I $D(IBERRM) D
 ...
 Q
 ;
MCRDED ; Medicare deductible rate for CY 2010
 ; check to see if rate already entered.
 N IBA,IBERRM,IBIEN,IBRN,IBTYPE,DA,DIK
 S IBTYPE="Medicare Deductible"
 D BMES^XPDUTL("Filing Medicare Deductible Rate for 01/01/2010")
 S IBIEN=0
...
PRIOR ;This code sets up the variables and calls the routine to print or print-and-update the
 ...
 ;
 Q:'$D(^IBA(354.1,"APRIOR",3081201))  ; quit if the "APRIOR" x-ref is not set for 12/1/08.
 ...
 S IBBMES=$S($D(ZTSK):"This job has been queued for NOW, as task number "_ZTSK_".",1:"This job could not be queued. Please edit the 12/1/09 threshold through the 'Add Income Thresholds' option, which allows you to queue this job.")
...
MMSG ; MailMan message to report update problem to billing groups, patch installer and patch developer
 ...
 S IBPARAM("FROM")="PATCH IB*2.0*425 CY 2010 RATE UPDATE"
 F IBGROUP="IB EDI SUPERVISOR","IB ERROR","MCCR" D
 . I $D(^XMB(3.8,"B",IBGROUP)) S IBGROUP="G."_IBGROUP,XMY(IBGROUP)=""
 S XMY(DUZ)=""
 ;
 S IBC=0
 S IBC=IBC+1,IBTXT(IBC)="This message has been sent by patch IB*2.0*425. If you have received this"
 S IBC=IBC+1,IBTXT(IBC)="message, it indicates that the patch encountered some difficulty in filing"
 S IBC=IBC+1,IBTXT(IBC)="the CY 2010 "_IBTYPE_" rates as outlined in the patch description."
 ...
 S IBC=IBC+1,IBTXT(IBC)="You can consult the IB*2.0*425 patch description for additional information."
