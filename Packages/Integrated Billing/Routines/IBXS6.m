IBXS6 ; GENERATED FROM 'IB SCREEN6' INPUT TEMPLATE(#512), FILE 399;10/26/11
 D DE G BEGIN
...
X12 I '$$INPAT^IBCEF(DA)!($$FT^IBCEF(DA)'=3) S Y="@6121"
 Q
13 S DQ=14 ;@6121
14 D:$D(DG)>9 F^DIE17,DE S DQ=14,DW="0;22",DV="P40.8'",DU="",DLB="DEFAULT DIVISION",DIFLD=.22
...
C14F1 S DIEZRXR(399,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 ...
 Q
X14 Q
15 D:$D(DG)>9 F^DIE17,DE S DQ=15,DW="U3;2",DV="*P8932.1'",DU="",DLB="SERVICE FACILITY TAXONOMY",DIFLD=243
 S DU="USC(8932.1,"
 G RE
...
X16 D DISPTAX^IBCEP81($P($G(^DGCR(399,DA,"U3")),U,2),"Service Facility")
 Q
17 S DW="0;27",DV="S",DU="",DLB="BILL CHARGE TYPE",DIFLD=.27
 S DE(DW)="C17^IBXS6"
...
X20 D CKFT^IBCIUT1(IBIFN)
 Q
21 S DQ=22 ;@614
22 D:$D(DG)>9 F^DIE17,DE S DQ=22,DW="0;19",DV="R*P353'",DU="",DLB="FORM TYPE",DIFLD=.19
...
X25 W !,*7,"Must be a printable national form type"
 Q
26 D:$D(DG)>9 F^DIE17,DE S DQ=26,DW="0;19",DV="R*P353'",DU="",DLB="FORM TYPE",DIFLD=.19
...
X31 S:IBDR20'["62" Y="@63"
 Q
32 D:$D(DG)>9 F^DIE17,DE S DQ=32,DW="U;5",DV="RFOX",DU="",DLB="IS THIS A SENSITIVE RECORD?",DIFLD=155
...
X33 S:X=0 Y=156
 Q
34 S DW="U;7",DV="FOX",DU="",DLB="R.O.I. FORM(S) COMPLETED?",DIFLD=157
...
X34 I $D(X) D YN^IBCU S:$G(X)=1 X=$$ROI399^IBNCPDR4($G(DA))
 ...
 Q
 ;
35 S DW="U;6",DV="RFOX",DU="",DLB="ASSIGNMENT OF BENEFITS",DIFLD=156
...
X36 S:'$D(IBOX) Y="@63"
 Q
37 S DW="U;3",DV="RFOX",DU="",DLB="POWER OF ATTORNEY COMPLETED?",DIFLD=153
...
X39 S:IBDR20'["63" Y="@64"
 Q
40 S DW="U;1",DV="RDX",DU="",DLB="STATEMENT COVERS FROM",DIFLD=151
 S DE(DW)="C40^IBXS6"
 G RE
...
X40 S %DT="ETP" D ^%DT S X=Y K:Y<1 X I $D(X) D DDAT^IBCU4 K IB00
 Q
 ;
41 D:$D(DG)>9 F^DIE17,DE S DQ=41,DW="U;2",DV="RDX",DU="",DLB="STATEMENT COVERS TO",DIFLD=152
 S DE(DW)="C41^IBXS6"
 G RE
...
X43 S:IBDR20'["64" Y="@65"
 Q
44 D:$D(DG)>9 F^DIE17,DE S DQ=44,DW="U;11",DV="*P399.1'",DU="",DLB="DISCHARGE BEDSECTION",DIFLD=161
 S DE(DW)="C44^IBXS6"
 S DU="DGCR(399.1,"
 G RE
...
X44 S DIC("S")="I $P(^DGCR(399.1,+Y,0),""^"",5)=1" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
45 D:$D(DG)>9 F^DIE17,DE S DQ=45,DW="U;15",DV="F",DU="",DLB="LENGTH OF STAY",DIFLD=165
 S DE(DW)="C45^IBXS6"
 G RE
...
49 D:$D(DG)>9 F^DIE17,DE S DQ=49,D=0 K DE(1) ;42
 S DIFLD=42,DGO="^IBXS65",DC="15^399.042IPA^RC^",DV="399.042MR*P399.2'",DW="0;1",DOW="REVENUE CODE",DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="DGCR(399.2,"
