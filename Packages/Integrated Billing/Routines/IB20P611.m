IB20P611 ;DEV/BJR - POST-INIT FOR IB*2.0*611 ;10/13/2017 10:50am
 ;;2.0;INTEGRATED BILLING;**611**;21-MAR-94;Build 4
 ;;Per VA Directive 6402, this routine should not be modified.
 ;
 Q
PRE ; set up check points for pre/post-init
 N %
 S %=$$NEWCP^XPDUTL("THRESH","THRESH^IB20P611")
 S %=$$NEWCP^XPDUTL("MCRDED","MCRDED^IB20P611")
 S %=$$NEWCP^XPDUTL("PRIOR","PRIOR^IB20P611")
 Q
 ;
THRESH ; Pension Threshold
 N IBA,IBERRM,IBRN,IBTYPE,IBX,DA,DIK,IBADLDEP
 S IBTYPE="Pension Threshold"
 D BMES^XPDUTL("Filing CY 2018 Pension Threshold rates.")
 S IBX=3161200 ;set IBX so that it will pick up all record on or after the new effective date
 F  S IBX=$O(^IBE(354.3,"B",IBX)) Q:'IBX  D  ; remove any records since 12/01/2017
 . S IBRN=0
 . F  S IBRN=$O(^IBE(354.3,"B",IBX,IBRN)) Q:'IBRN  D
 .. S DIK="^IBE(354.3,",DA=IBRN D ^DIK
 S IBA(354.3,"+1,",.01)=3171201 ; effective date for CY 2018 values
 S IBA(354.3,"+1,",.02)=1 ;     internal value 1 = BASIC PENSION
 S IBA(354.3,"+1,",.03)="13166" ;  base rate for veteran
 S IBA(354.3,"+1,",.04)="17241" ; 1 dependent
 S IBADLDEP="2250" ;  additional dependent amount
 ...
 D UPDATE^DIE("","IBA","","IBERRM") ; file the new record for CY 2018
 I $D(IBERRM) D
...
PRIOR ;This code sets up the variables and calls the routine to print or print-and-update the
 ...
 ;
 Q:'$D(^IBA(354.1,"APRIOR",3161201))  ; quit if the "APRIOR" x-ref is not set for 12/1/16.
 ...
 S IBBMES=$S($D(ZTSK):"This job has been queued for NOW, as task number "_ZTSK_".",1:"This job could not be queued. Please edit the 12/1/17 threshold through the 'Add Income Thresholds' option, which allows you to queue this job.")
...
MMSG ; MailMan message to report update problem to billing groups, patch installer and patch developer
 ...
 S IBPARAM("FROM")="PATCH IB*2.0*611 CY 2018 RATE UPDATE"
 F IBGROUP="IB EDI SUPERVISOR","IB ERROR","MCCR" D
 . I $D(^XMB(3.8,"B",IBGROUP)) S IBGROUP="G."_IBGROUP,XMY(IBGROUP)=""
 S XMY(DUZ)=""
 ;
 S IBC=0
 S IBC=IBC+1,IBTXT(IBC)="This message has been sent by patch IB*2.0*611. If you have received this"
 S IBC=IBC+1,IBTXT(IBC)="message, it indicates that the patch encountered some difficulty in filing"
 S IBC=IBC+1,IBTXT(IBC)="the CY 2018 "_IBTYPE_" rates as outlined in the patch description."
 ...
 S IBC=IBC+1,IBTXT(IBC)="You can consult the IB*2.0*611 patch description for additional information."
