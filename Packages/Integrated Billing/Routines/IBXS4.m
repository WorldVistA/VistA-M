IBXS4 ; GENERATED FROM 'IB SCREEN4' INPUT TEMPLATE(#510), FILE 399;09/02/16
 D DE G BEGIN
...
X2 I $P($G(^DGCR(399,DA,0)),U,8) S Y="@411"
 Q
3 S DW="U;20",DV="FXO",DU="",DIFLD=159.5,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X3 D NOPTF^IBCU
 ...
 ;
4 S DQ=5 ;@411
5 S DW="U;10",DV="FX",DU="",DIFLD=160,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
X5 K:$L(X)>2!($L(X)<1)!(X=99) X
 I $D(X),X'?.ANP K X
 Q
 ;
6 S DW="U;9",DV="S",DU="",DIFLD=159,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 ...
 G RE
X6 Q
7 S DW="U;8",DV="S",DU="",DIFLD=158,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X10 S:IBDR20'["42" Y="@43"
 Q
11 S DW="U;12",DV="*P399.1'",DU="",DIFLD=162,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="DGCR(399.1,"
 G RE
...
X15 S:IBDR20'["46" Y="@47"
 Q
16 S D=0 K DE(1) ;41
 S DIFLD=41,DGO="^IBXS42",DC="4^399.041IPA^OC^",DV="399.041M*P399.1'X",DW="0;1",DOW=$$LABEL^DIALOGZ(DP,DIFLD),DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="DGCR(399.1,"
...
X18 S:IBDR20'["47" Y="@44"
 Q
19 S D=0 K DE(1) ;40
 S DIFLD=40,DGO="^IBXS43",DC="1^399.04PA^CC^",DV="399.04M*P399.1'X",DW="0;1",DOW=$$LABEL^DIALOGZ(DP,DIFLD),DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="DGCR(399.1,"
...
X22 S IBZ20=$P(^DGCR(399,DA,0),U,9)
 Q
23 S DW="0;9",DV="SX",DU="",DIFLD=.09,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X29 I $P(^DGCR(399,DA,0),U,19)=2 S Y="@49"
 Q
30 S D=0 K DE(1) ;47
 S DIFLD=47,DGO="^IBXS44",DC="2^399.047PA^CV^",DV="399.047M*P399.1'X",DW="0;1",DOW=$$LABEL^DIALOGZ(DP,DIFLD),DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="DGCR(399.1,"
