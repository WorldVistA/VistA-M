...
X2 S IBMAED=1,DIE("NO^")=""
 Q
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW="0;2",DV="RNJ10,2",DU="",DLB="CHARGES",DIFLD=.02
 S DE(DW)="C3^IBXS73"
 G RE
...
X4 K IBMAED,DIE("NO^")
 Q
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW="0;3",DV="RNJ6,0X",DU="",DLB="UNITS OF SERVICE",DIFLD=.03
 S DE(DW)="C5^IBXS73"
 G RE
...
X6 K:X?1.10N.1".".2N X
 Q
 ;
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW="0;5",DV="R*P399.1'",DU="",DLB="BEDSECTION",DIFLD=.05
 S DE(DW)="C7^IBXS73"
 S DU="DGCR(399.1,"
 G RE
...
X7 S DIC("S")="I $P(^(0),U,5)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
8 D:$D(DG)>9 F^DIE17,DE S DQ=8,DW="0;9",DV="NJ10,2",DU="",DLB="NON-COVERED CHARGE",DIFLD=.09
 G RE
X8 S:X["$" X=$P(X,"$",2) K:X'?.N.1".".2N!(X>9999999.99)!(X<0) X
 Q
 ;
9 S DW="0;6",DV="*P81'",DU="",DLB="PROCEDURE",DIFLD=.06
 S DE(DW)="C9^IBXS73"
 S DU="ICPT("
 G RE
...
X10 I '$P(^DGCR(399,DA(1),"RC",DA,0),U,6) S Y="@758"
 Q
11 D:$D(DG)>9 F^DIE17,DE S DQ=11,DW="0;7",DV="P40.8'X",DU="",DLB="DIVISION",DIFLD=.07
 S DE(DW)="C11^IBXS73"
 S DU="DG(40.8,"
...
X13 I +$P(^DGCR(399,DA(1),"RC",DA,0),U,8) W !," AUTO ADDED CHARGE - NO CHANGE TO TYPE/COMPONENT" S Y="@759"
 Q
14 D:$D(DG)>9 F^DIE17,DE S DQ=14,DW="0;10",DV="S",DU="",DLB="TYPE",DIFLD=.1
 S DE(DW)="C14^IBXS73"
...
C14F1 Q
X14 Q
15 D:$D(DG)>9 F^DIE17,DE S DQ=15,DW="0;12",DV="S",DU="",DLB="COMPONENT",DIFLD=.12
...
X19 D LINKRX^IBCEU5(DA(1),DA)
 Q
20 S DW="0;15",DV="FXO",DU="",DLB="RX PROCEDURE",DIFLD=.15
