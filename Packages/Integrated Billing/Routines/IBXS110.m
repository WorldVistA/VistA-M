...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=2,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 I $D(^(.121)) S %Z=^(.121) S %=$P(%Z,U,5) S:%]"" DE(1)=% S %=$P(%Z,U,10) S:%]"" DE(3)=% S %=$P(%Z,U,12) S:%]"" DE(2)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="IBXS110",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW=".121;5",DV="*P5'X",DU="",DIFLD=.1215,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C1^IBXS110",DE(DW,"INDEX")=1
 S DU="DIC(5,"
 G RE
...
C1F1 S DIEZRXR(2,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 ...
 Q
X1 S DIC("S")="I $P(^DIC(5,Y,0),U,6)=1" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X I $D(X) S DFN=DA D TAD^DGLOCK Q
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW=".121;12",DV="FOX",DU="",DIFLD=.12112,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DQ(2,2)="S Y(0)=Y D ZIPOUT^VAFADDR"
 S DE(DW)="C2^IBXS110",DE(DW,"INDEX")=1
 G RE
...
C2F1 S DIEZRXR(2,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=603 S DIEZRXR(2,DIXR)=""
 Q
X2 K:X[""""!($A(X)=45) X I $D(X) S DFN=DA D TAD^DGLOCK I $D(X) K:$L(X)>20!($L(X)<5) X I $D(X) D ZIPIN^VAFADDR
 I $D(X),X'?.ANP K X
 Q
 ;
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW=".121;10",DV="FX",DU="",DIFLD=.1219,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C3^IBXS110"
 G RE
...
C3S S X="" G:DG(DQ)=X C3F1 K DB
 ...
 D EVENT^IVMPLOG(DA)
C3F1 Q
X3 K:$L(X)>20!($L(X)<4) X I $D(X) S DFN=DA D TAD^DGLOCK
 I $D(X),X'?.ANP K X
 Q
 ;
4 S DQ=5 ;@915
