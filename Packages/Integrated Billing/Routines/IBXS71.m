IBXS71 ; ;01/15/20
 D DE G BEGIN
DE S DIE="^DGCR(399,D0,""OP"",",DIC=DIE,DP=399.043,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^DGCR(399,D0,"OP",DA,""))=""
...
BEGIN S DNM="IBXS71",DQ=1+D G B
1 S DW="0;1",DV="MDX#",DU="",DIFLD=.01,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C1^IBXS71"
 G RE:'D S DQ=2 G 2
...
C1S S X="" G:DG(DQ)=X C1F1 K DB
 ...
 S DGRVRCAL=1
C1F1 Q
X1 S %DT="E" D ^%DT S X=Y K:Y<1!('$D(IBIFN))!('$$OPV^IBCU41(X,IBIFN)) X I $D(X) S DINUM=X
 Q
 ;
2 G 1^DIE17
