...
DE S DIE="^DGCR(399,D0,""RC"",",DIC=DIE,DP=399.042,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^DGCR(399,D0,"RC",DA,""))=""
 ...
 I  S %=$P(%Z,U,9) S:%]"" DE(6)=% S %=$P(%Z,U,10) S:%]"" DE(12)=% S %=$P(%Z,U,12) S:%]"" DE(13)=% S %=$P(%Z,U,15) S:%]"" DE(18)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="IBXS77",DQ=1+D G B
1 S DW="0;1",DV="MR*P399.2'",DU="",DLB="REVENUE CODE",DIFLD=.01
 S DE(DW)="C1^IBXS77",DE(DW,"INDEX")=1
 S DU="DGCR(399.2,"
 G RE:'D S DQ=2 G 2
...
X1 S DIC("S")="I +$P(^(0),U,3)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="0;2",DV="RNJ10,2",DU="",DLB="CHARGES",DIFLD=.02
 S DE(DW)="C2^IBXS77"
 G RE
...
C2S S X="" G:DG(DQ)=X C2F1 K DB
 ...
 D 21^IBCU2
C2F1 Q
X2 S:X["$" X=$P(X,"$",2) K:X'?.N.1".".2N!(X>9999999.99)!(X<0) X
 Q
 ;
...
X5 S DIC("S")="I $P(^(0),U,5)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW="0;9",DV="NJ10,2",DU="",DLB="NON-COVERED CHARGE",DIFLD=.09
 G RE
X6 S:X["$" X=$P(X,"$",2) K:X'?.N.1".".2N!(X>9999999.99)!(X<0) X
 Q
 ;
...
X8 I '$P(^DGCR(399,DA(1),"RC",DA,0),U,6) S Y="@758"
 Q
...
X11 I +$P(^DGCR(399,DA(1),"RC",DA,0),U,8) W !," AUTO ADDED CHARGE - NO CHANGE TO TYPE/COMPONENT" S Y="@759"
 Q
...
X14 I $S($P($G(^DGCR(399,DA(1),"RC",DA,0)),U,10)=3:0,1:$P($G(^(0)),U,10)'=4)!$P($G(^(0)),U,8) S Y="@759"
 Q
15 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=15 D X15 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X15 I $P($G(^DGCR(399,DA(1),"RC",DA,0)),U,10)=4 S Y="@7581"
 Q
...
X17 D LINKRX^IBCEU5(DA(1),DA)
 Q
18 S DW="0;15",DV="FXO",DU="",DLB="RX PROCEDURE",DIFLD=.15
 S DQ(18,2)="S Y(0)=Y S Y=Y_"" - ""_$P($$PRCNM^IBCSCH1($P($G(^DGCR(399,D0,""CP"",+Y,0)),U)),U)"
 S DE(DW)="C18^IBXS77",DE(DW,"INDEX")=1
 G RE
...
C18F1 N X,X1,X2 S DIXR=53 D C18X1(U) K X2 M X2=X D C18X1("O") K X1 M X1=X
 I $G(X(1))]"" D
 . I X(2)'=""&'$D(^TMP("IBCRRX",$J)) D DELPR^IBCU1(DA(1),X(2))
 G C18F2
C18X1(DION) K X
 ...
 Q
C18F2 Q
X18 S X=$$RXPRLOOK^IBCEU4(X) K:'X X
 I $D(X),X'?.ANP K X
 Q
 ;
