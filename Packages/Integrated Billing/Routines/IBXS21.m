...
X2 K:$L(X)>30!($L(X)<1) X
 ...
 Q
 ;
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW=".311;15",DV="RSa",DU="",DLB="EMPLOYMENT STATUS",DIFLD=.31115
 S DE(DW)="C3^IBXS21"
...
X17 S:IBDR20'["22" Y="@42"
 Q
18 S DW=".25;1",DV="FX",DU="",DLB="SPOUSE'S EMPLOYER NAME",DIFLD=.251
 S DE(DW)="C18^IBXS21"
 G RE
...
X19 S:X']"" Y="@42"
 Q
20 D:$D(DG)>9 F^DIE17,DE S DQ=20,DW=".25;2",DV="FX",DU="",DLB="SPOUSE'S EMP STREET [LINE 1]",DIFLD=.252
 S DE(DW)="C20^IBXS21"
 G RE
...
X21 S:X']"" Y=.255
 Q
22 D:$D(DG)>9 F^DIE17,DE S DQ=22,DW=".25;3",DV="FX",DU="",DLB="SPOUSE'S EMP STREET [LINE 2]",DIFLD=.253
 S DE(DW)="C22^IBXS21"
 G RE
...
X23 S:X']"" Y=.255
 Q
24 D:$D(DG)>9 F^DIE17,DE S DQ=24,DW=".25;4",DV="FX",DU="",DLB="SPOUSE'S EMP STREET [LINE 3]",DIFLD=.254
 G RE
X24 K:$L(X)>35!($L(X)<3) X I $D(X) S DFN=DA D SE^DGLOCK2
 I $D(X),X'?.ANP K X
 Q
 ;
25 S DW=".25;5",DV="FX",DU="",DLB="SPOUSE'S EMPLOYER'S CITY",DIFLD=.255
 G RE
X25 K:$L(X)>20!($L(X)<2) X I $D(X) S DFN=DA D SE^DGLOCK2
 I $D(X),X'?.ANP K X
 Q
 ;
26 S DW=".25;6",DV="P5'X",DU="",DLB="SPOUSE'S EMPLOYER'S STATE",DIFLD=.256
 S DU="DIC(5,"
 G RE
X26 S DFN=DA D SE^DGLOCK2
 Q
 ;
27 S DW=".22;6",DV="FOX",DU="",DLB="SPOUSE'S EMP ZIP+4",DIFLD=.2206
...
X27 K:X[""""!($A(X)=45) X I $D(X) S DFN=DA D SE^DGLOCK2 I $D(X) K:$L(X)>15!($L(X)<5) X I $D(X) D ZIPIN^VAFADDR
 ...
 Q
 ;
28 D:$D(DG)>9 F^DIE17,DE S DQ=28,DW=".25;8",DV="FOX",DU="",DLB="SPOUSE'S EMP PHONE NUMBER",DIFLD=.258
 G RE
