...
BEGIN S DNM="IBXSAH10",DQ=1+D G B
1 S DW="0;1",DV="M*P8932.1'",DU="",DIFLD=.01,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C1^IBXSAH10"
 S DU="USC(8932.1,"
 G RE:'D S DQ=2 G 2
...
C1F1 Q
X1 S DIC("S")="I $TR($P($G(^(90002)),U,2),""IN"",""21"")=$P(^IBA(355.93,DA(1),0),U,2)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="0;2",DV="RS",DU="",DIFLD=.02,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C2^IBXSAH10"
 S DU="1:YES;0:NO;"
 G RE
...
C2F1 Q
X2 Q
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW="0;3",DV="RS",DU="",DIFLD=.03,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C3^IBXSAH10"
 S DU="A:ACTIVE;I:INACTIVE;"
 G RE
