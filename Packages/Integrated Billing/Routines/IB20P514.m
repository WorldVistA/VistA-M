IB20P514 ;ALB/NCA - POST-INIT FOR IB*2.0*514 ; 11/20/12 2:28pm
 ;;2.0;INTEGRATED BILLING;**514**;21-MAR-94;Build 25
 ;
PRE ; set up check points for pre/post-init
 N %
 S %=$$NEWCP^XPDUTL("THRESH","THRESH^IB20P514")
 S %=$$NEWCP^XPDUTL("MCRDED","MCRDED^IB20P514")
 S %=$$NEWCP^XPDUTL("PRIOR","PRIOR^IB20P514")
 Q
 ;
THRESH ; Pension Threshold
 N IBA,IBERRM,IBRN,IBTYPE,IBX,DA,DIK
 S IBTYPE="Pension Threshold"
 D BMES^XPDUTL("Filing CY 2014 Pension Threshold rates.")
 S IBX=3131201
 ...
 S IBA(354.3,"+1,",.01)=3131201 ; effective date for CY 2013 values
 S IBA(354.3,"+1,",.02)=1 ;     internal value 1 = BASIC PENSION
 S IBA(354.3,"+1,",.03)=12652 ;  base rate for veteran
 S IBA(354.3,"+1,",.04)=16569 ; 1 dependent
 S IBA(354.3,"+1,",.05)=18730 ; 2 dependents
 S IBA(354.3,"+1,",.06)=20891 ; 3 dependents
 S IBA(354.3,"+1,",.07)=23052 ; 4 dependents
 S IBA(354.3,"+1,",.08)=25213 ; 5 dependents
 S IBA(354.3,"+1,",.09)=27374 ; 6 dependents
 S IBA(354.3,"+1,",.10)=29535 ; 7 dependents
 S IBA(354.3,"+1,",.11)=31696 ; 8 dependents
 S IBA(354.3,"+1,",.12)=2161  ;  additional dependent amount
 D UPDATE^DIE("","IBA","","IBERRM") ; file the new record for CY 2014
 I $D(IBERRM) D
...
PRIOR ;This code sets up the variables and calls the routine to print or print-and-update the
 ...
 ;
 Q:'$D(^IBA(354.1,"APRIOR",3121201))  ; quit if the "APRIOR" x-ref is not set for 12/1/12.
 ...
 S IBBMES=$S($D(ZTSK):"This job has been queued for NOW, as task number "_ZTSK_".",1:"This job could not be queued. Please edit the 12/1/13 threshold through the 'Add Income Thresholds' option, which allows you to queue this job.")
...
MMSG ; MailMan message to report update problem to billing groups, patch installer and patch developer
 ...
 S IBPARAM("FROM")="PATCH IB*2.0*514 CY 2014 RATE UPDATE"
 F IBGROUP="IB EDI SUPERVISOR","IB ERROR","MCCR" D
 . I $D(^XMB(3.8,"B",IBGROUP)) S IBGROUP="G."_IBGROUP,XMY(IBGROUP)=""
 S XMY(DUZ)=""
 ;
 S IBC=0
 S IBC=IBC+1,IBTXT(IBC)="This message has been sent by patch IB*2.0*514. If you have received this"
 S IBC=IBC+1,IBTXT(IBC)="message, it indicates that the patch encountered some difficulty in filing"
 S IBC=IBC+1,IBTXT(IBC)="the CY 2014 "_IBTYPE_" rates as outlined in the patch description."
 ...
 S IBC=IBC+1,IBTXT(IBC)="You can consult the IB*2.0*514 patch description for additional information."
