IBXS81 ; ;03/14/19
 D DE G BEGIN
DE S DIE="^DGCR(399,D0,""DEN1"",",DIC=DIE,DP=399.096,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^DGCR(399,D0,"DEN1",DA,""))=""
...
BEGIN S DNM="IBXS81",DQ=1+D G B
1 S DW="0;1",DV="MNJ2,0",DU="",DIFLD=.01,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C1^IBXS81"
 G RE:'D S DQ=2 G 2
...
C1F1 Q
X1 K:+X'=X!(X>32)!(X<1)!(X?.E1"."1N.N) X
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="0;2",DV="S",DU="",DIFLD=.02,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="E:EXTRACTED;M:MISSING;"
 G RE
X2 Q
3 G 1^DIE17
