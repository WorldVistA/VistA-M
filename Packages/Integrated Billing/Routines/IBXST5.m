...
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGCR(399,DA,""))=""
 I $D(^("S")) S %Z=^("S") S %=$P(%Z,U,9) S:%]"" DE(7)=% S %=$P(%Z,U,14) S:%]"" DE(4)=%
 I $D(^("TX")) S %Z=^("TX") S %=$P(%Z,U,5) S:%]"" DE(1)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="IBXST5",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="TX;5",DV="S",DU="",DLB="CLAIM MRA STATUS",DIFLD=24
 S DE(DW)="C1^IBXST5"
 S DU="0:NO MRA NEEDED;1N:MRA NEEDED/NOT YET REQUESTED;1R:MRA REQUESTED;C:VALID MRA RECEIVED;A:MRA SKIPPED;"
 S X="1R"
 S Y=X
...
X2 S Y="@99"
 Q
3 S DQ=4 ;@94
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW="S;14",DV="D",DU="",DLB="DATE LAST PRINTED",DIFLD=14
 S DE(DW)="C4^IBXST5"
 S X=DT
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X5 S Y="@99"
 Q
6 S DQ=7 ;@902
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW="S;9",DV="FOX",DU="",DLB="AUTHORIZE BILL GENERATION?",DIFLD=9
