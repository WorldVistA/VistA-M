IBXSA2 ; GENERATED FROM 'IB SCREEN102' INPUT TEMPLATE(#1753), FILE 399;12/30/17
 D DE G BEGIN
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGCR(399,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,16) S:%]"" DE(40)=%
 I $D(^("M2")) S %Z=^("M2") S %=$P(%Z,U,1) S:%]"" DE(53)=%,DE(61)=% S %=$P(%Z,U,2) S:%]"" DE(57)=% S %=$P(%Z,U,3) S:%]"" DE(67)=%,DE(75)=% S %=$P(%Z,U,4) S:%]"" DE(71)=%
 ...
 I $D(^("U3")) S %Z=^("U3") S %=$P(%Z,U,3) S:%]"" DE(37)=% S %=$P(%Z,U,11) S:%]"" DE(44)=%
...
X8 K:$L(X)>50!($L(X)<3) X
 ...
 ;
9 S DQ=10 ;@10111
10 S DW="U;13",DV="FJ50",DU="",DIFLD=163,DLB="PRIMARY AUTHORIZATION CODE"
 G RE
X10 K:$L(X)>50!($L(X)<1) X
 I $D(X),X'?.ANP K X
 Q
 ;
11 S DW="UF32;1",DV="FJ50",DU="",DIFLD=253,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
X11 K:$L(X)>50!($L(X)<1) X
 I $D(X),X'?.ANP K X
 Q
 ;
...
X12 S:'DIPA("I2") Y="@1011"
 Q
13 S DW="U2;8",DV="FXJ50",DU="",DIFLD=230,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
X13 K:$L(X)>50!($L(X)<1) X
 I $D(X),X'?.ANP K X
 Q
 ;
14 S DW="UF32;2",DV="FJ50",DU="",DIFLD=254,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
X14 K:$L(X)>50!($L(X)<1) X
 I $D(X),X'?.ANP K X
 Q
 ;
...
X15 S:'DIPA("I3") Y="@1011"
 Q
16 S DW="U2;9",DV="FJ50",DU="",DIFLD=231,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
X16 K:$L(X)>50!($L(X)<1) X
 I $D(X),X'?.ANP K X
 Q
 ;
17 S DW="UF32;3",DV="FJ50",DU="",DIFLD=255,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
...
39 S DW="U2;11",DV="S",DU="",DIFLD=233,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 ...
 G RE
X39 Q
40 S DW="0;16",DV="D",DU="",DIFLD=.16,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
...
X43 S:IBDR20'["105" Y="@106"
 Q
44 S DW="U3;11",DV="*P8932.1'",DU="",DIFLD=252,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="USC(8932.1,"
 G RE
X44 S DIC("S")="I $P($G(^(90002)),U,2)=""N""" D ^DIC K DIC S DIC=$G(DIE),X=+Y K:Y<0 X
 Q
 ;
45 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=45 D X45 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X45 D DISPTAX^IBCEP81($P($G(^DGCR(399,DA,"U3")),U,11),"Billing Provider")
 Q
46 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=46 D X46 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X46 I $P($G(^DGCR(399,DA,"U3")),U,11) N X,Y,DIR S DIR(0)="EA",DIR("A")="Press Return to continue" D ^DIR K DIR
 Q
47 S DQ=48 ;@106
...
X66 S DIPA("OLDALTT2")=$P($G(^DGCR(399,DA,"M2")),U,3)
 Q
67 D:$D(DG)>9 F^DIE17,DE S DQ=67,DW="M2;3",DV="*P355.98'",DU="",DIFLD=142,DLB="Secondary Payer - Alt Inst Prim Payer ID Type"
 S DE(DW)="C67^IBXSA2"
 S DU="IBA(355.98,"
 G RE
...
X80 S DIPA("OLDALTT2")=$P($G(^DGCR(399,DA,"M2")),U,5)
 Q
81 D:$D(DG)>9 F^DIE17 G ^IBXSA25
