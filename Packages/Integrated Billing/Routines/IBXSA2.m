IBXSA2 ; GENERATED FROM 'IB SCREEN102' INPUT TEMPLATE(#1753), FILE 399;04/19/17
 D DE G BEGIN
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGCR(399,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,16) S:%]"" DE(40)=%
 I $D(^("TX")) S %Z=^("TX") S %=$P(%Z,U,8) S:%]"" DE(52)=% S %=$P(%Z,U,9) S:%]"" DE(58)=%
...
X61 N X,Y,DIR S DIR(0)="EA",DIR("A",1)="NO FIELDS AVAILABLE TO EDIT",DIR("A")="PRESS RETURN TO CONTINUE: " W ! D ^DIR K DIR
 Q
62 S DQ=63 ;@1069
63 S DQ=64 ;@107
64 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=64 D X64 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X64 S:IBDR20'["107" Y="@109"
 Q
65 S DQ=66 ;@109
66 G 0^DIE17
