IBCCPT ;ALB/LDB - MCCR OUTPATIENT VISITS LISTING CONT. ;29 MAY 90
 ;;2.0;INTEGRATED BILLING;**55,62,52,91,106,125,51,148,174,182,245,266,260,339,432,592,742,796**;21-MAR-94;Build 34
...
EN D:$D(DIR) HLP W @IOF S DGU=0 K DGCPT,^UTILITY($J) D VST(.IBQUERY)
 ...
 ;IB*2.0*796 - Grab Units from data
 S (DGCNT,DGCNT1)=0 F  S DGCNT=$O(^UTILITY($J,"CPT-CNT",DGCNT)) Q:'DGCNT  D  I DGU="^" S DGCNT=DGCNT-1 Q
 . S DGNOD=^(DGCNT),DGCPT=+DGNOD,DGDAT=$P(DGNOD,"^",2),DGBIL=$P(DGNOD,"^",3),DGASC=$P(DGNOD,"^",4),DGDIV=$P(DGNOD,"^",5),DGUNIT=$P(DGNOD,"^",16),DGCNT1=DGCNT1+1
 . D CPRT
...
FILE1 ;  file procedures, if BASC, only for 1 visit date
 ...
 I +$P(DGNOD,"^",16) S DR=DR_";92////"_+$P(DGNOD,"^",16) ;IB*2.0*796 - Add quantity/units
 ;
...
CPRT D:$Y+6>IOSL SCR Q:DGU="^"
 ...
 ;IB*2.0*796 - Add Unit column to output
 W !,DGCNT,")",?5,$P(IBCPTNM,U,2),?12,$$RJ^XLFSTR(DGUNIT,4),?18,$E($P(IBCPTNM,U,3),1,24),?44,$E($P($G(^SC(+$P(DGNOD,U,9),0)),U,1),1,15),?61,$$FMTE^XLFDT(DGDAT,2)
 I +DGBIL,+$P($G(DGNOD),U,6) S IBNBM="  *ON BILL/"_$E($P(DGNOD,U,7),1,4)_"*"
 ;IB*2.0*796 - Change from ON THIS BILL to ON BILL to keep line under 80 chars
 I IBNBM="",DGBIL S IBNBM="  *ON BILL*"
 I IBNBM="",+$P($G(DGNOD),U,6) S IBNBM="  "_$E($P(DGNOD,U,7),1,12)
 W ?67,$E(IBNBM,1,12) ;IB*2.0*796 - Move over to make room for UNIT column, ensure doesn't run line over 80 chars
 ;
 S IBMODS=$P($G(DGNOD),U,10) F J=1:1 S IBZ=$P(IBMODS,",",J) Q:IBZ=""  S IBZ=$$MOD^ICPTMOD(IBZ,"I",IBDATE) W !,?8,$P(IBZ,U,2),?13,$P(IBZ,U,3)
 Q
 ;IB*2.0*796 - Change report to include quantity of CPT code
CHDR W @IOF,!,?15,"<<CURRENT PROCEDURAL TERMINOLOGY CODES>>",!!,?10,"LISTING FROM VISIT DATES WITH ASSOCIATED CPT CODES",!,?22,"IN OUTPT ENCOUNTERS FILE",!
 ...
 S L="",$P(L,"=",80)="" W L,!,"NO.",?5,"CODE",?12,"UNIT",?18,"SHORT NAME",?44,"CLINIC",?61,"DATE",!,L K L
 Q
...
EXTPROC(IBIFN,IBOE,IBOE0,IBCNT) ; Extract procedures for an encounter
 ...
 . ;IB*2.0*796 - Add quantity as a field and stop repeating codes based on quantity
 . S $P(IBCPTDAT,U,16)=$P(IBCPTS(I2),U,16)
 . ;F Z=1:1:$P(IBCPTS(I2),U,16) S IBCNT=IBCNT+1,^UTILITY($J,"CPT-CNT",IBCNT)=IBCPTDAT,^UTILITY($J,"CPT-CNT",IBCNT,"DX")=$G(IBDX)
 . S IBCNT=IBCNT+1,^UTILITY($J,"CPT-CNT",IBCNT)=IBCPTDAT,^UTILITY($J,"CPT-CNT",IBCNT,"DX")=$G(IBDX)
 . K IBDX
