...
1 S D=0 K DE(1) ;24
 ...
 S DU="PRC(442.7,"
 G RE:D I $D(DSC(442.15))#2,$P(DSC(442.15),"I $D(^UTILITY(",1)="" X DSC(442.15) S D=$O(^(0)) S:D="" D=-1 G M1
 S D=$S($D(^PRC(442,DA,15,0)):$P(^(0),U,3,4),$O(^(0))'="":$O(^(0)),1:-1)
M1 I D>0 S DC=DC_D I $D(^PRC(442,DA,15,+D,0)) S DE(1)=$P(^(0),U,1)
 G RE
R1 D DE
 S D=$S($D(^PRC(442,DA,15,0)):$P(^(0),U,3,4),1:1) G 1+1
 ;
2 S D=0 K DE(1) ;20
