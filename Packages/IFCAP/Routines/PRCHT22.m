...
DE S DIE="^PRC(442,",DIC=DIE,DP=442,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^PRC(442,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,14) S:%]"" DE(3)=%
 I $D(^(1)) S %Z=^(1) S %=$P(%Z,U,10) S:%]"" DE(1)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET I X'?.ANP S DDER=1 Q 
 ...
 Q
BEGIN S DNM="PRCHT22",DQ=1
1 S DW="1;10",DV="R*P200'",DU="",DLB="PURCHASING/PPM AGENT",DIFLD=16
 S DU="VA(200,"
 G RE
X1 S DIC("S")="I $D(^(400)),$P(^(400),U,1)>1 Q:$D(PRCHNRQ)  I $P(^(400),U,1)>2" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
2 S D=0 K DE(1) ;20
 S Y="COMMENTS^W^^0;1^Q",DG="4",DC="^442.04" D DIEN^DIWE K DE(1) G A
 ;
3 S DW="0;14",DV="RNJ3,0",DU="",DLB="LINE ITEM COUNT",DIFLD=15
 G RE
