PRCHAMY2 ;WISC/DJM-PRINT AMENDMENT, ROUTINE #2 ;9/15/95  11:40 AM
...
E22 ;LINE ITEM Delete
 ...
 ..S QTY=$$FETCH(2,ITEM)
 ..S AUC=$$FETCH(5,ITEM)
 ..S DATA="    "_QTY_" "_$P($G(^PRCD(420.5,$P(ITEM0,U,3),0)),U)_" at $"_$J(AUC,12,4)_" = $"_$J(QTY*AUC,9,2) D DATA^PRCHPAM5(.LCNT,DATA),LCNT1^PRCHPAM5(LCNT)
 Q
 ;
E23 ;LINE ITEM Edit
 N FIELD,CHANGE,CHANGES,OLD,ITEM,ITEM0,ITEM1,ITEMZ,QTY,AUC,UOP,UOP1,NSN,UCF,LCNT,DATA,VAL,ABC,VSN,CONOLD,CON442
 S FIELD=0 K ITEM D LCNT^PRCHPAM5(.LCNT)
 F  S FIELD=$O(^PRC(442,PRCHPO,6,PRCHAM,3,"AC",AMEND,FIELD)) Q:FIELD'>0  D
 .S CHANGE=0 F  S CHANGE=$O(^PRC(442,PRCHPO,6,PRCHAM,3,"AC",AMEND,FIELD,CHANGE)) Q:CHANGE'>0  D
 ..S CHANGES=$G(^PRC(442,PRCHPO,6,PRCHAM,3,CHANGE,0)),OLD=$G(^PRC(442,PRCHPO,6,PRCHAM,3,CHANGE,1,1,0))
 ...
 ..D DATA^PRCHPAM5(.LCNT,DATA)
 ..I $L(ABC)>0 D OLD^PRCHAMY3
 ..I $L(ABC)'>0 S ITEMZ=ITEM1 D NEW^PRCHAMY3
 ...
 ..D DATA^PRCHPAM5(.LCNT,DATA)
 ..S:$D(ITEMZ) ITEM1=ITEMZ D NEW^PRCHAMY3
 ..S UOP1=$S($P(ITEM0,U,3)>0:$P($G(^PRCD(420.5,$P(ITEM0,U,3),0)),U),1:"")
 ..S DATA="    Items per "_UOP1_": "_$P(ITEM0,U,12)
 ..F I=1:1:26-$L(DATA) S DATA=DATA_" "
 ..S DATA=DATA_"NSN: "_$P(ITEM0,U,13) D DATA^PRCHPAM5(.LCNT,DATA)
 ..I $P(ITEM0,U,5)]"" S DATA="    STK:# "_$P(ITEM0,U,6) D DATA^PRCHPAM5(.LCNT,DATA)
 ..S UOP1=$S($P(ITEM0,U,3)>0:$P($G(^PRCD(420.5,$P(ITEM0,U,3),0)),U),1:"")
...
FETCH(FIELD,ITEM) ;EXTRINSIC FUNCTION TO RETURN THE 'VALUE' FOR A FIELD FROM 'LINE ITEM
 ;AMENDMENT' OPTIONS.
 N VAL1
 S VAL=0 F  S VAL=$O(^PRC(442,PRCHPO,6,PRCHAM,3,"AC",21,FIELD,VAL)) Q:VAL'>0  S VAL1=$P($G(^PRC(442,PRCHPO,6,PRCHAM,3,VAL,0)),U,4) I VAL1=ITEM S VAL1=0 G EXIT
 S VAL=0 F  S VAL=$O(^PRC(442,PRCHPO,6,PRCHAM,3,"AC",22,FIELD,VAL)) Q:VAL'>0  S VAL1=$P($G(^PRC(442,PRCHPO,6,PRCHAM,3,VAL,0)),U,4) I VAL1=ITEM D  G EXIT
 .S VAL1=$G(^PRC(442,PRCHPO,6,PRCHAM,3,VAL,1,1,0))
 .Q
 S VAL=0 F  S VAL=$O(^PRC(442,PRCHPO,6,PRCHAM,3,"AC",23,FIELD,VAL)) Q:VAL'>0  S VAL1=$P($G(^PRC(442,PRCHPO,6,PRCHAM,3,VAL,0)),U,4) I VAL1=ITEM D  G EXIT
 .S VAL1=$G(^PRC(442,PRCHPO,6,PRCHAM,3,VAL,1,1,0))
 .Q
 S VAL1=""
EXIT Q VAL1
