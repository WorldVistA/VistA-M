PRCHDAM6 ;WISC/DJM-PRINT AMENDMENT,ROUTINE 65 ;6/3/94  8:46 AM
...
E36 ;ITEM DISCOUNT Add/Edit PRINT
 ...
 S FIELD=0 K ITEMD D LCNT^PRCHDAM4(.LCNT)
 ...
 ..K DIS2,PCT2 ;removing old values before resetting them in the loop.
 ..S:FIELD=.01 DIS2=OLD
 ..S:FIELD=1 PCT2=OLD
 ...
 ..S ITEMD0=$G(^PRC(443.6,PRCHPO,3,ITEMD,0)) I ITEMD0="" Q
 ...
 ..;If an entry is found in 'AC' x-ref for the ITEM field #.01, then lookup for the PERCENT/DOLLAR AMOUNT field #1 and for the same item define PCT2 var
 ...
 ..;   I DIS2=0 S DATA=" *ADDED THROUGH AMENDMENT*" D DATA^PRCHDAM4(.LCNT,DATA) D
 ...
 ..I PCT2]"" S DATA=" The discount on item "_NEWI_" will now be AMENDED to read "_$S(NEWP["$":NEWP,1:NEWP_"%") D DATA^PRCHDAM4(.LCNT,DATA) Q
 ..;If only field ITEM #.01 is changed.
 ..I DIS2]"" S DATA=" The item(s) "_DIS2_" will now be AMENDED to read "_NEWI D DATA^PRCHDAM4(.LCNT,DATA) Q
 .Q
...
E37 ;ITEM DISCOUNT Delete PRINT
 ...
 S FIELD=0 D LCNT^PRCHDAM4(.LCNT)
 F  S FIELD=$O(^PRC(443.6,PRCHPO,6,PRCHAM,3,"AC",AMEND,FIELD)) Q:FIELD'>0  D
 .S CHANGE=0 F  S CHANGE=$O(^PRC(443.6,PRCHPO,6,PRCHAM,3,"AC",AMEND,FIELD,CHANGE)) Q:CHANGE'>0  D
 ...
 ..D LINE^PRCHDAM4(.LCNT,2)
 ..S DATA=" *DELETED THROUGH AMENDMENT*" D DATA^PRCHDAM4(.LCNT,DATA)
 ..S DATA=$S(OLD["$":OLD,1:OLD_"%")_" Discount For Items: "_DIS_" is DELETED" D DATA^PRCHDAM4(.LCNT,DATA),LCNT1^PRCHDAM4(LCNT)
 ..Q
 .Q
 Q
