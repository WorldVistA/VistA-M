...
DE S DIE="^PRCS(410,",DIC=DIE,DP=410,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^PRCS(410,DA,""))=""
 ...
 I $D(^(9)) S %Z=^(9) S %=$P(%Z,U,1) S:%]"" DE(10)=% S %=$P(%Z,U,4) S:%]"" DE(9)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
5 S DQ=6 ;@1
6 S D=0 K DE(1) ;10
 S DIFLD=10,DGO="^PRCST42",DC="22^410.02AI^IT^",DV="410.02MRNJ3,0",DW="0;1",DOW="LINE ITEM NUMBER",DLB="Select "_DOW S:D DC=DC_D
...
X8 I $D(PRCSERR),PRCSERR S Y="@1" K PRCSERR
 Q
9 S DW="9;4",DV="NJ7,2X",DU="",DLB="EST. SHIPPING AND/OR HANDLING",DIFLD=48.1
 G RE
X9 S:X["$" X=$P(X,"$",2) K:X'?.N.1".".2N!(X>9999.99)!(X<1) X
 Q
 ;
