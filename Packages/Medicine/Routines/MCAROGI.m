MCAROGI ; GENERATED FROM 'MCARGICON' PRINT TEMPLATE (#982) ; 10/04/96 ; (FILE 699, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 S X=$G(^MCAR(699,D0,0)) W ?22 S Y=$P(X,U,8) S Y=$S(Y="":Y,$D(^VA(200,Y,0))#2:$P(^(0),U,1),1:Y) W $E(Y,1,35)
 D T Q:'DN  D N D N:$X>0 Q:'DN  W ?0 W "Consultation Type:  "
 S X=$G(^MCAR(699,D0,15)) W ?22 S Y=$P(X,U,11) S Y=$S(Y="":Y,$D(^MCAR(699.82,Y,0))#2:$P(^(0),U,1),1:Y) W $E(Y,1,30)
...
D1R ;
 ...
 S X=$G(^MCAR(699,D0,0)) D N:$X>24 Q:'DN  S DIWL=25,DIWR=80 S Y=$P(X,U,6) S X=Y D ^DIWP
 D 0^DIWW K DIP K:DN Y
 W ?24 D DISP^MCMAG K DIP K:DN Y
 D ^DIWW K Y K DIWF
 Q
HEAD ;
 W !,"--------------------------------------------------------------------------------",!!
