RTCP ; GENERATED FROM 'RT PULL LIST' INPUT TEMPLATE(#892), FILE 194.2;07/15/96
 D DE G BEGIN
DE S DIE="^RTV(194.2,",DIC=DIE,DP=194.2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^RTV(194.2,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,1) S:%]"" DE(3)=% S %=$P(%Z,U,2) S:%]"" DE(16)=% S %=$P(%Z,U,4) S:%]"" DE(8)=%,DE(13)=% S %=$P(%Z,U,11) S:%]"" DE(10)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="RTCP",DQ=1
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIEZ=892,U="^"
...
X2 S:'$D(RTSHOW) Y="@10"
 Q
3 S DW="0;1",DV="RF",DU="",DLB="PULL LIST",DIFLD=.01
 S DE(DW)="C3^RTCP"
 G RE
