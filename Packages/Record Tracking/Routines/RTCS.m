RTCS ; GENERATED FROM 'RT CHANGE REQUEST STATUS' INPUT TEMPLATE(#883), FILE 190.1;07/15/96
 D DE G BEGIN
DE S DIE="^RTV(190.1,",DIC=DIE,DP=190.1,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^RTV(190.1,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,6) S:%]"" DE(2)=% S %=$P(%Z,U,7) S:%]"" DE(3)=% S %=$P(%Z,U,8) S:%]"" DE(4)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET I X'?.ANP S DDER=1 Q 
 ...
 Q
BEGIN S DNM="RTCS",DQ=1
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIEZ=883,U="^"
1 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=1 D X1 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X1 D DUZ^RTPSET
 Q
2 S DW="0;6",DV="S",DU="",DLB="REQUEST STATUS",DIFLD=6
 ...
 G Z
X2 Q
3 S DW="0;7",DV="D",DU="",DLB="DATE/TIME CURRENT STATUS",DIFLD=7
 S X="NOW"
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X3 S %DT="TX" D ^%DT S X=Y K:Y<1 X
 Q
 ;
4 S DW="0;8",DV="P200'",DU="",DLB="USER RESPONSIBLE FOR STATUS",DIFLD=8
 S DU="VA(200,"
 S X=RTDUZ
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
