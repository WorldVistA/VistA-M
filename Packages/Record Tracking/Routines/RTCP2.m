...
DE S DIE="^RTV(194.2,",DIC=DIE,DP=194.2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^RTV(194.2,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,6) S:%]"" DE(1)=% S %=$P(%Z,U,7) S:%]"" DE(2)=% S %=$P(%Z,U,8) S:%]"" DE(3)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET I X'?.ANP S DDER=1 Q 
 ...
 Q
BEGIN S DNM="RTCP2",DQ=1
1 S DW="0;6",DV="S",DU="",DLB="PULL LIST STATUS",DIFLD=6
 ...
 G Z
X1 Q
2 S DW="0;7",DV="D",DU="",DLB="DATE/TIME CURRENT STATUS",DIFLD=7
 S X="NOW"
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X2 S %DT="TX" D ^%DT S X=Y K:Y<1 X
 Q
 ;
3 S DW="0;8",DV="P200'",DU="",DLB="USER RESPONSIBLE FOR STATUS",DIFLD=8
 S DU="VA(200,"
 S X=RTDUZ
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
