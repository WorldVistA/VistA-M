MAGT7SO ;WOIFO/MLH/PMK - telepathology - create HL7 message to DPS - segment build - ORC ; 24 Jul 2013 8:23 PM
...
ORCSEG(SEGELTS,FILE,STATE,IENS,ACNUMB) ; FUNCTION - main entry point - create an ORC segment
 ...
 . . D SET^HLOAPI(.SEGELTS,ORCTRL,FLDORCTRLCD)
 . . Q
 ...
 . . D SET^HLOAPI(.SEGELTS,ORSTATUS,FLDORSTATUS)
 . . Q
 . D  Q:ERRSTAT  ; ORC-9-date/time of transaction
 . . D SETTS^HLOAPI4(.SEGELTS,$$NOW^XLFDT,FLDDTXACT)
 . . Q
 . D  Q:ERRSTAT  ; ORC-10-entered by
 . . S ERRSTAT=$$NPNAME^MAG7UNM(.ENTBY,DUZ)
 . . D:'ERRSTAT
 . . . D SET^HLOAPI(.SEGELTS,DUZ,FLDENTBY,1)
 . . . D SET^HLOAPI(.SEGELTS,$G(ENTBY("FAMILY")),FLDENTBY,2)
 . . . D SET^HLOAPI(.SEGELTS,$G(ENTBY("GIVEN")),FLDENTBY,3)
 . . . D SET^HLOAPI(.SEGELTS,$G(ENTBY("MIDDLE")),FLDENTBY,4)
 . . . Q
 . . Q
 ...
 . . D SET^HLOAPI(.SEGELTS,NAME,FLDORDFACNAM,1) ; organization name
 . . D SET^HLOAPI(.SEGELTS,"FI",FLDORDFACNAM,7) ; abbreviation for facility id
 . . D SET^HLOAPI(.SEGELTS,$$STATNUMB^MAGDFCNV(),FLDORDFACNAM,10) ; organization identifier (DIVISION)
 . . Q
 . Q
 Q ERRSTAT
 ;
CALLBACK(SEGELTS,ORDPVDRNO,FLDCALBKPHN) ; call back phone number (in both ORC and OBR segments)
 ...
 . D SET^HLOAPI(.SEGELTS,CALBAKFON(IREP,2,1),FLDCALBKPHN,2,1,IREP)
 . D SET^HLOAPI(.SEGELTS,CALBAKFON(IREP,3,1),FLDCALBKPHN,3,1,IREP)
 . D SET^HLOAPI(.SEGELTS,CALBAKFON(IREP,1,1),FLDCALBKPHN,12,1,IREP)
 . Q
 Q ERRSTAT
