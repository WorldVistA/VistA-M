ALPBHL1 ;OIFO-DALLAS MW,SED,KC - BCBU main HL7 message processor ;2/6/21  15:33
 ;;3.0;BAR CODE MED ADMIN;**7,8,73,108**;Mar 2004;Build 33
 ;
 ...
 .I ALPBSEG="PID"!(ALPBSEG="ORC")!(ALPBSEG="PV1")!(ALPBSEG="RXO")!(ALPBSEG="ZZZ") S ALPBMTXT(ALPBSEG)=HLNODE    ;*108  add ZZZ seg
 ...
 K ALPBDATA
 ;
 ; retrieve Hazardous to Handle & Dispose flags from ZZZ segment...
 D GETZZZ^ALPBUTL2($G(ALPBMTXT("ZZZ")),$G(ALPBFS),$G(ALPBCS),.ALPBDATA)
 S ALPBHAZH=$G(ALPBDATA(1))
 S ALPBHAZD=$G(ALPBDATA(2))
 K ALPBDATA
 ;
 ...
 .S:ALPBHAZH]"" ALPBFILE(53.702,"+"_ALPBOIEN_","_ALPBIEN_",",14)=ALPBHAZH           ;*108
 .S:ALPBHAZD]"" ALPBFILE(53.702,"+"_ALPBOIEN_","_ALPBIEN_",",15)=ALPBHAZD           ;*108
...
PM ; process the message segments...
 ...
 ..K ALPBFERR,ALPBNTE
 .; Hazardous drug flags segment...
 .I ALPBSEG="ZZZ" D
 ..D ZZZ^ALPBHL1U(+$G(ALPBIEN),+$G(ALPBOIEN),$G(ALPBDATA),$G(ALPBFS),$G(ALPBCS),.ALPBFERR)
 ..I +$G(ALPBFERR("DIERR")) D ERRLOG^ALPBUTL1(+$G(ALPBIEN),+$G(ALPBOIEN),$G(ALPBHREC),"ZZZ",$G(ALPBDATA),.ALPBFERR)
 ..K ALPBFERR
 .K ALPBDATA,ALPBSEG
 ;
...
CLEAN K ALPBCS,ALPBDATA,ALPBECH,ALPBFS,ALPBHREC,ALPBIEN,ALPBMLOG,ALPBMTXT
 ...
 K ALPBPNAM,ALPBPSEX,ALPBPSSN,ALPBSEG,ALPBHAZH,ALPBHAZD
 Q
