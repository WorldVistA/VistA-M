PXRMDLL ;SLC/PJH - REMINDER DIALOG LOADER ;01/24/2017
 ;;2.0;CLINICAL REMINDERS;**10,6,12,17,18,26,47**;Feb 04, 2005;Build 289
 ;
...
GROUP(DIEN,DSUB,BLFAIL) ;Dialog group
 ...
 .S DGSUB=$O(^PXRMD(801.41,DIEN,10,"B",DGSEQ,"")) Q:'DGSUB!(BLFAIL=1)
 .S DGCNT=DGCNT+1
 ...
 .I $G(TERMNODE)'="" D  Q:TERMSTAT=0!(BLFAIL=1)
 ..S TERMSTAT=1
 ..D REPLACE^PXRMDLLB(DFN,TERMNODE,.DGIEN,.DATA,.TERMSTAT,.BLFAIL)
 .;Exclude from P/N
 .S DEXC=$P(DATA,U,8)
 ...
 .I DTYP="G" D GROUP(DGIEN,DGRP,.BLFAIL) Q
...
LOAD(DIEN,DFN) ;Load dialog questions into array
 N BLFAIL,DARRAY,DATA,DIALOGIEN,DITEM,DFIND,DFIEN,DFTYP,DPCE,DRES,DSEQ,DSUB,DTXT,DTYP,OCNT
 N DDIS,DEXC,DHIDE,DCHECK,DDIS,DHIS,DMHEX,DRESL,DSUPP,SUB,IDENT,TXTCNT,REINDX
 ;Check Status of dialog
 S BLFAIL=0,DIALOGIEN=DIEN
 ...
 .S DSUB=$O(^PXRMD(801.41,DIEN,10,"B",DSEQ,"")) Q:'DSUB!(BLFAIL=1)
 ...
 .I $G(TERMNODE)'="" D  Q:TERMSTAT=0!(BLFAIL=1)
 ..S TERMSTAT=1
 ..D REPLACE^PXRMDLLB(DFN,TERMNODE,.DITEM,.DATA,.TERMSTAT,.BLFAIL)
 ...
 .;Dialog Group
 .I DTYP="G" D GROUP(DITEM,DSUB,.BLFAIL) Q
