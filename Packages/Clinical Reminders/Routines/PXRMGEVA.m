PXRMGEVA ;SLC/AGP,RFR - Generic entry point to run different Reminder Evaluation ;Apr 09, 2025@11:38:57
 ;;2.0;CLINICAL REMINDERS;**45,71,84,87**;Feb 04, 2005;Build 35
 ;
 ...
 ;
ERROR(RESULT,MSG) ;
 S @RESULT@(0)=-1_U_$S($P($G(@RESULT@(0)),U,2)'="":$P(@RESULT@(0),U,2)_"; ",1:"")_MSG
 Q
 ;
...
REM(RESULT,INPUT) ; controller for reminder evaluation types
 I $D(INPUT("LR")) D REMLIST(.RESULT,.INPUT)
 I $D(INPUT("REMINDERS")) D REMEVAL(.RESULT,.INPUT)
 I $D(INPUT("ROC ORDERS")) D REMOC(.RESULT,.INPUT)
 I $D(INPUT("TERMS")) D TERMEVAL(.RESULT,.INPUT)
 Q
 ;
...
REMEVAL(RESULT,INPUT) ;
 ...
 I $G(INPUT("SUB","REMINDERS"))'="" D
 .S RESULT=$G(INPUT("SUB","REMINDERS"))
 .K ^TMP($J,RESULT)
 .S RESULT=$NA(^TMP($J,RESULT)),@RESULT@(0)=0
 S PXRMSRCFF=1
 K ^TMP("PXRHM",$J)
...
MAINDF(DFN,PXRMITEM,OUTTYPE,EVALDT,SAVEFIE) ;
 ...
 Q
 ;
TERMEVAL(RESULT,INPUT) ;
 N CNT,FIEVAL,FINDING,IFIEVAL,IND,NAME,MAXOCC,NLINES,NODE,OCC,PXRMDATE,PXRMRM,TERMARR,RETTYPE,RETVALUE
 N TERMIEN,TEXT,X
 I $G(INPUT("SUB","TERMS"))'="" D
 .S RESULT=$G(INPUT("SUB","TERMS"))
 .K ^TMP($J,RESULT)
 .S RESULT=$NA(^TMP($J,RESULT)),@RESULT@(0)=0
 S TERMIEN=0 F  S TERMIEN=$O(INPUT("TERMS",TERMIEN)) Q:'+TERMIEN  D
 .N IND,MAXOCC,NODE,TERMARR,OCC,FINDPA,FIEVAL
 .S NODE=$G(INPUT("TERMS",TERMIEN)),NAME=$P(NODE,U)
 .I +$P(NODE,U,2)>0 S PXRMDATE=$P(NODE,U)
 .S RETTYPE=$P(NODE,U,4),RETVALUE=$S(RETTYPE="IEN":TERMIEN,1:NAME)
 ...
 .D IEVALTER^PXRMTERM(INPUT("DFN"),.FINDPA,.TERMARR,1,.FIEVAL)
 .S @RESULT@(0)=1
 .S @RESULT@("TERMS",RETVALUE)=$G(FIEVAL(1))
 .I +$P(NODE,U,3)=1 M @RESULT@("TERMS",RETVALUE,"FIEVAL")=FIEVAL
 .I +$P(NODE,U,4)=0 Q
 .S FIEVAL("TERM")=NAME,FIEVAL("TERM IEN")=TERMARR("IEN")
 .S FIEVAL("TERM NAME")=NAME
 .S NLINES=0,CNT=1,PXRMRM=80
 .S IND=0 F  S IND=$O(FIEVAL(1,IND)) Q:IND'>0  D
 ..K IFIEVAL,NLINES,TEXT
 ..I +$G(FIEVAL(1,IND))<1 Q
 ..S FIEVAL(1,"TERM")=NAME
 ..M IFIEVAL(1)=FIEVAL(1,IND)
 ..S IFIEVAL("FINDING")=$G(IFIEVAL(1,"FINDING"))
 ..S IFIEVAL("FILE NUMBER")=$G(IFIEVAL(1,"FILE NUMBER"))
 ..S NLINES=0,FINDING=1
 ..D OUTPUT^PXRMTERM(1,.IFIEVAL,.NLINES,.TEXT)
 ..F X=2:1:NLINES S CNT=CNT+1,@RESULT@("TERMS",RETVALUE,"DETAIL TEXT",CNT,0)=TEXT(X)
 Q
 ;
REMLIST(RESULT,INPUT) ;
 N BEG,END,CNT,DATA,DATANAM,DATAVAL,FAIL,INC,LIEN,LNAME,NODE,NUM,OVER,PAT
 N PFNAME,PNAME,PLIST,PATCREAT,RETDATA,SECURE,INSTANCE
 I $G(INPUT("SUB","LR"))'="" D
 .S RESULT=$G(INPUT("SUB","LR"))
 .K ^TMP($J,RESULT)
 .S RESULT=$NA(^TMP($J,RESULT)),@RESULT@(0)=0
 S LNAME="",FAIL=0 F  S LNAME=$O(INPUT("LR",LNAME)) Q:LNAME=""!(FAIL=1)  D
 .S LIEN=$G(INPUT("LR",LNAME,"IEN"))
 .S INSTANCE="" F  S INSTANCE=$O(INPUT("LR",LNAME,INSTANCE)) Q:'+INSTANCE!(FAIL=1)  D
 ...
 ..S NODE=$G(INPUT("LR",LNAME,INSTANCE)),PNAME=$P(NODE,U),BEG=$P(NODE,U,2),END=$P(NODE,U,3),SECURE=$P(NODE,U,4),OVER=$P(NODE,U,5),RETDATA=+$P(NODE,U,6)
 ..S PLIST=$$REMPLCRE(PNAME,SECURE,OVER) I +PLIST'>0 D ERROR(.RESULT,"Error could not find or create patient list "_PNAME) S FAIL=1 Q
 ..S PFNAME=$P(PLIST,U,2)
 ..D RUN^PXRMLCR(LIEN,+PLIST,"PXRMRULE",BEG,END,0,1)
 ..S @RESULT@(LNAME,INSTANCE)=+$P($G(^PXRMXP(810.5,+PLIST,30,0)),U,4)
 ...
 ....S @RESULT@(LNAME,INSTANCE,PAT,"DATA",DATANAM)=DATAVAL
 ..S @RESULT@(LNAME,INSTANCE,"PATIENT LIST CREATED")=PFNAME
 ..S @RESULT@(0)=1
 Q
 ;
REMPLCRE(PLNAME,SECURE,OVER) ;
 N FDA,IENS,MSG,NAME,NUM,RESULT,UNIQUE
 S (NUM,RESULT,UNIQUE)=0
 ;if overwrite check to see if the list exist
 I OVER=1 S RESULT=$O(^PXRMXP(810.5,"B",PLNAME,""))
 I RESULT>0 Q RESULT_U_PLNAME
 S NAME=PLNAME
