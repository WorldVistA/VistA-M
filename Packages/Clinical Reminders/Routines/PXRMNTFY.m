PXRMNTFY ;SLC/PKR - Routines for notifications. ;Dec 20, 2019@15:03
 ;;2.0;CLINICAL REMINDERS;**24,45**;Feb 04, 2005;Build 566
 ;
 ;========================
...
WH(EVENT,DFN,VISIT,GMPLIFN) ;Determine whether to send pregnancy/lactation status change
 ...
 N CODELIST,DEFARR,EXIT,FIEV,NAME,NODE
 N PROVARR,PXRMSDT,RIEN,RNAME,SEND,SENDMSG,STATUS,MESSAGES
 N TAX,TAXIEN,WHDATA,CACHE
 ...
 I '$D(CODELIST) Q
 ;
 ;check if patient is applicable for WH
 S PXRMSDT=DT,NAME="VA-WH POTENTIALLY UNSAFE MEDICATIONS REPORT - COHORT"
 S RIEN=$O(^PXD(811.9,"B",NAME,"")) I RIEN'>0 Q
 ...
 I $$STATMTCH^PXRMORCH(STATUS,"N") Q
 ;
 S SENDMSG=0
 F TAX="VA-WH CURRENTLY PREGNANT","VA-WH RECENTLY PREGNANT","VA-WH POSSIBLE PREGNANCY","VA-WH CURRENTLY LACTATING"  D
 ...
 ...I $D(^PXD(811.2,TAXIEN,20,"AE",CODESYS,CODE)) S SENDMSG=SENDMSG+1,SENDMSG(SENDMSG)=TAX_U_CODESYS_"|"_CODE_"|"_CODELIST(CODESYS,CODE)
 I SENDMSG=0 Q
 S SEND=0
 F CODE=1:1:SENDMSG Q:$G(CACHE)=-1  D
 .I SENDMSG(CODE)["PREGNAN" D
 ..I '$D(CACHE("PREGNANCY")) D WHEVAL(.CACHE,"VA-WH UPDATE PREGNANCY STATUS",.PXRMSDT)
 ..Q:CACHE=-1
 ..I ((SENDMSG(CODE)["CURRENTLY")!(SENDMSG(CODE)["POSSIBLE"))&((CACHE("PREGNANCY","STATE")'="PREGNANT")!((CACHE("PREGNANCY","STATE")="PREGNANT")&(CACHE("PREGNANCY","OUT DATED")))) S SEND=1 Q
 ..I SENDMSG(CODE)["RECENTLY",CACHE("PREGNANCY","STATE")="PREGNANT" S SEND=1
 .I SENDMSG(CODE)["LACTATING" D
 ..I '$D(CACHE("LACTATION")) D WHEVAL(.CACHE,"VA-WH UPDATE LACTATION STATUS",.PXRMSDT)
 ..Q:CACHE=-1
 ..I CACHE("LACTATION","STATE")'="LACTATING" S SEND=2
 .I SEND>0 D
 ..I '$D(MESSAGES(SEND,"PXRMPROV")) D
 ...N PTYPE,PCP,PXRMPROV
 ...I SEND=1,$G(CACHE("PREGNANCY","MUC"))=1 S PTYPE="M",PCP=+$P($$OUTPTPR^SDUTL3(DFN,$$NOW^PXRMDATE),U)
 ...I (SEND=2)!('+$G(CACHE("PREGNANCY","MUC"))) S PTYPE="CM"
 ...I $G(PTYPE)'="" D PATMGR^WVRPCPT1(.PXRMPROV,DFN,PTYPE,,$P(SENDMSG(CODE),U,3))
 ...I +$G(PXRMPROV(0))=-1,$P(PXRMPROV(0),U,2)'="Patient record not found" D
 ....S ERROR(1,0)="Error retrieving Women's Health managers: "_$P(PXRMPROV(0),U,2)
 ....D ERROR(.ERROR)
 ...I (+$G(PXRMPROV(0))=-1)!($G(PTYPE)="") K PXRMPROV M PXRMPROV=PROVARR
 ...I +$G(PCP)>0 S PXRMPROV(PCP)=""
