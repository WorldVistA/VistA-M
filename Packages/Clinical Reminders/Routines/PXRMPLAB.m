...
EN(MSG) ;
 ...
 N TERMARR,PKGIFN,OBX,DATA,EXIT,PXRMPROV,MUC,ITEM,ITEMTYPE,STATUS,INST
...
OBX ;process the OBX segments
 ...
 .I ITEMIEN'>0!(ITEM="") Q
 .I $$EVAL(PAT,ITEMIEN,ITEM,ITEMTYPE,$P($G(BEGDATE),"."),PROV,$G(DATE),$G(LOC),.PXRMPROV,.ERROR,.MUC,INST)>0 D
 ..S DATA("ID")=PAT_U_"P"
...
EVAL(PAT,ITEMIEN,ITEM,ITEMTYPE,BEGDATE,PROV,DATE,LOC,PXRMPROV,ERROR,MUC,INST) ;evaluation of a reminder
 ...
 I RESULT=1,'$G(FIEV(4)) D
 .D PATMGR^WVRPCPT1(.PXRMPROV,PAT,"CM",,INST)
 .I +$G(PXRMPROV(0))=-1 D
 ..I $P(PXRMPROV(0),U,2)="Patient record not found" K PXRMPROV Q
 ..S RESULT=0
 ..D ERROR("Error retrieving Women's Health managers: "_$P(PXRMPROV(0),U,2),.ERROR) G EVALX
 I RESULT=1,$G(FIEV(4)) D
 .S MUC=1,PCP=+$P($$OUTPTPR^SDUTL3(PAT,$$NOW^PXRMDATE),U)
 .I PCP>0 S PXRMPROV(PCP)=""
 S PXRMPROV(PROV)=""
EVALX ;
 Q RESULT
 ;
