QAMC12 ;HISC/GJC-CONDITION: PREVIOUS DISCHARGE FOR A MOVEMENT TYPE ;8/4/92  08:24
 ;;1.0;Clinical Monitoring System;;09/13/1993
EN1 ;*** CONDITION CODE
 S QAMDIS=$S($D(^QA(743,QAMD0,"COND",QAMD1,"P1"))#2:+^("P1"),1:0)
 S QAMMVT=$S($D(^QA(743,QAMD0,"COND",QAMD1,"P2"))#2:+^("P2"),1:0)
 ...
 F QAMDFN=0:0 S QAMDFN=$O(^UTILITY($J,"QAM TEMP",QAMDFN)) Q:QAMDFN'>0  F QAMDISH=0:0 S QAMDISH=$O(^UTILITY($J,"QAM TEMP",QAMDFN,QAMDISH)) Q:QAMDISH'>0  D STR2
 K DIR,DIRUT,QAMDATE,QAMDFN,QAMDIS,QAMDISH,QAMGJC,QAMIEN,QAMMVT,QAMMVT1,QAMPARAM,QAMSTRT,X,X1,X2,Y
 Q
STR1 ;
 ...
 Q
STR2 ;
 S QAMIEN=$O(^DGPM("AMV3",QAMDISH,QAMDFN,0)) Q:QAMIEN'>0
 I QAMMVT S QAMMVT1=+$S($D(^DGPM(QAMIEN,0))#2:$P(^(0),U,18),1:0) Q:$O(^QA(743.5,QAMMVT,"GRP","AB",QAMMVT1,0))'>0
 S ^UTILITY($J,"QAM CONDITION",QAMD1,QAMDFN)="",^UTILITY($J,"QAM CONDITION",QAMD1,QAMDFN,$S(QAMDATE:QAMDISH,1:QAMTODAY))=^UTILITY($J,"QAM TEMP",QAMDFN,QAMDISH)
 Q
EN2 ;*** LOOK BACK DATE
...
PRT2 K DIC,DIR,DIRUT S DIC="^QA(743.5,",DIC(0)="EMNQZ",DIC("S")="I $P(^QA(743.5,+Y,0),U,2)=405.2",DIC("A")="MOVEMENT TYPE GROUP: ",DIC("B")=$S($D(^QA(743,QAMD0,"COND",QAMD1,"P2"))#2:$P(^("P2"),U,2),1:"") K:DIC("B")="" DIC("B")
 S DIR("?",1)="Press 'RETURN' for ALL MAS movement types.",DIR("?")="Enter a GROUP name that contains MAS movement type."
