YTFILE ;SLC/TGA,HIOFO/FT - INSTRUMENT FILE MGMT. ;2/20/13 9:37pm
 ;;5.01;MENTAL HEALTH;**13,77,108**;Dec 30, 1994;Build 17
 ...
 Q
 ;ENKIL ;
 ;L +^YTD(601.4,YSDFN):DILOCKTM Q:'$T
 ;K ^YTD(601.4,YSDFN,1,YSENT) I '$D(YSCLERK) K ^YTD(601.4,YSDFN,1,"B",YSTEST)
 ...
 ;.K ^YTD(601.4,YSDFN),^YTD(601.4,"B",YSDFN) L +^YTD(601.4,0):DILOCKTM Q:'$T  S X=^YTD(601.4,0),X4=$P(X,U,4),X3=$P(X,U,3),X4=X4-1 S:X4<0 X4=0 S:'$O(^YTD(601.4,0)) X3="" S ^YTD(601.4,0)=$P(X,U,1,2)_"^"_X3_"^"_X4
 ;.L -^YTD(601.4,0)
 ;L -^YTD(601.4,YSDFN)
 ;Q
 ;
 ;EN4 ;
 ;I YS4D L +^YTD(601.4,YSDFN):DILOCKTM Q:'$T
 ;D EN40:'YS4D,42 L -^YTD(601.4,YSDFN) Q
 ;EN40 ;
 ;S YS4D=1 L +^YTD(601.4,YSDFN):DILOCKTM Q:'$T
 ;I '$D(^YTD(601.4,YSDFN,0)) L +^YTD(601.4,0):DILOCKTM Q:'$T  S X=^YTD(601.4,0),X(4)=$P(X,U,4),X(3)=$P(X,U,3),X(4)=X(4)+1 S:YSDFN>X(3) X(3)=YSDFN S X=$P(X,U,1,2)_"^"_X(3)_"^"_X(4) L -^YTD(601.4,0)
