YSGAF ;ALB/ASF,HIOFO/FT - GLOBAL ASSESSMENT OF FUNCTIONING ;1/28/13  2:52pm
 ;;5.01;MENTAL HEALTH;**33,37,40,42,43,51,49,108**;Dec 30, 1994;Build 17
 ...
 ;
 ;The following entry points are Supported calls documented in ICR 2535:
 ; RET
 ; UPD
 Q
CLENT ;entry point YSGAF CLINIC ENTRY option
 Q  ;DSM 5 released, no new GAFs allowed
...
CK ;check last Axis 5
 ...
 Q
PRINT ;entry point for YSGAF PRINT CLINIC option
...
TOP ;print header
 ...
 W:$Y>0 @IOF,"GAF List   Clinic: ",YSCNAME,"    **= > than ",YSDAYS," days"
...
BOT ;page end
 ...
 Q
PTENT ;patient entry
 ;entry point for YSGAF SINGLE PT option
 Q  ;DSM 5 released, no new GAFs allowed
 ;N %DT,DA,DIE,DIR,DIRUT,DLAYGO,DR,K,VA,VADM,X,X1,X2,Y,YSCLIN,YSCNAME,YSDA,YSDATE,YSDAYS,YSDD,YSDXEG,YSDXEL,YSDXEN,YSG,YSGAFLC,YSGAFLD,YSGAFLN,YSGC,YSGD,YSGN,YSGR,YSGT,YSLINE,YSN,YSONLY,YSOUT,YSPAGE,YSPTN,YSRULE,YSSTOP,YSGAFER
...
DISP5 ;display last axis5
 Q:'$D(DFN)
 D DEM^VADPT
 W !,VADM(1),?35,"SSN: ","xxx-xx-"_$E(VA("PID"),8,11),?55,"DOB: ",$P(VADM(3),U,2)
DISP51 D CK
 I YSGAFLN D
...
UPD(YSPN,YSGN,YSGD,YSGC,YSPT) ;Update GAF information
 ...
 S YSERR=1
 W !,"  Sorry, with the release of DSM 5 no new GAF entries are allowed."
 Q
 ;S YSERR=0
 ;I '$G(YSPN) D
