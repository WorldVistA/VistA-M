PXAPI ;ISL/DEE,PKR - PCE's APIs ;Aug 04, 2025@08:32:11
 ;;1.0;PCE PATIENT CARE ENCOUNTER;**15,14,27,28,124,164,210,211,217,244**;Aug 12, 1996;Build 37
 Q
 ;
...
VIS(PXRESULT,PXVIS,PXDATE) ;Return Vaccine Information Statement entry
 ...
 Q
 ;
VISITDETAIL(RESULT,VISIT) ;
 N NODE,SAS,TEMP
 S NODE=$G(^AUPNVSIT(VISIT,0))
 S RESULT("DATE/TIME")=$P(NODE,U)
 S RESULT("PATIENT")=$P(NODE,U,5)
 S RESULT("LOCATION")=$P(NODE,U,6)
 S RESULT("SERVICE CATEGORY")=$P(NODE,U,7)
 S RESULT("CLINIC STOP")=$P(NODE,U,8)
 D GETSAFORVISITDET^PXSPECAUTH(.SAS,.TEMP,VISIT)
 M RESULT("SPECIAL AUTHORITIES")=SAS
 Q
 ;
