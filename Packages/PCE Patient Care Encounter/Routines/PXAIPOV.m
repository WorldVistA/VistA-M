PXAIPOV ;ISL/JVS,PKR,ESW - SET THE DIAGNOSIS/PROBLEM LIST NODES ;Oct 29, 2025@14:09:42
 ;;1.0;PCE PATIENT CARE ENCOUNTER;**28,73,69,108,112,130,124,174,168,203,199,211,228,244**;Aug 12, 1996;Build 37
 ;
 Q
...
SET ;--SET AND NEW VARIABLES
 ...
 N BEFOR0,BEFOR12,BEFOR800,BEFOR801,BEFOR802,BEFOR811,BEFOR812,BEFORE900,CODE,FPRI
 N EVENTDT,GMPSAVED,IENB,J,LNARR,NOPLLIST,PIECE,POVI,PRI,PRVDR,PXAA,PXAAX,PXAB,PXARRAY
 N PXAIVDT,PXBCNT,PXBCNTPL,PXBKY,PXBPMT,PXBSAM,PXBSKY,PXDIGNS,PXKDONE,PXSAS
 N STOP,SUB,VAR,VNODE,VSA,SAINPUTS,TMPSA
 ;
 K PXAERR
 S PXAERR(8)=PXAK
 S PXAERR(7)="DX/PL"
 ...
 .I $P(SUB," ")="PL" S CODE=$P(SUB," ",2) I CODE'="",$$FINDBYCODE^PXSPECAUTH(CODE) S PXSAS(CODE)=@PXADATA@("DX/PL",PXAK,SUB) Q
...
SETVARA ;--SET VISIT VARIABLES
 ...
 ;
 I $G(PXAVISIT) D
 .S VNODE=$G(^AUPNVSIT(PXAVISIT,0))
 .D GETPATSA^PXSPECAUTH($P(VNODE,U,5),$P(VNODE,U),$P(VNODE,U,22),PXAVISIT,.TMPSA)
 .S CODE="" F  S CODE=$O(TMPSA(CODE)) Q:CODE=""  D
 ..;not eligible
 ..I +$P(TMPSA(CODE),U)=0 K PXSAS(CODE) Q
 ..;eligible
 ..I +TMPSA(CODE)=1 I $G(PXSAS(CODE))'="",($G(PXSAS(CODE))<-1!(PXSAS(CODE)>1)) S PXSAS(CODE)=""
 .D SETOLD800^PXSPECAUTH(.AFTER800,.PXSAS,1)
...
SETPXKA ;--SET PXK ARRAY AFTER
 ...
 M ^TMP("PXK",$J,"POV",PXAK,900,"AFTER")=AFTER900
 ;
...
BEFOR ;
 I $G(IENB) D
 .F PIECE=0,12,802,811,812 S ^TMP("PXK",$J,"POV",PXAK,PIECE,"BEFORE")=$G(^AUPNVPOV(IENB,PIECE))
 .D GETSAFORVPOVDET^PXSPECAUTH(.BEFORE900,.BEFOR800,IENB)
 E  D
 .S (BEFOR0,BEFOR12,BEFOR800,BEFOR802,BEFOR811,BEFOR812,BEFORE900)=""
 .;
SETPXKB .;--SET PXK ARRAY BEFORE
 ...
 .S ^TMP("PXK",$J,"POV",PXAK,800,"BEFORE")=BEFOR800
 .M ^TMP("PXK",$J,"POV",PXAK,900,"BEFORE")=BEFORE900
