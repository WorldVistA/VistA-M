PXKMASC ;ISL/JVS - Build and Pass to auto-check-out ;Dec 31, 2025@13:23:21
 ;;1.0;PCE PATIENT CARE ENCOUNTER;**22,41,73,164,210,211,244**;Aug 12, 1996;Build 37
 ; Builds and passes data to MAS for Auto-checkout
 ;
 ; Reference to ^SCE("AVSIT" in ICR #2045
 ; Reference to ^SCE(IEN,0) in ICR #402
 ; Reference to GETACCT^IBBAPI in ICR #4664
 ; Reference to ORACTREF^ORWPFSS in ICR #4673
 ; Reference to EN^SDPCE in ICR #2026
 ; Reference to ^DIC(8, in ICR #427
 ; Reference to FINAL^SCDXHLDR in ICR #2220
 ; Reference to GETARN^SDPFSS2 in ICR #4668
 ;
 ;
...
DUP ;Overwrite if a duplicate just the After Node
 ...
 I PXKSUB=900 D  Q
 . I PXKCAT'="VST",PXKCAT'="POV" Q
 . I PXKCAT="POV" D GETSAFORVPOVDET^PXSPECAUTH(.NODE900,.NODE800,PXKPIEN)
 . I PXKCAT="VST" D GETSAFORVISITDET^PXSPECAUTH(.NODE900,.NODE800,PXKPIEN)
 . M ^TMP("PXKCO",$J,PXKVST,PXKCAT,PXKPIEN,PXKSUB,"AFTER")=NODE900
 . S ^TMP("PXKCO",$J,PXKVST,PXKCAT,PXKPIEN,800,"AFTER")=NODE800
 I PXKSUB'=1 D  Q
...
ORG ;If original copy both
 ...
 . I PXKCAT="VST" D GETSAFORVISITDET^PXSPECAUTH(.NODE900,.NODE800,PXKPIEN)
 . I PXKCAT="POV" D GETSAFORVPOVDET^PXSPECAUTH(.NODE900,.NODE800,PXKPIEN)
 . S ^TMP("PXKCO",$J,PXKVST,PXKCAT,PXKPIEN,800,"BEFORE")=PXKBEF(800)
 . M ^TMP("PXKCO",$J,PXKVST,PXKCAT,PXKPIEN,900,"BEFORE")=PXKBEF(900)
 ;
 I PXKSUB'=1 D  Q
 . S ^TMP("PXKCO",$J,PXKVST,PXKCAT,PXKPIEN,PXKSUB,"AFTER")=$G(@PXKGN)
...
EVENT ; EVENT TO PRESENT THE DATA TO OTHER USERS
 I '$D(^TMP("PXKCO",$J)) Q
 N NODE800,NODE900,OUTPATENCIEN,SAS,SIDX,SOR,SOURCE
 N CODE,DFN,IND,OLDCODES,OUTPATENCIEN,VDT,X
 F X=1:1:8 D
 .S CODE=$$NODETOCODE^PXSPECAUTH(X) I CODE="" Q
 .S OLDCODES(CODE)=X
 S SOR=$G(PXKCO("SOR"))
 ...
 D GETSAFORVISITDET^PXSPECAUTH(.NODE900,.NODE800,PXKVVST)
 ;
EVENT1 ;
 S ^TMP("PXKCO",$J,PXKVVST,"VST",PXKVVST,800,"AFTER")=NODE800
 S IND=0
 F  S IND=+$O(NODE900(IND)) Q:IND=0  S ^TMP("PXKCO",$J,PXKVVST,"VST",PXKVVST,900,"AFTER",IND,0)=NODE900(IND,0)
 ;M ^TMP("PXKCO",$J,PXKVVST,"VST",PXKVVST,900,"AFTER")=SAS
 ;S ^TMP("PXKCO",$J,PXKVVST,"VST",PXKVVST,800,"AFTER")=OLD800
