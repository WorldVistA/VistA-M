PXCEAE1 ;ISL/DEE,ISA/KWP,SLC/AJB - Builds the List Manager display of a visit and related v-files ;Aug 04, 2025@08:42:49
 ;;1.0;PCE PATIENT CARE ENCOUNTER;**22,73,199,201,210,215,211,244**;Aug 12, 1996;Build 37
 ;; ;
 Q
 ;
...
DISPLAY(ENTRY,PXCECODE,ARRAY,ARRAYIX,LINE,COUNT,VIEW) ; -- display the data
 ...
 . I PXCECODE="PXCESIT",+PXCETEXT=800 Q
 ...
 N IDX,NODE,NODE800,NODE900,SAIDX,SATMP,SA0
 I VIEW'="D" Q
 I PXCECODE'="PXCESIT",PXCECODE'="PXCEPOV" Q
 I IEN=0 Q
 I PXCECODE="PXCESIT" D GETSAFORVISITDET^PXSPECAUTH(.NODE900,.NODE800,IEN)
 I PXCECODE="PXCEPOV" D GETSAFORVPOVDET^PXSPECAUTH(.NODE900,.NODE800,IEN)
 S IDX=0 F  S IDX=$O(NODE900(IDX)) Q:IDX'>0  D
 .S NODE=NODE900(IDX,0),SAIDX=+NODE I SAIDX=0 Q
 .S SA0=$$GETZERO^PXSPECAUTH(SAIDX)
 .S SATMP($P(SA0,U,4))=$TR($P(SA0,U,3),"&","")_": "_U_$S(+$P(NODE,U,2)=1:"Yes",+$P(NODE,U,2)=0:"No",1:"Unanswered")
 S IDX=0 F  S IDX=$O(SATMP(IDX)) Q:IDX'>0  D
 .S NODE=SATMP(IDX)
 .S $P(PXCETEXT,"~",5)=$P(NODE,U),PXCEEXT=$P(NODE,U,2)
 .D ADDLINE
 Q
ADDLINE ;
 S LINE=LINE+1
