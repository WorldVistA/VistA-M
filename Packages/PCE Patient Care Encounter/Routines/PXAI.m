PXAI ;ISL/JVS,PKR ISA/KWP,ESW - PCE DRIVING RTN FOR 'DATA2PCE' API ;Aug 04, 2025@09:04:35
 ;;1.0;PCE PATIENT CARE ENCOUNTER;**15,74,69,102,111,112,130,164,168,215,211,244**;Aug 12, 1996;Build 37
 Q
 ;
 ;+  1       2       3        4        5       6      7      8       9
DATA2PCE(PXADATA,PXAPKG,PXASOURC,PXAVISIT,PXAUSER,PXANOT,ERRRET,PXAPREDT,PXAPROB,PXACCNT) ;+API to pass data for add/edit/delete to PCE.
 ...
 ;+      PXAPROB($J,1,"WARNING3","ENCOUNTER",1,N_"W")=REASON
 ;+
...
NEW ;--NEW VARIABLES
 ...
 K ^TMP("PXK",$J),^TMP("DIERR",$J),^TMP("PXAIADDPRV",$J),^TMP("PXSCSA",$J)
 ;
VST ;--VISIT
...
SC ;--STANDARD CODES
 ...
 ;
 ;D SCSAWARNINGS^PKRUTLSCC(PXAVISIT)
 ;
 S PXACCNT=$P($G(^AUPNVSIT(PXAVISIT,0)),"^",26) ;PX*1.0*164 ;Sets the PFSS Account Reference, if any
 K ^TMP("PXAIADDPRV",$J),^TMP("PXK",$J),^TMP("PXSCSA",$J),PXAERR,PXKERROR
...
EXTERNAL ;---SEND ERRORS TO SCREEN
 ...
 .;Do not build the warning dialog if there are no warnings.
 . S NUMWARNINGS=$$NUMWARNINGS^PXAIERR(.PXAERR)
 . I NUMWARNINGS>0 D
