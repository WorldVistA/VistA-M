PXPXRM ;SLC/PKR - APIs for Clinical Reminder indexes. ;08/23/2017
 ;;1.0;PCE PATIENT CARE ENCOUNTER;**119,199,210,215,216,211**;Aug 12, 1996;Build 244
 ;
...
VCPT(DA,DATA) ;Return data for a specified V CPT entry.
 ...
 S DATA("EVENT DATE AND TIME")=$P($G(^AUPNVCPT(DA,12)),U,1)
...
VHF(DA,DATA) ;Return data for a specified V Health Factor entry.
 ...
 S DATA("EVENT DATE AND TIME")=$P($G(^AUPNVHF(DA,12)),U,1)
...
VIMM2(DA,DATA) ; Internal function
 ...
 S PXX=$P(PXTEMP,U,7)
 S DATA("LOT NUMBER")=PXX_$S(PXX:(U_$P($G(^AUTTIML(+PXX,0)),U,1)),1:"")
...
VPEDU(DA,DATA) ;Return data, for a specified V Patient ED entry.
 ...
 S DATA("EVENT DATE AND TIME")=$P($G(^AUPNVPED(DA,12)),U,1)
...
VSCDATA(DA,DATA) ;Return data for a specified V Standard Codes entry.
 ...
 S DATA("VISIT")=$P(TEMP,U,3)
 S DATA("PROVIDER NARRATIVE")=$P(TEMP,U,4)
 S DATA("EVENT DATE AND TIME")=$P($G(^AUPNVSC(DA,12)),U,1)
...
VXAM(DA,DATA) ;Return data, for a specified V Exam entry.
 ...
 S DATA("EVENT DATE AND TIME")=$P($G(^AUPNVXAM(DA,12)),U,1)
