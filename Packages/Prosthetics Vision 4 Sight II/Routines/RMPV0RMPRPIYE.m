RMPV0RMPRPIYE ; OIT/JDA - Adapted from RMPRPIYE; Dec 01, 2024@21:29:56
 ;;1.0;PROSTHETICS VISION 4 SIGHT II;**2**;Jan 31, 2025;Build 38
 ;Per VHA Directive 10-93-142, this routine should not be modified.
 ;
 ; Reference to ^PRCP(445, supported by ICR #7547
 ...
 ; Reference to EXIT^RMPRPIYF supported by ICR #7540
 ; Reference to POST^RMPRPIYF supported by ICR #7540
 ; Reference to DEL1^RMPRPIYF supported by ICR #7540
 ; Reference to CPT^RMPRCPTU supported by ICR #7543
 ; Reference to LOWBA^RMPRPIYI supported by ICR #7544
 ; Reference to HCPCS3^RMPRPIY1 supported by ICR #7555
 ; Reference to routine ^RMPRD1 supported by ICR #7553
 ; Reference to ^RMPRPIYK supported by ICR #7559
 ;
...
EN ;EDIT STOCK ISSUES
 ...
 D HOMEZIS^RMPVRT,DIV4^RMPV0RMPRSIT G:$D(X) EXIT^RMPRPIYF
 S DIC("S")="S RM661=$G(^RMPR(660,+Y,1)) I ($P(^RMPR(660,+Y,0),U,13)=11),($P(^(0),U,10)=RMPR(""STA""))"
 ;I RMPRSITE=1 S DIC("S")=DIC("S")_"!($P(^(0),U,10)="""")"
 S DIC="^RMPR(660,",DIC(0)="AEMQ",DIC("A")="Select PATIENT: ",DIC("W")="D ^RMPRD1" D %DIC^RMPVFM G:Y<0 EXIT^RMPRPIYF
 S RMPRIEN=+Y
 L +^RMPR(660,+Y):1 I $T=0 D WRITECTL^RMPVIO("!"),WRITECTL^RMPVIO("?5"),WRITE^RMPVIO($C(7)),WRITE^RMPVIO("Someone else is Editing this entry!") G EXIT^RMPRPIYF
...
EDX ;POST
 ...
 D %DIR^RMPVFM K DIR G:Y="P" POST^RMPRPIYF G:Y="D" DEL1^RMPRPIYF
 G:Y="E" EDT
 I $D(DIRUT)!$D(DUOUT)!$D(DTOUT) G EXIT^RMPRPIYF
DEL ;
 S DIR(0)="SBO^E:EDIT;D:DELETE",DIR("B")="E"
 S DIR("A")="Would you like to EDIT/DELETE this Transaction",DIR("?")="Answer 'E' to EDIT the transaction or 'D' to DELETE the transaction." D %DIR^RMPVFM
...
EDU S RMTY=$P(R1(0),U,4)
 K DIR D WRITECTL^RMPVIO("!") S DIR(0)="660,2",DIR("B")=$S(RMTY="I":"INITIAL ISSUE",RMTY="X":"REPAIR",RMTY="R":"REPLACE",RMTY="S":"SPARE",1:"")
 D %DIR^RMPVFM G:$D(DIRUT) CO S $P(R1(0),U,4)=Y,$P(R3("D"),U,4)=$S(Y="I":"INITIAL ISSUE",Y="X":"REPAIR",Y="R":"REPLACE",Y="S":"SPARE",1:"")
 S DIR(0)="660,62",DIR("B")=$P(R4("D"),U,3) D %DIR^RMPVFM G:$D(DIRUT) CO S $P(R1("AM"),U,3)=Y,$P(R4("D"),U,3)=$S(Y=1:"SC/OP",Y=2:"SC/IP",Y=3:"NSC/IP",Y=4:"NSC/OP",1:"")
 I Y<4 S $P(R1("AM"),U,4)="",$P(R4("D"),U,4)="" G 2
 K DIR I Y=4 S DIR(0)="660,63" S:$P(R1("AM"),U,4)?1N.N DIR("B")=$P(R4("D"),U,4) D %DIR^RMPVFM G:$D(DUOUT)!$D(DTOUT) CO G:$D(DIRUT)!(X="") 2
...
HCPCS ;scanning an item is mandatory.
 ...
 I $P(R3("D"),U,6)&((RMPREXC="^")!(RMPREXC="P")) G CO^RMPV0RMPRPIYE
 I RMPREXC="^" G CO^RMPV0RMPRPIYE
 I RMPREXC="P" G CO^RMPV0RMPRPIYE
 I RMPREXC="T" G CO^RMPV0RMPRPIYE
...
CPT ;ask for CPT Modifier
 ...
 I RMHCOLD'=RMDAHC D CPT^RMPRCPTU(RDA) G:$D(DUOUT)!$D(DTOUT) CO S $P(R1(1),U,6)=$G(RMCPT) D:$G(REDIT) WRITECTL^RMPVIO("!"),WRITE^RMPVIO("NEW CPT MODIFIER: "),WRITE^RMPVIO($G(RMCPT))
 ...
 .. D:RMCPOLD=RMCPT WRITECTL^RMPVIO("!!"),WRITE^RMPVIO("*** Based on the information given above, CPT Modifier string has not changed!!!"),WRITECTL^RMPVIO("!")
 .. D:RMCPOLD'=RMCPT WRITECTL^RMPVIO("!"),WRITE^RMPVIO("NEW CPT MODIFIER: "),WRITE^RMPVIO($G(RMCPT))
 K DIR
 ;
VEN0 ;process vendor
 K DIC,DIR
 ...
 S DIC("A")="VENDOR:"
 S DIC="^PRC(440,",DIC(0)="AEQM" D %DIC^RMPVFM I $D(DUOUT)!$D(DTOUT) G CO
 G:+Y<0 VEN0
 S $P(R1(0),U,9)=+Y,$P(R3("D"),U,9)=$P(Y,U,2) K DIR,DIC
 ;
SOURCE ;
 K DIR S DIR(0)="660,12",DIR("B")=$P(R1(0),U,14),DIR("A")="SOURCE"
 D %DIR^RMPVFM G:$D(DIRUT)!$D(DUOUT) CO^RMPV0RMPRPIYE G:$D(DTOUT) EXIT^RMPRPIYF
 S $P(R1(0),U,14)=Y,$P(R3("D"),U,14)=$S(Y="C":"Commercial",1:"VA")
 ;
QTY K DIR S DIR("A")="QUANTITY"
 S DIR(0)="660,5",DIR("B")=$P(R1(0),U,7)
 D %DIR^RMPVFM G:$D(DIRUT)!$D(DUOUT) CO^RMPV0RMPRPIYE G:$D(DTOUT) EXIT^RMPRPIYF
 I $D(RMUBA),((RMUBA+$P(R1(0),U,7))-Y<0) D LOWBA^RMPRPIYI G HCPCS^RMPV0RMPRPIYE
