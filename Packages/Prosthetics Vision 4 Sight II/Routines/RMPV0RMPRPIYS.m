RMPV0RMPRPIYS ; OIT/JDA - Adapted from RMPRPIYS; Dec 01, 2024@21:44:42
 ;;1.0;PROSTHETICS VISION 4 SIGHT II;**2**;Jan 31, 2025;Build 38
 ;Per VHA Directive 10-93-142, this routine should not be modified.
 ;
 ; Reference to CHK^RMPRED5 supported by ICR #6545
 ; Reference to HPACT^RMPRPIX1 supported by ICR #7116
 ; Reference to GET^RMPRPIX1 supported by ICR #7116
 ; Reference to ETOI^RMPRPIX1 supported by ICR #7116
 ; Reference to ^RMPR(661.1, supported by ICR #6506
 ; Reference to GET^RMPRPIX6 supported by ICR #7118
 ; Reference to VNDIEN^RMPRPIX6 supported by ICR #7118
 ; Reference to CPT^RMPRCPTU supported by ICR #7543
 ;
...
STN S RMPROVAL=$G(RMPRSTN("IEN"))
 D WRITEIND^RMPVIO("@IOF") S RMPRERR=$$STN^RMPRPIY1(.RMPRSTN,.RMPREXC)
 I RMPRERR G PBX
 I RMPREXC'="" G PBX
 I RMPROVAL'=RMPRSTN("IEN") K RMPR1,RMPR11
 ;
 ;***** HCPCS - prompt for HCPCS and Item
HCPCS D WRITECTL^RMPVIO("!!"),WRITE^RMPVIO("Print Bar code Labels for current inventory..."),WRITECTL^RMPVIO("!")
HCPCS2 D HCPCS^RMPRPIY7(RMPRSTN("IEN"),$G(RMPR1("HCPCS")),.RMPR1,.RMPR11,.RMPREXC)
 I RMPREXC="T" G PBX
 I RMPREXC="P" G STN
 I RMPREXC="^" D  G PBX
 . D WRITECTL^RMPVIO("!"),WRITE^RMPVIO("** No HCPCS selected...") H 1
 . Q 
...
HCPCS4 D WRITECTL^RMPVIO("!!"),WRITE^RMPVIO("HCPCS: "_RMPR1("HCPCS")_" "_RMPR1("SHORT DESC"))
 D WRITECTL^RMPVIO("!!"),WRITE^RMPVIO("IFCAP Item: "),WRITE^RMPVIO(RMPR11("ITEM MASTER"))
 D WRITECTL^RMPVIO("!!"),WRITE^RMPVIO("PIP Item desc.: "),WRITE^RMPVIO(RMPR11("DESCRIPTION"))
 ;
...
CURST S RMPRLCN=""
 ...
 . D WRITECTL^RMPVIO("!"),WRITE^RMPVIO("This item is not currently in stock."),WRITECTL^RMPVIO("!!")
 . Q 
 K RMPR7I
...
NLAB S RMPRNLAB=RMPR7("QUANTITY")
 D WRITECTL^RMPVIO("!") D NLABP(.RMPRNLAB,RMPR7("QUANTITY"),.RMPREXC)
 I RMPREXC="T" G PBX
 I RMPREXC="P" G HCPCS
 I RMPREXC="^" G HCPCS
 ;
...
SELD S %ZIS("A")="Select Bar Code Printer: "
 S %ZIS="QM" K IOP D WRITECTL^RMPVIO("!") D ^%ZIS G:POP PRINTX
 I $G(IOST)'["P-ZEBRA" D 
 . D WRITECTL^RMPVIO("!!"),WRITE^RMPVIO("** WARNING - This is NOT a Zebra Bar Code Printer!!"),WRITECTL^RMPVIO("!!")
 ...
 D ^%ZTLOAD D:$D(ZTSK) WRITECTL^RMPVIO("!"),WRITE^RMPVIO("REQUEST QUEUED!") H 2 G PRINTC
 ;
...
NLABP(RMPRNLAB,RMPRMAX,RMPREXC) ;
 ...
 S RMPRNLAB=+Y
NLABPX Q 
NLABPH2 D WRITE^RMPVIO("Type in the number of bar code labels you want to print for the"),WRITECTL^RMPVIO("!")
 D WRITE^RMPVIO("inventory item you have selected."),WRITECTL^RMPVIO("!")
 Q 
 ;
...
BARC1 D %DIR^RMPVFM
 ...
 S RMPRBARC=X
BARCX Q 
BARCH D WRITE^RMPVIO("If you have access to a bar code scanner, use it to scan the item bar code now."),WRITECTL^RMPVIO("!")
 D WRITE^RMPVIO("Don't press the [Enter] key as the scanner should do this automatically."),WRITECTL^RMPVIO("!")
 D WRITE^RMPVIO("If the scanner cannot read the bar code, type in the character sequence"),WRITECTL^RMPVIO("!")
 D WRITE^RMPVIO("immediately below the bar code."),WRITECTL^RMPVIO("!")
...
SCAN1 D BARC(.RMPRBARC,.RMPREXC)
 ...
 I $P(RMPRBARC,"-")="" D WRITECTL^RMPVIO("!"),WRITE^RMPVIO("** No HCPCS Selected due to null HCPCS entered...") G SCAN
 ...
 I '$G(RMDAHC) D WRITECTL^RMPVIO("!"),WRITE^RMPVIO("** No HCPCS Selected or Unable to Select Inactive HCPCS...") G SCAN
 ...
 I $G(RMPR7("IEN"))="" D WRITECTL^RMPVIO("!"),WRITE^RMPVIO("*** The Item scanned is not available, please update your inventory !!!") G SCAN1
...
SCANX Q 
SCANE D WRITECTL^RMPVIO("!"),WRITE^RMPVIO("A problem has occurred with the scan, please try again."),WRITECTL^RMPVIO("!")
 Q 
 ;
...
CPT(RDA) ;
 ...
 D:$G(REDIT) WRITECTL^RMPVIO("!"),WRITE^RMPVIO("OLD CPT MODIFIER: "),WRITE^RMPVIO($P(R1(1),U,6))
 I RMHCOLD'=RMDAHC D CPT^RMPRCPTU(RDA)
 I $D(DUOUT) S RMPREXC="^" G CPTX
 I $D(DTOUT) S RMPREXC="T" G CPTX
 S $P(R1(1),U,6)=$G(RMCPT)
 D:$G(REDIT) WRITECTL^RMPVIO("!"),WRITE^RMPVIO("NEW CPT MODIFIER: "),WRITE^RMPVIO($G(RMCPT))
 I RMHCOLD'="",(RMHCOLD=RMDAHC),$G(REDIT) D 
 . S DIR(0)="Y",DIR("A")="Would you like to Edit CPT MODIFIER Entry  ",DIR("B")="N" D %DIR^RMPVFM Q:$D(DTOUT)!$D(DUOUT) 
 . I $G(Y) D 
 .. D CPT^RMPRCPTU(RDA)
 .. I $D(DUOUT) S RMPREXC="^"
 .. I $D(DTOUT) S RMPREXC="T"
 .. D:RMCPT=$P(R1(1),U,6) WRITECTL^RMPVIO("!!"),WRITE^RMPVIO("***Based on the information given above, CPT modifier string has not changed!!!"),WRITECTL^RMPVIO("!")
 .. D:RMCPT'=$P(R1(1),U,6) WRITECTL^RMPVIO("!"),WRITE^RMPVIO("NEW CPT MODIFIER: "),WRITE^RMPVIO($G(RMCPT))
 .. S $P(R1(1),U,6)=$G(RMCPT)
CPTX Q 
UPCASE(RMPRSTR) ;
