...
QTY K DIR,Y S DIR(0)="660,5",DIR("B")=1 S:$P(R1(0),U,7) DIR("B")=$P(R1(0),U,7)
 D %DIR^RMPVFM I $P(R1(0),U,7)'=""&$D(DUOUT) G LIST
 I $D(DTOUT) X CK2 G ^RMPV0RMPRPIYI
 I $D(DIRUT) G ^RMPV0RMPRPIYI
 I $G(RMUBA),((RMUBA-Y)<0) D LOWBA^RMPRPIYI G 2^RMPV0RMPRPIYI
 I $G(RMITQTY),RMITQTY<Y D WRITECTL^RMPVIO("!"),WRITE^RMPVIO("Issue quantity exceeds on-hand ("),WRITE^RMPVIO(RMITQTY),WRITE^RMPVIO(") for scanned item bar code!!"),WRITECTL^RMPVIO("!") G QTY
...
SERV ;date of service
 ...
 D %DIR^RMPVFM K DIR I $D(DTOUT) X CK2 G ^RMPV0RMPRPIYI
 I $D(DUOUT),$G(REDIT) G LIST
 I (X="")!(X="@") D WRITECTL^RMPVIO("!"),WRITE^RMPVIO("This field is mandatory!!!"),WRITECTL^RMPVIO("!") G SERV
 S $P(R1(1),U,8)=Y D DD^%DT S $P(R1("D"),U,8)=Y
 ;
LI S DIR(0)="660,9" S:$P(R1(0),U,11)'="" DIR("B")=$P(R1(0),U,11)
 D %DIR^RMPVFM I $D(DTOUT) X CK1 Q 
 G:$D(DUOUT) LIST
 I X["^" D WRITECTL^RMPVIO("!"),WRITE^RMPVIO("Jumping not allowed") G LI
 I $P(R1(0),U,11)'=""&(X="@") S $P(R1(0),U,11)="" D WRITE^RMPVIO($C(7)),WRITECTL^RMPVIO("!?5"),WRITE^RMPVIO("Deleted...") H 1 G LOT
 S $P(R1(0),U,11)=X
 ;
LOT ;
 ;
 ...
 I $P(R1(0),U,24)'=""&(X="@") S $P(R1(0),U,24)="" D WRITE^RMPVIO($C(7)),WRITECTL^RMPVIO("!?5"),WRITE^RMPVIO("Deleted...") H 1 G REMA
 S $P(R1(0),U,24)=X
 ;
REMA ;
 ;
 ...
 I $P(R1(0),U,18)'=""&(X="@") S $P(R1(0),U,18)="" D WRITE^RMPVIO($C(7)),WRITECTL^RMPVIO("!?5"),WRITE^RMPVIO("Deleted...") H 1 G LIST
 S $P(R1(0),U,18)=X
 ;
LIST ;ENTRY POINT FOR STOCK ISSUE ROUTINES TO DISPLAY TRANSACTION DATA
 ...
 D %DIR^RMPVFM K DIR G:Y="P" POST G:Y="D" DEA
 I Y="E" S REDIT=1 G 1^RMPV0RMPRPIYI
 I $D(DIRUT)!$D(DUOUT)!$D(DTOUT) G ^RMPV0RMPRPIYI
 ;
DEA ;
 K DIR
 S DIR("A")="Are you sure you want to DELETE this entry"
 S DIR("B")="N",DIR(0)="Y"
 D %DIR^RMPVFM I $D(DTOUT)!$D(DUOUT)!$D(DIRUT) X CK Q 
 I Y=1 D WRITECTL^RMPVIO("!!"),WRITE^RMPVIO($C(7)),WRITECTL^RMPVIO("?50"),WRITE^RMPVIO(" Deleted...") H 2 K DIR G RES^RMPV0RMPRPIYI
 G LIST
 ;
POST ;
...
GGC ;
 ...
 I RMPRERR=9 D LOWBA^RMPRPIYI G 2^RMPV0RMPRPIYI
 I RMPRERR D WRITECTL^RMPVIO("!"),WRITE^RMPVIO("*** ERROR in API RMPRPIU6, ERROR = "),WRITE^RMPVIO(RMPRERR),WRITE^RMPVIO(" !!!") G EXIT
 S ^TMP($J,"RMPRPCE",660,RMPR60("IEN"))=RMPR699("AMIS GROUPER")_"^"_$G(RMPRDFN)
 ;
 D WRITECTL^RMPVIO("!"),WRITE^RMPVIO("Posted to 2319...") H 3
 G RES^RMPV0RMPRPIYI
 ;
