RMPV0RMPRPIYF ; OIT/JDA - Adapted from RMPRPIYF; Dec 01, 2024@21:29:57
 ;;1.0;PROSTHETICS VISION 4 SIGHT II;**2**;Jan 31, 2025;Build 38
 ;Per VHA Directive 10-93-142, this routine should not be modified.
 ;
 ; Reference to UPD^RMPRPIX2 supported by ICR #7117
 ; Reference to UPD^RMPRPIX6 supported by ICR #7118
 ; Reference to CRE^RMPRPIX6 supported by ICR #7118
 ; Reference to SET60^RMPRPIYE supported by ICR #7562
 ; Reference to DEL1^RMPRPIFD supported by ICR #7542
...
COST ;
 ...
 ;
DATE S:$P(R1(1),U,8) DIR("B")=$P(R1("D"),U,8) S DIR("A")="DATE OF SERVICE",DIR(0)="660,39" D %DIR^RMPVFM K DIR
 G:X["^" CO^RMPV0RMPRPIYE G:$D(DTOUT) EXIT I $P(R1(1),U,8)&(X="@") D WRITECTL^RMPVIO("!"),WRITE^RMPVIO("This field is mandatory!!!"),WRITECTL^RMPVIO("!") G DATE
 I X="" D WRITECTL^RMPVIO("!"),WRITE^RMPVIO("This field is mandatory!!!"),WRITECTL^RMPVIO("!") G DATE
 S $P(R1(1),U,8)=Y,Y=$P(R1(1),U,8) D DD^%DT S $P(R1("D"),U,8)=Y
 ;
REQ S DIR(0)="660,9" S:$P(R1(0),U,11)'="" DIR("B")=$P(R1(0),U,11) D %DIR^RMPVFM G:$D(DUOUT) CO^RMPV0RMPRPIYE G:$D(DTOUT) EXIT
 I X["^" D WRITECTL^RMPVIO("!"),WRITE^RMPVIO("Jumping not allowed!") G REQ
 I $P(R1(0),U,11)'=""&(X="@") D WRITECTL^RMPVIO("!?5"),WRITE^RMPVIO("Deleted...") H 1 S $P(R1(0),U,11)="" G LOT
 S $P(R1(0),U,11)=X
 ;
LOT K DIR S DIR(0)="660,21" S:$P(R1(0),U,24)'="" DIR("B")=$P(R1(0),U,24) D %DIR^RMPVFM G:$D(DUOUT) CO^RMPV0RMPRPIYE
 I X["^" D WRITECTL^RMPVIO("!"),WRITE^RMPVIO("Jumping not allowed!") G LOT
 I $P(R1(0),U,24)'=""&(X="@") D WRITECTL^RMPVIO("!?5"),WRITE^RMPVIO("Deleted...") H 1 S $P(R1(0),U,24)="" G REMA
 S $P(R1(0),U,24)=X
 ;
REMA K DIR S DIR(0)="660,16" S:$P(R1(0),U,18)'="" DIR("B")=$P(R1(0),U,18) D %DIR^RMPVFM G:$D(DUOUT) CO^RMPV0RMPRPIYE G:$D(DTOUT) EXIT
 I X["^" D WRITECTL^RMPVIO("!"),WRITE^RMPVIO("Jumping not allowed!") G REMA
 I $P(R1(0),U,18)'=""&(X="@") D WRITECTL^RMPVIO("!?5"),WRITE^RMPVIO("Deleted...") H 1 S $P(R1(0),U,18)="" G CC
 S $P(R1(0),U,18)=X
CC G CO^RMPV0RMPRPIYE
 ;
POST ;POSTS EDITED TRANSACTION TO 660
 D WRITECTL^RMPVIO("!"),WRITE^RMPVIO("Posting....")
...
API ;call API for 660, 661.7, 661.6, 661.63, 661.9
 ...
 I RMFLG=2 D QUAN
 D UP660
 I $G(RMPRERR) D WRITECTL^RMPVIO("!!"),WRITE^RMPVIO("*** ERROR in 2319 UPDATE, Please notify your IRM..IEN = "),WRITE^RMPVIO($G(RMPR60("IEN"))),WRITECTL^RMPVIO("!!") H 3
 ;
PCE ;update PCE data
 ;
...
UP660 ;update 660
 ...
 I $G(RMPRERR) D WRITECTL^RMPVIO("!"),WRITE^RMPVIO("*** Error in API RMPRPIX2, ERROR = "),WRITE^RMPVIO(RMPRERR),WRITECTL^RMPVIO("!"),WRITE^RMPVIO("*** Please inform your IRM !!"),WRITECTL^RMPVIO("!")
 Q 
 ;
...
QUAN ;only update quantity
 ...
 Q 
 ;
ERR D WRITECTL^RMPVIO("!!"),WRITE^RMPVIO("Error encountered while posting to PIP.  Patient 10-2319 not deleted!! Please check with your Application Coordinator.") H 5 G EXIT
