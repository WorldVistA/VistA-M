SOWKCLSW ;B'HAM ISC/SAB-Routine to print caseload summary for all workers ; 08 Apr 93 / 4:01 PM [ 09/26/94  1:00 PM ]
...
DEV W !!,"WARNING !!!",!?5,"This report is formatted for 132 columns and will be",!?5,"difficult to read if printed to the screen.",!
 ...
 K SOWKION I $D(IO("Q")) S ZTDESC="CLINICAL SUMMARY FOR ALL WORKERS",ZTRTN="EN^SOWKCLSW" F G="SOWKED","SOWKBEG","COM","SOWKBG" S:$D(@G) ZTSAVE(G)=""
...
CLOS W ! W:$E(IOST)'["C" @IOF D ^%ZISC K COM,STA,STN,SOWKED,^TMP($J),G,PG,SOWKBEG,%DT,SOWKI,SOWKI1,CN,C,D,I,M,OC,OUT,PR,PAT,T,X,Y,TD,WRK,%H,DS,SOWKBG D KVA^VADPT D:$D(ZTSK) KILL^%ZTLOAD
 Q
SETUP S C=0,(PR,OC)="" F I=0:0 S C=C+1,I=$O(^SOWK(650,SOWKI1,2,I)) Q:'I!(C>8)  S PR=PR_$P(^SOWK(650,SOWKI1,2,I,0),"^")_" ",OC=OC_$P(^(0),"^",2)_" "
 ...
 S DFN=$P(CN,U,8) D PID^VADPT6 ;this will return the var. VA("BID")
...
PRI U IO I ($Y+5)>IOSL D HDR Q:$G(OUT)=1
 ...
 Q
HDR D:PG'=0 CHK Q:$G(OUT)=1  D
 .S PG=PG+1 U IO W !,"SOCIAL WORKER CLINICAL SUMMARY - ALL WORKERS"_$S(COM=2:" (CLOSED CASES)",COM=3:" (OPENED CASES)",1:" (ALL CASES)"),?89,TD,?121,"PAGE: "_PG,!,STA_" ("_STN_")",?45,"REPORTING PERIOD "_SOWKBG_" TO "_SOWKED
