...
DEV ; -- select device, run option
 ...
 I $D(IO("Q")) F I="DG1","DG6","IBTY","IBBDT","IBEDT","IBDIAG(","IBTY1" S ZTSAVE(I)=""
 I $D(IO("Q")) S ZTRTN="START^IBQLR4",ZTDESC="UM - ACUTE REPORT" D ^%ZTLOAD K IO("Q"),ZTSK D HOME^%ZIS G END
 U IO
 ;
START ;
 K ^TMP("IBQLR4",$J) S IBDDT=IBBDT-.01,(IBPAG,IBQUIT,IBMCT)=0
 ...
 D PRINT^IBQLR4A
END ; -- Clean up
 W ! K ^TMP("IBQLR4",$J),IB,IBMONTH,IBDDT,IBBDT,IBEDT,IBTRN,IBTRND,IBTY,IBTEXT,IBDATA,IBHDR,IBQUIT,IBPAG,IBTRV,IBMONTH,MSTRING,IBREA,I,N,X,IBRES,IBCAT,IBMTH,IBMD,IBDIAG
...
DATA ;
 ...
 .S ^($E(IBDDT,1,5))=$G(^TMP("IBQLR4",$J,IBDIAG1,1,"CNTA",$E(IBDDT,1,5)))+1
 ; -- count non-acute admissions
 E  D
 .S ^($E(IBDDT,1,5))=$G(^TMP("IBQLR4",$J,IBDIAG1,1,"CNTN",$E(IBDDT,1,5)))+1
 .F I=1:1 S IBR=$P(IB(1.03)," ",I) Q:'IBR  S ^($E(IBDDT,1,5))=$G(^TMP("IBQLR4",$J,IBDIAG1,1,"REA",IBR,$E(IBDDT,1,5)))+1
 ; --continued stay days
 ...
 ..F I=1:1 S IBR=$P(IB(13.06)," ",I) Q:'IBR  S ^($E(IBDDT,1,5))=$G(^TMP("IBQLR4",$J,IBDIAG1,2,"REA",IBR,$E(IBDDT,1,5)))+1
 Q
 ;
