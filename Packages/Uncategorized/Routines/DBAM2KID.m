DBAM2KID ; Save patches to HFS files in respective folders ;12/14/98  10:56
 ...
 I '$D(DUZ) D ^XUP
 W !
 W !,"This routine takes each FORUM VistA patch message and separates it into "
 W !,"one txt file and one KIDS build, except in the event of informational patches. "
 W !
 W !,"Informational patches do not have a KIDS build associated with them "
 W !,"and will only be displayed as a txt file. "
 W !
 W !,"Informational patches may or may not have a separate "
 W !,"KIDS build that has been downloaded, reviewed and scrubbed."
 W !
...
GETTXT .D   ; GET TEXT
 ..;
 ..S LINE=.99 F  S LINE=$O(^TMP($J,"MSG",LINE)) Q:LINE'>0  S TEXT=^TMP($J,"MSG",LINE,0) D ADDLINE("TEXT",TEXT) Q:TEXT["$END TXT"  S SAVTXT=1
 .;
GETKID .D   ; GET KIDS
 ..;
 ...
 ..D ADDLINE("KIDS","")
 ..F  S LINE=$O(^TMP($J,"MSG",LINE)) Q:LINE'>0  S TEXT=^TMP($J,"MSG",LINE,0) Q:TEXT["$END KID"  D ADDLINE("KIDS",TEXT) S SAVKID=1
 ..D ADDLINE("KIDS","**END**")
 ..D ADDLINE("KIDS","**END**")
 ..D ADDLINE("KIDS","")
 .;
...
SAVKID .; SAVE KIDS
 ...
 .S REDLIST="" F  S REDLIST=$O(REDLIST(REDLIST)) Q:'REDLIST  W !!,REDLIST(REDLIST,"SUBJ"),!,?5,REDLIST(REDLIST,"TEXT")
 ;
...
REDACT ;
 ...
 .S TEXT=$$REPLACE(TEXT,"va.gov","domain.ext")
 .;
 .I ($$UPCASE(TEXT)["VA.GOV") S REDLIST=$G(REDLIST)+1,REDLIST(REDLIST,"TEXT")=TEXT,REDLIST(REDLIST,"SUBJ")=SUBJECT
 .I ($$UPCASE(TEXT)?.E1"10."1.3N1"."1.3N1"."1.3N.E)  S REDLIST=$G(REDLIST)+1,REDLIST(REDLIST,"TEXT")=TEXT,REDLIST(REDLIST,"SUBJ")=SUBJECT
 .;
 .S ^TMP($J,SUB,D0)=TEXT
 ;
 Q
