ICD112A ;ALB/MJF - YEARLY DRG UPDATE; October 01, 2020@15:42
 ...
 ;
 ;Update the (#80.2) DRG file with FY 2023 DRG Grouper MS-DRG codes.
 ;
 Q
 ;
 ;Routines ICD112* contain each FY 2023 MS-DRG code update values
...
DRG ;post-install driver (#80.2) DRG updates
 ...
 ;   ICDTOT - Total MS-DRG codes filed
 ;
 D BMES^XPDUTL(">>> Adding FY 2023 DRG Grouper updates to (#80.2) DRG file...")
 N ICDRTN,ICDI,ICDSUB,ICDEDIT,ICDADD,ICDTMP
 S (ICDEDIT,ICDADD)=0
 S ICDTOT=$G(ICDTOT) I ICDTOT']"" S ICDTOT=0
 S ICDTMP=$G(ICDTMP)
 I ICDTMP']"" S ICDTMP=$NA(^TMP("DRGFY2023",$J)) D
 . K @ICDTMP
 . S @ICDTMP@(0)="PATCH FY 2023 DRG UPDATE^"_$$NOW^XLFDT
 ;
 ;loop each sub-routine
 S ICDSUB="FGHIJK"
 F ICDI=1:1:6 S ICDRTN="^ICD112"_$E(ICDSUB,ICDI) D
 .Q:($T(@ICDRTN)="")
...
WEIGHTS(ICDLINE,ICDTMP) ;--80.22D subfile - #20 FISCAL YEAR WEIGHTS&TRIM
 ...
 S ICDFYR=3230000,ICDLOW=1,ICDHIGH=99  ; *** default Low/High ???
 S ICDDRG=+$P(ICDLINE,U)
 I '$D(^ICD(ICDDRG,0)) D  Q
 . S @ICDTMP@("ERROR",ICDDRG,0)="MISSING (#80.2) DRG FILE RECORD"
 ;
