ICD125A ;ALB/DMR - YEARLY DRG UPDATE; October 01, 2025@15:42
 ;;18.0;DRG Grouper;**125**;October 20, 2000;Build 4
 ;Per VA Directive 6402, this routine should not be modified.
 ;
 ;Update the (#80.2) DRG file with FY 2026 DRG Grouper MS-DRG codes.
 ;
 Q
 ;
 ;Routines ICD125* contain each FY 2026 MS-DRG code update values
...
DRG ;post-install driver (#80.2) DRG updates
 ...
 ;element values used to create the FY 2026 MS-DRG updates.
 ; Input:
 ...
 ;   ICDTOT - Total MS-DRG codes filed
 ;
 D BMES^XPDUTL(">>> Adding FY 2026 DRG Grouper updates to (#80.2) DRG file...")
 N ICDRTN,ICDI,ICDSUB,ICDEDIT,ICDADD,ICDTMP
 S (ICDEDIT,ICDADD)=0
 S ICDTOT=$G(ICDTOT) I ICDTOT']"" S ICDTOT=0
 S ICDTMP=$G(ICDTMP)
 I ICDTMP']"" S ICDTMP=$NA(^TMP("DRGFY2026",$J)) D
 . K @ICDTMP
 . S @ICDTMP@(0)="PATCH FY 2026 DRG UPDATE^"_$$NOW^XLFDT
 ;
 ;loop each sub-routine
 S ICDSUB="FGHIJK"
 F ICDI=1:1:6 S ICDRTN="^ICD125"_$E(ICDSUB,ICDI) D
 .Q:($T(@ICDRTN)="")
...
DESCA(ICDDRG,ICDFY,ICDTMP) ;-- 80.268D - #68 DESCRIPTION
 ...
 D UPDATE^DIE("","ICDFDA","","ICDERR") K ICDFDA
 S ^DMRTRACE(ICDDRG,"DESCB")="ICDDRG: "_ICDDRG_" ICDTMP: "_ICDTMP_" ICDFY: "_ICDFY
 I $D(ICDERR) D  K ICDERR
...
DESCB(ICDDRG,ICDFY,ICDDESC,ICDTMP) ;-- 80.2681 - #68 DESCRIPTION
 ...
 D UPDATE^DIE("","ICDFDA","","ICDERR") K ICDFDA
 S ^DMRTRACE(ICDDRG,"DESCB")="ICDDRG: "_ICDDRG_" ICDDESC: "_ICDDESC_" ICDTMP: "_ICDTMP_" ICDFY: "_ICDFY
 I $D(ICDERR) D  K ICDERR
...
WEIGHTS(ICDLINE,ICDTMP) ;--80.22D subfile - #20 FISCAL YEAR WEIGHTS&TRIM
 ...
 S ICDFYR=3260000,ICDLOW=1,ICDHIGH=99  ; *** default Low/High ???
 S ICDDRG=+$P(ICDLINE,U)
 I '$D(^ICD(ICDDRG,0)) D  Q
 . S @ICDTMP@("ERROR",ICDDRG,0)="MISSING (#80.2) DRG FILE RECORD"
 ;
