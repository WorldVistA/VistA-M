MPIFA31B ;BP/CMC-BUILD A31 MSGS ;7/15/21  15:04
 ;;1.0;MASTER PATIENT INDEX VISTA;**22,24,27,28,31,25,44,46,54,59,60,64,68,75,78**;30 Apr 99;Build 1
 ;
...
A31(DFN) ;BUILD AND SEND A31
 ...
 N ZEL,ZSP,SIDG,NAMECOMP,DODF,DODD,DODNP,DODOPT,DODDISDT,SECLVL,SEXOR,SEXORD,PRON,PROND
 K HLA("HLA"),HLA("HLS")
 ...
 .D SEXOR^VAFCSB(DFN,.SEXOR) ;**78, VAMPI-11114 (dri)
 .D SEXORD^VAFCSB(DFN,.SEXORD) ;**78, VAMPI-11114 (dri)
 .D PRON^VAFCSB(DFN,.PRON) ;**78, VAMPI-11118 (dri)
 .D PROND^VAFCSB(DFN,.PROND) ;**78, VAMPI-11118 (dri)
 ...
 I $O(SEXOR(0)) N CNT S CNT=0 F  S CNT=$O(SEXOR(CNT)) Q:'CNT  S HLA("HLS",EN)=SEXOR(CNT),EN=EN+1 ;**78, VAMPI-11114 (dri)
 I $O(SEXORD(0)) D  S EN=EN+1 ;**78, VAMPI-11114 (dri)
 .N CNT,LVL
 .S LVL=1,CNT=0 F  S CNT=$O(SEXORD(CNT)) Q:'CNT  D
 ..I CNT=1 S HLA("HLS",EN)=SEXORD(CNT)
 ..I CNT>1 S HLA("HLS",EN,LVL)=SEXORD(CNT),LVL=LVL+1
 I $O(PRON(0)) N CNT S CNT=0 F  S CNT=$O(PRON(CNT)) Q:'CNT  S HLA("HLS",EN)=PRON(CNT),EN=EN+1 ;**78, VAMPI-11118 (dri)
 I $O(PROND(0)) D  S EN=EN+1 ;**78, VAMPI-11118 (dri)
 .N CNT,LVL
