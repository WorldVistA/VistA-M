GMPLSAVE ; ISL/MKB,KER,JER,TC,PWC -- Save Problem List data ; Dec 11, 2024@14:00
 ;;2.0;Problem List;**26,31,35,37,38,36,42,47,45,49,54,57,58**;Aug 25, 1994;Build 33
 ;
 ; External References
 ;   Reference to $$CSI^ICDEX,$$SAB^ICDEX,$$CODECS^ICDEX in ICR #5747
 ;   Reference to $$GETDES^LEXTRAN1 in ICR #5009
...
EN ; Save Changes made to Existing Problem
 ...
 . Q:'$D(GMPFLD(FLD))
 . N DOAUDIT S DOAUDIT=1
 . I FLD=1.1!(FLD=1.11)!(FLD=1.12)!(FLD=1.13)!(FLD=1.15)!(FLD=1.16)!(FLD=1.17)!(FLD=1.18) S GMPOLDIND(FLD)=GMPFLD(FLD),DOAUDIT=0
 . Q:$P($G(GMPORIG(FLD)),U)=$P($G(GMPFLD(FLD)),U)
 . ; VSR pwc GMPL*2*54 - replace four slashes (////) with FileMan database calls
 ...
 . D:DOAUDIT AUDIT^GMPLX(CHNGE,"")
 D FILE^DIE("","GMPLFDA","GMPLERR")
 S GMPSAVED=1
 D SET^GMPLSPECAUTH(GMPIFN,.GMPFLD,.GMPOLDIND,$G(GMPROV),.GMPORIG)   ;add orig New SA 2 mult nodes plus auditing changes, 58.
...
NEW ; Save Collected Values in new Problem Entry
 ...
 ;   Set Node 1
 S DATA=$P(GMPFLD(1.01),U) F I=1.02:.01:1.18 D
 .I I=1.1!(I=1.11)!(I=1.12)!(I=1.13)!(I=1.15)!(I=1.16)!(I=1.17)!(I=1.18) S:$D(GMPFLD(I)) GMPOLDIND(I)=GMPFLD(I)
 .S DATA=DATA_U_$S($P($G(GMPFLD(+I)),U)="@":"",1:$P($G(GMPFLD(+I)),U))
 ...
 ; broadcast event
 D SET^GMPLSPECAUTH(DA,.GMPFLD,.GMPOLDIND,0)   ;58
