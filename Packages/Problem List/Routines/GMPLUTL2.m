GMPLUTL2 ; ISL/MKB,KER,JER,TC -- PL Utilities (OE/TIU) ; Mar 3, 2026@15:40
 ;;2.0;Problem List;**10,18,21,26,35,36,42,45,49,58**;Aug 25, 1994;Build 33
 ;
 ; External References
 ;   DBIA   348  ^DPT(  file #2
 ;    ICR  5747  $$SAB^ICDEX, $$CODECS^ICDEX, $$CSI^ICDEX, $$ICDDX^ICDEX, $$ICDD^ICDEX
...
DETAIL(IFN,GMPL) ; Returns Detailed Data for Problem
 ...
 ;
 Q:'$D(^AUPNPROB(IFN,0))
 N GMPL0,GMPL1,GMPL800,GMPL802,GMPL803,GMPLP,X,I,FAC,CNT,NIFN,GMPLNTDT,GMPLATHR,GMPLTEXT,GMPLCSIEN
 K ^TMP("GMPLCMT",$J)
 S GMPLP=+($$PTR^GMPLUTL4)
 S GMPL0=$G(^AUPNPROB(IFN,0)),GMPL1=$G(^(1)),GMPL800=$G(^(800)),GMPL802=$G(^(802)),GMPL803=$G(^(803,0))
 S GMPL("DTINTEREST")=$S(+$P(GMPL802,U,1):$P(GMPL802,U,1),1:$P(GMPL0,U,8))
 S GMPLCSIEN=$$CSI^ICDEX(80,+GMPL0),GMPL("CSYS")=$S($P(GMPL802,U,2)]"":$P(GMPL802,U,2),1:$$SAB^ICDEX(GMPLCSIEN,GMPL("DTINTEREST")))
 S GMPL("DIAGNOSIS")=$P($$ICDDX^ICDEX(+GMPL0,GMPL("DTINTEREST"),GMPLCSIEN,"I"),U,2)
 ...
 . S GMPL("ICD9MLTP")=CNT
 D DETAIL^GMPLSPECAUTH(.GMPL,IFN,GMPL1)   ;58 use new SA structure api
...
ICDDESC(GMPLCODE,GMPDT,GMPLCSYS) ; Get description for ICD Code
 ...
 S ICDY=$$ICDD^ICDEX(GMPLCODE,.ICDD,GMPDT,GMPLCSYS)
 I 'ICDY G ICDDESQ
 S GMPY=$G(ICDD(1))
ICDDESQ Q GMPY
