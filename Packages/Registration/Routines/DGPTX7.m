DGPTX7 ; GENERATED FROM 'DG701' INPUT TEMPLATE(#432), FILE 45;06/24/20
 D DE G BEGIN
DE S DIE="^DGPT(",DIC=DIE,DP=45,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGPT(DA,""))=""
 ...
 I $D(^(82)) S %Z=^(82) S %=$P(%Z,U,1) S:%]"" DE(9)=% S %=$P(%Z,U,2) S:%]"" DE(19)=% S %=$P(%Z,U,3) S:%]"" DE(29)=% S %=$P(%Z,U,4) S:%]"" DE(39)=% S %=$P(%Z,U,5) S:%]"" DE(49)=% S %=$P(%Z,U,6) S:%]"" DE(59)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
56 S DW="70;20",DV="*P80'X",DU="",DIFLD=79.201,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C56^DGPTX7",DE(DW,"INDEX")=1
 S DU="ICD9("
 G RE
...
X58 I DGCODSYS="ICD9"!(DGXX="")!(DGTYPE=2) S Y="@51"
 Q
...
X63 S:$P($G(^DGPT(D0,70)),U,20,24)_$P($G(^DGPT(D0,71)),U,1,3)?."^" Y="@120"
 Q
64 S DQ=65 ;@55
...
66 S DW="70;21",DV="*P80'X",DU="",DIFLD=79.21,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C66^DGPTX7",DE(DW,"INDEX")=1
 S DU="ICD9("
 G RE
...
X68 I DGCODSYS="ICD9"!(DGXX="")!(DGTYPE=2) S Y="@56"
 Q
69 D:$D(DG)>9 F^DIE17 G ^DGPTX73
