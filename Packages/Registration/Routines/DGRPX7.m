DGRPX7 ; GENERATED FROM 'DG LOAD EDIT SCREEN 7' INPUT TEMPLATE(#420), FILE 2;03/08/11
 D DE G BEGIN
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 ...
 I $D(^(.3)) S %Z=^(.3) S %=$P(%Z,U,1) S:%]"" DE(5)=% S %=$P(%Z,U,2) S:%]"" DE(7)=% S %=$P(%Z,U,4) S:%]"" DE(11)=% S %=$P(%Z,U,5) S:%]"" DE(9)=% S %=$P(%Z,U,11) S:%]"" DE(28)=% S %=$P(%Z,U,12) S:%]"" DE(8)=% S %=$P(%Z,U,13) S:%]"" DE(13)=%
 I $D(^(.31)) S %Z=^(.31) S %=$P(%Z,U,3) S:%]"" DE(21)=% S %=$P(%Z,U,4) S:%]"" DE(22)=%
 I $D(^(.362)) S %Z=^(.362) S %=$P(%Z,U,6) S:%]"" DE(33)=% S %=$P(%Z,U,12) S:%]"" DE(25)=% S %=$P(%Z,U,13) S:%]"" DE(26)=% S %=$P(%Z,U,14) S:%]"" DE(27)=% S %=$P(%Z,U,17) S:%]"" DE(31)=% S %=$P(%Z,U,20) S:%]"" DE(30)=%
...
X20 S %DT="E" D ^%DT S X=Y K:Y<1 X I $D(X) S DFN=DA D INCOM^DGLOCK
 Q
 ;
21 S DW=".31;3",DV="FXOa",DU="",DLB="CLAIM NUMBER",DIFLD=.313
...
X26 S DFN=DA D MV^DGLOCK I $D(X) S DFN=DA D EV^DGLOCK
 Q
 ;
27 D:$D(DG)>9 F^DIE17,DE S DQ=27,DW=".362;14",DV="SX",DU="",DLB="RECEIVING A VA PENSION?",DIFLD=.36235
 S DE(DW)="C27^DGRPX7"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 G RE
...
C27S S X="" G:DG(DQ)=X C27F1 K DB
 D ^DGRPX74
C27F1 Q
X27 S DFN=DA D MV^DGLOCK
 Q
 ;
28 D:$D(DG)>9 F^DIE17,DE S DQ=28,DW=".3;11",DV="SX",DU="",DLB="RECEIVING VA DISABILITY?",DIFLD=.3025
 S DE(DW)="C28^DGRPX7"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 G RE
...
C28S S X="" G:DG(DQ)=X C28F1 K DB
 D ^DGRPX76
C28F1 Q
X28 S DFN=DA D MV^DGLOCK I $D(X),X="Y" D EC^DGLOCK1
 Q
 ;
29 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=29 D X29 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X29 S:(X'="Y")&($P($G(^DPT(DA,.362)),U,12,14)'["Y") Y=.36265
 Q
30 D:$D(DG)>9 F^DIE17,DE S DQ=30,DW=".362;20",DV="NJ8,2X",DU="",DLB="TOTAL ANNUAL VA CHECK AMOUNT",DIFLD=.36295
 S DE(DW)="C30^DGRPX7"
 G RE
...
C30S S X="" G:DG(DQ)=X C30F1 K DB
 D ^DGRPX77
C30F1 Q
X30 D DOL^DGLOCK2 K:+X'=X&(X'?.N1"."2N)!(X>99999)!(X<0) X I $D(X) S DFN=DA D MV^DGLOCK I $D(X),('$$TOTCHK^DGLOCK2(DFN)) D TOTCKMSG^DGLOCK2 K X
 Q
 ;
31 D:$D(DG)>9 F^DIE17,DE S DQ=31,DW=".362;17",DV="SX",DU="",DLB="GI INSURANCE POLICY?",DIFLD=.36265
 S DE(DW)="C31^DGRPX7"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 G RE
...
C31S S X="" G:DG(DQ)=X C31F1 K DB
 D ^DGRPX79
C31F1 Q
X31 S DFN=DA D MV^DGLOCK Q
 Q
 ;
32 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=32 D X32 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X32 S:X'="Y" Y="@703"
 Q
33 D:$D(DG)>9 F^DIE17,DE S DQ=33,DW=".362;6",DV="NJ8,2X",DU="",DLB="AMOUNT OF GI INSURANCE",DIFLD=.3626
 G RE
X33 D DOL^DGLOCK2 K:+X'=X&(X'?.N1"."2N)!(X>999999)!(X<1) X I $D(X) S DFN=DA D MV^DGLOCK I $D(X),$D(^DPT(DA,.362)),$P(^(.362),U,17)'="Y" W !?4,*7,"Applicant doesn't have GI Insurance." K X
 Q
 ;
34 S DQ=35 ;@703
35 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=35 D X35 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X35 S:DGDR'["703" Y="@704"
 Q
36 D:$D(DG)>9 F^DIE17 G ^DGRPX710
