DGRPXR ; GENERATED FROM 'DGRP COLLATERAL REGISTER' INPUT TEMPLATE(#422), FILE 2;12/08/20
 D DE G BEGIN
...
BEGIN S DNM="DGRPXR",DQ=1
 N DIEZTMP,DIEZAR,DIEZRXR,DIIENS,DIXR K DIEFIRE,DIEBADK S DIEZTMP=$$GETTMP^DIKC1("DIEZ")
 M DIEZAR=^DIE(422,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=422,U="^"
1 S DW="VET;1",DV="RSXa",DU="",DIFLD=1901,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C1^DGRPXR"
 S DU="Y:YES;N:NO;"
 S X="N"
 S Y=X
...
X1 I $D(X) S:'$D(DPTX) DFN=DA D:'$D(^XUSEC("DG ELIGIBILITY",DUZ)) VAGE^DGLOCK:X="Y" I $D(X) D:$D(DFN) EV^DGLOCK
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW=".36;1",DV="*P8'Xa",DU="",DIFLD=.361,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C2^DGRPXR",DE(DW,"INDEX")=1
 S DU="DIC(8,"
 S X=DGRPX
 S Y=X
...
C2F1 N X,X1,X2 S DIXR=815 D C2X1(U) K X2 M X2=X D C2X1("O") K X1 M X1=X
 D
...
C2F2 Q
X2 Q
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW=".32;3",DV="*P21'Xa",DU="",DIFLD=.323,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C3^DGRPXR"
 S DU="DIC(21,"
