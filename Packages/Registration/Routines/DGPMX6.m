...
DE S DIE="^DGPM(",DIC=DIE,DP=405,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGPM(DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,1) S:%]"" DE(2)=% S %=$P(%Z,U,4) S:%]"" DE(6)=%,DE(11)=% S %=$P(%Z,U,8) S:%]"" DE(16)=% S %=$P(%Z,U,9) S:%]"" DE(14)=% S %=$P(%Z,U,19) S:%]"" DE(17)=%
 I $D(^("USR")) S %Z=^("USR") S %=$P(%Z,U,3) S:%]"" DE(23)=% S %=$P(%Z,U,4) S:%]"" DE(25)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="DGPMX6",DQ=1
 N DIEZTMP,DIEZAR,DIEZRXR,DIIENS,DIXR K DIEFIRE,DIEBADK S DIEZTMP=$$GETTMP^DIKC1("DIEZ")
 M DIEZAR=^DIE(451,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=451,U="^"
1 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=1 D X1 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X1 S:$S(DGPMN:1,DGPMY=+^DGPM(DA,0):1,1:0) Y="@10"
 Q
...
X5 D ONLY^DGPMV36 S:'$D(DGPMSPI) Y="@20"
 Q
...
X10 S ^DISV(DUZ,"^DG(405.1,")=$S($D(^DISV(DUZ,"DGPM6")):^("DGPM6"),1:"")
 Q
11 D:$D(DG)>9 F^DIE17,DE S DQ=11,DW="0;4",DV="R*P405.1'X",DU="",DLB="SPECIALTY TRANSFER TYPE",DIFLD=.04
 S DE(DW)="C11^DGPMX6"
 S DU="DG(405.1,"
 G RE
...
X12 S ^DISV(DUZ,"DGPM6")=$S($D(^DISV(DUZ,"^DG(405.1,")):^("^DG(405.1,"),1:"")
 Q
13 S DQ=14 ;@30
14 D:$D(DG)>9 F^DIE17,DE S DQ=14,DW="0;9",DV="R*P45.7'a",DU="",DLB="FACILITY TREATING SPECIALTY",DIFLD=.09
 S DE(DW)="C14^DGPMX6"
 S DU="DIC(45.7,"
 G RE
...
C14F1 Q
X14 S DIC("S")="I $$ACTIVE^DGACT(45.7,Y,$P($G(^DGPM(DA,0)),U))" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
15 S DQ=16 ;@35
16 D:$D(DG)>9 F^DIE17,DE S DQ=16,DW="0;8",DV="*P200'",DU="",DLB="PRIMARY PHYSICIAN",DIFLD=.08
 S DE(DW)="C16^DGPMX6"
 S DU="VA(200,"
 G RE
...
C16F1 Q
X16 S DIC("S")="I $$SCREEN^DGPMDD(Y,DA,+$G(^DGPM(DA,0)))" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
17 D:$D(DG)>9 F^DIE17,DE S DQ=17,DW="0;19",DV="R*P200'",DU="",DLB="ATTENDING PHYSICIAN",DIFLD=.19
 S DE(DW)="C17^DGPMX6"
 S DU="VA(200,"
 G RE
...
C17F1 Q
X17 S DIC("S")="I $$SCREEN^DGPMDD(Y,DA,+$G(^DGPM(DA,0)))" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
X19 S:$D(DGPMBYP) Y="@40"
 Q
20 D:$D(DG)>9 F^DIE17,DE S DQ=20,D=0 K DE(1) ;99
...
X22 S:DGPMP=^DGPM(DA,0) Y=""
 Q
23 S DW="USR;3",DV="RP200'",DU="",DLB="LAST EDITED BY",DIFLD=102
 S DU="VA(200,"
 S X=DUZ
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
25 S DW="USR;4",DV="RD",DU="",DLB="LAST EDITED ON",DIFLD=103
 S %=$P($H,",",2),X=DT_(%\60#60/100+(%\3600)+(%#60/10000)/100)
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X25 S %DT="STX" D ^%DT S X=Y K:Y<1 X
 Q
 ;
26 G 0^DIE17
