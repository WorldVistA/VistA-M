DGX6 ; GENERATED FROM 'DG601-10P' INPUT TEMPLATE(#1766), FILE 45;02/01/23
 D DE G BEGIN
...
BEGIN S DNM="DGX6",DQ=1
 N DIEZTMP,DIEZAR,DIEZRXR,DIIENS,DIXR K DIEFIRE,DIEBADK S DIEZTMP=$$GETTMP^DIKC1("DIEZ")
 M DIEZAR=^DIE(1766,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=1766,U="^"
...
X3 S DGHOLD=$G(^DGPT(DGPTF,"P",+P(DGZP,1),0)) S:DGHOLD]"" DGHOLD1=$G(^(1))
 Q
4 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=4 D X4 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X4 D CENSUS^DGPTIC10(DA)
 Q
5 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=5 D X5 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X5 S DGTYPE=$P(^DGPT(D0,0),U,11),DGCODSYS=$$CODESYS^DGPTIC10(D0)
 Q
6 S D=0 K DE(1) ;60
 S DIFLD=60,DGO="^DGX61",DC="29^45.05D^P^",DV="45.05MDX",DW="0;1",DOW=$$LABEL^DIALOGZ(DP,DIFLD),DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 G RE:D I $D(DSC(45.05))#2,$P(DSC(45.05),"I $D(^UTILITY(",1)="" X DSC(45.05) S D=$O(^(0)) S:D="" D=-1 G M6
 S D=$S($D(^DGPT(DA,"P",0)):$P(^(0),U,3,4),$O(^(0))'="":$O(^(0)),1:-1)
M6 I D>0 S DC=DC_D I $D(^DGPT(DA,"P",+D,0)) S DE(6)=$P(^(0),U,1)
 S X=P(DGZP,1)
 S Y=X
