DGRPTX ; GENERATED FROM 'DGRPT 10-10T REGISTRATION' INPUT TEMPLATE(#1476), FILE 2;05/14/26
 D DE G BEGIN
...
X18 S Z0=$S($D(^DPT(D0,.11)):+$P(^(.11),"^",5),1:0) K:'Z0 X Q:'Z0!'$D(^DIC(5,Z0,1,0))  S DIC="^DIC(5,Z0,1,",DIC(0)="QEM" D ^DIC S X=+Y K:Y'>0 X K Z0,DIC
 Q
 ;
19 D:$D(DG)>9 F^DIE17,DE S DQ=19,DW=".13;1",DV="FaXJ17",DU="",DIFLD=.131,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C19^DGRPTX"
 G RE
...
C19S S X="" G:DG(DQ)=X C19F1 K DB
 D ^DGRPTX5
C19F1 Q
X19 S X=$TR(X,"x","X") K:$L(X)>17 X I $D(X) K:'(X?10N!(X?10N1"X"1.6N)) X
 I $D(X),X'?.ANP K X
 Q
 ;
20 D:$D(DG)>9 F^DIE17,DE S DQ=20,DW=".13;2",DV="FaXJ17",DU="",DIFLD=.132,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C20^DGRPTX"
 G RE
...
C20S S X="" G:DG(DQ)=X C20F1 K DB
 D ^DGRPTX7
C20F1 Q
X20 S X=$TR(X,"x","X") K:$L(X)>17 X I $D(X) K:'(X?10N!(X?10N1"X"1.6N)) X
 I $D(X),X'?.ANP K X
 Q
 ;
...
X47 S:X="" Y="@40"
 Q
48 D:$D(DG)>9 F^DIE17 G ^DGRPTX28
