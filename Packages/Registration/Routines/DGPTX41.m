...
C5F1 S DIEZRXR(45.01,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 ...
 Q
X5 S %DT="ETX" D ^%DT S X=+Y K:Y<1 X I $D(X) D DT401^DGPTUTL1
 Q
 ;
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW="0;3",DV="RP45.3'",DU="",DIFLD=3,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="DIC(45.3,"
 G RE
X6 Q
7 S DW="0;4",DV="S",DU="",DIFLD=4,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
9 S DW="0;6",DV="S",DU="",DIFLD=6,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 ...
 G RE
X9 Q
10 S DW="0;7",DV="S",DU="",DIFLD=7,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X14 I DGADD,$P(DGHOLD,U,8)]"" S Y="@30"
 Q
15 S DW="0;8",DV="*P80.1'X",DU="",DIFLD=8,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C15^DGPTX41",DE(DW,"INDEX")=1
 S DU="ICD0("
 G RE
C15 G C15S:$D(DE(15))[0 K DB
C15S S X="" G:DG(DQ)=X C15F1 K DB
C15F1 N X,X1,X2 S DIXR=1290 D C15X1(U) K X2 M X2=X D C15X1("O") K X1 M X1=X
 I $G(X(1))]"" D
...
X19 I DGADD,$P(DGHOLD,U,9)]"" S Y="@40"
 Q
20 D:$D(DG)>9 F^DIE17,DE S DQ=20,DW="0;9",DV="*P80.1'X",DU="",DIFLD=9,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C20^DGPTX41",DE(DW,"INDEX")=1
 S DU="ICD0("
 G RE
C20 G C20S:$D(DE(20))[0 K DB
C20S S X="" G:DG(DQ)=X C20F1 K DB
C20F1 N X,X1,X2 S DIXR=1301 D C20X1(U) K X2 M X2=X D C20X1("O") K X1 M X1=X
 I $G(X(1))]"" D
 . K ^DGPT(DA(1),"S","AO",X,DA)
 K X M X=X2 I $G(X(1))]"" D
 . S ^DGPT(DA(1),"S","AO",X,DA)=""
...
X24 I DGADD,$P(DGHOLD,U,10)]"" S Y="@50"
 Q
25 D:$D(DG)>9 F^DIE17,DE S DQ=25,DW="0;10",DV="*P80.1'X",DU="",DIFLD=10,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C25^DGPTX41",DE(DW,"INDEX")=1
 S DU="ICD0("
 G RE
C25 G C25S:$D(DE(25))[0 K DB
C25S S X="" G:DG(DQ)=X C25F1 K DB
C25F1 N X,X1,X2 S DIXR=1308 D C25X1(U) K X2 M X2=X D C25X1("O") K X1 M X1=X
 I $G(X(1))]"" D
 . K ^DGPT(DA(1),"S","AO",X,DA)
 K X M X=X2 I $G(X(1))]"" D
 . S ^DGPT(DA(1),"S","AO",X,DA)=""
...
X29 I DGADD,$P(DGHOLD,U,11)]"" S Y="@60"
 Q
30 D:$D(DG)>9 F^DIE17,DE S DQ=30,DW="0;11",DV="*P80.1'X",DU="",DIFLD=11,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C30^DGPTX41",DE(DW,"INDEX")=1
 S DU="ICD0("
 G RE
C30 G C30S:$D(DE(30))[0 K DB
C30S S X="" G:DG(DQ)=X C30F1 K DB
C30F1 N X,X1,X2 S DIXR=1309 D C30X1(U) K X2 M X2=X D C30X1("O") K X1 M X1=X
 I $G(X(1))]"" D
 . K ^DGPT(DA(1),"S","AO",X,DA)
 K X M X=X2 I $G(X(1))]"" D
 . S ^DGPT(DA(1),"S","AO",X,DA)=""
...
X34 I DGADD,$P(DGHOLD,U,12)]"" S Y="@70"
 Q
35 D:$D(DG)>9 F^DIE17,DE S DQ=35,DW="0;12",DV="*P80.1'X",DU="",DIFLD=12,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C35^DGPTX41",DE(DW,"INDEX")=1
 S DU="ICD0("
 G RE
C35 G C35S:$D(DE(35))[0 K DB
C35S S X="" G:DG(DQ)=X C35F1 K DB
C35F1 N X,X1,X2 S DIXR=1310 D C35X1(U) K X2 M X2=X D C35X1("O") K X1 M X1=X
 I $G(X(1))]"" D
 . K ^DGPT(DA(1),"S","AO",X,DA)
 K X M X=X2 I $G(X(1))]"" D
 . S ^DGPT(DA(1),"S","AO",X,DA)=""
...
X41 I '$D(DGBPC(1)) S Y="@899"
 Q
42 D:$D(DG)>9 F^DIE17,DE S DQ=42,DW="300;1",DV="SX",DU="",DIFLD=300.01,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
