DGPTX1 ; GENERATED FROM 'DG101' INPUT TEMPLATE(#426), FILE 45;02/25/22
 D DE G BEGIN
...
X3 S DGJUMP=$P(DGJUMP,"1,",2)
 Q
4 S DW="0;3",DV="RNJ3,0X",DU="",DIFLD=3,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X4 S DIC="^DIC(4,",DIC(0)="M" D ^DIC S X=$S($D(^DIC(4,+Y,99)):+^(99),1:"") K:Y'>0 X
 Q
 ;
5 S DW="0;5",DV="FX",DU="",DIFLD=5,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C5^DGPTX1"
 G RE
...
X5 D UP^DGHELP S DIC="^DIC(45.81,",DIC(0)="EFC",D="D1" D IX^DIC S DIC=$S($D(DIE):DIE,1:DIC) I Y'>0!(X=" ")!('$$ACTIVE^DGPTDD($G(X),+Y,+$G(PTF))) K X
 ...
 Q
 ;
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW="101;1",DV="*P45.1'R",DU="",DIFLD=20,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="DIC(45.1,"
 G RE
X6 S DIC("S")="S DGER=0 D SA^DGPTFJ K DGSU0,DGSU1 I 'DGER" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
7 S DW="101;3",DV="S",DU="",DIFLD=22,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="1:CONTRACT-PUBLIC&PRIV;2:SHARING;3:CONTRACT-MILT&FED AGENCY;4:PAID UNAUTH;"
 G RE
X7 Q
8 S DW="101;5",DV="NJ3,0X",DU="",DIFLD=21.1,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
X8 S DIC="^DIC(4,",DIC(0)="ME" D ^DIC S X=$S($D(^DIC(4,+Y,99)):$E(^(99),1,3),1:"") K:Y'>0 X
 Q
 ;
9 S DW="101;6",DV="FX",DU="",DIFLD=21.2,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
X9 D UP^DGHELP S DIC(0)="ECF",DIC="^DIC(45.81,",D="D1" D IX^DIC S DIC=$S($D(DIE):DIE,1:DIC) I +Y'>0!(X=" ") K X
 I $D(X),X'?.ANP K X
 Q
 ;
10 S DW="101;8",DV="P8'",DU="",DIFLD=20.1,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="DIC(8,"
...
X11 I DGPTFMT>1 S Y="@10"
 Q
12 S DW="101;4",DV="P45.82'X",DU="",DIFLD=23,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C12^DGPTX1"
 S DU="DIC(45.82,"
 G RE
...
X21 S DGJUMP=$P(DGJUMP,"5,",2)
 Q
22 D:$D(DG)>9 F^DIE17,DE S DQ=22,DW="70;6",DV="P45.6'",DU="",DIFLD=75,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="DIC(45.6,"
 G RE
X22 Q
...
X25 S DGJUMP=$P(DGJUMP,"6,",2)
 Q
26 S DW="70;4",DV="S",DU="",DIFLD=73,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="1:YES;3:NO;"
 G RE
X26 Q
27 S DW="70;5",DV="S",DU="",DIFLD=74,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="1:YES;2:NO;"
 G RE
X27 Q
...
X30 S DGJUMP=$P(DGJUMP,"7,",2)
 Q
31 S DW="70;12",DV="NJ3,0X",DU="",DIFLD=76.1,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
X31 S DIC="^DIC(4,",DIC(0)="ME" D ^DIC S DIC=$S($D(DIE):DIE,1:DIC) S X=$S($D(^DIC(4,+Y,99)):$E(^(99),1,3),1:"") K:Y'>0 X
 Q
 ;
32 S DW="70;13",DV="FX",DU="",DIFLD=76.2,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
X32 D UP^DGHELP S DIC(0)="EFC",DIC="^DIC(45.81,",D="D1" D IX^DIC S DIC=DIE I +Y'>0!(X=" ") K X
 I $D(X),X'?.ANP K X
 Q
 ;
33 S DW="70;9",DV="S",DU="",DIFLD=78,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 ...
 G RE
X33 Q
34 S DW="70;8",DV="NJ6,0",DU="",DIFLD=77,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 G RE
