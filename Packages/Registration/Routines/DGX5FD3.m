...
DE S DIE="^DGPT(D0,""M"",",DIC=DIE,DP=45.02,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^DGPT(D0,"M",DA,""))=""
 ...
 I $D(^(81)) S %Z=^(81) S %=$P(%Z,U,1) S:%]"" DE(51)=% S %=$P(%Z,U,2) S:%]"" DE(61)=%
 I $D(^(82)) S %Z=^(82) S %=$P(%Z,U,6) S:%]"" DE(4)=% S %=$P(%Z,U,7) S:%]"" DE(14)=% S %=$P(%Z,U,8) S:%]"" DE(24)=% S %=$P(%Z,U,9) S:%]"" DE(34)=% S %=$P(%Z,U,10) S:%]"" DE(44)=% S %=$P(%Z,U,11) S:%]"" DE(54)=% S %=$P(%Z,U,12) S:%]"" DE(64)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
C1F1 S DIEZRXR(45.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=835,1183 S DIEZRXR(45.02,DIXR)=""
 Q
...
11 S DW="0;12",DV="*P80'X",DU="",DIFLD=12,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C11^DGX5FD3",DE(DW,"INDEX")=1
 S DU="ICD9("
 G RE
...
C11F1 S DIEZRXR(45.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=835,1184 S DIEZRXR(45.02,DIXR)=""
 Q
...
21 S DW="0;13",DV="*P80'X",DU="",DIFLD=13,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C21^DGX5FD3",DE(DW,"INDEX")=1
 S DU="ICD9("
 G RE
...
C21F1 S DIEZRXR(45.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=835,1185 S DIEZRXR(45.02,DIXR)=""
 Q
...
31 S DW="0;14",DV="*P80'X",DU="",DIFLD=14,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C31^DGX5FD3",DE(DW,"INDEX")=1
 S DU="ICD9("
 G RE
...
C31F1 S DIEZRXR(45.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=835,1186 S DIEZRXR(45.02,DIXR)=""
 Q
...
41 S DW="0;15",DV="*P80'X",DU="",DIFLD=15,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C41^DGX5FD3",DE(DW,"INDEX")=1
 S DU="ICD9("
 G RE
...
C41F1 S DIEZRXR(45.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=835,1178 S DIEZRXR(45.02,DIXR)=""
 Q
...
51 S DW="81;1",DV="*P80'X",DU="",DIFLD=81.01,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C51^DGX5FD3",DE(DW,"INDEX")=1
 S DU="ICD9("
 G RE
...
C51F1 S DIEZRXR(45.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=835,1224 S DIEZRXR(45.02,DIXR)=""
 Q
...
X60 S DGNFLD="@150"
 Q
61 S DW="81;2",DV="*P80'X",DU="",DIFLD=81.02,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C61^DGX5FD3",DE(DW,"INDEX")=1
 S DU="ICD9("
 G RE
...
C61F1 S DIEZRXR(45.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=835,1225 S DIEZRXR(45.02,DIXR)=""
 Q
...
X63 I DGXX="" S Y="@141"
 Q
64 D:$D(DG)>9 F^DIE17,DE S DQ=64,DW="82;12",DV="SX",DU="",DIFLD=82.12,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DU="Y:Present on Admission;N:Not Present on Admission;U:Insufficient Docum to Present on Admission;W:Can't Determine if Present on Admission;1:Exempt;"
 G RE
X64 I X]"",$G(DA),$G(DA(1)) K:'$$POA501^DGPTFUT1(X,DA(1),DA,81,2) X
 Q
 ;
65 S DQ=66 ;@141
