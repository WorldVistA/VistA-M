...
SET I X'?.ANP S DDER=1 Q 
 ...
 Q
BEGIN S DNM="DGXTS2",DQ=1
1 S DW="0;2",DV="RNJ5,0",DU="",DLB="PATIENTS REMAINING",DIFLD=.02
 G RE
X1 K:+X'=X!(X>99999)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
...
X3 I '$D(DGTSPT) S Y=1
 Q
4 S DW="1;5",DV="NJ5,0",DU="",DLB="PASS PATIENTS REMAINING",DIFLD=5
 G RE
X4 K:+X'=X!(X>99999)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
5 S DW="1;6",DV="NJ5,0",DU="",DLB="AA PATIENTS REMAINING",DIFLD=6
 G RE
X5 K:+X'=X!(X>99999)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
6 S DW="1;7",DV="NJ5,0",DU="",DLB="UA PATIENTS REMAINING",DIFLD=7
 G RE
X6 K:+X'=X!(X>99999)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
7 S DW="1;8",DV="NJ5,0",DU="",DLB="ASIH PATIENTS REMAINING",DIFLD=8
 G RE
X7 K:+X'=X!(X>99999)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
8 G 1^DIE17
