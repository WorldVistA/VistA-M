...
DE S DIE="^DGPM(",DIC=DIE,DP=405,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGPM(DA,""))=""
 ...
 I $D(^("ODS")) S %Z=^("ODS") S %=$P(%Z,U,2) S:%]"" DE(4)=% S %=$P(%Z,U,3) S:%]"" DE(6)=% S %=$P(%Z,U,5) S:%]"" DE(8)=% S %=$P(%Z,U,6) S:%]"" DE(10)=%
 I $D(^("USR")) S %Z=^("USR") S %=$P(%Z,U,3) S:%]"" DE(13)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X3 S:'DGODS Y="@31"
 Q
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW="ODS;2",DV="S",DU="",DLB="NON-VA FACILITY?",DIFLD=11500.02
 S DE(DW)="C4^DGPMX31"
 S DU="1:YES;0:NO;"
 G RE
...
X5 S Y=$S('DGODSPT:11500.05,'X:"@31",1:11500.03)
 Q
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW="ODS;3",DV="S",DU="",DLB="AT VA EXPENSE?",DIFLD=11500.03
 S DU="1:YES;0:NO;"
 G RE
X6 Q
...
X7 S Y="@31"
 Q
8 S DW="ODS;5",DV="S",DU="",DLB="DISPLACED FOR ODS PATIENT?",DIFLD=11500.05
 S DU="1:YES;0:NO;"
 G RE
X8 Q
...
X9 S:'X Y="@31"
 Q
10 S DW="ODS;6",DV="NJ14,2",DU="",DLB="VA COST TO TRAVEL",DIFLD=11500.06
 S DE(DW)="C10^DGPMX31"
 G RE
...
C10S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 S A1B2TAG="ADM" D ^A1B2XFR
 Q
X10 S:X["$" X=$P(X,"$",2) K:X'?.N.1".".2N!(X>99999999999)!(X<0) X
 Q
 ;
11 S DQ=12 ;@31
12 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=12 D X12 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X12 I DGPMP=^DGPM(DA,0) S Y=""
 Q
13 D:$D(DG)>9 F^DIE17,DE S DQ=13,DW="USR;3",DV="RP200'",DU="",DLB="LAST EDITED BY",DIFLD=102
 S DU="VA(200,"
 S X=DUZ
 S Y=X
