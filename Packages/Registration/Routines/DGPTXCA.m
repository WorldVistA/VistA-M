DGPTXCA ; GENERATED FROM 'DG PTF POST CREATE' INPUT TEMPLATE(#455), FILE 45;06/13/96
 D DE G BEGIN
DE S DIE="^DGPT(",DIC=DIE,DP=45,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGPT(DA,""))=""
 I $D(^(101)) S %Z=^(101) S %=$P(%Z,U,1) S:%]"" DE(3)=%,DE(6)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET I X'?.ANP S DDER=1 Q 
 ...
 Q
BEGIN S DNM="DGPTXCA",DQ=1
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIEZ=455,U="^"
...
X2 S:$P(^DGPT(DA,0),U,4) Y="@20"
 Q
3 S DW="101;1",DV="*P45.1'",DU="",DLB="SOURCE OF ADMISSION",DIFLD=20
 S DU="DIC(45.1,"
 G RE
X3 S DIC("S")="S DGER=0 D SA^DGPTFJ K DGSU0,DGSU1 I 'DGER" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
X4 S Y="@20"
 Q
5 S DQ=6 ;@10
6 S DW="101;1",DV="*P45.1'",DU="",DLB="SOURCE OF ADMISSION",DIFLD=20
 S DU="DIC(45.1,"
 S X=DGPTSOA
 S Y=X
