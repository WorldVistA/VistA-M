...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=2,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 ...
 I $D(^(.52)) S %Z=^(.52) S %=$P(%Z,U,5) S:%]"" DE(17)=% S %=$P(%Z,U,6) S:%]"" DE(19)=%
...
X2 S DGJUMP=$P(DGJUMP,"2,",2)
 Q
3 S DW="0;5",DV="RP11'a",DU="",DIFLD=.05,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C3^DGPTX11"
 S DU="DIC(11,"
 G RE
...
R5 D DE
 ...
 ;
6 S DW="57;4",DV="S",DU="",DIFLD=57.4,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X9 S DGJUMP=$P(DGJUMP,"3,",2)
 Q
10 S DW=".321;1",DV="RSX",DU="",DIFLD=.32101,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C10^DGPTX11"
...
X10 S DFN=DA D SV^DGLOCK
 Q
 ;
11 D:$D(DG)>9 F^DIE17,DE S DQ=11,DW=".321;2",DV="RSX",DU="",DIFLD=.32102,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C11^DGPTX11"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 G RE
...
X12 S:X'="Y" Y=.32103
 Q
13 D:$D(DG)>9 F^DIE17,DE S DQ=13,DW=".321;13",DV="RSX",DU="",DIFLD=.3213,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C13^DGPTX11"
 S DU="B:BLUE WATER NAVY;K:KOREAN DMZ;V:VIETNAM;O:OTHER;"
 G RE
...
X15 S:X'="Y" Y=.525
 Q
16 D:$D(DG)>9 F^DIE17,DE S DQ=16,DW=".321;12",DV="SX",DU="",DIFLD=.3212,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C16^DGPTX11"
...
X16 S DFN=DA D IR^DGLOCK Q
 Q
 ;
17 D:$D(DG)>9 F^DIE17,DE S DQ=17,DW=".52;5",DV="RSX",DU="",DIFLD=.525,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X18 S:X'="Y" Y="@42"
 Q
19 D:$D(DG)>9 F^DIE17,DE S DQ=19,DW=".52;6",DV="RP22'X",DU="",DIFLD=.526,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C19^DGPTX11"
 S DU="DIC(22,"
 G RE
