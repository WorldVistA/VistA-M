...
DE S DIE="^DGPM(",DIC=DIE,DP=405,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGPM(DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,1) S:%]"" DE(2)=% S %=$P(%Z,U,4) S:%]"" DE(6)=% S %=$P(%Z,U,5) S:%]"" DE(9)=%
 I $D(^("ODS")) S %Z=^("ODS") S %=$P(%Z,U,2) S:%]"" DE(12)=% S %=$P(%Z,U,3) S:%]"" DE(14)=% S %=$P(%Z,U,5) S:%]"" DE(16)=% S %=$P(%Z,U,6) S:%]"" DE(18)=%
 I $D(^("USR")) S %Z=^("USR") S %=$P(%Z,U,3) S:%]"" DE(21)=% S %=$P(%Z,U,4) S:%]"" DE(24)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="DGPMX3",DQ=1
 ...
 M DIEZAR=^DIE(448,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=448,U="^"
...
X8 I $P(^DGPM(DA,0),"^",18)'=10 S Y="@31"
 Q
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW="0;5",DV="RP4'X",DU="",DLB="TRANSFER FACILITY",DIFLD=.05
 S DE(DW)="C9^DGPMX3"
 S DU="DIC(4,"
 G RE
...
X11 S:'DGODS Y="@31"
 Q
12 D:$D(DG)>9 F^DIE17,DE S DQ=12,DW="ODS;2",DV="S",DU="",DLB="NON-VA FACILITY?",DIFLD=11500.02
 S DE(DW)="C12^DGPMX3"
 S DU="1:YES;0:NO;"
 G RE
...
X13 S Y=$S('DGODSPT:11500.05,'X:"@31",1:11500.03)
 Q
14 D:$D(DG)>9 F^DIE17,DE S DQ=14,DW="ODS;3",DV="S",DU="",DLB="AT VA EXPENSE?",DIFLD=11500.03
 S DU="1:YES;0:NO;"
 G RE
X14 Q
...
X15 S Y="@31"
 Q
16 S DW="ODS;5",DV="S",DU="",DLB="DISPLACED FOR ODS PATIENT?",DIFLD=11500.05
 S DU="1:YES;0:NO;"
 G RE
X16 Q
...
X17 S:'X Y="@31"
 Q
18 S DW="ODS;6",DV="NJ14,2",DU="",DLB="VA COST TO TRAVEL",DIFLD=11500.06
 S DE(DW)="C18^DGPMX3"
 G RE
...
C18S S X="" G:DG(DQ)=X C18F1 K DB
 ...
 S A1B2TAG="ADM" D ^A1B2XFR
C18F1 Q
X18 S:X["$" X=$P(X,"$",2) K:X'?.N.1".".2N!(X>99999999999)!(X<0) X
 Q
 ;
19 S DQ=20 ;@31
...
X20 I DGPMP=^DGPM(DA,0) S Y=""
 Q
21 D:$D(DG)>9 F^DIE17,DE S DQ=21,DW="USR;3",DV="RP200'",DU="",DLB="LAST EDITED BY",DIFLD=102
 S DU="VA(200,"
 S X=DUZ
 S Y=X
...
24 S DW="USR;4",DV="RD",DU="",DLB="LAST EDITED ON",DIFLD=103
 S %=$P($H,",",2),X=DT_(%\60#60/100+(%\3600)+(%#60/10000)/100)
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X24 S %DT="STX" D ^%DT S X=Y K:Y<1 X
 Q
 ;
25 G 0^DIE17
