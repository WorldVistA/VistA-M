DGPMX3 ; GENERATED FROM 'DGPM DISCHARGE' INPUT TEMPLATE(#448), FILE 405;03/03/97
 D DE G BEGIN
...
BEGIN S DNM="DGPMX3",DQ=1
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIEZ=448,U="^"
...
X5 S ^DISV(DUZ,"DG(405.1,")=$S($D(^DISV(DUZ,"DGPM3")):^("DGPM3"),1:"")
 Q
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW="0;4",DV="R*P405.1'X",DU="",DLB="TYPE OF DISCHARGE",DIFLD=.04
 S DE(DW)="C6^DGPMX3"
 S DU="DG(405.1,"
 G RE
...
X7 S ^DISV(DUZ,"DGPM3")=$S($D(^DISV(DUZ,"^DG(405.1,")):^("^DG(405.1,"),1:"")
 Q
8 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=8 D X8 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X8 I $P(^DGPM(DA,0),"^",18)'=10 S Y="@31"
 Q
9 D:$D(DG)>9 F^DIE17 G ^DGPMX31
