...
DE S DIE="^DGPT(",DIC=DIE,DP=45,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGPT(DA,""))=""
 ...
 I $D(^(71)) S %Z=^(71) S %=$P(%Z,U,1) S:%]"" DE(31)=% S %=$P(%Z,U,2) S:%]"" DE(41)=% S %=$P(%Z,U,3) S:%]"" DE(51)=% S %=$P(%Z,U,4) S:%]"" DE(61)=%
 I $D(^(82)) S %Z=^(82) S %=$P(%Z,U,8) S:%]"" DE(4)=% S %=$P(%Z,U,9) S:%]"" DE(14)=% S %=$P(%Z,U,10) S:%]"" DE(24)=% S %=$P(%Z,U,11) S:%]"" DE(34)=% S %=$P(%Z,U,12) S:%]"" DE(44)=% S %=$P(%Z,U,13) S:%]"" DE(54)=% S %=$P(%Z,U,14) S:%]"" DE(64)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
1 S DW="70;22",DV="*P80'X",DU="",DIFLD=79.22,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
X8 S:$P($G(^DGPT(D0,70)),U,22,24)_$P($G(^DGPT(D0,71)),U,1,15)?."^" Y="@240"
 Q
9 S DQ=10 ;@70
10 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=10 D X10 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X10 S DGNFLD="@80"
 Q
11 S DW="70;23",DV="*P80'X",DU="",DIFLD=79.23,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
C61F1 S DIEZRXR(45,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=1191,1591 S DIEZRXR(45,DIXR)=""
 Q
