...
DE S DIE="^DGPM(",DIC=DIE,DP=405,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGPM(DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,1) S:%]"" DE(2)=% S %=$P(%Z,U,4) S:%]"" DE(6)=% S %=$P(%Z,U,5) S:%]"" DE(12)=%,DE(15)=% S %=$P(%Z,U,6) S:%]"" DE(9)=% S %=$P(%Z,U,7) S:%]"" DE(10)=%,DE(20)=%
 I $D(^("LD")) S %Z=^("LD") S %=$P(%Z,U,1) S:%]"" DE(22)=% S %=$P(%Z,U,2) S:%]"" DE(23)=%
 I $D(^("USR")) S %Z=^("USR") S %=$P(%Z,U,3) S:%]"" DE(25)=% S %=$P(%Z,U,4) S:%]"" DE(27)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X13 S Y="@42"
 Q
14 S DQ=15 ;@41
15 D:$D(DG)>9 F^DIE17,DE S DQ=15,DW="0;5",DV="RP4'X",DU="",DLB="TRANSFER FACILITY",DIFLD=.05
 S DE(DW)="C15^DGPMX4"
 S DU="DIC(4,"
 G RE
...
X18 S DGWARD(405,DA_",",.06)=""
 Q
19 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=19 D X19 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X19 D FILE^DIE("","DGWARD","")
 Q
20 D:$D(DG)>9 F^DIE17,DE S DQ=20,DW="0;7",DV="*P405.4'X",DU="",DLB="ROOM-BED",DIFLD=.07
 S DE(DW)="C20^DGPMX4"
 S DU="DG(405.4,"
 S Y="@"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
C20S S X="" G:DG(DQ)=X C20F1 K DB
 ...
 S DGPMDDF=7,DGPMDDT=1 D ^DGPMDDCN
C20F1 Q
X20 K:'$D(DGPMT) X I $D(X) S DIC("S")="I $D(^DG(405.4,""W"",+$P(^DGPM(DA,0),""^"",6),+Y)) D OCC^DGPMRB I 'DGPMOC" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X I $D(X) D ROOM^DGPMVDD K:$D(DGOOS) X K DGOOS
 Q
 ;
21 S DQ=22 ;@42
22 D:$D(DG)>9 F^DIE17,DE S DQ=22,DW="LD;1",DV="RP406.41'",DU="",DLB="REASON FOR LODGING",DIFLD=30.01
 S DU="DG(406.41,"
 G RE
X22 Q
23 S DW="LD;2",DV="F",DU="",DLB="LODGING COMMENTS",DIFLD=30.02
 G RE
X23 K:$L(X)>30!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
 ;
24 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=24 D X24 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X24 I DGPMP=^DGPM(DA,0) S Y=""
 Q
25 S DW="USR;3",DV="RP200'",DU="",DLB="LAST EDITED BY",DIFLD=102
 S DU="VA(200,"
 S X=DUZ
 S Y=X
...
27 S DW="USR;4",DV="RD",DU="",DLB="LAST EDITED ON",DIFLD=103
 S %=$P($H,",",2),X=DT_(%\60#60/100+(%\3600)+(%#60/10000)/100)
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X27 S %DT="STX" D ^%DT S X=Y K:Y<1 X
 Q
 ;
28 G 0^DIE17
