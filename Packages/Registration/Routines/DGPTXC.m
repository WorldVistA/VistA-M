DGPTXC ; GENERATED FROM 'DG PTF CREATE PTF ENTRY' INPUT TEMPLATE(#443), FILE 45;02/01/23
 D DE G BEGIN
...
1 S DW="0;2",DV="RDX",DU="",DIFLD=2,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
C1F1 S DIEZRXR(45,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=401,1176,1187,1188,1189,1190,1191,1192,1193,1194,1195,1196,1197,1198,1199,1239,1240,1241,1242,1243,1244,1245,1246,1247,1248,1249 S DIEZRXR(45,DIXR)=""
 Q
X1 Q
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="0;4",DV="S",DU="",DIFLD=4,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C2^DGPTXC"
 S DU="1:FEE BASIS;"
 S X=$P(DGPTDATA,U,3)
 S Y=X
...
C2F1 Q
X2 Q
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW="0;6",DV="S",DU="",DIFLD=6,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C3^DGPTXC"
...
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,D=0 K DE(1) ;50
 S DIFLD=50,DGO="^DGPTXC1",DC="77^45.02AI^M^",DV="45.02MNJ6,1X",DW="0;1",DOW=$$LABEL^DIALOGZ(DP,DIFLD),DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
...
R4 D DE
 G A
 ;
5 S DW="0;11",DV="RS",DU="",DIFLD=11,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
...
C5F1 S DIEZRXR(45,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=401,833,1176,1187,1188,1189,1190,1191,1192,1193,1194,1195,1196,1197,1198,1199,1239,1240,1241,1242,1243,1244,1245,1246,1247,1248,1249,1691 S DIEZRXR(45,DIXR)=""
 Q
X5 Q
6 G 0^DIE17
