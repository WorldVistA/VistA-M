DVBAXA ; GENERATED FROM 'DVBA C ADD 2507 PAT' INPUT TEMPLATE(#965), FILE 2;05/14/26
 D DE G BEGIN
...
X22 S Z0=$S($D(^DPT(D0,.11)):+$P(^(.11),"^",5),1:0) K:'Z0 X Q:'Z0!'$D(^DIC(5,Z0,1,0))  S DIC="^DIC(5,Z0,1,",DIC(0)="QEM" D ^DIC S X=+Y K:Y'>0 X K Z0,DIC
 Q
 ;
23 D:$D(DG)>9 F^DIE17,DE S DQ=23,DW=".13;1",DV="FaXJ17",DU="",DIFLD=.131,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C23^DVBAXA"
 G RE
...
C23S S X="" G:DG(DQ)=X C23F1 K DB
 D ^DVBAXA18
C23F1 Q
X23 S X=$TR(X,"x","X") K:$L(X)>17 X I $D(X) K:'(X?10N!(X?10N1"X"1.6N)) X
 I $D(X),X'?.ANP K X
 Q
 ;
24 D:$D(DG)>9 F^DIE17,DE S DQ=24,DW=".13;2",DV="FaXJ17",DU="",DIFLD=.132,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C24^DVBAXA"
 G RE
...
C24S S X="" G:DG(DQ)=X C24F1 K DB
 D ^DVBAXA20
C24F1 Q
X24 S X=$TR(X,"x","X") K:$L(X)>17 X I $D(X) K:'(X?10N!(X?10N1"X"1.6N)) X
 I $D(X),X'?.ANP K X
 Q
 ;
