DVBA159P ;ALB/KCL,RLC,ECF - PATCH DVBA*2.7*159 INSTALL UTILITIES ; 10/20/2010
...
DISABLE(NM) ;Disable matching CAPRI template entries
 ...
 ;for templates loaded by the patch on the target system (i.e. ~159T5).
 ;
 ...
 N DVBABST  ;template NAME field (i.e. DBQ PARKINSONS~154T5)
...
RENAME(NM) ;Rename CAPRI templates loaded by the patch
 ...
 ;the imported version of a template (i.e. DBQ PARKINSONS~159F) to its
 ;new name/version (i.e. DBQ PARKINSONS~159T5).
 ;
 N DVBABIEN,DVBABIE2 ;ien of CAPRI TEMPLATE DEFINITIONS file
 N DVBABST  ;template NAME field (i.e. AUDIO~159F)
 N DVBABS2  ;template NAME field (i.e. AUDIO~159T5)
 ...
 F  S DVBABIEN=$O(^DVB(396.18,DVBABIEN)) Q:'DVBABIEN  D
 .S DVBABST=$P($G(^DVB(396.18,DVBABIEN,0)),"^",1) ;template name
 .;look for template versions just loaded by the patch (~159F)
 ...
 ..;name to the new version name (i.e. DBQ PARKINSONS~159F --> DBQ PARKINSONS~159T5)
 ..I DVBACH=0  D
...
TXTOLD ;
 ;;235;DBQ LEUKEMIA;DBQ LEUKEMIA;22;DVBCQBC1;I; ; ;
 ;;236;DBQ PARKINSONS;DBQ PARKINSONS DISEASE;13;DVBCQPD1;I; ; ;
 ;;237;DBQ ISCHEMIC HEART DISEASE;DBQ ISCHEMIC HEART DIS;6;DVBCQIH1;I; ;
 ;;QUIT
 ;
 ; New exam(s) to activate
 ; format:  ien;exam name;print name;body system;routine;status;;wks#
TXTNEW ;
 ;;300;DBQ PARKINSONS;DBQ PARKINSONS DISEASE;13;DVBCQPD3;A; ; ;
 ;;301;DBQ ISCHEMIC HEART DISEASE;DBQ ISCHEMIC HEART DIS;6;DVBCQIH3;A; ;
 ;;302;DBQ LEUKEMIA;DBQ LEUKEMIA;22;DVBCQBC3;A; ; ;
 ;;QUIT
