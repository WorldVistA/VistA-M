DVBA161P ;ALB/KCL,RLC,ECF - PATCH DVBA*2.7*161 INSTALL UTILITIES ; 10/20/2010
...
DEACT ;De-activate older versions of CAPRI templates
 ...
 D MES^XPDUTL(" ")
 ;
 D DISABLE("DBQ EATING DISORDERS")
 D DISABLE("DBQ HEMATOLOGIC AND LYMPHATIC CONDITIONS, INCLUDING LEUKEMIA")
 D DISABLE("DBQ INITIAL PTSD")
 D DISABLE("DBQ ISCHEMIC HEART DISEASE")
 D DISABLE("DBQ MENTAL DISORDERS (OTHER THAN PTSD AND EATING DISORDERS)")
 D DISABLE("DBQ PROSTATE CANCER")
 D DISABLE("DBQ REVIEW PTSD")
 ;
...
ACTIVATE ;Activate CAPRI templates that were loaded by the patch
 ...
 D MES^XPDUTL(" ")
 ;
 D RENAME("DBQ EATING DISORDERS")
 D RENAME("DBQ HEMATOLOGIC AND LYMPHATIC CONDITIONS, INCLUDING LEUKEMIA")
 D RENAME("DBQ INITIAL PTSD")
 D RENAME("DBQ ISCHEMIC HEART DISEASE")
 D RENAME("DBQ MENTAL DISORDERS (OTHER THAN PTSD AND EATING DISORDERS)")
 D RENAME("DBQ PROSTATE CANCER")
 D RENAME("DBQ REVIEW PTSD")
 ;
...
DISABLE(NM) ;Disable matching CAPRI template entries
 ...
 ;suffix (i.e. ~161) does not match the version suffix that will be used
 ;for templates loaded by the patch on the target system (i.e. ~161).
 ;
 ...
 N DVBABST  ;template NAME field (i.e. DBQ PARKINSONS~161)
...
RENAME(NM) ;Rename CAPRI templates loaded by the patch
 ...
 ;the imported version of a template (i.e. DBQ PARKINSONS~161F) to its
 ;new name/version (i.e. DBQ PARKINSONS~161).
 ;
 N DVBABIEN,DVBABIE2 ;ien of CAPRI TEMPLATE DEFINITIONS file
 N DVBABST  ;template NAME field (i.e. AUDIO~161F)
 N DVBABS2  ;template NAME field (i.e. AUDIO~161)
 ...
 F  S DVBABIEN=$O(^DVB(396.18,DVBABIEN)) Q:'DVBABIEN  D
 .S DVBABST=$P($G(^DVB(396.18,DVBABIEN,0)),"^",1) ;template name
 .;look for template versions just loaded by the patch (~161F)
 ...
 ..;name to the new version name (i.e. DBQ PARKINSONS~161F --> DBQ PARKINSONS~161)
 ..I DVBACH=0  D
...
TXTOLD ;
 ;;301;DBQ ISCHEMIC HEART DISEASE;DBQ ISCHEMIC HEART DIS;6;DVBCQIH3;I; ; ;
 ;;QUIT
 ;
 ; New exam(s) to activate
 ; format:  ien;exam name;print name;body system;routine;status;;wks#
TXTNEW ;
 ;;303;DBQ EATING DISORDERS;DBQ EATING DISORDERS;14;DVBCQEA1;A; ; ;
 ;;304;DBQ HEMIC AND LYMPHATIC CONDITIONS INCLUDING LEUKEMIA;DBQ HEMIC AND LYMPHATIC;22;DVBCQHA1;A; ; ;
 ;;305;DBQ INITIAL PTSD;DBQ INITIAL PTSD;14;DVBCQPT1;A; ; ;
 ;;307;DBQ MENTAL DISORDERS (EXCEPT PTSD AND EATING DISORDERS);DBQ MENTAL DISORDER;14;DVBCQMD1;A; ; ;
 ;;308;DBQ PROSTATE CANCER;DBQ PROSTATE CANCER;8;DVBCQPC1;A; ; ; 
 ;;309;DBQ REVIEW PTSD;DBQ REVIEW PTSD;14;DVBCQPR1;A; ; ;
 ;;322;DBQ ISCHEMIC HEART DISEASE;DBQ ISCHEMIC HEART DIS;6;DVBCQIH5;A; ; ;
 ;;QUIT
