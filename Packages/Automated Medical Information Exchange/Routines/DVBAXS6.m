...
DE S DIE="^DVB(396,",DIC=DIE,DP=396,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DVB(396,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,28) S:%]"" DE(1)=%
 I $D(^(1)) S %Z=^(1) S %=$P(%Z,U,10) S:%]"" DE(4)=%,DE(8)=%
 I $D(^(2)) S %Z=^(2) S %=$P(%Z,U,5) S:%]"" DE(7)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
SET I X'?.ANP S DDER=1 Q 
 ...
 Q
BEGIN S DNM="DVBAXS6",DQ=1
1 S DW="0;28",DV="S",DU="",DLB="STATUS-BEG DATE/CARE (CHAP 17)",DIFLD=21
 S DU="P:PENDING;C:COMPLETED;"
 G RE
X1 Q
2 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=2 D X2 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X2 S DVBASTAT=X S:X="C" Y="@21"
 Q
3 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=3 D X3 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X3 S DVBADT=$S($D(^DVB(396,D0,1)):$P(^(1),U,10),1:"") S:X="P"&(DVBADT="") Y="@22"
 Q
4 S DW="1;10",DV="D",DU="",DLB="BEG/DATE/CARE COMPLETION DATE",DIFLD=21.5
 S Y="@"
...
X5 S Y="@22"
 Q
6 S DQ=7 ;@21
7 S DW="2;5",DV="F",DU="",DLB="EDIT21",DIFLD=21.7
 S X=OPER
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G Z
X7 Q
8 S DW="1;10",DV="D",DU="",DLB="BEG/DATE/CARE COMPLETION DATE",DIFLD=21.5
 G RE
X8 S %DT="EX" D ^%DT S X=Y K:Y<1 X
 Q
 ;
9 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=9 D X9 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X9 I X=""&(DVBASTAT="C") W *7,!!,"Completed status must have date.",!! S Y="@21"
 Q
10 S DQ=11 ;@22
11 G 0^DIE17
