LEX2127P ;ISL/AEB - LEX*2.0*127 Pre/Post Install ;04/19/2020
 ;;2.0;LEXICON UTILITY;**127**;Sep 23, 1996;Build 1
 ;               
 ; Global Variables 
 ;    ^LEXM(              N/A
 ;               
...
CON ; Conversion of data
 N CODE,CIEN,CEFD,EIEN,ZZERR,U
 S CIEN=0,U="^"
 F  S CIEN=$O(^LEX(757.02,CIEN)) Q:CIEN>355287  D
 .S CODE=$P(^LEX(757.02,CIEN,0),U,2)
 .I ($E(CODE,1,3)'="U00")!(CIEN=355286)!(CIEN=355287)!(CIEN=266158) Q
 .S CEFD=$O(^LEX(757.02,CIEN,4,"B"),-1)
 .Q:'$P(^LEX(757.02,CIEN,4,CEFD,0),"^",2)
 .S CEFD=$P(^LEX(757.02,CIEN,4,CEFD,0),"^")+1
 .K FDA,ZZERR
 .S FDA(757.28,"+1,"_CIEN_",",.01)=CEFD
 .S FDA(757.28,"+1,"_CIEN_",",1)=0
