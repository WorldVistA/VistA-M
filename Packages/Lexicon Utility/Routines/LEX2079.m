LEX2079 ;ISL/KER - LEX*2.0*79 Env Check ;07/01/2011
 ;;2.0;LEXICON UTILITY;**79**;Sep 23, 1996;Build 2
 ;               
 ; Global Variables
 ;    ^LEXM               N/A
 ;               
...
ENV ; Environment Check
 ...
 . S (LEXR,LEXC)=0 S LEXC1=5,(LEXC2,LEXC3,LEXC4)=18
 . S LEXI=0  F  S LEXI=$O(LEXREQP(LEXI)) Q:+LEXI'>0  D
 . . S LEXC=LEXC+1,LEXPAT=$G(LEXREQP(LEXI))
 . . S:$P(LEXPAT,"^",2)?7N LEXR=LEXR+1,LEXC3=LEXC2+12,LEXC4=LEXC3+12
 . S LEXI=0  F  S LEXI=$O(LEXREQP(LEXI)) Q:+LEXI'>0  D
 ...
 . . I LEXI=1 D
 . . . W !,?3,"Checking for ",?LEXC2,$S(+($G(LEXR))>0:"Released",1:"")
 . . . W ?LEXC3,$S(+($G(LEXR))>0:"Installed",1:"")
 . . S LEXTX=$J(" ",LEXC1)_LEXPAT
 . . S:LEXREL?7N LEXTX=LEXTX_$J(" ",(LEXC2-$L(LEXTX)))_$TR($$FMTE^XLFDT(LEXREL,"5DZ"),"@"," ")
 . . I +LEXPN>0 D
 . . . H 1 S LEXO=+($G(LEXO))+1 S LEXTX=LEXTX_$J(" ",(LEXC3-$L(LEXTX)))
...
IMP ;   Import names
 ...
 S LEXREQP(1)="LEX*2.0*78^3111018^Code Set FY12 1st Quarter Update"
 S LEXREQP(2)="ICD*18.0*56^3111128^DRG Grouper FY12 1st Quarter Update"
 ;     This Build Name
 S LEXBUILD="LEX*2.0*79"
 ;     This Build's Export Global Host Filename
 S LEXIGHF="LEX_2_79.GBL"
 ;     Fiscal Year
 S LEXFY="FY12"
 ;     Quarter
 S LEXQTR="2nd"
