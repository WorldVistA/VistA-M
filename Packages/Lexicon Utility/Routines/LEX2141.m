LEX2141 ;ISL/TJH - LEX*2.0*141 Env Check ; 07/11/2022
 ;;2.0;LEXICON UTILITY;**141**;Sep 23, 1996;Build 5
 ;
 ; Global Variables
 ;    ^LEXM               N/A
 ;               
 ...
 ; Reference to $$IENS^DILF in ICR #2054
 ; Reference to EN^DIQ1 in ICR #10015
 ; Reference to $$FMTE^XLFDT in ICR #10103
 ; Reference to $$PATCH^XPDUTL in ICR #10141
 ; Reference to $$PKG^XPDUTL in ICR #10141
 ; Reference to $$VER^XPDUTL in ICR #10141
 ; Reference to $$VERSION^XPDUTL in ICR #10141
...
IMP ;   Import names
 ...
 ;     Revision
 S LEXLREV=141
 ;     Requird Builds Array
 ;        LEX(1)=build SEQ #^released date^subject
 ;        LEX(n)=build SEQ #^released date^subject
 S LEXREQP(1)="LEX*2.0*140 SEQ #131^3220609^CODE SET FY22 4TH QTR UPDATE: COVID-19"  ;seq # from FORUM ; date is release date ; patch subject
 ;     This Build Name
 S LEXBUILD="LEX*2.0*141"
 ;     This Build's Export Global Host Filename
 S LEXIGHF="LEX_2_141.GBL"
 ;     Fiscal Year
 S LEXFY="FY23"
 ;     Quarter
 S LEXQTR="1st"
