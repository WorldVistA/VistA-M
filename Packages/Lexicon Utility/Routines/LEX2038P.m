LEX2038P ; ISL/KER - Pre/Post Install LEX*2.0*38 ; 02/02/2006
 ;;2.0;LEXICON UTILITY;**38**;Sep 23, 1996
 ;
POST ; Main Entry for LEX*2.0*38
 D FIX W ! D LEXM
 Q
FIX ; Software/Code Fixes
 D EN^LEX2038A,EN^LEX2038B,EN^LEX2038C,EN^LEX2038D
 Q
LEXM ; LEXM Global Install
 ...
 ;
 ;-----------------------------
 ;   Re-Index Files - N/A for LEX*2.0*38
...
LOAD ; Load Data from ^LEXM into IC*/LEX Files
 N LEXB,LEXBUILD,LEXCD,LEXIGHF,LEXLAST,LEXLREV D IMP^LEX2038
 S U="^",LEXB=$G(^LEXM(0,"BUILD")) Q:LEXB=""  Q:LEXBUILD=""
 S LEXCD=0 S LEXCD=+($$CPD^LEX2038)
...
MSG ; Send Installation Message to G.LEXICON
 Q:+($G(DUZ))=0!($$NOTDEF^LEX2038($G(DUZ)))
 D HOME^%ZIS N DIFROM,LEXLREV,LEXLAST,LEXBUILD,LEXIGHF
 D IMP^LEX2038,POST^LEXXFI Q
 ;
...
KLEXM ; Subscripted Kill of ^LEXM
 ...
 Q
 ;
PRE ; LEX*2.0*38 Pre-Install   (N/A for patch 38)
 Q
 ;
RX ; Reindex files            (N/A for patch 38)
 Q
 ...
 Q
 ;
CON ; Conversion of data       (N/A for patch 38)
 Q
REM(X,Y) ;
