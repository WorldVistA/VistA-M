LEXRXC ;ISL/KER - Re-Index 757.01 B/ADC/ADTERM ;05/23/2017
 ;;2.0;LEXICON UTILITY;**81,80,86,103**;Sep 23, 1996;Build 2
 ;               
 ; Global Variables
 ;    ^LEX(               SACC 1.3
 ;    ^LEX(757.01)        SACC 1.3
...
R75701 ; Repair file 757.01
 D MC,RB,RADC,RADTERM,RAH,R75701^LEXRXC2,R75701^LEXRXC3
 D:+($G(^TMP("LEXRX",$J,"ERR",757.01)))>0 SET
 Q
RB ;   Index    ^LEX(757.01,"B",TXT,IEN) 
 ;            ^LEX(757.01,IEN,4,"B",NEG,IEN2)
 ;            ^LEX(757.01,IEN,5,"B",WORD,IEN2)
 S:$D(ZTQUEUED) ZTREQ="@" W:'$D(ZTQUEUED) ! N DA,DIK,LEXBEG,LEXDIF,LEXDF,LEXELP,LEXEND,LEXERR,LEXEXP,LEXFI,LEXIDX,LEXIDXT,LEXIDNT,LEXIDST
 N LEXIEN,LEXMC,LEXNDS,LEXNDSN,LEXNDSS,LEXNER,LEXOK,LEXS,LEXSER,LEXSTR S LEXFI="757.01"
 ...
 . . N LEXOK,LEXEXP,LEXDF S LEXEXP=$$UP^XLFSTR($G(^LEX(LEXFI,LEXIEN,0)))
 . . S LEXDF=+($P($G(^LEX(LEXFI,LEXIEN,1)),"^",5))
 . . I $E(LEXEXP,1,63)'=LEXSTR,+LEXDF'>0 D
 . . . S LEXERR=LEXERR+1 K:'$D(LEXTEST) ^LEX(LEXFI,LEXIDX,LEXSTR,LEXIEN)
 . . . Q:'$L(LEXEXP)  S:LEXDF'>0 ^LEX(LEXFI,LEXIDX,$E(LEXEXP,1,63),LEXIEN)=""
 ...
 . N DA,DIK,X,LEXDF S DA=LEXIEN,X=$$UP^XLFSTR($G(^LEX(LEXFI,LEXIEN,0))) Q:'$L(X)
 . S LEXDF=+($P($G(^LEX(LEXFI,LEXIEN,1)),"^",5))
 . I '$D(^LEX(LEXFI,"B",$E(X,1,63),DA))&(LEXDF'>0) D
 . . S LEXERR=LEXERR+1 I '$D(ZTQUEUED) W !,?8,LEXFI,?19,LEXIDX,?30,"Missing",?58,"  ",DA
 . S:$L(X)&(LEXDF'>0) ^LEX(LEXFI,"B",$E(X,1,63),DA)=""
...
RADC ;   Index    ^LEX(757.01,"ADC",1,IEN)
 S:$D(ZTQUEUED) ZTREQ="@" W:'$D(ZTQUEUED) ! N DA,DIK,LEXBEG,LEXDF,LEXDIF,LEXELP,LEXEND,LEXERR,LEXFI,LEXIDX,LEXIDXT,LEXIEN,LEXNDS,LEXOK,LEXSTR
 S LEXFI="757.01"
...
RADTERM ;   Index    ^LEX(757.01,"ADTERM",DT,IEN) 
 S:$D(ZTQUEUED) ZTREQ="@" N DA,DIK,LEXBEG,LEXDE,LEXDF,LEXDIF,LEXELP,LEXEND,LEXERR,LEXFI,LEXIDX,LEXIDXT,LEXIEN,LEXNDS,LEXOK,LEXSTR
 S LEXFI="757.01"
 ...
 . . S LEXDE=$$UP^XLFSTR($E($P($G(^LEX(LEXFI,LEXIEN,0)),U,1),1,63))
 . . I LEXDE'=LEXSTR,+LEXDF>0 D
 . . . S LEXERR=LEXERR+1
 . . . K:'$D(LEXTEST) ^LEX(LEXFI,LEXIDX,LEXSTR,LEXIEN) S ^LEX(LEXFI,LEXIDX,LEXDE,LEXIEN)=""
 ...
 . . . I '$D(ZTQUEUED) W !,?8,LEXFI,?19,LEXIDX,?30,$E(LEXSTR,1,20),$S($L(LEXSTR)>20:"...",1:""),?58,"  ",LEXIEN
 S LEXIEN=0 F  S LEXIEN=$O(^LEX(LEXFI,LEXIEN)) Q:+LEXIEN'>0  D
 . N DA,DIK,LEXDF,X,UX S DA=LEXIEN,X=$P($G(^LEX(LEXFI,DA,0)),"^",1) Q:'$L(X)
 . S UX=$$UP^XLFSTR(X) S LEXDF=+($P($G(^LEX(LEXFI,DA,1)),"^",5))
 . I +LEXDF>0,'$D(^LEX(LEXFI,LEXIDX,$E(X,1,63),DA)) D
 . . S LEXERR=LEXERR+1 I '$D(ZTQUEUED) W !,?8,LEXFI,?19,LEXIDX,?30,"Missing",?58,"  ",DA
 . I +LEXDF'>0,$D(^LEX(LEXFI,LEXIDX,$E(X,1,63),DA)) D
 . . S LEXERR=LEXERR+1 I '$D(ZTQUEUED) W !,?8,LEXFI,?19,LEXIDX,?30,"Invalid (deleted)",?58,"  ",DA
 . S:+LEXDF>0 ^LEX(LEXFI,LEXIDX,$E(UX,1,63),DA)=""
 . K:+LEXDF'>0 ^LEX(LEXFI,LEXIDX,$E(X,1,63),DA),^LEX(LEXFI,LEXIDX,$E(UX,1,63),DA)
 S LEXERR=$S(+LEXERR>0:LEXERR,1:"") I '$D(ZTQUEUED) W !,$J(LEXERR,5),?8,LEXFI,?19,LEXIDX,?30,LEXIDXT
 S LEXEND=$$NOW^XLFDT,LEXELP=$$FMDIFF^XLFDT(LEXEND,LEXBEG,3)
 S:$E(LEXELP,1)=" "&($E(LEXELP,3)=":") LEXELP=$TR(LEXELP," ","0")
 D REP^LEXRXXS(LEXFI,LEXFI,LEXIDX,LEXNDS,LEXERR,LEXIDXT,LEXELP)
...
RAH ;   Index    ^LEX(757.01,"AH",HA,IEN,IEN2) 
 S:$D(ZTQUEUED) ZTREQ="@" N DA,DIK,LEXBEG,LEXDE,LEXDF,LEXDIF,LEXELP,LEXEND,LEXERR,LEXFI,LEXIDX,LEXIDXT,LEXEX,LEXEX2,LEXNDS,LEXOK,LEXSTR
 S LEXFI="757.01"
 N LEXTC S LEXTC=$$UPD^LEXRXXT3($G(LEXNAM),,"Repairing File #757.01 ""AH""") Q:LEXTC=1
 S LEXBEG=$$NOW^XLFDT,(LEXNDS,LEXERR)=0,LEXSTR="",LEXFI=757.01,LEXIDX="AH",LEXIDXT="^LEX(757.01,""AH"",HA,IEN,IEN2)"
 F  S LEXSTR=$O(^LEX(LEXFI,LEXIDX,LEXSTR)) Q:'$L(LEXSTR)  D
 . N LEXEX S LEXEX=0 F  S LEXEX=$O(^LEX(LEXFI,LEXIDX,LEXSTR,LEXEX)) Q:+LEXEX'>0  D
 . . N LEXEX2 S LEXEX2=0 F  S LEXEX2=$O(^LEX(LEXFI,LEXIDX,LEXSTR,LEXEX,LEXEX2)) Q:+LEXEX2'>0  D
 . . . N LEXND,LEXHA S LEXND=$G(^LEX(LEXFI,LEXEX,7,LEXEX2,0)),LEXHA=$P(LEXND,"^",3)
 . . . S LEXNDS=LEXNDS+1 I '$D(^LEX(LEXFI,LEXEX,7,LEXEX2,0)) D  Q
 . . . . S LEXERR=LEXERR+1 K:'$D(LEXTEST) ^LEX(LEXFI,LEXIDX,LEXSTR,LEXEX,LEXEX2)
 . . . . I '$D(ZTQUEUED) W !,?8,LEXFI,?19,LEXIDX,?30,LEXSTR,?58,"  ",LEXEX
 . . . I $L(LEXHA),LEXSTR'=LEXHA D
 . . . . S LEXERR=LEXERR+1
 . . . . K:'$D(LEXTEST) ^LEX(LEXFI,LEXIDX,LEXSTR,LEXEX,LEXEX2) S ^LEX(LEXFI,LEXIDX,LEXHA,LEXEX,LEXEX2)=""
 . . . . I '$D(ZTQUEUED) W !,?8,LEXFI,?19,LEXIDX,?30,$E(LEXSTR,1,20),$S($L(LEXSTR)>20:"...",1:""),?58,"  ",LEXEX
 S LEXEX=0 F  S LEXEX=$O(^LEX(LEXFI,LEXEX)) Q:+LEXEX'>0  D
 . N LEXEX2 S LEXEX2=0 F  S LEXEX2=$O(^LEX(LEXFI,LEXEX,7,LEXEX2)) Q:+LEXEX2'>0  D
 . . N DA,DIK,LEXND,LEXHA,X S DA(1)=LEXEX,DA=LEXEX2,LEXND=$G(^LEX(LEXFI,DA(1),7,DA,0))
 . . S (X,LEXHA)=$P(LEXND,"^",3) Q:'$L(LEXHA)
 . . I '$D(^LEX(LEXFI,LEXIDX,X,DA(1),DA)) D
 . . . S LEXERR=LEXERR+1 I '$D(ZTQUEUED) W !,?8,LEXFI,?19,LEXIDX,?30,"Missing",?58,"  ",DA(1)
 . . S:$L(X) ^LEX(LEXFI,LEXIDX,X,DA(1),DA)=""
 S LEXERR=$S(+LEXERR>0:LEXERR,1:"") I '$D(ZTQUEUED) W !,$J(LEXERR,5),?8,LEXFI,?19,LEXIDX,?30,LEXIDXT
 S LEXEND=$$NOW^XLFDT,LEXELP=$$FMDIFF^XLFDT(LEXEND,LEXBEG,3)
 S:$E(LEXELP,1)=" "&($E(LEXELP,3)=":") LEXELP=$TR(LEXELP," ","0")
 D REP^LEXRXXS(LEXFI,LEXFI,LEXIDX,LEXNDS,LEXERR,LEXIDXT,LEXELP)
...
SET ;   Re-Index (some indexes in file 757.01, set logic only)
 S:$D(ZTQUEUED) ZTREQ="@" Q:'$D(LEXSET)  N LEXTC,LEXPRE,LEXBEG,LEXEND,LEXELP,LEXNM,LEXFI,LEXRT,LEXIEN,LEXP3,LEXP4
...
MC ;     Major Concept Map File
 N DIK S DIK="^LEX(757," D IXALL^DIK
 Q
RL ;     Re-Link 757.05
 N LEX,DIK S LEX=" " F  S LEX=$O(^LEX(757.05,LEX)) Q:'$L(LEX)  K:LEX?1U.U ^LEX(757.05,LEX)
 S DIK="^LEX(757.05," D IXALL^DIK
 Q
