LEX2037P ; ISL/KER - Pre/Post Install ; 09/02/2005
 ;;2.0;LEXICON UTILITY;**37**;Sep 23, 1996
 ;
 ; External References
 ;   DBIA 10086  HOME^%ZIS
 ;   DBIA  2052  $$GET1^DID
...
POST ; LEX*2.0*37 Post-Install
 ...
 ;
 ;-----------------------------
 ;   Re-Index Files - N/A for LEX*2.0*37
...
LOAD ; Load Data from ^LEXM into IC*/LEX Files
 N LEXB,LEXBUILD,LEXCD,LEXIGHF,LEXLAST,LEXLREV D IMP^LEX2037
 S U="^",LEXB=$G(^LEXM(0,"BUILD")) Q:LEXB=""  Q:LEXBUILD=""
 S LEXCD=0 S LEXCD=+($$CPD^LEX2037)
...
MSG ; Send Installation Message to G.LEXICON
 Q:+($G(DUZ))=0!($$NOTDEF^LEX2037($G(DUZ)))
 D HOME^%ZIS N DIFROM,LEXLREV,LEXLAST,LEXBUILD,LEXIGHF
 D IMP^LEX2037,POST^LEXXFI Q
 ;
...
KLEXM ; Subscripted Kill of ^LEXM
 ...
 Q
 ;
PRE ; LEX*2.0*37 Pre-Install   (N/A for patch 37)
 Q
 ;
RX ; Reindex files            (N/A for patch 37)
 Q
 ...
 Q
 ;
CON ; Conversion of data       (N/A for patch 37)
 Q
