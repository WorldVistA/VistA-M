LEX2035P ; ISA/FJF-Pre/Post Install; 02/16/2005
 ;;2.0;LEXICON UTILITY;**35**;Sep 23, 1996
 ;
 ; External References
 ;   DBIA 10086  HOME^%ZIS
 ;   DBIA  2052  $$GET1^DID
...
POST ; LEX*2.0*35 Post-Install
 ...
 ;   Data Conversion
 D CON
 ;   Re-Index Files - N/A for LEX*2.0*35
 ;   Do not use for Annual/Quarterly Updates, it disrupts the Protocol
 ;   D RX
 ;   Fire Protocol - N/A for LEX*2.0*35
 ; D NOTIFY^LEXXGI
...
LOAD ; Load Data from ^LEXM into IC*/LEX Files
 N LEXB,LEXBUILD,LEXCD,LEXIGHF,LEXLAST,LEXLREV D IMP^LEX2035
 S U="^",LEXB=$G(^LEXM(0,"BUILD")) Q:LEXB=""  Q:LEXBUILD=""
 S LEXCD=0 S LEXCD=+($$CPD^LEX2035)
...
MSG ; Send Installation Message to G.LEXICON
 Q:+($G(DUZ))=0!($$NOTDEF^LEX2035($G(DUZ)))
 D HOME^%ZIS N DIFROM,LEXLREV,LEXLAST,LEXBUILD,LEXIGHF
 D IMP^LEX2035,SEND^LEXXST Q
 ;
...
KLEXM ; Subscripted Kill of ^LEXM
 ...
 Q
 ;
PRE ; LEX*2.0*35 Pre-Install   (N/A for patch 35)
 Q
 ;
RX ; Reindex files
 ; N/A for patch 35
 ...
 Q
 ;
CON ; Conversion of data       (N/A for patch 35 only)
 Q
