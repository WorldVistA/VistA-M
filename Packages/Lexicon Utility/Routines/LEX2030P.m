LEX2030P ;ISA/FJF-Pre/Post Install; 08/04/2004
 ;;2.0;LEXICON UTILITY;**30**;Sep 23, 1996
 ;
 ; External References
 ;   DBIA 10086  HOME^%ZIS
 ;   DBIA  2052  $$GET1^DID
...
LOAD ; Load Data from ^LEXM into IC*/LEX Files
 N LEXB,LEXBUILD,LEXCD,LEXIGHF,LEXLAST,LEXLREV D IMP^LEX2030
 S U="^",LEXB=$G(^LEXM(0,"BUILD")) Q:LEXB=""  Q:LEXBUILD=""
 S LEXCD=0 S LEXCD=+($$CPD^LEX2030)
...
MSG ; Send Installation Message to G.LEXICON
 Q:+($G(DUZ))=0!($$NOTDEF^LEX2030($G(DUZ)))
 D HOME^%ZIS N DIFROM,LEXLREV,LEXLAST,LEXBUILD,LEXIGHF
 D IMP^LEX2030,SEND^LEXXST Q
 ;
...
KLEXM ; Subscripted Kill of ^LEXM
 ...
 Q
 ;
PRE ; LEX*2.0*30 Pre-Install   (N/A for patch 30)
 Q
 ;
RX ; Reindex files 80, 80.1, 80.2, 81 and 81.3
 ...
 Q
 ;
CON ; Conversion of data       (N/A for patch 30)
 Q
