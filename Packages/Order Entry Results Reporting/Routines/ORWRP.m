ORWRP ; ALB/MJK,DCM - Report Calls ; Jul 03, 2024@07:35:55
 ;;3.0;ORDER ENTRY/RESULTS REPORTING;**1,10,85,109,132,160,194,227,215,262,243,280,377,498,586**;Dec 17, 1997;Build 5
 ;
...
RPT(ROOT,DFN,RPTID,HSTYPE,DTRANGE,EXAMID,ALPHA,OMEGA) ; -- return report text
 ...
 K ^TMP("ORDATA",$J) S ROOT=$NA(^TMP("ORDATA",$J,1))
 I '$L($G(RPTID)) D SETITEM(.ROOT,"Report ID is Missing.") Q
 S TAB="R"
 I $E(RPTID,1,2)="L:" S TAB="L",RPTID=$P(RPTID,":",2,999) ;an ID beginning with "L:" forces TAB to LAB - "L:" added in GUI code
 S HSTAG=$P($G(RPTID),"~",2),RPTID=$P($G(RPTID),"~"),REMOTE=+$P(RPTID,";",2),RPTID=$P($P(RPTID,";"),":")
 S ORRPTIEN=""
