ORAM1 ;POR/RSF - ANTICOAGULATION MANAGEMENT RPCS (2 of 4) ; Oct 31, 2024@10:29:16
 ;;3.0;ORDER ENTRY/RESULTS REPORTING;**307,339,351,361,391,474,463,617**;Dec 17, 1997;Build 5
 ;;Per VHA Directive 2004-038, this routine should not be modified
 ;
 ; Reference to ^LAB(60 supported by IA #91
 ;
 Q
...
FLOWTT(RESULT,DFN) ; GETS ANTICOAG FLOWSHEET DATA
 ...
 S (ORI,ORAMI,ORX)=0
 F  S ORI=$O(^ORAM(103,DFN,3,ORI)) Q:+ORI'>0  D
 . N ORAMACFS,ORAMCOM,ORAMCOMP,ORAMCC,ORAMPROV,ORAMLCNT,ORAMTOT,ORAMT,OUTSIDEHCT
 . S ORAMACFS=^ORAM(103,DFN,3,ORI,0),ORAMCC=$P(ORAMACFS,U,2)
 . S OUTSIDEHCT=$P(^ORAM(103,DFN,6),U,4)
 . S ORAMCOM=$P(ORAMACFS,U),$P(ORAMACFS,U)=$$FMTE^XLFDT(ORAMCOM,"2P"),$P(ORAMACFS,U,2)=ORI
 . S ORAMPROV=$P(ORAMACFS,U,10) I +$G(ORAMPROV) S $P(ORAMACFS,U,10)=$$GET1^DIQ(200,+ORAMPROV_",",.01)
 . S ORAMLCNT=+$O(^ORAM(103,DFN,3,ORI,1,""),-1)
 . S ORAMCOMP=$O(^ORAM(103,DFN,3,ORI,2,""),-1)
 . S ORAMT=ORAMLCNT
 ...
 .. I +$G(ORAMLCNT) F ORX=(ORAMI+1):1:(ORAMLCNT+ORAMI) S RESULT(ORX)=$G(^ORAM(103,DFN,3,ORI,1,(ORX-ORAMI),0))
 .. I +$G(ORAMCC),+$G(ORAMCOMP) D
 ... S ORAMI=ORX+2,RESULT(ORAMI)="Complications noted:"
 ... F ORX=(ORAMI+2):1:(ORAMCOMP+ORAMI) D
 .... N ORAMCMPL
 .... S ORAMCMPL=$G(^ORAM(103,DFN,3,ORI,2,(ORX-ORAMI),0))
...
LOCK(RESULT,DATA) ; Locks form   RPC=ORAM1 LOCK
 ...
 S $P(^ORAM(103,$P(DATA,"^"),0),"^",19)=1_"|"_$$GET1^DIQ(200,+$P(DATA,"^",2)_",",.01)_"|"_ORAMTDAY,RESULT=1
 Q
 ;
