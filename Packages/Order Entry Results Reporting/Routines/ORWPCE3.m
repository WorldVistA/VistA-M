ORWPCE3 ; SLC/KCM/REV/JM/TC - Get a PCE encounter for a TIU document ;Dec 03, 2025@07:26:33
 ;;3.0;ORDER ENTRY/RESULTS REPORTING;**10,85,116,190,280,306,371,361,385,377,498,405,598,588,606,508**;Dec 17, 1997;Build 39
 ;
 ...
 ; Reference to $$GETCODE^PXSPECAUTH in ICR #7506
 ;
PCE4NOTE(LST,IEN,DFN,VSITSTR) ; Return encounter for an associated note   IA#4214,6132,6614
 ...
 N X,X0,X2,X12,X13,X16,X802,X811,VTYP,IDX,IPOV,ICPT,IPRV,IIMM,ISK,IPED,IHF,IXAM,ITRT
 N ICOM,ISSECV,MIDX,MIEN,MCNT,MODS,VIS,VISCNT,VISTEMP,CPIECE,GFINDS,GCNT,ITEM,ORDATA
 N ORPARR,ORPVAL,ORSUB,TOPLST,IST,X220,MAG,UCUM,PRIMARY,SUB,TEMP,SYSTEM,SYSIEN,SAIDX
 S PRIMARY=0
 ...
 N VAL,SCNODE
 S IDX=0 F  S IDX=$O(^TMP("PXKENC",$J,VISIT,"VST",VISIT,900,IDX)) Q:IDX'>0  D
 .S SCNODE=$G(^TMP("PXKENC",$J,VISIT,"VST",VISIT,900,IDX,0)),SAIDX=$P(SCNODE,U),CODE=$$GETCODE^PXSPECAUTH(SAIDX)
 .S ILST=ILST+1,LST(ILST)="VST^"_CODE_U_$S(+$P(SCNODE,U,2)>-1:$P(SCNODE,U,2),1:"")
