ORWDX1 ; SLC/KCM/REV - Utilities for Order Dialogs ; Oct 03, 2025@11:12:00
 ;;3.0;ORDER ENTRY/RESULTS REPORTING;**85,187,195,215,243,296,341,350,461,625**;Dec 17, 1997;Build 10
 ;
 ...
 ;Reference to ^VA( supported by ICR #4329
 ;
...
CHANGE(ORLST,ORCLST,DFN,ISIMO) ;
 N CNT,PACKIEN,INP,IVM,INPDIEN,IVMDIEN,CIEN,CIVIEN,UDIEN
 N ISCONV,ORIEN,ORLOC,ORDG,DIAL,ORDGRP,ISIMOLOC
 S (INP,IVM,INPDIEN,IVMDIEN,UDIEN,CIEN,CIVIEN)=0
 ...
 S CIVIEN=$O(^ORD(100.98,"B","CLINIC INFUSIONS","")) Q:CIVIEN'>0
 ...
 .S DIAL=$P($G(^OR(100,ORIEN,3)),U,4)
 .;Remove Treating Specialty if the order location is the clinic
 .I $P($G(^OR(100,ORIEN,0)),U,10)=(ORLOC_";SC("),$P($G(^SC(ORLOC,0)),U,3)="C" D  Q
 ..S $P(^OR(100,ORIEN,0),U,13)=""
 .;
 .;Change patient location and patient class.
 .S $P(^OR(100,ORIEN,0),U,10)=ORLOC_";SC("
 .S PACKIEN=$P(^OR(100,ORIEN,0),U,14)
 .I $$GET1^DIQ(9.4,PACKIEN_",",1)'="PSO" S $P(^OR(100,ORIEN,0),U,12)="I"
 .;
 .; DM 625 Updates order when converting to IMO location.
 .D IMOLOC^ORIMO(.ISIMOLOC,ORLOC,DFN,0)
 .I (ISIMOLOC>=0),(ORDG=INPDIEN)!(ORDG=IVMDIEN)!(ORDG=UDIEN) D
 ..I $P(DIAL,";",2)="ORD(101.41," S $P(^OR(100,ORIEN,0),U,11)=$S(ORDG=IVMDIEN:CIVIEN,1:CIEN),ISCONV=1
 ..S ORDG=$P(^OR(100,ORIEN,0),U,11)
 ..S ORDGRP=$P($G(^ORD(100.98,ORDG,0)),U,4)
 ..S ($P(^OR(100,ORIEN,0),U,5),$P(^OR(100,ORIEN,3),U,4))=ORDGRP_";ORD(101.41,"
 .;
 .; DM 625 Updates display group for the order.
 .I 'ISCONV D
 ..I $P(DIAL,";",2)="ORD(101.41," S $P(^OR(100,ORIEN,0),U,11)=$P($G(^ORD(101.41,+DIAL,0)),U,5)
