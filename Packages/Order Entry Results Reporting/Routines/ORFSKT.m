...
BLDSTRS(RESULTS,CNT,PARM,EXT,INT,PIECE,ID,NAME,TYPE,DATETIME,DELSTR,DELSTR1,DELSTR2,ADMINDATE,ENCTYPE,PRMPTS,PLACEIEN) ;
 ...
 .I "AIDH"[ENCTYPE D  Q
 ..;I (TYPE=0!(TYPE=1))&($P(INT,".")<$P(DATETIME,".")) S RESULTS(CNT)="-1^"_ORTEMP_" Date cannot be before the encounter date/time." Q
 ..I INT>$$NOW^XLFDT() S RESULTS(CNT)="-1^"_ORTEMP_" Date/Time cannot be a future date/time." Q
 ..;
 ...
 .I INT>$$NOW^XLFDT() S RESULTS(CNT)="-1^"_ORTEMP_" Date/Time cannot be a future date/time." Q
 .S ADMINDATE=INT,$P(DELSTR,U,PIECE)=INT
 ;
...
GETPOSS(ID,NAME,TYPE,DATETIME,ENCTYPE,LOC,VIMMDOC,PLACE,CNT,RESULT,DEFAULTS,PATIENT) ;
 ...
 ..I $$FMDIFF^XLFDT(DATETIME,$$NOW^XLFDT(),2)>0 S DEFAULTS("VISIT DATE TIME")=U_$$NOW^XLFDT() Q
