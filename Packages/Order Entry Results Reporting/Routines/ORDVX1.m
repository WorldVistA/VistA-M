ORDVX1 ; slc/dcm - OE/RR Extract Lab AP Reports ;01/08/15  08:44
 ;;3.0;ORDER ENTRY/RESULTS REPORTING;**243,315,377**;Dec 17, 199;Build 582
 ;
...
APSET(LRSS)   ; Sets ^TMP("OROOT",$J
 N ACC,CDT,DA,DIC,DIQ,DR,GMW,SN,X,YR,OACC,TIUDA
 ...
 I $D(^LR(LRDFN,LRSS,IX,.05))  D
 . S TIUDA=0 F  S TIUDA=$O(^LR(LRDFN,LRSS,IX,.05,"C",TIUDA)) Q:'TIUDA  S ^TMP("OROOT",$J,IX,LRSS,.05)=TIUDA
