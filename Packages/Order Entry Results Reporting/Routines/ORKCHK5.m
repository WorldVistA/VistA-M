ORKCHK5 ;SLC/CLA - SUPPORT ROUTINE FOR ACCEPT MODE ORDER CHECKS ;Aug 09, 2021@10:50:14
 ;;3.0;ORDER ENTRY/RESULTS REPORTING;**6,32,74,94,123,190,280,357,345,269,382,539,562,405,626**;Dec 17, 1997;Build 19
 ;
 ...
 ; Reference to BDA^PSSPGXOR is supported by ICR 7524
 ; Reference to PGX^PSSPGXOR is supported by ICR 7524
 Q
...
PHARM ;process pharmacy order checks:
 ...
 D:+ORDODSG PGX(.ORKS,ORKDFN,.OROIL,ORKA) ;do pharmacogenomic checks
...
DSGCHK(ORRET,ORDFN,OROIL,ORKA) ;DO DOSAGE ORDER CHECKS
 ...
 Q
 ;
PGX(ORRET,ORDFN,OROIL,ORKA) ;DO PGX ORDER CHECKS
 N ORTYPE,ORY,ORI,ORKNUM,ORKFLAG,ORKDNGR,ORPSDRUG,ORLOOP1,ORLOOP2,ORCNT,ORNODE,ORNAME,ORPGXOI,ORPOROI,ORPGXMON,ORPGXLP,ORPGXPK,ORMONCT,ORPGXMEN,ORPGXHEN,ORPGXSUP,ORPGXNM,ORPGXSEV,ORPGXTXT
 D PARAMS("PHARMACOGENOMICS HIGH",.ORKNUM,.ORKFLAG,.ORKDNGR)
 S ORPGXHEN=$S(ORKFLAG="D":0,1:1) S ORPGXNM("HIGH","NUM")=ORKNUM,ORPGXNM("HIGH","DANGER")=ORKDNGR
 K ORKNUM,ORKFLAG,ORKDNGR
 D PARAMS("PHARMACOGENOMICS MODERATE",.ORKNUM,.ORKFLAG,.ORKDNGR)
 S ORPGXMEN=$S(ORKFLAG="D":0,1:1) S ORPGXNM("MODERATE","NUM")=ORKNUM,ORPGXNM("MODERATE","DANGER")=ORKDNGR
 I 'ORPGXHEN,'ORPGXMEN Q
 S ORPGXSUP=$S('ORPGXMEN:"M",'ORPGXHEN:"H",1:"N")
 S ORTYPE=$P(ORKA,"|",2)
 S ORPSDRUG=$P(ORKA,"^",4)
 S ORPSDRUG=$G(ORPSDRUG)
 I 'ORPSDRUG S (ORPGXOI,ORPOROI)=0 D
 .S ORCNT="" F  S ORCNT=$O(OROIL(ORCNT)) Q:ORCNT=""!(ORPGXOI)  D
 ..S ORNODE=$G(OROIL(ORCNT))  ;Q:$P(ORNODE,U,2)="PSIV"
 ..S ORNAME=$P(ORNODE,U,2) I ORNAME="ORDERABLE" S ORPGXOI=$P(ORNODE,U,4)
 I 'ORPSDRUG,$G(ORPGXOI) D
 .S ORPOROI=$P($P(^ORD(101.43,ORPGXOI,0),U,2),";")
 I 'ORPSDRUG,'ORPOROI Q
 I 'ORPSDRUG,ORPOROI S ORPGXPK=$S(ORKDG="PSIV":"I",ORKDG="PSI":"U",ORKDG="PSH":"X",1:"O"),ORPSDRUG=$$BDA^PSSPGXOR(ORPOROI,ORPGXPK)
 Q:'ORPSDRUG
 S ORPSDRUG=+ORPSDRUG
 S ORPGXPK=$S(ORKDG["PSI":"I",1:"O")
 ;For IV's a call is made through here for each individual drug of the IV
 K ^TMP($J,"ORPGX") D PGX^PSSPGXOR(ORDFN,"ORPGX",ORPGXPK,ORPSDRUG,1,ORPGXSUP)
 S (ORPGXMON,ORMONCT)=0 F  S ORMONCT=$O(^TMP($J,"ORMONOGRAPH",ORMONCT)) Q:'ORMONCT  S ORPGXMON=ORMONCT
 S ORLOOP1="" F  S ORLOOP1=$O(^TMP($J,"ORPGX",ORLOOP1)) Q:ORLOOP1=""  D
 .S ORLOOP2="" F  S ORLOOP2=$O(^TMP($J,"ORPGX",ORLOOP1,ORLOOP2)) Q:ORLOOP2=""  D
 ..S ORPGXTXT=^TMP($J,"ORPGX",ORLOOP1,ORLOOP2,"TEXT")
 ..S ORPGXSEV=^TMP($J,"ORPGX",ORLOOP1,ORLOOP2,"SEV") S ORPGXSEV=$S(ORPGXSEV="HIGH":ORPGXNM("HIGH","DANGER"),1:ORPGXNM("MODERATE","DANGER"))
 ..S ORKNUM=$S(ORPGXSEV=1:ORPGXNM("HIGH","NUM"),1:ORPGXNM("MODERATE","NUM"))
 ..I $D(^TMP($J,"ORPGX",ORLOOP1,ORLOOP2,"PGXMON")) D
 ...S ORPGXMON=ORPGXMON+1
 ...S ^TMP($J,"ORMONOGRAPH")=ORPGXMON
 ...S ^TMP($J,"ORMONOGRAPH",ORPGXMON,"INT")=$G(^TMP($J,"ORPGX",ORLOOP1,ORLOOP2,"PGXTITLE"))
 ...S ORPGXLP="" F  S ORPGXLP=$O(^TMP($J,"ORPGX",ORLOOP1,ORLOOP2,"PGXMON",ORPGXLP)) Q:ORPGXLP=""  D
 ....S ^TMP($J,"ORMONOGRAPH",ORPGXMON,"DATA",ORPGXLP,0)=^TMP($J,"ORPGX",ORLOOP1,ORLOOP2,"PGXMON",ORPGXLP)
 ....S ^TMP($J,"ORMONOGRAPH",ORPGXMON,"OC")=ORPGXTXT
 ..S ORRET("ORK",ORPGXSEV_",,"_"  "_$G(ORNUM)_","_$E(ORPGXTXT,1,225))=""_U_ORKNUM_U_ORPGXSEV_U_$G(ORPGXTXT)
 K ^TMP($J,"ORPGX")
 Q
