...
BEGIN S DNM="A1CKC2",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,D=0 K DE(1) ;.3721
 S DIFLD=.3721,DGO="^A1CKC3",DC="6^2.04P^.372^",DV="2.04MP31'X",DW="0;1",DOW=$$LABEL^DIALOGZ(DP,DIFLD),DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="DIC(31,"
...
X4 S Y=$P(STR,"^"),STR=$P(STR,"^",2,99)
 Q
5 S DQ=6 ;@100
6 S DW=".3;11",DV="SX",DU="",DIFLD=.3025,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C6^A1CKC2"
...
X7 S Y=$P(STR,"^"),STR=$P(STR,"^",2,99)
 Q
8 S DQ=9 ;@200
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW=".362;14",DV="SX",DU="",DIFLD=.36235,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C9^A1CKC2"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 S X=PE
 S Y=X
...
X10 S Y=$P(STR,"^"),STR=$P(STR,"^",2,99)
 Q
11 S DQ=12 ;@300
12 D:$D(DG)>9 F^DIE17,DE S DQ=12,DW=".362;12",DV="SX",DU="",DIFLD=.36205,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C12^A1CKC2"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 S X=AA
 S Y=X
...
X13 S Y=$P(STR,"^"),STR=$P(STR,"^",2,99)
 Q
14 S DQ=15 ;@400
15 D:$D(DG)>9 F^DIE17,DE S DQ=15,DW=".362;13",DV="SX",DU="",DIFLD=.36215,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C15^A1CKC2"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 S X=HB
 S Y=X
