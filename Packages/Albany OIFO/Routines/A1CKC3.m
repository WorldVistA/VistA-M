...
BEGIN S DNM="A1CKC3",DQ=1+D G B
1 S DW="0;1",DV="MP31'X",DU="",DIFLD=.01,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C1^A1CKC3",DE(DW,"INDEX")=1
 S DU="DIC(31,"
 S X="`"_ISC
 S Y=X
...
X1 I $D(X) D EK^DGLOCK Q
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="0;2",DV="RNJ3,0X",DU="",DIFLD=2,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C2^A1CKC3",DE(DW,"INDEX")=1
 S X=+SCI(ISC)
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
...
X2 K:+X'=X!(X>100)!(X<0)!(X?.E1"."1N.N) X I $D(X) D EK^DGLOCK
 Q
 ;
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW="0;3",DV="SX",DU="",DIFLD=3,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C3^A1CKC3",DE(DW,"INDEX")=1
 S DU="0:NO;1:YES;"
 S Y="1"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
