FBCTAU ; GENERATED FROM 'FBAA AUTHORIZATION' INPUT TEMPLATE(#1015), FILE 161;10/31/14
 D DE G BEGIN
...
X1 S FBPRG=""
 Q
2 S D=0 K DE(1) ;1
 S DIFLD=1,DGO="^FBCTAU1",DC="26^161.01ID^1^",DV="161.01DX",DW="0;1",DOW="FROM DATE",DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
...
X5 S HID=$S($D(^FBAAA(DA,4)):$P(^(4),"^"),1:"")
 Q
6 S DW="4;1",DV="FX",DU="",DLB="FEE ID CARD NUMBER",DIFLD=.5
 S DE(DW)="C6^FBCTAU"
 G RE
...
X7 S NID=X
 Q
8 D:$D(DG)>9 F^DIE17,DE S DQ=8,DW="4;2",DV="D",DU="",DLB="FEE ID CARD ISSUE DATE",DIFLD=.6
 S X="TODAY"
 S Y=X
 G Y
...
X9 S Y=$S(HID="":"",HID=NID:"",NID="":.65,1:.7)
 Q
10 S DW="4;4",DV="D",DU="",DLB="FEE ID CARD EXPIRATION DATE",DIFLD=.65
 G RE
X10 S %DT="EX" D ^%DT S X=Y K:Y<1 X
 Q
 ;
11 S DW="4;3",DV="RF",DU="",DLB="REASON FOR CARD NUMBER CHANGE",DIFLD=.7
 G RE
