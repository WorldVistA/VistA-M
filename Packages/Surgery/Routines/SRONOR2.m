SRONOR2 ;B'HAM ISC/MAM - REPORT OF NON-O.R. PROCEDURES ; Apr 07, 2025@10:00
 ;;3.0;Surgery;**50,175,182,219**;24 Jun 93;Build 2
 ;
...
LOOP ; break procedure if greater than 50 characters
 ...
 Q
CAN(SROP,SRST) ; Cancel/abort check - returns Canceled, Aborted or Null - called by SRONOR2,3,4,5,7 and 8   SR*3.0*219
 I ($P($G(^SRF(SROP,30)),"^")=""),($P($G(^SRF(SROP,31)),"^",8)="") Q "" ; No Canc date/time OR reason code: not canceled
 I SRST]"" Q "*ABORTED*" ; Procedure started
 I $P($G(^SRF(SROP,.2)),"^",1) Q "*ABORTED*"   ; Anesthesia started
 I $P($G(^SRF(SROP,30)),"^",6)>1 Q "*ABORTED*" ; abort flag >1
 Q "*CANCELED*" ; canceled before procedure started.
