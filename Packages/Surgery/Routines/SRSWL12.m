SRSWL12 ;BIR/MAM - WAITING LIST, BRIEF-ALL ; [ 12/06/99  9:45 AM ]
 ...
 I $D(IO("Q")) K IO("Q") S ZTDESC="SURGERY WAITING LIST",ZTRTN="BEG^SRSWL12" D ^%ZTLOAD G END
BEG ; entry when queued
 ...
 ; of Operation, and corresponding record and subrecord numbers.
 ;
 ...
 .D PAGE Q:SRSOUT  S SROPDT="" F  S SROPDT=$O(^TMP("SR",$J,SRSNM,SROPDT)) Q:SROPDT=""!SRSOUT  S SRSS="" F  S SRSS=$O(^TMP("SR",$J,SRSNM,SROPDT,SRSS)) Q:SRSS=""!SRSOUT  D ANOTHER
END I $E(IOST)="P" S SRSOUT=1 W @IOF
 I $D(ZTQUEUED) K ^TMP("SR",$J) Q:$G(ZTSTOP)  S ZTREQ="@" Q
 I 'SRSOUT W !!,"Press RETURN to continue, or '^' to quit:  " R X:DTIME W @IOF
