...
LA1 S:$D(ZTQUEUED) ZTREQ="@" S LANM=$T(+0),TSK=$O(^LAB(62.4,"C",LANM,0)),U="^" Q:TSK<1
 Q:'$D(^LA(TSK,"I",0))
 S LRTOP=$P(^LAB(69.9,1,1),U,1) D ^LASET Q:'TSK  S X="TRAP^"_LANM,@^%ZOSF("TRAP")
 S SS="CH"
 F I=1:1:12 S ZZ(I)=$P("DATE^TEST^WBC^RBC^HGB^HCT^MCV^MCH^MCHC^RDW^PLT^",U,I)
LA2 K Y S A=0,TOUT=0 D IN G QUIT:TOUT,C3:IN["DATE",LA2:IN'["-------"
 ;
C2 S TOUT=0 D IN G QUIT:TOUT
C3 S A=A+1,Y(A)=""
 S X="" F I=1:1:$L(IN) S:$A(IN,I)-32 X=X_$E(IN,I)
 S NM=$P(X,",",1),NM=$S(NM?1P.A:$E(NM,2,9),1:NM) F A=1:1:12 Q:ZZ(A)=NM
 G C2:A>11 S Y(A)=$P(X,",",2),F(A)=""
 S X=Y(A),Y(A)=$S(".....-----+++++"[X:"",1:+X)_$S(A>2&(F(A)]" "):F(A),1:"") G C2:A<11
