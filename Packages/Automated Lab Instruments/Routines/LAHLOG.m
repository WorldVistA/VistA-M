...
LA2 K TV,Y S TOUT=0,A=1 D IN G QUIT:TOUT,LA2:IN'?4N.E D QC
 S TOUT=0,BAD=0,A=2 D IN,QC G QUIT:TOUT
 F I=0:0 S I=$O(TC(I)) Q:I<1  X TC(I,2) S:V @TC(I,1)=V ;V COULD BE MODIFIED ON THIS LINE
 ;S X=$E(Y(),,) F I=$L(X):-1:1 I $A(X,I)-32 S RMK=$E(X,1,I) Q
 S V=$E(Y(1),6,8) D NUM S ID=+V,V=$E(Y(1),10,15) D NUM S IDE=+V,TRAY=1,CUP=ID
