LR582 ;SLC/PBB - LR*5.2*582 PATCH UNIVERSAL ENVIRONMENT CHECK ROUTINE ;01/21/06
 ;;5.2;LAB SERVICE;**582**;Sep 27, 1994;Build 6
 ;;
EN ; Does not prevent loading of the transport global.
 ...
 Q
 ;
PRE(ZZP,ZZV) ; KIDS Pre install for LR*5.2*582
 ;
 N XQA,XQAMSG,LRZZP
 S LRZZP="LR"_ZZP ;"LR582"
 S LRZZPP=LRZZP_" " ;"LR582 "
 ;K ^XTEMP($J,"LR582") ;Keep field 80 identifier
 ...
 Q
 ;
POST(ZZP,ZZV) ; KIDS Post install for LR*5.2*582
 ;
 ...
 N X S X="LRXXX" X ^%ZOSF("DEL") ;Delete obsolate install routine.
 Q
NLT1 ;
 N I
 ...
 ;S Y=$$NOW^XLFDT\1 D DD^%DT S Y="2.52 ;;5.2;LAB SERVICE;**582**;"_Y_";Build 01"
 ;S Y="5.2;LAB SERVICE;**582**;MAY 05, 2016;Build 1"
 ;S Y="2.80;;5.2;LAB SERVICE;**582**;Sep 27, 1994"
...
REINDEX ;Reindex LAM to fire new x-refs
 ...
 . S $P(^LAM(0),U,3)=99999
 ;K ^XTMP("LRNLTERR","LR582") S ^XTMP("LRNLTERR",0)=$$HTFM^XLFDT($H+90,1)_"^"_DT_"^LR52 582 Error Messages"
 K ^XTMP("LRNLTERR",LRZZP) S ^XTMP("LRNLTERR",0)=$$HTFM^XLFDT($H+90,1)_"^"_DT_"^LR52 "_ZZP_" Error Messages"
 ;K ^XTMP("LRNLT","LR582")
 K ^XTMP("LRNLT",LRZZP)
 ;S ^XTMP("LRNLT",0)=$$HTFM^XLFDT($H+90,1)_"^"_DT_"^LR52 582 Messages"
...
POST1 ;TRANSPORT FILE 64.81 INTO FILE 64 IF REQUIRED
 N LRREC,LRREC9
 ;K ^XTMP("LRNLT","LR582 ") D
 K ^XTMP("LRNLT",LRZZPP) D
 . ;S ^XTMP("LRNLT",0)=$$HTFM^XLFDT($H+90,1)_U_DT_U_"LR582  Added NLT Codes List"
 . S ^XTMP("LRNLT",0)=$$HTFM^XLFDT($H+90,1)_U_DT_U_LRZZPP_" Added NLT Codes List"
 . ;S ^XTMP("LRNLT","LR582 ",0)=""
...
P1 F  L +^LAM:10 Q:$T  D BMES^XPDUTL("Attempting to Lock ^LAM Global.")
 ...
 S $P(^LAM(0),U,3)=99999
 D:'$G(LRDBUG)
 . ;S $ZE="LR582 : Application Error! please disregard NO need to report it" D ^%ZTER ;MAIL^LR582POA
...
SCR(LRMSG) ;Store message in ^XTMP("LRNLT" Global
 ;S LRSCR=$G(^XTMP("LRNLT","LR582 ",1,0))+1,^(0)=LRSCR
 S LRSCR=$G(^XTMP("LRNLT",LRZZPP,1,0))+1,^(0)=LRSCR
 ;S ^XTMP("LRNLT","LR582 ",1,LRSCR)=LRSCR_"|"_LRMSG
