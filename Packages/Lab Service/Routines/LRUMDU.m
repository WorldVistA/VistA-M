...
Z Q:LR("Q")  W !,"-----------------|-------|-------|-------|-------|-------|-------|-------|" Q
 ;
R W !!,"SURE YOU WANT TO DELETE ALL THE LISTS " S %=2 D YN^LRU Q:%'=1  K ^LRO(69.2,LRAA,7,DUZ,60) S ^(60,0)="^69.35A^0^0" Q
 ;
D R !,"Select list number to delete: ",X:DTIME G:X=""!(X[U) LRUMDU I '$D(N(X)) W $C(7),!,"Enter the test list number",! G D
 K ^LRO(69.2,LRAA,7,DUZ,60,X) S X(1)=$O(^LRO(69.2,LRAA,7,DUZ,60,0)) S:'X(1) X(1)=0 S X=^LRO(69.2,LRAA,7,DUZ,60,0),^(0)=$P(X,"^",1,2)_"^"_X(1)_"^"_$S(X(1)=0:X(1),1:($P(X,"^",4)-1)) G LRUMDU
 ;
...
C I X="@" W !?3,"SURE YOU WANT TO DELETE ? " S %=0 D RX^LRU G:%=1 K W " <NOTHING DELETED>" Q
 ...
 ;
P S ZTRTN="QUE^LRUMDU" D BEG^LRUTL G:POP!($D(ZTSK)) END
...
B D A Q:X=""!(X[U)
G I '$D(^LRO(69.2,LRAA,7,DUZ,60,L,0)) S ^(0)=L,Z=^LRO(69.2,LRAA,7,DUZ,60,0),^(0)=$P(Z,"^",1,2)_"^"_L_"^"_($P(Z,"^",4)+1)
 I $D(^LRO(69.2,LRAA,7,DUZ,60,L,1,O,0)) S ^(0)=T Q
 S:'$D(^LRO(69.2,LRAA,7,DUZ,60,L,1,0)) ^(0)="^69.36PA^0^0" S Z=^(0),^(0)=$P(Z,"^",1,2)_"^"_O_"^"_($P(Z,"^",4)+1),^LRO(69.2,LRAA,7,DUZ,60,L,1,O,0)=T Q
 ;
...
W W !!?3,"Enter test list number (1-99) then a ',' then test order number (1-7)",!,"[Entering 2,3 would put the test selected in test list 2 and test order 3]",! Q
 ;
END S Z=0 F X=0:0 S X=$O(^LRO(69.2,LRAA,7,DUZ,60,X)) Q:'X!(Z=1)  F Y=0:0 S Y=$O(^LRO(69.2,LRAA,7,DUZ,60,X,1,Y)) Q:'Y  S Z=1 Q
 G:Z LRUMDU Q
K K ^LRO(69.2,LRAA,7,DUZ,60,L,1,O) S X(1)=$O(^LRO(69.2,LRAA,7,DUZ,60,L,1,0)) S:'X(1) X(1)=0 S X=^LRO(69.2,LRAA,7,DUZ,60,L,1,0),^(0)=$P(X,"^",1,2)_"^"_X(1)_"^"_$S(X(1)=0:X(1),1:($P(X,"^",4)-1)) D K^LRU,F,Z Q
F W @IOF
F1 F A=0:0 S A=$O(^LRO(69.2,LRAA,7,DUZ,60,A)) Q:'A  F B=0:0 S B=$O(^LRO(69.2,LRAA,7,DUZ,60,A,1,B)) Q:'B  S C=^(B,0),N(A,B)=$P(^LAB(60,C,.1),"^"),L(A,B)=$P($P(^(0),U,5),";",2)
...
I W ! S DIC=200,DIC(0)="AEQM" D ^DIC K DIC Q:Y<1  S X=+Y I '$O(^LRO(69.2,LRAA,7,X,60,0)) W $C(7),!,$P(Y,"^",2)," has no test lists." G I
 S %X="^LRO(69.2,LRAA,7,X,60,",%Y="^LRO(69.2,LRAA,7,DUZ,60," D %XY^%RCR Q
