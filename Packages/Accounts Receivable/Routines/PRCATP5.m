PRCATP5 ; GENERATED FROM 'PRCA 3RD PROFILE' PRINT TEMPLATE (#386) ; 01/14/26 ; (FILE 430, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 S X=$G(^PRCA(430,D0,0)) S Y=$P(X,U,7) S Y=$S(Y="":Y,$D(^DPT(Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 D N:$X>39 Q:'DN  W ?39 W "BILL #: "
 W ?0,$E($P(X,U,1),1,14)
 W ?49 D SVCINFO^PRCAAPR1 K DIP K:DN Y
 ...
 D N:$X>60 Q:'DN  W ?60,$E($P(X,U,6),1,17)
 ...
 S X=$G(^PRCA(430,D0,0)) W ?31 S Y=$P(X,U,19) S Y=$S(Y="":Y,$D(^DIC(36,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 D N:$X>0 Q:'DN  W ?0 W "TERTIARY INSUANCE COMPANY: "
 W ?29 S Y=$P(X,U,20) S Y=$S(Y="":Y,$D(^DIC(36,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
