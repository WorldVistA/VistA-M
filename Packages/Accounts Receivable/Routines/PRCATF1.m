PRCATF1 ; GENERATED FROM 'PRCA FMS STATUS' PRINT TEMPLATE (#442) ; 06/27/96 ; (FILE 347, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 F X=0:0 S X=$O(^DIPT(442,"DXS",X)) Q:'X  S Y=$O(^(X,"")) F X=X:0 Q:Y=""  S DXS(X,Y)=^(Y),Y=$O(^(Y))
 ...
 S X=$G(^RC(347,D0,0)) S Y=$P(X,U,7) S Y=$S(Y="":Y,$D(^PRCA(430,Y,0))#2:$P(^(0),U,1),1:Y) W $E(Y,1,30)
