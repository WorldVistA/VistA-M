...
DE S DIE="^PRCA(430,D0,2,",DIC=DIE,DP=430.01,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^PRCA(430,D0,2,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,1) S:%]"" DE(1)=% S %=$P(%Z,U,8) S:%]"" DE(2)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="PRCATE2",DQ=1+D G B
...
X1 K:X'?2N X I $D(X) S DINUM=$S(+X<70:"3"_X,1:"2"_X)
 ...
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="0;8",DV="RNJ12,2",DU="",DIFLD=7,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C2^PRCATE2"
 G RE
...
C2F1 Q
X2 S:X["$" X=$P(X,"$",2) K:X'?.N.1".".2N!(X>999999999.99)!(X<0) X
 Q
 ;
3 G 1^DIE17
