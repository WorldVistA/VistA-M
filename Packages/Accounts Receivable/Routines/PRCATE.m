PRCATE ; GENERATED FROM 'PRCA SET' INPUT TEMPLATE(#799), FILE 430;05/04/10
 D DE G BEGIN
...
BEGIN S DNM="PRCATE",DQ=1
 ...
 M DIEZAR=^DIE(799,"AR") S DICRREC="TRIG^DIE17"
 S:$D(DTIME)[0 DTIME=300 S D0=DA,DIIENS=DA_",",DIEZ=799,U="^"
...
X12 I PRCAT'["T" S Y="@4"
 Q
13 D:$D(DG)>9 F^DIE17,DE S DQ=13,DW="202;1",DV="F",DU="",DLB="INSURED NAME",DIFLD=239
 G RE
...
X14 S:X="" Y=247
 Q
15 S DW="202;2",DV="S",DU="",DLB="INSURED SEX",DIFLD=240
 S DU="M:MALE;F:FEMALE;U:UNKNOWN;"
 G RE
X15 Q
16 S DW="202;4",DV="F",DU="",DLB="CERT SSN HIC ID NO.",DIFLD=242
 G RE
X16 K:$L(X)>20!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
 ;
17 S DW="202;5",DV="F",DU="",DLB="GROUP NAME",DIFLD=243
 G RE
X17 K:$L(X)>20!($L(X)<1) X
 I $D(X),X'?.ANP K X
 Q
 ;
18 S DW="202;6",DV="F",DU="",DLB="GROUP NUMBER",DIFLD=244
 G RE
X18 K:$L(X)>17!($L(X)<1) X
 I $D(X),X'?.ANP K X
 Q
 ;
19 S DW="202;9",DV="F",DU="",DLB="EMPLOYER NAME",DIFLD=247
 G RE
...
X20 S:X="" Y="@4"
 Q
21 S DW="202;10",DV="F",DU="",DLB="EMPLOYEE ID NUMBER",DIFLD=248
 G RE
X21 K:$L(X)>11!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
 ;
22 S DW="202;11",DV="F",DU="",DLB="EMPLOYER LOCATION",DIFLD=249
 G RE
X22 K:$L(X)>40!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
 ;
23 S DW="0;19",DV="P36'",DU="",DLB="SECONDARY INSURANCE CARRIER",DIFLD=19
 S DU="DIC(36,"
 G RE
X23 Q
...
X24 S:X="" Y="@4"
 Q
25 S DW="0;20",DV="P36'",DU="",DLB="TERTIARY INSURANCE CARRIER",DIFLD=19.1
 S DU="DIC(36,"
 G RE
X25 Q
26 S DQ=27 ;@4
27 S DW="0;5",DV="R*P430.6'OX",DU="",DLB="BILL RESULTING FROM",DIFLD=4.5
...
X28 S $P(^PRCA(430,D0,7),U,1)=X
 Q
29 S DW="0;10",DV="RDX",DU="",DLB="DATE BILL PREPARED",DIFLD=10
 S DE(DW)="C29^PRCATE"
 G RE
...
31 D:$D(DG)>9 F^DIE17,DE S DQ=31,D=0 K DE(1) ;1
 S DIFLD=1,DGO="^PRCATE2",DC="7^430.01IA^2^",DV="430.01MRFX",DW="0;1",DOW="FISCAL YEAR",DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
...
R31 D DE
 ...
 ;
32 S DW="100;2",DV="RP49'",DU="",DLB="SERVICE",DIFLD=101
 S DU="DIC(49,"
 G RE
X32 Q
33 S DW="104;1",DV="P200'",DU="",DLB="APPROVING OFFICIAL (SERVICE)",DIFLD=111
 S DU="VA(200,"
 G RE
X33 Q
