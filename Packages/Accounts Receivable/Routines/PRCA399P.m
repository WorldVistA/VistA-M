PRCA399P ;MNTVBB/RGB - Patient bills reset AFTER 382/385 ; 2/22/22
...
EN S PRCASITE=$E($$SITE^RCMSITE(),1,3),PRCASTNM=$P(^DIC(4,PRCASITE,0),U)
 I $D(^XTMP("PRCA399",$J,998)) W !!,"*********CANNOT RUN BACK TO BACK WITHOUT SPECIAL RESTORE ********" Q
 D MES^XPDUTL("Reset all AR debtors bills that had an incorrect posting")
 D MES^XPDUTL("of Covid decrease adjustments for interest/admin fees")
 D MES^XPDUTL("")
 D MES^XPDUTL("SITE: "_PRCASITE_" - "_PRCASTNM)
 K ^XTMP("PRCA399")
 S ^XTMP("PRCA399",0)=$$FMADD^XLFDT(DT,150)_"^"_DT
 S ^XTMP("PRCA399",$J,1,0)=0,^XTMP("PRCA399",$J,2,0)=0
 D NOW^%DTC S ^XTMP("PRCA399",$J,999,"ASTART382/385 FIX")=%
 D NOW^%DTC S ^XTMP("PRCA399",$J,999,"B1FINDBILS399")=%
 D SETTAB,FINDF1ST,BILLDT
 S U="^",(PRCAT,PRCAT1,PRCAT2,PRCAT3,PRCAT4,PRCAT5,PRCAPRT)=0,PRCAZ=+$G(^XTMP("PRCA399",$J,0,0))
 S PRCATX=PRCAF1ST
 F  S PRCATX=$O(^PRCA(433,PRCATX)) Q:'PRCATX  D
 .S PRCAR=$G(^PRCA(433,PRCATX,5)) Q:PRCAR'["PANDEMIC REVERSAL"
 .S PRCAT=PRCAT+1
 .S PRCABILL=$P($G(^PRCA(433,PRCATX,0)),U,2),PRCADEB=$P(^PRCA(430,PRCABILL,0),U,9)
 .I '$D(^RCD(340,PRCADEB,0))!($P(^RCD(340,PRCADEB,0),U)'[";DPT") Q
 .I ";16;42;"'[(";"_$P($G(^PRCA(430,PRCABILL,0)),U,8)_";") Q:'$D(^XTMP("PRCA399",$J,3,PRCADEB,PRCABILL))  S PRCAT5=PRCAT5+1
 .I '$D(^XTMP("PRCA399",$J,2,1,PRCADEB)) D
 ...
 .I '$D(^XTMP("PRCA399",$J,1,1,PRCABILL)) D
 ..S PRCAT1=PRCAT1+1
 ..S ^XTMP("PRCA399",$J,2,1,PRCADEB,0,PRCABILL)="0^0"
 ..S ^XTMP("PRCA399",$J,2,1,PRCADEB,2,PRCABILL)=""
 ..F J=0,6,7 S ^XTMP("PRCA399",$J,1,1,PRCABILL,J)=$G(^PRCA(430,PRCABILL,J))
 ...
 D MES^XPDUTL("FIRST TRANSACTION SET FOR INT/ADMIN PANDEMIC OFFSETS: "_PRCAF1ST)
 D MES^XPDUTL("TOTAL DEBTORS FOUND WITH PANDEMIC OFFSETS: "_PRCAT2)
 D MES^XPDUTL("TOTAL BILLS FOUND FOR PANDEMIC RESET (382/385): "_PRCAT1)
 D MES^XPDUTL("TOTAL PANDEMIC ADJ TX FOUND FORCING BILL RESETS: "_PRCAT)
 D MES^XPDUTL("TOTAL BILLS WITH '-' IN NODE 7: "_PRCAT3)
 ...
 D ALLDEBTS
 D EN^PRCA399E,5^PRCA399E
 D EN^PRCA399A
RESET ;SET BAD TX,SET 7 NODES,REPORT OF EXCEPTIONS,REPORT OF PREPAYS
...
FINDF1ST ;Searches file 433 for first 'PANDEMIC OFFSET' to drive bill reset process
 S PRCADEB=0,PRCABTX=$P(^PRCA(433,0),U,3)
 ...
 S PRCAF1ST=PRCABTX-10,^XTMP("PRCA399",$J,1,0,0)=PRCAF1ST
 Q
...
CURTX N PRCATX,PRCADTCK S PRCADTCK=PRCASETD
 F  S PRCADTCK=$O(^PRCA(433,"ATD",PRCADB,PRCADTCK)),PRCATX=0 Q:'PRCADTCK  D
 .S PRCATX=0
...
TXCODES ;TRANSACTION CODES
 ...
 ;;23:WRITE-OFF^WO^109^0^1^1^D^^D1;rep tot
